                            ...
                            const RECT* overlayRect, DWORD drawFlags)
{
    ...
    // TemporarilySimpleIcons is set while switching panel modes
    ...
    if (!TemporarilySimpleIcons && UseSystemIcons)
    {
        if (symbolIndex != symbolsUpDir && symbolIndex != symbolsArchive)
        {
            ...
            if (GetPluginIconsType() != pitFromPlugin || !Is(ptPluginFS))
            {
                if (isDir) // it's a directory
                {
                    ...
                    *(DWORD*)(fileName + f->NameLen) = 0;
                    ...
                    if (!IconCache->GetIndex(fileName, icon, NULL, NULL) ||                             // the icon-thread isn't loading it
                        IconCache->At(icon).GetFlag() != 1 && IconCache->At(icon).GetFlag() != 2 ||     // the icon is neither new nor old
                ...
                {
                    int index;
                    BOOL exceptions = *(DWORD*)lowerExtension == *(DWORD*)"scr" || // icons stored in the file,
                                      *(DWORD*)lowerExtension == *(DWORD*)"pif" || // even though it isn't in the Registry
                    ...
                    if (exceptions || Associations.GetIndex(lowerExtension, index)) // the extension has an icon (association)
                    {
                        ...
                        {                                                             // icon in the file
                            ...
                            *(DWORD*)(fileName + f->NameLen) = 0;
                            ...
                            index = -2; // a dynamic icon will be shown
                        }
...
// the outer rectangle oRect and the inner rectangle iRect
//
...
int DrawItemAlpDx[1024]; // for width calculations of FixedWidth columns and elastic columns with smart mode
...
void CFilesWindow::DrawBriefDetailedItem(HDC hTgtDC, int itemIndex, RECT* itemRect, DWORD drawFlags)
{
    ...
    HDC hDC;
    int cacheValidWidth; // meaningful only when using the cache and specifies the width,
    ...
    if (drawFlags & DRAWFLAG_SELFOC_CHANGE)
    {
        ...
        // if only the cursor position changed, skip visibility tests
        drawFlags |= DRAWFLAG_SKIP_VISTEST;

        // we are drawing only a small area, so using the cache is fine
...
*/
    ...
    if (!(drawFlags & DRAWFLAG_ICON_ONLY))
    {
        ...
        // set the active font, background color and text color
        ...
        // valid if the focus frame is shortened
        ...
        // (GDI functions not returning BOOL call GdiFlush())
        //
        //    GdiFlush();

        if (drawFlags & DRAWFLAG_MASK)
        {
            SetTextColor(hDC, RGB(0, 0, 0)); // we want black text on black background to handle anti-aliased fonts
        ...
        if (drawFlags & DRAWFLAG_SKIP_VISTEST || RectVisible(hDC, &r))
        {
            ...
            {
                r.right -= 10; // correction - we must create space for the drag rectangle
            ...
            if (isItemUpDir)
            {
                ...
                if (GetViewMode() == vmBrief && !Configuration.FullRowSelect)
                {
                    adjR.right -= 10; // subtract 10 so we don't stretch across the full width
                    r.right -= 10;
                    focusFrameRightValid = TRUE;
            ...
            if (!isItemUpDir && !Configuration.FullRowSelect)
            {
                ...
                // focus frame will also be shorter
                focusFrameRightValid = TRUE;
                focusFrameRight = r.right;
            }
            if (!isItemUpDir && GetViewMode() == vmDetailed && (column->FixedWidth == 1 || NarrowedNameColumn))
            {
                ...
                {
                    // no width measured yet - measuring now
                    // the string may be longer than available space and must end with "..."
                    textWidth = nameWidth - 1 - IconSizes[ICONSIZE_16] - 1 - 2 - SPACE_WIDTH;
                    GetTextExtentExPoint(hDC, TransferBuffer, nameLen, textWidth,
                                         &fitChars, DrawItemAlpDx, &fnSZ);
                }
                if (fitChars < nameLen)
                {
                    // find from the end the character after which we can copy "..." so it fits in the column
                                            ...
                                            !(drawFlags & DRAWFLAG_DIRTY_ONLY) && !(drawFlags & DRAWFLAG_SELFOC_CHANGE)))
        {
            ...
            // callbacks that fill the buffer with text and then we draw the column
            ...
            int deltaX;
            // skip the Name column (i==0) - it is already drawn
            int i;
            for (i = 1; i < Columns.Count; i++)
            {
                ...
                if (drawFlags & DRAWFLAG_SKIP_VISTEST || RectVisible(hDC, &r))
                {
                    ...
                    {
                        if (column->FixedWidth == 1) // NarrowedNameColumn does not apply here (not the Name column)
                        {
                            int fitChars;
        ...
        //
        // if FullRowSelect is off and the name wasn't drawn, we won't bother with the frame
            ...
            (Configuration.FullRowSelect || focusFrameRightValid))
        {
            ...
            /*
      // for debugging the dual cursor issue we need to distinguish normal focus and the drop target
      if (itemIndex == DropTargetIndex)
      {
...
*/
    ...
    if (drawFlags & DRAWFLAG_SELFOC_CHANGE)
    {
        // if we used the cache, blit it to the screen
...
// out1      [OUT] first output line without terminator
               ...
               char* out2, int* out2Len, int* out2Width)
{
    ...
    if (sz.cx > *maxWidth)
    {
        ...
        int w = 0;
        int index = 0;
        while (index < maxW) // this condition should not be triggered
        {
            if (text[index] == ' ')
        ...
        else
        {
            ...
            *out1Width = DrawItemAlpDx[backTrackIndex - 1] + TextEllipsisWidth;
            ...
            // look for a space where we can continue on the next line
            while (index < textLen)
            {
                if (text[index++] == ' ')
                    break;
...
*/
    ...
    if (drawFlags & DRAWFLAG_SKIP_VISTEST || RectVisible(hDC, &rect))
    {
        ...
        RECT innerRect;
        BOOL thickFrame = FALSE; // for Thumbnails only -- should the frame be double?
        if (GetViewMode() == vmThumbnails)
        {
                  ...
                  BOOL isDisk)
{
    ...
    *out1Len = (int)strlen(out1);
    ...
    if (!validDate) // no date set, so set some value...
    {
        st.wYear = 2000;
        st.wMonth = 12;
        st.wDay = 24;
    ...
    if (!validTime) // no time set, so set some value...
    {
        st.wHour = 0;
        st.wMinute = 0;
        st.wSecond = 0;
                                ...
                                CIconSizeEnum iconSize)
{
    ...
    if (!(drawFlags & DRAWFLAG_ICON_ONLY))
    {
        ...
        // inner rectangle we will clear to
        RECT r;
        ...
        int textX = r.left + 2;
        int textY = rect.top + (rect.bottom - rect.top - textH) / 2; // center it

        r.top = textY;
        r.bottom = textY + textH;
        RECT focusR = r; // backup for drawing the focus
        ...
        //    if (drawFocusFrame) // shrink if the frame will not be drawn
        //      InflateRect(&r, -1, -1);

        // display the first line and also clear the area above it
        r.bottom = textY + FontCharHeight;
        if (out1[0] == 0 && out2[0] == 0) // if this is the last line, clear the area below it
        {
            r.bottom += 2;
            if (drawFocusFrame)
                r.bottom--;
        ...
        if (out1[0] != 0)
        {
            ...
            if (out2[0] == 0) // if this is the last line, clear the area below it
            {
                r.bottom += 2;
                if (drawFocusFrame)
                    r.bottom--;
        ...
        // display the third line and clear the area below it
        if (out2[0] != 0)
        {
            r.top = r.bottom;
