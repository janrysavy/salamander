...
// this number keeps growing and is used as a source for unique IDs
...
CCommonDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        // prevent background panels from refreshing while modal dialogs or message boxes are open
        if (Modal && MainWindow != NULL && Parent != NULL && Parent == MainWindow->HWindow)
        {
            BeginStopRefresh(FALSE, TRUE); // the sniffer can take a break
        ...
        /* j.r.: the VK_ESCAPE variant seems better because clicking IDCANCEL does not set the variable
    case WM_COMMAND:
    {
    ...
    */
    ...
    case WM_DESTROY:
    {
        ...
        // (for example pasted text from an edit line), so we'll verify it
...
CSelectDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        InstallWordBreakProc(GetDlgItem(HWindow, IDE_FILEMASK)); // install WordBreakProc for the combobox
    ...
    if (ti.Type == ttDataToWindow)
    {
        ...
        for (i = 0; i < SALCFG_ROOTS_COUNT; i++)
        {
            if (ConfigurationExist[i])
            {
                ...
                if (selIndex == 0)
                    selIndex = 1; // the last configuration becomes the default
            }
        }
...
int CLanguageSelectorDialog::GetPreferredLanguageIndex(const char* selectSLGName, BOOL exactMatch)
{
    ...
    int primarylocaleIndex = -1; // index corresponding to the user's primary language
...
CLanguageSelectorDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // JRY: For AS 2.53 which ships with Czech, German and English we send
        // other translations to the "Translations" forum
        //     https://forum.altap.cz/viewforum.php?f=23 in the hope that
        // someone will be motivated to create a translation.
        ...
        else
        {
            if (PluginName != NULL)
            {
                ...
                buf[99] = 0; // use only 100 characters for the plug-in name so some space remains for the original title
