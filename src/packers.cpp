...
void CPackerConfig::AddDefault(int SalamVersion)
{
    ...
    case 9: // 1.6b6 - due to switching from exe name to variable in custom packers
        ...
        // a crude approach since the new text is shorter -> overwrite directly
        for (index = 0; index < GetPackersCount(); index++)
        {
            if ((GetPackerOldType(index) && GetPackerType(index) != 1) ||
                (!GetPackerOldType(index) && GetPackerType(index) != CUSTOMPACKER_EXTERNAL))
    ...
    case 10: // 1.6b6 - due to renaming "XXX (Internal)" to "XXX (Plugin)" in the Pack and Unpack dialogs
             //         and because the ANSI "list of files" file is used for (un)packers ACE32 and PKZIP25
    ...
    case 18: // 2.5b3 - only to transfer plugin configuration from version 2.5b2
    case 19: // 2.5b4 - only to transfer plugin configuration from version 2.5b3
    case 20: // 2.5b5 - only to transfer plugin configuration from version 2.5b4
    case 21: // 2.5b6 - only to transfer plugin configuration from version 2.5b5(a)
    case 22: // 2.5b6 - filters in panels -- unified into a single history
    case 23: // 2.5b6 - new panel view (Tiles)
    case 24: // 2.5b7 - only to transfer plugin configuration from version 2.5b6
    case 25: // 2.5b7 - plugins: show in plugin bar -> variable moved to CPluginData
    case 26: // 2.5b8 - only to transfer plugin configuration from version 2.5b7
    case 27: // 2.5b9 - only to transfer plugin configuration from version 2.5b8
    case 28: // 2.5b9 - new color scheme based on the old DOS Navigator -> convert 'scheme'
    case 29: // 2.5b10 - only to transfer plugin configuration from version 2.5b9
    case 30: // 2.5b11 - only to transfer plugin configuration from version 2.5b10
