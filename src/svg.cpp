...
// (a shorter version might exist)
//
// the following solution has the advantage that constants are evaluated during preprocessing
...
// render icons for which we have an SVG representation
void RenderSVGImage(NSVGrasterizer* rast, HDC hDC, int x, int y, const char* svgName, int iconSize, COLORREF bkColor, BOOL enabled)
{
    ...
    if (svg != NULL)
    {
        ...
        if (!enabled)
        {
            DWORD disabledColor = GetSVGSysColor(COLOR_BTNSHADOW); // JRYFIXME - initial attempt, where will we get the disabled color?
