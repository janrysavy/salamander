...
SECLoadRegistry()
{
  ...
  {
    // always load data after the first launch
...
// - since version 2.5 RC2 we no longer use: - delete files from TEMP (default value in HKEY_CLASSES_ROOT\CLSID\{C78B6131-F3EA-11D2-94A1-00E0292A01E3}\InProcServer32)
//   (keep in mind they may not be deletable immediately - be able to schedule it after the machine reboots)
// - since version 3.0 B1: assume utils\salextx86.dll and salextx64.dll may not be deletable immediately - be able to schedule it after the machine reboots
