...
#include <shlobj.h>
//
// Initialize GUIDs (should be done only once per DLL/EXE)
//
...
#define NOHANDLES(function) function // defense against CheckHnd polluting the source with HANDLES macros
...
SECLoadRegistry()
{
  ...
  // open the Shell Extensions registry key
  ...
  {
    // always reload data after the first launch
...
//    MessageBox(NULL, "loading registry", "shellext.dll", MB_OK);
    ...
    {
      // now create and load one configuration item
    ...
    // load the individual configuration variables
    bufferSize = sizeof(BOOL);
...
{
    DWORD createType; // indicates whether the key was created or just opened
...
// our version of RegQueryValueEx; unlike the API function it ensures
// a null terminator is added for the REG_SZ, REG_MULTI_SZ, and REG_EXPAND_SZ types
...
// - since version 2.5 RC2 we no longer do the following: delete files from TEMP (default value in HKEY_CLASSES_ROOT\CLSID\{C78B6131-F3EA-11D2-94A1-00E0292A01E3}\InProcServer32)
//   (keep in mind they may not be deletable immediately - support scheduling the deletion after the machine reboots)
// - since version 3.0 B1: assume utils\salextx86.dll and salextx64.dll may not be deletable immediately - support scheduling the deletion after the machine reboots
...
// - everything stated for the HKEY_CLASSES_ROOT key must also be attempted for the
//   HKEY_CURRENT_USER\Software\Classes key (used when the user does not have permission to write to
//   HKEY_CLASSES_ROOT)
BOOL SECRegisterToRegistry(const char* shellExtensionPath, BOOL doNotLoadDLL, REGSAM regView)
{
    ...
    if (NOHANDLES(RegOpenKeyEx(HKEY_CLASSES_ROOT, key, 0, KEY_READ | regView, &hKey)) == ERROR_SUCCESS)
    {
        ...
        {
            if (doNotLoadDLL && FileExists(shellExtPath) ||       // when the DLL cannot be loaded, at least verify that it exists
                !doNotLoadDLL && CheckVersionOfDLL(shellExtPath)) // otherwise load it and read back its version
            {
    ...
    REG_TRY_AGAIN:
        ...
        // else;  // the failure to open the key under HKEY_CLASSES_ROOT is handled further below (doing it here would be pointless)
            ...
            if (classesKey == HKEY_CLASSES_ROOT)
            {
                                           ...
                                           KEY_READ | KEY_WRITE | regView, &classesKey)) == ERROR_SUCCESS)
                {
                    if (MyCreateKey(classesKey, "CLSID", &hKey, regView))
                        NOHANDLES(RegCloseKey(hKey)); // the "CLSID" key may not exist yet, so create it
                    goto REG_TRY_AGAIN;
                }
            }
            if (classesKey != HKEY_CLASSES_ROOT)
        ...
        // without "As Admin" this code path is dead code, at least on Vista+
        ...
        // this should tell the shell that it needs to reload the shell extensions
