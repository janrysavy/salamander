...
void CProgressDialog::SetWindowIcon()
{
    if (RunningInOwnThread)
    {
        // from Windows XP the taskbar shows the application icon even for windows without an assigned icon
...
CProgressDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_USER_PROGRDLGSTART: // probably needless on W2K+: just a delayed worker thread start
    {
        ...
        else
        {
            if (RunningInOwnThread)
            {
                ...
                if (startPaused)
                    PostMessage(HWindow, WM_COMMAND, IDB_MINIMIZE, 0); // minimize the "waiting" operation immediately (nothing to watch, saves one step)
                else
    ...
    case WM_TIMER:
    {
        ...
        {
            // textual operation status (transfer speed, etc.)
            if (Status != NULL)
            {
                char buf[300];
                buf[0] = 0;
...
*/
                             ...
                             secs < TimeLeftLastValue / CQuadWord(2, 0)))
                        {
                            ...
                            // slow down updating the time-left information based on the calculated time (the longer the time, the fewer updates)
                            if (secs.Value <= 10)
...
*/
        ...
        if (setSpeedLimit && Script != NULL)
        {
            ...
            if (!useSpeedLimit && speedLimit == 1)
                speedLimit = Configuration.LastUsedSpeedLimit; // on first enable use the last used speed limit
    ...
    case WM_USER_CANCELPROGRDLG:
    {
        if (CanClose)
        {
            if (!IsWindowEnabled(HWindow)) // there is a modal dialog above this one (a message box asking about canceling the operation or reporting an error)
    ...
    case WM_USER_FOCUSPROGRDLG:
    {
        ...
        if (!IsWindowEnabled(HWindow)) // there is a modal window above the dialog (a message box asking about canceling the operation or reporting an error)
        {
    ...
    case WM_USER_PROGRDLGEND: // probably needless on W2K+: we had to delay the dialog termination a bit
    {
        ...
        {
            // Prior to Windows Vista we only called GetNextWindow(), which was
            // enough to reach the next window in the Z-order. Vista introduced
            // hidden helper windows such as "MSCTFIME UI" and "Default IME" that
            // sit between our windows, so we skip hidden ones here.
            BOOL valid;
            HWND hNext = HWindow;
            do
            {
    ...
    case WM_COMMAND:
    {
        ...
        case IDCANCEL: // abort
        {
            if (CanClose && !CancelWorker)
            {
                ...
                int ret = SalMessageBox(HWindow, LoadStr(IDS_CANCELOPERATION),
                                        ...
                                        MB_YESNO | MB_ICONQUESTION /*| MSGBOXEX_ESCAPEENABLED*/); // Escape key is not allowed here
                // Idea: Zarevak accidentally started deleting a large batch of files
                // and began hitting Escape (the machine was heavily loaded so it did
                // not respond immediately) canceling the confirmation, therefore the
                // confirmation can no longer be closed with Escape.
            ...
            return TRUE; // terminate the dialog only after the worker uses IDOK ...
        }
        ...
        case IDB_MINIMIZE:
        {
            ...
            // MinimizeApp is called and the app behaves incorrectly
    ...
    case WM_DESTROY:
    {
        ...
        // on Windows XP SP1 with a top-most window open after closing this dialog
        ...
        // window a a question pops up (overwrite confirmation, cancel confirmation, etc.) -- this
        // system messes up the Z-order and without the following hack after closing the progress window
        // activates the top-most window that is highest; see \Source\zorder and
