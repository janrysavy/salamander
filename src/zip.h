...
{
protected:
    const char* RemapNameFrom; // mapping of names originating in the temporary directory
    const char* RemapNameTo;   // mapping target: the name of the archive we are unpacking from
...
protected:
    ...
    BOOL HasTwoProgress(); // TRUE if the dialog shows two progress bars; otherwise FALSE
    ...
    const char* Title;   // dialog title pointer inside the LoadStr buffer (do not keep for long)
    ...
    int CacheIndex;                            // index within LinesCache to be filled by the next line
    BOOL CacheIsDirty;                         // TRUE if the cache needs to be sent to the screen

    CQuadWord TotalSize,
        ActualSize;
    CQuadWord TotalSize2,
        ActualSize2;
    BOOL SizeIsDirty;  // TRUE when Size needs to be pushed to the screen
    BOOL Size2IsDirty; // TRUE when Size2 needs to be pushed to the screen
...
protected:
    CFilesWindow* Panel;
    CZIPUnpackProgress UnpackProgress; // UnpackProgress dialog instance
    BOOL ProgressDialog2;              // TRUE = UnpackProgress dialog with two progress bars, FALSE = single-progress dialog
    HWND FocusWnd;

    // control mechanism
    DWORD ThreadID; // only the thread in which our pointer was passed may call us
...
public:
    ...
    // PROGRESS DIALOG: the dialog contains one or two progress meters (depending on 'twoProgressBars')
    ...
    // prints the text 'txt' (splits multiple lines) into the progress dialog
    ...
    // (for a progress dialog with a single progress meter, 'size2' must be CQuadWord(-1, -1)); returns whether the action should continue (FALSE = end)
...
public:
    ...
    // calls 'pluginData'.ReleaseFilesOrDirs (releasing plug-in data) for all files (if 'releaseFiles' is TRUE) and all directories
    // (if 'releaseDirs' is TRUE)
    ...
    // returns the salamander-dir for the specified directory; if 'readOnly' is TRUE, the returned salamander-dir object must not be modified
    CSalamanderDirectory* GetSalamanderDir(const char* path, BOOL readOnly);
    // returns the salamander-dir for the specified directory index; the returned salamander-dir object must not be modified
...
// checks the free space at 'path' and, if it is below totalSize, asks the user whether to continue
...
{
public:
    BOOL Move; // should files be moved (instead of copied) into the archive?
...
public:
    ...
    BOOL MovePacker(int srcIndex, int dstIndex);    // moves an item within the array
...
public:
    ...
    BOOL MoveUnpacker(int srcIndex, int dstIndex);      // moves an item within the array
