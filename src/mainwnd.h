...
// we keep some margin because the path may contain long variables that "shrink" after expansion,
...
struct CHotPathItem
{
    ...
    // MAX_PATH would be too small for Path because of escaping and variables
...
public:
    ...
    // 'emptyEcho' - insert an informative item if the menu is empty
    // 'customize' - append a separator and customize option
    // 'topSeparator' - place a separator above inserted paths
...
//
// returns success - continue retrieving more names? (FALSE ends the enumeration)
...
struct CDynString
{
    ...
    BOOL Append(const char* str, int len); // returns TRUE on success; if 'len' is -1 the length is calculated using strlen
...
#define COMPARE_DIRECTORIES_ONEPANELDIRS 0x00000040 // mark directories that exist only in one panel (doesn't recurse)
...
public:
    ...
    BOOL CanCloseButInEndSuspendMode; // TRUE if CanClose was TRUE but temporarily FALSE while handling WM_USER_END_SUSPMODE
...
public:
    ...
    BOOL GetViewersAssoc(int wantedViewerType, CDynString* strViewerMasks); // helper: collects all masks associated with the given viewer type; returns TRUE on success (when enough memory for the string)
    ...
    // and to all opened FS from plugins (both panels and FS may refresh their contents);
    ...
    // if the plugin does not want to close the FS, asks the user whether to force close (as if canForce==TRUE)
    ...
    // interfaces that must be set to NULL after closing so that future
    // allocations do not accidentally match the old pointers
    ...
    void EditWindowSetDirectory(); // sets the text before the command line and enables/disables it
    ...
    // fills the menu with UserMenuItems
    ...
    // 'popup' is the popup we are filling
    ...
    // have already been created and must be assigned to their controls.
    // reloadUMIcons determines whether the UserMenu icons are reloaded. WARNING:
    // if the items are on a network share this can be extremely slow
    ...
    BOOL ProcessHelpMsg(MSG& msg, DWORD* pContext, HWND* hDirtyWindow); // hDirtyWindow: window that received WM_USER_HELP_MOUSEMOVE and needs WM_USER_HELP_MOUSELEAVE
    void ExitHelpMode();
    ...
    // if the drive bar has a different disk mask than 'drivesMask' or if 'checkCloudStorages' is TRUE
    // and the drive bar has a different cloud mask than 'cloudStoragesMask', it will be regenerated
    void RebuildDriveBarsIfNeeded(BOOL useDrivesMask, DWORD drivesMask, BOOL checkCloudStorages,
                                  DWORD cloudStoragesMask);

    // sets DoNotLoadAnyPlugins and when FALSE sends refreshes to panels loading thumbnails
    void SetDoNotLoadAnyPlugins(BOOL doNotLoad);

    // shows or hides two drive bars based on 'show'
    ...
    // connects or disconnects the main window from shell change notifications (adding/removing drives)
