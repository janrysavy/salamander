...
BOOL AgreeMask(const char* filename, const char* mask, BOOL hasExtension, BOOL extendedMode)
{
    ...
    {
        if (*mask == 0)
            return FALSE; // mask too short
        BOOL agree;
        if (extendedMode)
            agree = (LowerCase[*filename] == LowerCase[*mask] || *mask == '?' || // match or '?' represents any char, '#' represents any digit
                     (*mask == '#' && *filename >= '0' && *filename <= '9'));
        else
            agree = (LowerCase[*filename] == LowerCase[*mask] || *mask == '?'); // match or '?' represents any char
        if (agree)
        {
            filename++;
            mask++;
    ...
    if (*mask == '*')
        mask++;                        // asterisk at the end represents an empty string, all OK
...
char* MaskName(char* buffer, int bufSize, const char* name, const char* mask)
{
    ...
    // "d" + "*.old" = "d.old" -> the result is "a.b.c.d.old")

    int ignPoints = 0; // how many dots the name contains (this section corresponds to mask text up to the first dot); the rest matches the extension
    const char* n = name;
    while (*n != 0)
        if (*n++ == '.')
            ignPoints++;
    ...
    while (*s != 0)
        if (*s++ == '.')
        {
            ignPoints--;
            break;
        } // only one dot makes sense in the mask (name.ext); the part after the second dot will be expanded at the end of the name
    //  while (*s != 0) if (*s++ == '.') {ignPoints--;}  // in this variant the name and mask sections match from the end ("a.b.c.d" + "*.*.old": "a.b" + "*"; "c" + "*"; "d" + "old" -> "a.b.c.old")
    if (ignPoints < 0)
        ignPoints = 0;
    //  if (ignPoints == 0 && *name == '.') ignPoints++;   // a leading dot should be ignored (not an extension); fix: ".cvspass" is an extension in Windows...
    ...
    while (*s != 0 && d < endBuf)
    {
        switch (*s)
        {
        case '*': // copy the remainder of the name section (up to the "ignPoints+1"-th dot)
        {
            while (*n != 0)
            {
                if (*n == '.')
...
// and it should be easy to type (see the '<' on the German keyboard;
...
void PrepareQSMask(char* mask, const char* src)
{
    ...
    while (*src != 0)
    {
        if (IsQSWildChar(*src))
        {
            ...
            {
                // convert other wildcard characters to '/'
                src++;
                *mask++ = (lastChar = '/');
            }
        }
    ...
    if (MasksHashArray != NULL)
    {
        ...
        for (i = 0; i < MasksHashArraySize; i++)
        {
            if (MasksHashArray[i].Mask != NULL) // if this array element is not empty
            {
                ...
                while (next != NULL) // if there are multiple masks at the same hashed entry
                {
...
{
    NeedPrepare = TRUE; // unfortunately also used as a write buffer, so allow another PrepareMasks
    return MasksString;
}
...
BOOL CMaskGroup::PrepareMasks(int& errorPos, const char* masksString)
{
    ...
    while (1)
    {
        ...
        *mask = 0;
        if (*s != 0 && *s != ';' && (*s != '|' || excludePos != -1)) // the '|' character may appear only once in the mask
        {
            ...
            for (i2 = PreparedMasks.Count - 1; i2 >= 0; i2--)
            {
                ...
                { // this mask can be hashed; add it to the hash array
                    ...
                    if (MasksHashArray[hash].Mask == NULL)
                    {
                        ...
                        if (!PreparedMasks.IsGood())
                            PreparedMasks.ResetState(); // Detach always succeeds (at worst the array shifts, which is fine)
                    }
                    else
                    {
...
BOOL CMaskGroup::AgreeMasks(const char* fileName, const char* fileExt)
{
    ...
    if (fileExt == NULL)
    {
        ...
        while (--fileExt >= fileName && *fileExt != '.')
            ;
        if (fileExt < fileName)
            fileExt = fileName + tmpLen; // under Windows ".cvspass" is an extension ...
        else
            fileExt++;
    }
