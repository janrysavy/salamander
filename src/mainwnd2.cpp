...
#include <shlwapi.h>
#undef PathIsPrefix // otherwise it would clash with CSalamanderGeneral::PathIsPrefix
...
// 6 = 1.6b4.x
// 7 = 1.6b5.x          needed for correct conversion of supported plug-in functions (see CPlugins::Load)
...
// 16 = 2.5b2           added coloring of Encrypted files and folders (added when loading config + included in default config)
...
// 65 = 2.55b1 (DB72)   external archivers: identify by UID instead of Title (which is translated) - changing language lost paths to external archivers
...
// 101 = 3.08           only to transfer plug-in configuration from version 4.00b1 (DB168) - by mistake 3.08 and DB171 share the same version number 101, sorry
...
// Older versions follow toward higher indices.
// The last index contains NULL and serves as a terminator when working with the array.
// When creating a new configuration version (stored separately in the registry)
...
const char* SALAMANDER_AUTO_IMPORT_CONFIG = "AutoImportConfig"; // value exists only during upgrade: installer overwrites the old version with the new and stores this value pointing to the old configuration key from which to import settings
...
// If not found or if the key stored in AutoImportConfig does not exist (points to this version, which makes no sense)
...
//
// If the configuration key of this version also contains the "Configuration" key (expected to be a saved configuration),
// we ask the user whether to:
//   - Use the current configuration and ignore the old one (we do not delete it so the user does not lose data). In this case
//     delete AutoImportConfig immediately. This is done silently if AutoImportConfig points to this version's key
    ...
    do
    {
        ...
        if (cfgFound)
        {
            ...
            if (rootIndex != 0 &&                      // only for one of the older keys
                configVersion >= 59 /* 2.53 beta 2 */) // before 2.53 beta 2 there was only English, so reading makes no sense; offer system default language or manual selection
            {
...
BOOL MyRegRenameKey(HKEY key, const char* name, const char* newName)
{
    ...
    // and probably the key opened via NtOpenKey with OBJECT_ATTRIBUTES initialized by
    // InitializeObjectAttributes). It's overly complicated and not performance critical,
    // so we'll do it the slow but simple way... copy the key to a new one and then delete the original
    HKEY newKey;
    if (!OpenKeyAux(NULL, key, newName, newKey)) // verify the target key does not already exist
    {
...
// If it succeeds, 'loadConfiguration' is set and the function returns TRUE.
...
// When it finds one, a dialog is shown offering to convert it into the current configuration
...
{
    HKEY hRootKey;
    loadConfiguration = NULL; // we don't want to load any configuration - defaults will be used
    int rootIndex = 0;
    const char* root;
    DWORD saveInProgress; // dummy
    HKEY hCfgKey;
    ...
    BOOL offerImportDlg = FALSE; // if an old configuration or key exists, offer import

    LoadSaveToRegistryMutex.Enter();

    char backup[200];
    sprintf_s(backup, "%s.backup.63A7CD13", SalamanderConfigurationRoots[0]); // "63A7CD13" prevents collision with user key names
    HKEY backupKey;
    ...
    if (backupFound)
    {
        ...
        if (!copyIsOK) // delete the bad backup and pretend it never existed (it probably wasn't fully created)
        {
    ...
    do
    {
        ...
        { // this configuration is corrupted
            ...
            {
                char corrupted[200];
                sprintf_s(corrupted, "%s.corrupted.63A7CD13", root); // "63A7CD13" prevents collision with user key names
            ...
            if (rootIndex == 0) // for the active version inform the user about the corrupted configuration and let them back up the key, then try to delete it (older versions simply ignore the corrupted config)
            {
                char buf[1500];
   ...
   we let the script handle button hotkey conflicts by simulating a menu
...
*/
        ...
        if (cfgFound) // a key is considered a configuration key only if the "Configuration" subkey exists (mere presence of the key isn't enough because it may contain only "AutoImportConfig")
        {
            CloseKeyAux(hCfgKey);
            if (rootIndex == 0)
            {
...
void CMainWindow::SaveConfig(HWND parent)
{
    ...
    {
        TRACE_E("SALAMANDER_ROOT_REG == NULL"); // not necessarily an error: during UPGRADE we may exit without saving if not all plug-ins are installed and the user chooses Exit
        return;
    }
    ...
    if (GlobalSaveWaitWindow == NULL)
    {
        ...
        analysing.SetProgressMax(7 /* MUST BE SYNCHRONIZED with CMainWindow::WindowProc::WM_USER_CLOSE_MAINWND !!! */); // one less so they can enjoy seeing 100%
        analysing.Create();
    ...
    if (CreateKey(HKEY_CURRENT_USER, SALAMANDER_ROOT_REG, salamander))
    {
        ...
        if (cfgIsOK)
        {
            ...
            if (CreateKey(salamander, SALAMANDER_CONFIG_REG, actKey))
            {
                ...
                // WARNING: when an icon overlay handler crashes these values are written directly to the registry
            ...
            if (Configuration.ConfigVersion < 6 && // only for old configs, otherwise we neither create nor clear the key
    ...
    if (OpenKey(HKEY_CURRENT_USER, SALAMANDER_ROOT_REG, salamander))
    {
        ...
        if (OpenKey(salamander, SALAMANDER_COLORS_REG, actKey))
        {
            ...
            if (!LoadRGBF(actKey, SALAMANDER_CLR_FOCUS_BK_INACTIVE_NORMAL_REG, UserColors[FOCUS_BK_INACTIVE_NORMAL]))
                UserColors[FOCUS_BK_INACTIVE_NORMAL] = UserColors[ITEM_BK_NORMAL]; // convert older configurations
            if (!LoadRGBF(actKey, SALAMANDER_CLR_FOCUS_BK_INACTIVE_SELECTED_REG, UserColors[FOCUS_BK_INACTIVE_SELECTED]))
                UserColors[FOCUS_BK_INACTIVE_SELECTED] = UserColors[ITEM_BK_NORMAL]; // convert older configurations
                             ...
                             &closeShell, sizeof(DWORD)))
                {
                        ...
                        !GetValue(subKey, USERMENU_ARGUMENTS_REG, REG_SZ, arguments, USRMNUARGS_MAXLEN))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                *s++ = 0; // cut off the command, set s to the first argument character
                                char* st = arguments;
        ...
        if (OpenKey(salamander, SALAMANDER_CONFIG_REG, actKey))
        {
            ...
            GetValue(actKey, CONFIG_FINDFULLROW_REG, REG_DWORD,
                     &Configuration.FindFullRowSelect, sizeof(DWORD));
            if (Configuration.ConfigVersion <= 6)
                Configuration.ShowGrepErrors = FALSE; // force FALSE so we don't annoy users unnecessarily (others do it too)
            ...
            // if this is an old configuration and the user menu contains items,
        ...
        if (useWinPlacement)
        {
            ...
            if (!Configuration.StatusArea)
            {
                ...
                // the shortcut setting takes precedence over the configuration
