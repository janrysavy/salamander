...
// value at which custom salspawn.exe errors start
#define SPAWN_ERR_BASE 10000
// program for running 16-bit archivers (-c must equal SPAWN_ERR_BASE)
...
// structure holding the path to the packer
...
struct SPackFormat
{
    ...
    const char* MultiExtension; // extension format for split archives
...
//
// Structure for the definition table of external packers - modifying operations
//
struct SPackModifyTable
{
    ...
    BOOL CanPackToDir;           // TRUE if the archiver supports packing to a directory

    //
    // items for deletion from the archive
    //
    const char* DeleteInitDir; // directory in which the delete command is run
    ...
    //
    const char* MoveInitDir; // directory where the move command runs; NULL if unsupported
    const char* MoveCommand; // command for moving files to the archive; NULL if unsupported
...
struct SPackBrowseTable
{
    ...
    //
    const char* ListInitDir; // directory where the listing command is run
    const char* ListCommand; // command for listing archive contents
    FPackList SpecialList;   // if not NULL, function for parsing the listing
                             // then the following items need not have meaning
    const char* StartString; // how the last header line begins
    int LinesToSkip;         // number of lines to ignore after StartString
    int AlwaysSkip;          // number of lines always ignored after StartString (stop string not checked)
    ...
    short AttrIdx;  // index of the attributes on the listing line
...
public:
    ...
    const char* PackerExecutable;   // packer program name for disk lookup
...
public:
    ...
    void AddDefault(int SalamVersion); // adds archivers introduced since SalamVersion; when SalamVersion = -1 adds all of them (default config)
...
public:
    ...
    BOOL OldType; // denotes old data (version < 6) - assumes internal ZIP+TAR+PAK?
...
public:
    ...
    void AddDefault(int SalamVersion); // adds packers introduced since SalamVersion; when SalamVersion = -1 adds all of them (default config)
    ...
    // creates search data; if anything fails it returns false and can also set the line and column of the error
...
// initialization of the spawn.exe path
