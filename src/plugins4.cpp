    ...
    if (timer != NULL)
    {
        ...
        if (PluginFSTimers.IsGood())
        {
            ...
            if (index == 0) // the nearest timeout changed, adjust the Windows timer
            {
                ...
                if ((int)ti > 0) // if the new timer hasn't expired yet (the difference can even be negative), adjust or start the Windows timer
                {
...
void CPlugins::HandlePluginFSTimers()
{
    ...
    if (!StopTimerHandlerRecursion) // protection against recursion
    {
        ...
        {
            CPluginFSTimer* timer = PluginFSTimers[i];
            if ((int)(timer->AbsTimeout - timeNow) <= 0 &&     // the timer expired
                timer->TimerAddedTime < startTimerTimeCounter) // it's an "old" timer (we block newly added timers to prevent an endless loop)
            {
                ...
                if (fs != NULL)
                {
                    ...
                    i = -1;                                  // if PluginFSTimers changes, start again from the beginning
                }
                else
        ...
        {
            DWORD ti = PluginFSTimers[0]->AbsTimeout - GetTickCount();
            if ((int)ti > 0) // if the new timer hasn't already expired (the difference can even be negative), modify or start the Windows timer
                SetTimer(MainWindow->HWindow, IDT_PLUGINFSTIMERS, ti, NULL);
            else
                PostMessage(MainWindow->HWindow, WM_TIMER, IDT_PLUGINFSTIMERS, 0); // process the next timeout as soon as possible
        }
