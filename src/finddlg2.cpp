...
void CFindDialog::OnHideDuplicateNames()
{
    ...
    if (totalCount - deletedCount > 0)
    {
        // simply select the first item... could be improved in future
    ...
    case WM_TIMER:
    {
        ...
        if (wParam == IDT_FLASHICON)
        {
            ...
            if (FlashIconCounter <= 0)
            {
                ...
                {
                    // user probably did not see the flashing, repeat it next time Find is activated
...
void CFindDialog::UninitializeOle()
{
    if (OleInitialized)
    {
        __try
        {
            OleFlushClipboard(); // hand over data from the IDataObject left on the clipboard (it gets released here)
            OleUninitialize();
        }
...
static char MyEnumFileNamesBuffer[MAX_PATH]; // called from GUI => cannot run in multiple threads, so a static buffer is fine
...
void CFindDialog::OnDrag(BOOL rightMouseButton)
{
    ...
    if (hr == DRAGDROP_S_DROP)
    {
        ...
        // shortened: an optimized Move is performed, meaning no copy to the destination followed by deletion
        //            of the original. To avoid unintentionally deleting the original (which might not be moved yet),
        //            the operation returns DROPEFFECT_NONE or DROPEFFECT_COPY,
        // so we implement this workaround
        if (!rightMouseButton && // right button has a menu: dropSource->LastEffect contains the effect before showing the menu, not the final one; for Move it is 0 just like Cancel, so we cannot distinguish Move from Cancel and won't delete anything
...
void CFindDialog::OnContextMenu(int x, int y)
{
    ...
    if (ContextMenu != NULL)
    {
        ...
        if (cmd != 0)
        {
            ...
            if (cmdCut || cmdCopy)
            {
                // also set the preferred drop effect and mark the source as Salamander
...
void CFindDialog::OnOpen(BOOL onlyFocused)
{
    ...
    do
    {
        // Petr: replaced LVNI_SELECTED with LVNI_FOCUSED because users complained
        ...
        // (as in the panel; Explorer opens all selected, so this differs).
