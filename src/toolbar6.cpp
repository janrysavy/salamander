    ...
    : CToolBar(hNotifyWindow, origin)
{
    ...
    List = NULL;
    // this initialization is also done in WM_DESTROY because the window is only hidden and shown
    CheckedDrive[0] = 0;
    HDrivesIcons = NULL;
...
BOOL CDriveBar::CreateDriveButtons(CDriveBar* copyDrivesListFrom)
{
    ...
    // suppress drawing to avoid flickering of the checked item (opening and closing the plugins manager would flicker)
    // it also flickered when Salamander started
...
{
    CALL_STACK_MESSAGE_NONE
    // even if we don't hold any icon we will return the correct height
...
void CDriveBar::Execute(DWORD id)
{
    if (List != NULL)
    {
        ...
        if (List->ExecuteItem(index, MainWindow->HWindow, &r, &fromDropDown))
        {
            ...
            case drvtDropbox:
            case drvtOneDrive:    // either the button itself or selected from the drop-down menu
            case drvtOneDriveBus: // either the button itself or selected from the drop-down menu
            {
            ...
            case drvtPluginCmd:
            {
                ...
                if (data != NULL) // the plug-in exists, run the command
...
BOOL CDriveBar::OnContextMenu()
{
    ...
    if (List != NULL && index >= 0)
    {
        ...
        if (GetItemInfo2(index, TRUE, &tii))
        {
            ...
            if (PostCmd != 0) // set only for drvtPluginFS and drvtPluginCmd; drvtPluginFS cannot appear on the Drive bar
            {
                UpdateWindow(MainWindow->HWindow);

                CPluginData* data = Plugins.GetPluginData(dllName);
                if (data != NULL) // plug-in exists, execute the command
                {                 // post-cmd from the FS item context menu
