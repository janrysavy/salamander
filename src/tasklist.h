...
// Determined using a mutex in the global namespace, so it is visible to
// other sessions (remote desktop, fast user switching)
...
struct CProcessListItem
{
    ...
    DWORD SalmonPID;      // Salmon ProcessID, letting the attaching process guarantee it has
...
// WARNING, x64 and x86 processes communicate using the structure; watch out for types (e.g. HANDLE) that have different widths
...
struct CCommandLineParams
{
    ...
    char TitlePrefix[MAX_PATH];  // title prefix; if empty, leave unchanged; prefer declaring the length as MAX_PATH instead of TITLE_PREFIX_MAX, which might change unexpectedly
...
protected:
    ...
    BOOL OK;                   // was construction successful?
...
public:
    ...
    // If 'timeouted' is not NULL, it indicates whether the failure was caused by a timeout when waiting for shared memory
