...
protected:
    ...
    CPluginFSInterfaceAbstract* PluginFS; // only for Type==2: the last interface used for the FS path

    int TopIndex;      // top index when the panel state was saved
    char* FocusedName; // focus when the panel state was saved
...
public:
    ...
    // IDs will start from one and correspond to the index parameter when calling Execute()
...
// this set of variables is used for Salamander column internal callbacks
...
struct CViewTemplate
{
    ...
    char Name[VIEW_NAME_MAX]; // name under which the view appears in the configuration/menu;
                              // if it is an empty string, the view is not defined
    DWORD Flags;              // visibility of Salamander standard columns
                              // VIEW_SHOW_xxxx
...
public:
    ...
    // the contents of str will be copied into the allocated Text buffer
    // if subStr is not NULL, its contents will be inserted into str through sprintf
...
struct CSharesItem
{
    ...
    BOOL IsGood() { return LocalPath != NULL; } // if LocalPath is allocated, the rest will be as well
};
...
//
// If 'HCenterAgains' is not NULL, it is centered to it, otherwise to the parent.
...
public:
    ...
    // writes the item according to 'index' into 'buffer' (pass buffer size in 'buffMax')
    // 'index': value 0 yields the oldest item, value Count
    // yields the last added message
    // if the index is out of bounds, writes the text "error"
...
public:
    ...
    // if 'stop' is TRUE, stops storing waypoints (calls to Add are ignored);
    ...
    // writes the item according to 'index' into 'buffer' (pass buffer size in 'buffMax')
    // 'index': value 0 yields the oldest item, value Count
    // yields the last added waypoint
    // if the index is out of bounds, writes the text "error"
    void Print(char* buffer, int buffMax, int index);
};
...
extern CShares Shares; // stores the loaded shared directories here
...
void GetNetworkDrivesBody(DWORD& netDrives, char (*netRemotePath)[MAX_PATH], char* buffer); // helper used only by the bug-report diagnostics

// returns the SID (as a string) for our process
// release the returned SID by calling LocalFree
//   LPTSTR sid;
//   if (GetStringSid(&sid))
//     LocalFree(sid);
BOOL GetStringSid(LPTSTR* stringSid);

// returns the MD5 hash computed from the SID, yielding a 16-byte array from a variable-length SID
...
// denies WRITE_DAC | WRITE_OWNER access to the Everyone group; all other access remains allowed
// provides stronger security than a "NULL DACL", where the object is fully open to everyone
// works on any OS; returns a pointer on Windows 2000 and newer, otherwise returns NULL
// release any output parameter that receives a non-NULL pointer
