...
//
// 1) OLE caches (among other things) BSTRs, so under a normal OS IMallocSpy
//    reports bogus leaks. Caching can be disabled by setting the environment
//    variable OANOCACHE=1 and using the debug (Checked) version of OLE. Ideally
//    use the complete W2K Checked Build or extract the appropriate DLL from the
//    Checked version of the Service Pack which is freely available.
//
// 2) If leaks remain in OLE, an attempt to unregister the spy via
//    CoRevokeMallocSpy returns E_ACCESSDENIED and postpones the Release call.
//    In practice Release is never called for us (W2K). Some allocations are
//    freed only during DLL_PROCESS_DETACH in OLEAUT32.DLL, so the spy reports
...
//    time we need to keep this object alive as long as possible because OLE will
//    call our methods (especially PreFree and PostFree with fSpy == FALSE) even
//    after CoRevokeMallocSpy. We solve it by copying the object into static
//    memory in OleSpyRegister(). The destructor is thus never called and the
//    critical section is never destroyed.
//
// 4) When testing under W2K Checked Build we can get more reports from OLE by
//    adding a section to win.ini:
//    [CairOLE InfoLevels]
//    cairole=7
//    heap=7
//
//    if we want detailed messages including call stacks of individual allocations
//    we have to set both values to the decimal equivalent of 0xffffffff:
//    [CairOLE InfoLevels]
//    cairole=4294967295
//    heap=4294967295
//
//    CairOLE often reports more leaks than the spy detects. It is also common
//    that CairOLE reports leaks while the spy reports everything is OK. What
//    matters is that these leaks are constant (for example on Salamander
//    start/exit CairOLE reports 13 leaks and about 800 bytes). Since this number
//    does not grow, I consider it unimportant.
//
// 5) The spy will report leaks from functions like SHBrowseForFolder or
...
// IsBadReadPtr throws exceptions in the debug build and we don't want that
BOOL
...
struct SPYBLK
{
    ...
    SIZE_T cbRequest;                  // requested size
    DWORD cRealloc;                    // how many times the item was reallocated
    DWORD cOrder;                      // allocation order (used for breakpoints)
...
private:
    ...
    LONG _iBreakAlloc;    // allocation on which to trigger a break (-1 == no break)
...
CMallocSpy::PostFree(BOOL fSpyed)
{
    ...
    // These deallocations come regularly; we won't pollute the log with them
    //  if (!fSpyed)
    //  {
    //    char buff[200];
...
BOOL CMallocSpy::DumpLeaks()
{
    ...
    // go to the end and then forward
    if (psbWalk != NULL)
    ...
    while (psbWalk != NULL)
    {
        ...
        __try
        {
            ...
            if (psbWalk->cbRequest >= *((USHORT*)pvRequest) &&
                *((USHORT*)pvRequest) != 0 && // we will not consider two zeros at the memory start a PIDL
                IsGoodPIDL((LPCITEMIDLIST)pvRequest, (int)psbWalk->cbRequest))
            {
                ...
                else if (ROOT_IsValidID((LPITEMIDLIST)pvRequest))
                {
                     ...
                     *((DWORD*)pvRequest) != 0 && // the first 4 bytes are 0 -> so this definitely isn't a UNICODE string
