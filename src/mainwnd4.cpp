...
BOOL CMainWindow::CanUnloadPlugin(HWND parent, CPluginInterfaceAbstract* plugin)
{
    ...
    // find detached FS belonging to the plug-in and attempt to close them
    int i;
    for (i = DetachedFSList->Count - 1; i >= 0; i--) // iterate backwards; items may be removed (quadratic complexity)
    {
    ...
    // check if data from the plug-in are present in SalShExtPastedData
    // (panels leaving archives due to plug-in unload might have stored them there)
...
void CMainWindow::MakeFileList()
{
    ...
    BeginStopRefresh(); // sniffer takes a break
    ...
    EndStopRefresh(); // the sniffer will start again now
}
...
BOOL GetNextFileFromPanel(int index, char* path, char* name, void* param)
{
    ...
    else
    {
        ...
        data->Window->SetSel(FALSE, -1, TRUE);                        // force redraw
...
*/
    ...
    // WARNING: when a .bat file runs an .exe passing all parameters (%*), a long .exe name
    // can still trigger "too long name" even when respecting the limit here. Handling this
    // properly would require parsing .bat files etc., which is too crazy to implement.
    ...
    if (buildBat)                                   // launching via a .bat file
    {
        if (WindowsVistaAndLater)
            return cmdLineLen <= 8191; // Vista/Win7: in reality it's 32776 but only 8191 works (longer parameters probably cause a Windows bug wiping characters)
...
// cmdSize      - size of 'cmd'
// args         - buffer receiving arguments
// argsSize     - size of 'args'
// buildBat     - if TRUE, arguments are placed into 'cmd'
// initDir      - buffer with the working directory for execution
// initDirSize  - length of initDir
// item         - user-menu item
// path         - long path to the file
// longName     - long filename
// fileNameUsed - set to TRUE when an argument expansion uses the file name or path
// userMenuAdvancedData - advanced parameters for User Menu: the Arguments array
        ...
        _TRY_AGAIN:
            ...
            while (getNextFile(index, path, name, data))
            {
                ...
                BOOL expandOK = ExpandCommand2(parent,
                                               ...
                                               arguments, USRMNUARGS_MAXLEN, buildBat, // if running via a batch file, allow
                ...
                if (expandOK && (firstRound || fileNameUsed || strcmp(initDir, prevInitDir) != 0)) // block running the same command for all items (a user mistake that happens often)
                {
                    ...
                    else // direct execution
                    {
                        ...
                        // this will also honor startup policies
                ...
                if (userMenuValidationData.MustHandleItemsAsGroup)
                    break; // in this mode only one command is run for all selected items
            }
...
BOOL CMainWindow::HandleCtrlLetter(char c)
{
    ...
    {
        WPARAM cmd;
        switch (c) // everything here is upper-case
        {
        case 'A':
...
void CMainWindow::FocusPanel(CFilesWindow* focus, BOOL testIfMainWndActive)
{
    ...
    {
        if (!testIfMainWndActive || GetForegroundWindow() == HWindow) // focus only if main window is active (FTP plugin with non-modal Welcome Message stole focus when command line was closed)
    ...
    while (HelpMode)
    {
        ...
        else
        {
            if (first)
            {
                ...
                // when the tooltip vanished, WM_MOUSELEAVE was delivered (PeekMessage distributes messages).
                // The button under the cursor then drew as enabled and immediately after as disabled again.
...
BOOL CMainWindow::ProcessHelpMsg(MSG& msg, DWORD* pContext, HWND* hDirtyWindow)
{
            ...
            if (msg.message != WM_LBUTTONDOWN)
            {
                ...
                if (msg.message == WM_MOUSEMOVE)
                {
                    if (*hDirtyWindow != NULL && *hDirtyWindow != hWndHit)
                        SendMessage(*hDirtyWindow, WM_USER_HELP_MOUSELEAVE, 0, 0);
                    *hDirtyWindow = hWndHit; // this window will need a LEAVE message
             ...
             (msg.message >= WM_KEYFIRST && msg.message <= WM_KEYLAST))
    {
        ...
        if (PeekMessage(&msg, NULL, msg.message, msg.message, PM_NOREMOVE))
        {
            ...
            // send messages to our menu (avoiding the need for a keyboard hook)
