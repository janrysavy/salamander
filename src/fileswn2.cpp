...
void CFilesWindow::Execute(int index)
{
    ...
    if (Is(ptDisk))
    {
        ...
        {
            if (CheckPath(FALSE) != ERROR_SUCCESS) // current path not accessible
            {
            ...
            if (file->DosName != NULL)
            {
                    ...
                    GetLastError() == ERROR_FILE_NOT_FOUND)
                {
                    ...
                    if (SalPathAppend(fullPath, file->DosName, MAX_PATH) &&
                        SalGetFileAttributes(fullPath) != INVALID_FILE_ATTRIBUTES)
                    { // when full name not available (problem converting from multibyte to UNICODE), use DOS name
            ...
            if (StrICmp(file->Ext, "lnk") == 0) // not a directory shortcut?
            {
                                     ...
                                     (LPVOID*)&link) == S_OK)
                {
                    ...
                    if (link->QueryInterface(IID_IPersistFile, (LPVOID*)&fileInt) == S_OK)
                    {
                        if (fileInt->Load(oleName, STGM_READ) == S_OK)
                        {
                            ...
                            {                                     // test path for accessibility, Resolve may change it
                                ...
                                {
                                    err = ERROR_SUCCESS; // Resolve can change the path, then we test again
                                }
                            }
                            if (err == ERROR_SUCCESS)
                            {
                                if (link->Resolve(HWindow, SLR_ANY_MATCH | SLR_UPDATE) == NOERROR)
                                {
                                    if (link->GetPath(fullName, MAX_PATH, &data, SLGP_UNCPRIORITY) == NOERROR)
                                    {
                                        // final form of fullName - verify it is OK
                                        err = CheckPath(TRUE, fullName); // fullName is a full path (shortcuts support no other)
                                        if (err == ERROR_SUCCESS)
                                        {
                                            DWORD attr = SalGetFileAttributes(fullName); // obtained here because data.dwFileAttributes isn't filled
                                            if (attr != INVALID_FILE_ATTRIBUTES && (attr & FILE_ATTRIBUTE_DIRECTORY))
                                            {
                                                linkIsDir = TRUE; // OK try change-path-to-disk
                                            }
                                            else
                                            {
                                                linkIsFile = TRUE; // OK check if it's an archive
                                            }
                                        }
                                    }
                                    else // links directly to servers can be opened in Network plugin (Nethood)
                                    {
                                        if (Plugins.GetFirstNethoodPluginFSName(netFSName))
                                        {
                                            if (link->GetPath(fullName, MAX_PATH, NULL, SLGP_RAWPATH) != NOERROR)
                                            { // path not stored as text in link, only as an ID list
                                                fullName[0] = 0;
                                                ITEMIDLIST* pidl;
                                            ...
                                            { // check if it's a link to a server (path contains "\\\\server")
                                                char* backslash = fullName + 2;
                                                while (*backslash != 0 && *backslash != '\\')
                                                    backslash++;
                                                if (*backslash == '\\')
                                                    backslash++;
                                                if (*backslash == 0)  // accept only paths "\\\\", "\\\\server", "\\\\server\\"
                                                    linkIsNet = TRUE; // OK try change-path-to-FS
                                            }
                                        }
                                    }
                                }
            ...
            if (linkIsDir || linkIsNet) // link points to network or directory, path is OK, switch to it
            {
            ...
            if (PackerFormatConfig.PackIsArchive(linkIsFile ? fullName : fileName)) // is it an archive?
            {
                ...
                if (!linkIsFile)
                {
                    // construct full archive name for ChangePathToArchive
                    strcpy(fullName, GetPath());
                    if (!SalPathAppend(fullName, fileName, MAX_PATH))
                    {
                        SalMessageBox(HWindow, LoadStr(IDS_TOOLONGNAME), LoadStr(IDS_ERRORCHANGINGDIR),
                ...
                BOOL noChange;
                if (ChangePathToArchive(fullName, "", -1, NULL, FALSE, &noChange)) // succeeded entering the archive
                {
                    if (linkIsFile)
                        TopIndexMem.Clear(); // long jump
                    else
            ...
            // launch the default context menu item (association)
            HCURSOR oldCur = SetCursor(LoadCursor(NULL, IDC_WAIT));
            MainWindow->SetDefaultDirectories(); // ensure the launched process inherits the correct current directories
        ...
        else // directory
        {
            ...
            if (index == 0 && strcmp(dir->Name, "..") == 0) // ".. <Up>"
            {
                ...
                if (!CutDirectory(path, &prevDir))
                {
                    ...
                    EndStopRefresh();
                    return; // nothing to shorten or already on Nethood path
                }
            ...
            else // subdirectory
            {
                ...
                // backup caret index for access-denied directory
                ...
                // Vista: handle unlistable junction points - change path to junction target
                ...
                {
                    if (!IsTheSamePath(path, GetPath())) // not on original path -> long jump
                    {                                    // the condition "!noChange" is not enough - it signals "path changed or reloaded - access-denied-dir"
                        TopIndexMem.Clear();
                    }
                    else // path unchanged (immediately shortened back)
                    {
                        refresh = FALSE;
                        if (!noChange) // access-denied-dir: listing refreshed, but path unchanged
                        {              // restore listbox indices and finish "refresh" after ChangePathToDisk
    ...
    else
    {
        if (Is(ptZIPArchive))
        {
            ...
            else // directory
            {
                ...
                if (index == 0 && strcmp(dir->Name, "..") == 0) // ".. <Up>"
                {
                    if (GetZIPPath()[0] == 0) // exit the archive
                    {
                        const char* s = strrchr(GetZIPArchive(), '\\'); // ZIP archive doesn't contain an extra trailing '\\'
                        if (s != NULL)                                  // always true
                        {
                            strcpy(path, s + 1); // previous directory
                            ...
                            { // failed to shorten the path - reject-close-archive or long jump
                                ...
                                else
                                {
                                    ...
                                    {
                                        TopIndexMem.Push(GetPath(), topIndex); // restore top index for next time
                                    }
                                }
                            }
                        }
                    }
                    else // shortening path inside archive
                    {
                        // split zip-path into new zip-path and prev-dir
                        ...
                        // build shortened archive path and obtain top index
                ...
                else // subdirectory
                {
                    ...
                    else
                    {
                        ...
                        {
                            TopIndexMem.Push(doublePath, topIndex); // remember top index for return
                        }
                        else
                        {
                            if (!noChange)
        ...
        else
        {
            if (Is(ptPluginFS))
            {
                ...
                char fsNameBuf[MAX_PATH]; // GetPluginFS() may cease to exist, so copy fs name locally
...
void CFilesWindow::ChangeSortType(CSortType newType, BOOL reverse, BOOL force)
{
    ...
    //---  store the focused item and sort old items by name
    ...
    //---  select item for focus and perform final sorting
    CLessFunction lessDirs;  // used to compare which is smaller; needed for optimization in the search loop
    CLessFunction lessFiles; // used to compare which is smaller; needed for optimization in the search loop
    switch (SortType)
    {
    case stName:
    ...
    int i;
    int count;
    if (focusIndex < Dirs->Count) // searching for a directory
    {
        i = 0;
        count = Dirs->Count;
    }
    ...
    if (d1.Name != NULL)
    {
        if (i == 0 && Dirs->Count > 0 && strcmp(Dirs->At(0).Name, "..") == 0)
        {
            if (strcmp(d1.Name, "..") == 0)
            {
                ...
                i = 1; // must skip ".."; it's not included
        }
        for (; i < count; i++)
        {
            if (i < Dirs->Count)
    ...
    if (focusIndex >= count)
        focusIndex = count - 1;
    //---  use the acquired data for the final listbox setup
                                                  ...
                                                  BOOL canForce, int tryCloseReason, int* failReason)
{
        ...
        ifPathIsInaccessibleGoTo[0] != 0 && ifPathIsInaccessibleGoTo[1] == ':')
    {
        ...
        if (failReasonInt == CHPPFR_CANNOTCLOSEPATH)
        {
            ...
            return FALSE; // the issue "cannot close the current path" won't be solved by switching to a fixed drive (we would just ask about disconnect twice)
        }
        noChangeUsed = TRUE;
    }
    else
...
void CFilesWindow::DisconnectNet()
{
    ...
    // disconnect from the mapped drive, otherwise it cannot be detached silently (system warns it's in use)
...
void CFilesWindow::ToggleDirectoryLine()
{
    ...
    // if the middle toolbar is displayed, give it a chance to reposition itself
...
BOOL CFilesWindow::IsViewTemplateValid(int templateIndex)
{
    ...
    if (templateIndex < 1) // tree not supported yet
        return FALSE;
                                      ...
                                      BOOL preserveTopIndex, BOOL salamanderIsStarting)
{
    ...
    if (lstrlen(newTemplate->Name) == 0)
    {
        // undefined view not desired - force the "detailed" view which always exists
        templateIndex = 2;
    ...
    if (!calledFromPluginBeforeListing)
    {
        ...
        else
        {
            ...
            // let it compute item sizes and ensure the focused item is visible
            // if 'preserveTopIndex' is TRUE, we must not scroll the panel beyond the focus
            RefreshListBox(-1, preserveTopIndex ? ListBox->TopIndex : -1, FocusedIndex, FALSE, FALSE);
        }
    }

    // when the panel mode changes we must clear saved top-indices (stored data aren't compatible)
...
void CFilesWindow::RedrawIndex(int index)
{
    ...
    if (index >= 0 && index < Dirs->Count + Files->Count)
        ...
        ListBox->PaintAllItems(NULL, 0); // ensure the empty panel text is drawn
}
                                           ...
                                           int tryCloseReason)
{
    ...
    else
    {
        if (Is(ptZIPArchive))
        {
            ...
            if (AssocUsed)
            {
                // if the user didn't suppress it, show info about closing an archive that contains edited files
                ...
                // pack modified files again (only if not a critical shutdown) so they are ready for next use
                UnpackedAssocFiles.CheckAndPackAndClear(parent, &someFilesChanged);
                // during critical shutdown we pretend updated files don't exist because
                // there is no time to pack them back. We must not delete them so the user
                // can manually repack them after restart; exception is when nothing was
                // edited -- then everything may be deleted even during critical shutdown
            ...
            // in the other panel, remove its cached files; it will unpack again next time
            // (the archive might have been edited in the meantime)
            ...
            {
                StrICpy(buf, GetZIPArchive()); // the disk cache stores the archive name in lowercase (allows case-insensitive comparison of the Windows file system name)
                DiskCache.FlushCache(buf);
            }

            // call the plugin's CPluginInterfaceAbstract::CanCloseArchive
            BOOL canclose = TRUE;
            ...
            if (format != 0) // found a supported archive
            {
                ...
                if (index < 0) // view: internal processing (plugin)?
                {
                    plugin = Plugins.Get(-index - 1);
                    if (plugin != NULL)
                    {
                        if (!plugin->CanCloseArchive(this, GetZIPArchive(), CriticalShutdown)) // the plugin refuses to close
                        {
                            canclose = FALSE;
                ...
                if (PackerFormatConfig.GetUsePacker(format)) // supports editing?
                {
                    ...
                    if (index < 0) // internal processing (plugin)?
                    {
                        if (plugin != Plugins.Get(-index - 1)) // if view == edit, don't call again
                        {
                            ...
                            if (plugin != NULL)
                            {
                                if (!plugin->CanCloseArchive(this, GetZIPArchive(), userForce || CriticalShutdown)) // the plugin refuses to close
                                {
                                    canclose = FALSE;
        ...
        else
        {
            if (Is(ptPluginFS))
            {
                ...
                {
                    detachFS = FALSE; // ensure a known value in case the plugin doesn't modify it
                    BOOL r = GetPluginFS()->TryCloseOrDetach(FALSE, canDetach, detachFS, tryCloseReason);
                    if (!r || !canDetach)
                        detachFS = FALSE; // verify/correct the output value
                    return r;
                }
    ...
    if (Is(ptDisk))
    {
        if (!cancel)
        {
            if (UserWorkedOnThisPath)
            {
                ...
                // HICON hIcon = GetFileOrPathIconAux(path, FALSE, TRUE); // retrieve the icon
    ...
    else
    {
        if (Is(ptZIPArchive))
        {
            if (!cancel)
            {
                ...
                // if SalShExtPastedData is interested in the archive data, pass them on
                ...
                if (!cancel)
                {
                    ...
                    if (detachFS) // detaching only -> add to MainWindow->DetachedFS
                    {
                        ...
                        if (!MainWindow->DetachedFSList->IsGood())
                        {
                            ...
                            sendDetachEvent = TRUE; // the call mustn't happen here because the FS isn't detached yet (still in the panel)
                    }

                    if (!detachFS) // closing the FS; let it deallocate and display final message boxes
                    {
                    ...
                    if (sendDetachEvent && detachedFS != NULL /* always true */)
                        detachedFS->Event(FSE_DETACHED, GetPanelCode()); // send notification about successful plugin detachment
                }
                else
...
void CFilesWindow::RefreshPathHistoryData()
{
    ...
    if (index >= 0 && index < Files->Count + Dirs->Count) // as long as listing data aren't being changed
    {
        int topIndex = ListBox->GetTopIndex();
        CFileData* file = index < Dirs->Count ? &Dirs->At(index) : &Files->At(index - Dirs->Count);

        // try to record a new top-index and focus-name
        if (Is(ptZIPArchive))
        {
                                    ...
                                    BOOL shorterPathWarning, int tryCloseReason)
{
    ...
    lstrcpyn(backup, path, MAX_PATH); // must be done before UpdateDefaultDir (path may point to DefaultDir[])
    ...
    // restore panel state (top-index + focused-name) before potentially closing this path
    ...
    { // change within ptDisk or we can close the current path, try to open a new one
...
*/
            BOOL updateIcon;
            updateIcon = !Is(ptDisk) || // simplified because forceUpdate is commented out
            ...
            { // executed only during the first pass, so we can use "Is(ptDisk)" and "GetPath()"
                ...
                if (samePath)
                {
                    // do not remember the closed path (to avoid adding it when changing to the same path)
                    oldCanAddToDirHistory = MainWindow->CanAddToDirHistory;
                    MainWindow->CanAddToDirHistory = FALSE;
                }

                CloseCurrentPath(parent, FALSE, detachFS, samePath, isRefresh, !samePath); // success, switch to the new path
                ...
                // hide the throbber and security icon; we don't need them on a disk path
            ...
            SetPath(changedPath);
            if (updateIcon ||
                !GetNetworkDrive()) // ensure icons display correctly when switching to a mounted volume (doesn't slow on local machines)
            {
                UpdateDriveIcon(FALSE);
            }
            if (noChange != NULL)
                *noChange = FALSE; // the listing was released
            forceUpdateInt = TRUE; // something must load now, otherwise the panel will remain empty

            // let the new path load its contents
            BOOL cannotList;
            ...
            {                                                                                                                // auto-refresh failure; verify whether the directory displayed in the panel is being deleted from another machine. If ignored, the panel will never refresh because auto-refresh is broken
                Sleep(400);                                                                                                  // give the deletion a moment to proceed so the directory becomes unlistable
            ...
            FIXED_DRIVE:
                ...
                BOOL change = FALSE;
                if (fixedDrive || !CutDirectory(changedPath)) // attempt to shorten the path
                {
                    if (canTryUserRescuePath) // first try the user's "rescue path"
                    {
                        canTryUserRescuePath = FALSE; // don't try it more than once
                        openIfPathIsInaccessibleGoToCfg = TRUE;
                        fixedDrive = FALSE; // allow switching to a fixed drive (perhaps tried already but the user path had priority)
                        GetIfPathIsInaccessibleGoTo(changedPath);
                        shorterPathWarning = TRUE; // want to see errors for the rescue path
                        change = TRUE;
                    }
                    else
                    {
                        ...
                        // cannot shorten, find the system or first fixed drive (our "escape drive")
                        char sysDir[MAX_PATH];
                        char root[4] = " :\\";
                        BOOL done = FALSE;
                        if (GetWindowsDirectory(sysDir, MAX_PATH) != 0 && sysDir[0] != 0 && sysDir[1] == ':')
                        ...
                        {
                            DWORD disks = GetLogicalDrives();
                            disks >>= 2; // skip A: and B:, when formatting floppies they sometimes become DRIVE_FIXED
                            char d = 'C';
                            while (d <= 'Z')
                            {
                                if (disks & 1)
                        ...
                        if (done)
                        {
                            ...
                            {                                                    // UNC or another disk (like "c:\")
                                strcpy(changedPath, root);                       // try our "escape drive"
                                change = TRUE;
                            }
                        }
                    }
                ...
                if (change) // only for a new path; otherwise leave the panel empty (shouldn't happen, fixed drive is our safety net)
                {
                    ...
                    suggestedTopIndex = -1;
                    suggestedFocusName = NULL;
                    // the "new" path is changing (UserWorkedOnThisPath may have survived in CloseCurrentPath)
                    UserWorkedOnThisPath = FALSE;
                    // try listing the adjusted path
                    goto _TRY_AGAIN;
                }
                if (failReason != NULL)
                    *failReason = CHPPFR_INVALIDPATH;
            ...
            else
            {
                ...
                {                        // if it's not a refresh and warnings about shortened path should be shown ...
                    if (!refreshListBox) // we'll display a message, so refresh the listbox
                    {
                        RefreshListBox(0, -1, -1, FALSE, FALSE);
                    }
                    // report the error that caused the path to be shortened
        ...
        {
            if (err == ERROR_NOT_READY) // the drive isn't ready (removable media)
            {
                ...
                { // allow entering the root when a volume is mounted (F:\DRIVE_CD -> F:\)
            ...
            if (forceUpdateInt && !fixedDrive) // if an update is needed, try the fixed drive
            {
                fixedDrive = TRUE; // prevent looping + switch to the fixed drive
                goto FIXED_DRIVE;
            }
            if (failReason != NULL)
                *failReason = CHPPFR_INVALIDPATH;
                                       ...
                                       BOOL isHistory)
{
    ...
    BOOL sameArch;
    BOOL checkPath = TRUE;
    BOOL forceUpdateInt = FALSE; // path change required? (possibly even to disk)
    BOOL tryPathWithArchiveOnError = isHistory;
    if (!Is(ptZIPArchive) || StrICmp(GetZIPArchive(), archive) != 0) // not in an archive or a different archive
    {
    ...
    _REOPEN_ARCHIVE:
        ...
        { // the current path can be closed, try to open the new one
            ...
            ERROR_1:
                ...
                {                   // we're certainly in an archive (a panel refresh of an archive)
            ...
            { // path isn't accessible or it is truncated (the archive cannot be opened)
                ...
                if (tryPathWithArchiveOnError)
                    tryPathWithArchiveOnError = (err == ERROR_SUCCESS && !pathInvalid); // shorter path is accessible, try it
                if (!isRefresh)                                                         // during refresh path-shortening warnings are not shown
                {
            ...
            {
                // retrieve file info (exists?, size, date & time)
                DWORD err2 = NO_ERROR;
                ...
                if (file != INVALID_HANDLE_VALUE)
                {
                    ...
                    SalGetFileSize(file, archiveSize, err2); // returns "success?" - ignored, err2 is checked later
                ...
                if (err2 != NO_ERROR)
                {
                    if (!isRefresh) // during refresh no missing-path warnings are shown
                ...
                if (nullFile || PackList(this, archive, *newArchiveDir, pluginData, plugin))
                {
                    ...
                    BOOL isTheSamePath = FALSE; // TRUE if the path doesn't actually change
                    ...
                    // success, switch to the new path - listing archives can take time
                    // so the path changes even if the target doesn't exist; applies to
                    ...
                    // hide the throbber and security icon; archives don't need them
                    if (DirectoryLine != NULL)
                        DirectoryLine->HideThrobberAndSecurityIcon();
                    ...
                    if (noChange != NULL)
                        *noChange = FALSE;
                    // ZIPPath, Files and Dirs are set later once archivePath is assigned...
                    if (failReason != NULL)
                        *failReason = CHPPFR_SUCCESS;
                }
                else
    ...
    {
        if (forceUpdate) // should check whether the archive changed?
        {
            DWORD err;
            if ((err = CheckPath(!isRefresh)) == ERROR_SUCCESS) // no need to restore network connections here ...
            {
                ...
                if (file != INVALID_HANDLE_VALUE)
                {
                    ...
                    {
                        if (AssocUsed) // any files from the archive are edited?
                        {
                            // notify that there were changes and editors should be closed
            ...
            if (err != ERROR_SUCCESS) // switch to an existing path
            {
                ...
                {
                    // if possible, exit the archive (maybe all the way to the "fixed drive")
                    ChangePathToDisk(HWindow, GetPath(), -1, NULL, noChange, refreshListBox, FALSE, isRefresh);
                }
                else // user pressed ESC -> the path is probably inaccessible, go straight to the "fixed drive"
                {
    ...
    while (path[0] != 0 && GetArchiveDirFiles() == NULL)
    {
        ...
        useFileName = (canFocusFileName && suggestedFocusName == NULL && fileName == NULL); // allow focusing the file only for the first shortening when no external focus is suggested
        if (end != NULL)
        {
            *end = 0;
            fileName = end + 1;
        ...
        if (!sameArch)
        {
            // the "new" path is changing (CloseCurrentPath might have left UserWorkedOnThisPath == TRUE)
            UserWorkedOnThisPath = FALSE;
        }
    }

    if (!useFileName && sameArch && GetArchiveDir()->SalDirStrCmp(currentPath, GetZIPPath()) == 0) // not focusing a file and the shortened path matches the current one
    {                                                                                              // occurs for example when attempting to enter an inaccessible directory (immediate return)
        EndStopRefresh();
        if (setWait)
            SetCursor(oldCur);
    ...
    // this must succeed (at least the archive root always exists)
    ...
    { // attempt to focus a file failed -> it wasn't a file name
        if (failReason != NULL)
            *failReason = CHPPFR_SHORTERPATH;
    }
    ...
    // add the path we just left (paths inside the archive aren't closed,
                                         ...
                                         BOOL forceUpdate, char* cutFileName, BOOL* keepOldListing)
{
    ...
    const char* origUserPart; // user part to switch the path to
    int origFSNameIndex;
    if (fsUserPart == NULL) // detached FS - restoring the listing...
    {
    ...
    while (1)
    {
        ...
        { // the path looks OK
            if (pathWasCut && cutFileName != NULL && *cutFileName == 0)
                useCutFileName = FALSE;
            if (firstCall) // path change within the FS, original listing not released (would suffice?)
            {
