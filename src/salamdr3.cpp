...
BOOL SalPathAddExtension(char* path, const char* extension, int pathSize)
{
    ...
    while (iterator >= path)
    {
        if (*iterator == '.')
        {
            //      if (iterator != path && *(iterator - 1) != '\\')  // ".cvspass" is considered an extension on Windows ...
...
int HandleFileException(EXCEPTION_POINTERS* e, char* fileMem, DWORD fileMemSize)
{
    ...
    {
        if (e->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION &&    // an access violation only signals a file error when the failing address belongs to the file
            (e->ExceptionRecord->NumberParameters >= 2 &&                         // we have something to test
             e->ExceptionRecord->ExceptionInformation[1] >= (ULONG_PTR)fileMem && // pointer to the fault inside the mapped file
        ...
        {
            return EXCEPTION_CONTINUE_SEARCH; // pass the exception farther up the call stack
        }
    }
}
...
{
    char* d = afterRoot; // pointer just past the root portion of the path
    while (*d != 0)
    {
        while (*d != 0 && *d != '.')
            d++;
            ...
            if (d == afterRoot || d > afterRoot && *(d - 1) == '\\') // '.' right after the root path or "\."
            {
                if (*(d + 1) == '.' && (*(d + 2) == '\\' || *(d + 2) == 0)) // ".."
                {
                    ...
                    while (l > afterRoot && *(l - 1) != '\\')
                        l--;
                    if (l >= afterRoot) // drop the directory and ".."
                    {
                        if (*(d + 2) == 0)
                            *l = 0;
                        else
...
{
    WCHAR* d = afterRoot; // pointer just past the root portion of the path
    while (*d != 0)
    {
        while (*d != 0 && *d != L'.')
            d++;
                    ...
                    BOOL* callNethood, int nameBufSize, BOOL allowRelPathWithSpaces)
{
    ...
    int rootOffset = 3; // offset of the directory part of the path (3 for "c:\path")
    char* s = name;
    while (*s >= 1 && *s <= ' ')
        s++;
        ...
        else
        {
            ...
            else
            {
                ...
                if (s - name > MAX_PATH - 1)
                    err = IDS_SERVERNAMEMISSING; // the text found is too long to be a server name
                else
                {
                    if (*s == 0 || *s == '\\')
                    {
                        if (callNethood != NULL)
                            *callNethood = *s == 0 && (*(s - 1) != '.' || *(s - 2) != '\\') && (*(s - 1) != '\\' || *(s - 2) != '.' || *(s - 3) != '\\'); // not "\\." or "\\.\" (the start of a path like "\\.\C:\")
                    ...
                    else
                    {
                        ...
                        if ((s - name) + 1 > MAX_PATH - 1)
                            err = IDS_SHARENAMEMISSING; // the text found is too long to be a share name (+1 for the trailing backslash)
                        if (*s == '\\')
                            s++;
                    }
                }
                    ...
                    if (*s == '\\') // "\path...."
                    {
                        if (curDir[0] == '\\' && curDir[1] == '\\') // UNC
                        {
                            ...
                            if (l1 + (root - curDir) >= nameBufSize)
                                err = IDS_TOOLONGPATH;
                            else // build the path starting from the current drive root
                            {
    ...
    if (err == 0) // eliminate '.' and '..' in the path
    {
        if (!SalRemovePointsFromPath(s))
            err = IDS_PATHISINVALID;
    }

    if (err == 0) // remove any unwanted trailing backslash from the string
    {
...
void AuxThreadBody(BOOL add, HANDLE thread, BOOL testIfFinished)
{
    ...
    if (!finished) // after TerminateAuxThreads() nothing new is accepted
    {
        if (add)
        {
            // purge threads that have already finished
        ...
        else
        {
            ...
            for (int i = 0; i < AuxThreads.Count; i++)
            {
                ...
                { // the thread is still running, terminate it
                    TerminateThread(t, 666);
                    WaitForSingleObject(t, INFINITE); // wait for the thread to really finish, it sometimes takes a while
                }
...
class CStopRefreshStack
{
  protected:
    ...
    int Ignored;                                      // number of BeginStopRefresh() calls we had to ignore (STOPREFRESHSTACKSIZE too small -> enlarge if needed)

  public:
    CStopRefreshStack() {Count = 0; Ignored = 0;}
    ~CStopRefreshStack() {CheckIfEmpty(3);} // three BeginStopRefresh() calls are OK: both panels call BeginStopRefresh() and the third comes from WM_USER_CLOSE_MAINWND (invoked first)
...
{
    /*
#ifdef _DEBUG     // verify that BeginStopRefresh() and EndStopRefresh() are called from the same function (according to the caller's return address -> it will not detect a "mistake" when two different functions are both called from the same parent)
  DWORD *register_ebp;
...
*/
    ...
    else
    {
        if (--StopRefresh == 0)
        {
            ...
            // if we blocked a refresh, give it a chance to run now
            ...
            if (MainWindow != NULL && MainWindow->NeedToResentDispachChangeNotif &&
                !AlreadyInPlugin) // if we are still inside a plug-in, sending the notification is pointless
            {
                MainWindow->NeedToResentDispachChangeNotif = FALSE;

                // post a request to broadcast notifications about path changes
...
void RemoveTemporaryDir(const char* dir)
{
    ...
    SetCurrentDirectory(dir); // helps it run faster (the system prefers the current directory)
    if (strlen(dir) < MAX_PATH)
        _RemoveTemporaryDir(dir);
    SetCurrentDirectoryToSystem(); // we must leave it or it cannot be deleted
...
AGAIN:
    ...
    char name[MAX_PATH];
    if (attrs == 0xFFFFFFFF) // probably does not exist, allow creating it
    {
        char root[MAX_PATH];
        GetRootPath(root, dir);
        if (dirLen <= (int)strlen(root)) // the directory is a root folder
        {
        ...
        if (!quiet)
        {
            // if the user did not suppress it, show information about the missing directory
        ...
        if (quiet || msgBoxRet == IDOK)
        {
            ...
            while (*st != 0)
            {
                BOOL invalidName = manualCrDir && *st <= ' '; // leading spaces are undesirable when creating directories manually (Windows allows it, but it is potentially dangerous)
...
void CPathHistoryItem::GetPath(char* buffer, int bufferSize)
{
    ...
    // we must duplicate every '&' otherwise they become underlines
            ...
            else
            {
                if (Type == 2) // FS
                {
                    ...
                    if (MainWindow != NULL && PluginFS != NULL && // when the FS interface is known
                        (!panel->Is(ptPluginFS) ||                // and it is not currently in the panel
                         !panel->GetPluginFS()->Contains(PluginFS)))
                    {
                        ...
                        for (i = 0; i < list->Count; i++)
                        {
                            if (list->At(i)->Contains(PluginFS))
                            {
                                ...
                                // try to switch to the requested path (it was there last time, no need to test IsOurPath),
                    ...
                    // try to find a detached FS interface that can list it (to avoid opening a new FS unnecessarily)
                    int fsNameIndex;
                    ...
                    // if nothing else works, open a new FS interface or just change the path on the active FS interface
                    if (!done)
                    {
                                                         ...
                                                         FocusedName, FALSE, 2, NULL, TRUE, &failReason))
                        {
                            if (failReason == CHPPFR_SHORTERPATH ||   // almost success (the path was only shortened)
                                failReason == CHPPFR_FILENAMEFOCUSED) // almost success (the path changed to a file and it was focused)
                            {                                         // refresh the record about the FS interface
                                if (panel->Is(ptPluginFS))
                                    PluginFS = panel->GetPluginFS()->GetInterface();
                            }
                ...
                if (Type == 2) // FS
                {
                    if (StrICmp(PathOrArchiveOrFSName, item.PathOrArchiveOrFSName) == 0) // the FS name is case-insensitive
                    {
                        if (strcmp(ArchivePathOrFSUserPart, item.ArchivePathOrFSUserPart) == 0) // the FS user part is case-sensitive
                            return TRUE;
                        if (curPluginFS != NULL && // handle the scenario where both FS user parts match because the FS reports TRUE from IsCurrentPath for them (we would generally need a method to compare two FS user parts, but that feels excessive just for history purposes; maybe later...)
    ...
    if (Paths.Count > 0)
    {
        ...
        for (i = 0; i < Paths.Count; i++)
        {
            ...
            if (n->IsTheSamePath(*item, curPluginFS))
            {
                ...
                { // FS case: replace pluginFS so the path opens on the most recent FS used for it
                ...
                if (i < Paths.Count - 1)
                {
                    ...
                    Paths.Add(item);
                    if (Paths.IsGood())
                        Paths.Detach(i); // if the add succeeded, remove the original entry
                    if (!Paths.IsGood())
                        Paths.ResetState();
                }
                return; // same paths -> nothing to do
    ...
    if (CreateKey(hKey, name, historyKey))
    {
        ...
        if (!onlyClear) // if we are not just clearing the key, store the history values
        {
            ...
            for (i = 0; i < Paths.Count; i++)
            {
                ...
                // archive & FS: use ':' to separate the two parts of the path
                // during load we use this character to determine the path type
                case 1: // archive
                case 2: // FS
                {
                    strcpy(path, item->PathOrArchiveOrFSName);
...
CUserMenuIconDataArr::GiveIconForUMI(const char* fileName, DWORD iconIndex, const char* umCommand)
{
    ...
    for (int i = 0; i < Count; i++)
    {
            ...
            strcmp(item->UMCommand, umCommand) == 0)
        {
            ...
            item->Clear();                 // avoid compressing the array when deleting (slow and unnecessary); just clear the item so searches skip it quickly
            return icon;
        }
    }
...
void CUserMenuIconBkgndReader::StartBkgndReadingIcons(CUserMenuIconDataArr* bkgndReaderData)
{
    ...
    if (!AlreadyStopped && bkgndReaderData != NULL && bkgndReaderData->Count > 0)
    {
        ...
        if (thread != NULL)
        {
            // the main thread runs at a higher priority; to read icons as fast as before moving work to a background thread,
            // boost the priority here as well
            SetThreadPriority(thread, THREAD_PRIORITY_ABOVE_NORMAL);

            bkgndReaderData = NULL; // handed over to the thread, do not free them here
    ...
    // pause briefly; if the icons load quickly, the "simple" variants never appear (less flicker)
    // additionally, some users reported that reading icons into the panel in parallel noticeably slowed down
    // loading icons for the user menu, so the toolbar icons showed up with a large delay.
    // This delay should prevent that, affecting only slow user menu icon loading—which is the goal of this tweak.
    if (thread != NULL)
    {
...
void CUserMenuIconBkgndReader::EndUserMenuIconsInUse()
{
    ...
    else
    {
        ...
        { // last lock released: if there is pending data to process, send it
            ...
            else // nobody needs the data anymore, just free it
                delete UserMenuIIU_BkgndReaderData;
            UserMenuIIU_BkgndReaderData = NULL;
            UserMenuIIU_ThreadID = 0;
        }
...
CUserMenuItem::CUserMenuItem(CUserMenuItem& item, CUserMenuIconDataArr* bkgndReaderData)
{
    ...
    if (Type == umitItem)
    {
        ...
        {
            UMIcon = DuplicateIcon(NULL, item.UMIcon); // GetIconHandle() was unnecessarily slow here
...
BOOL CUserMenuItem::GetIconHandle(CUserMenuIconDataArr* bkgndReaderData, BOOL getIconsFromReader)
{
    ...
    if (bkgndReaderData == NULL && fileName[0] != 0 && // load icons immediately here
    ...
    if (bkgndReaderData == NULL && umCommand[0] != 0 && // load icons immediately here
        MainWindow->GetActivePanel() != NULL &&
    ...
    // fall back to the default icon from shell32.dll
...
BOOL CUserMenuItems::LoadUMI(CUserMenuItems& source, BOOL readNewIconsOnBkgnd)
{
    ...
    if (readNewIconsOnBkgnd)
        UserMenuIconBkgndReader.StartBkgndReadingIcons(bkgndReaderData); // NOTE: this call frees 'bkgndReaderData'
    return TRUE;
}
...
BOOL PostMouseWheelMessage(MSG* pMSG)
{
    ...
    if (hWindow != NULL)
    {
        ...
        if (GetClassName(hWindow, className, 100) != 0)
        {
            ...
            // under the cursor; in that case do not route the message to the "proper" window below the cursor,
            ...
            else
            {
                ...
                if (winProcessId != GetCurrentProcessId()) // WM_USER_* should not be sent outside our process
                    hWindow = pMSG->hwnd;
            }
        }
        else
...
LRESULT CALLBACK MenuWheelHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    // It worked for me (through this hook). After installing Intellipoint drivers v7 (previously I had no special drivers on Vista x64)
    // WM_MOUSEHWHEEL messages stopped flowing through here and went straight to the Salamander panels.
    // Therefore I disabled this route and captured the messages only in the panel.
    // Note: in theory we could drop WM_MOUSEWHEEL handling in the same way, but I do not want to risk breaking something on older OSes
    // (we can revisit this when moving to Windows 2000 and later).
    // Note 2: if it turns out we must capture WM_MOUSEHWHEEL via this hook as well, we should detect at runtime that the messages
    // pass through here and then disable their processing in the panels and the command line.

    // 30 Nov 2012 - a user on the forum had WM_MOUSEHWEEL bypass the message hook (same as Manison before):
    // https://forum.altap.cz/viewtopic.php?f=24&t=6039
    // so now we also capture the message in each window that might receive it (depending on focus)
                              ...
                              DWORD attr)
{
    ...
    if (ZIPRoot != NULL) // ensure the zip root has no '\\' at either end
    {
...
void CFileTimeStamps::CopyFilesTo(HWND parent, int* indexes, int count, const char* initPath)
{
                           ...
                           LoadStr(IDS_BROWSEARCUPDATETEXT), path, FALSE, initPath))
    {
        ...
        if (ok && !tooLongName)
        {
            ...
            // perform the actual copy—wonderfully easy, though it does crash for some users now and then ;-)
...
void CFileTimeStamps::CheckAndPackAndClear(HWND parent, BOOL* someFilesChanged, BOOL* archMaybeUpdated)
{
    ...
    if (List.Count > 0)
    {
        ...
        // but we must not delete them either—the user needs a chance to pack them manually after restart
        if (!CriticalShutdown)
        {
...
void CTopIndexMem::Push(const char* path, int topIndex)
{
    ...
    {
        if (TopIndexesCount == TOP_INDEX_MEM_SIZE) // need to drop the first stored top index
        {
            int i;
...
BOOL CTopIndexMem::FindAndPop(const char* path, int& topIndex)
{
    ...
    else // query for a different path -> clear the memory because a long jump occurred
    {
        Clear();
        return FALSE;
    }
...
BOOL CFileHistory::AddFile(CFileHistoryItemTypeEnum type, DWORD handlerID, const char* fileName)
{
    ...
    // search existing entries to see whether the item is already present
    int i;
    for (i = 0; i < Files.Count; i++)
    {
        ...
        {
            // if so, just bring it to the top
            if (i > 0)
            {
                Files.Detach(i);
                if (!Files.IsGood())
                    Files.ResetState(); // the only possible failure is insufficient memory when shrinking the array
                Files.Insert(0, item);
                if (!Files.IsGood())
                {
                    Files.ResetState();
...
{
    // starting with Windows Vista, SelectAll works natively, so let the controls handle it
    if (WindowsVistaAndLater)
        return FALSE;
