...
BOOL SalPathAppend(char* path, const char* name, int pathSize)
{
    ...
    if (*name != 0)
    {
        ...
        if (l + 1 + n < pathSize) // will it fit including the null terminator?
        {
            if (l != 0)
                path[l] = '\\';
            else
...
void SalPathRemoveExtension(char* path)
{
    ...
    while (iterator >= path)
    {
        if (*iterator == '.')
        {
            //      if (iterator != path && *(iterator - 1) != '\\')  // Windows treats '.cvspass' as an extension
            *iterator = 0;
            break;
        }
        if (*iterator == '\\')
...
BOOL SalPathRenameExtension(char* path, const char* extension, int pathSize)
{
    ...
    while (iterator >= path)
    {
        if (*iterator == '.')
        {
            //      if (iterator != path && *(iterator - 1) != '\\')  // '.cvspass' is treated as an extension in Windows
            //      {
...
int HandleFileException(EXCEPTION_POINTERS* e, char* fileMem, DWORD fileMemSize)
{
    ...
    {
        return EXCEPTION_EXECUTE_HANDLER; // invoke the __except block
    }
    else
    {
        if (e->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION &&    // access violation means a file error only if the fault address belongs to the file
            (e->ExceptionRecord->NumberParameters >= 2 &&                         // we have something to test
             e->ExceptionRecord->ExceptionInformation[1] >= (ULONG_PTR)fileMem && // fault pointer in the mapped file
...
{
    char* d = afterRoot; // pointer past the root path
    while (*d != 0)
    {
        while (*d != 0 && *d != '.')
            d++;
            ...
            if (d == afterRoot || d > afterRoot && *(d - 1) == '\\') // '.' after the root path or "\."
            {
                if (*(d + 1) == '.' && (*(d + 2) == '\\' || *(d + 2) == 0)) // ".."
                {
                    ...
                    while (l > afterRoot && *(l - 1) != '\\')
                        l--;
                    if (l >= afterRoot) // omitting the directory along with ".."
                    {
                        if (*(d + 2) == 0)
                            *l = 0;
                        else
...
{
    WCHAR* d = afterRoot; // pointer past the root path
    while (*d != 0)
    {
        while (*d != 0 && *d != L'.')
            d++;
                    ...
                    BOOL* callNethood, int nameBufSize, BOOL allowRelPathWithSpaces)
{
    ...
    int rootOffset = 3; // offset of the directory part start (3 for "c:\path")
    char* s = name;
    while (*s >= 1 && *s <= ' ')
        s++;
    if (*s == '\\' && *(s + 1) == '\\') // UNC (\\server\share\...)
    {                                   // removing leading spaces from the path
        if (s != name)
            memmove(name, s, strlen(s) + 1);
        s = name + 2;
        if (*s == '.' || *s == '?')
            err = IDS_PATHISINVALID; // paths like \\?\Volume{6e76293d-1828-11df-8f3c-806e6f6e6963}\ or \\.\PhysicalDisk5\ are not supported here
        else
        {
            if (*s == 0 || *s == '\\')
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        if (callNethood != NULL)
                            *callNethood = *s == 0 && (*(s - 1) != '.' || *(s - 2) != '\\') && (*(s - 1) != '\\' || *(s - 2) != '.' || *(s - 3) != '\\'); // not "\\." or "\\.\" (start of path "\\.\C:\")
    ...
    else // path specified using a drive (c:\...)
    {
        if (*s != 0)
        {
            ...
            else
            {
                if (curDir != NULL)
                {
                    // for relative paths without a leading '\\' we do not consider
                    // spaces an error when 'allowRelPathWithSpaces' is enabled
                    // a directory or file name can start with a space even though Windows and other software,
                    // including Salamander, try to avoid it
                    ...
                    if (*s == '\\') // "\path...."
                    {
                        if (curDir[0] == '\\' && curDir[1] == '\\') // UNC
                        {
                            ...
                            if (l1 + (root - curDir) >= nameBufSize)
                                err = IDS_TOOLONGPATH;
                            else // construct path from the current drive root
                            {
    ...
    if (err == 0) // remove any unwanted trailing backslash
    {
        ...
        {
            if (name[0] == '\\' && name[1] == '\\' && name[2] == '.' && name[3] == '\\' && name[4] != 0 && name[5] == ':') // path in the form "\\.\C:\"
            {
...
void AuxThreadBody(BOOL add, HANDLE thread, BOOL testIfFinished)
{
    ...
    if (!finished) // after TerminateAuxThreads() we no longer accept anything
    {
        if (add)
        {
        ...
        else
        {
            ...
            for (int i = 0; i < AuxThreads.Count; i++)
            {
                ...
                { // thread is still running, terminate it
                    ...
                    WaitForSingleObject(t, INFINITE); // wait until the thread actually ends, it sometimes takes a while
                }
...
{
  protected:
    DWORD CallerCalledFromArr[STOPREFRESHSTACKSIZE];  // array of return addresses of functions that called BeginStopRefresh()
    DWORD CalledFromArr[STOPREFRESHSTACKSIZE];        // array of addresses where BeginStopRefresh() was called
  ...
  public:
    ...
    ~CStopRefreshStack() {CheckIfEmpty(3);} // three BeginStopRefresh() calls are OK: one for each panel and the third from WM_USER_CLOSE_MAINWND (called first)
...
{
    /*
#ifdef _DEBUG     // verify that BeginStopRefresh() and EndStopRefresh() are called from the same function (using the caller's return address -> won't catch a "mistake" when called from different functions invoked from the same place)
  DWORD *register_ebp;
...
*/
    ...
    else
    {
        if (--StopRefresh == 0)
        {
            ...
            // if we blocked some refresh, give it a chance to run
            ...
            if (MainWindow != NULL && MainWindow->NeedToResentDispachChangeNotif &&
                !AlreadyInPlugin) // if still inside a plug-in, sending the message is pointless
            {
                MainWindow->NeedToResentDispachChangeNotif = FALSE;

                // post a request to dispatch change notifications for paths
...
void RemoveTemporaryDir(const char* dir)
{
    ...
    SetCurrentDirectory(dir); // speeds things up (the system likes the current directory)
...
AGAIN:
    ...
    if (attrs == 0xFFFFFFFF) // probably doesn't exist, allow creating it
    {
        ...
        if (!quiet)
        {
            // if the user did not suppress it, show information that the directory does not exist
        ...
        if (quiet || msgBoxRet == IDOK)
        {
            ...
            while (1) // find the first existing directory
            {
                ...
                {
                    strcpy(name, root);
                    break; // we are already at the root directory
                }
                attrs = SalGetFileAttributes(name);
                if (attrs != 0xFFFFFFFF) // the name exists
                {
                    if (attrs & FILE_ATTRIBUTE_DIRECTORY)
                        break; // build starting from this directory
            ...
            while (*st != 0)
            {
                BOOL invalidName = manualCrDir && *st <= ' '; // leading spaces in a newly created directory name are undesirable only when created manually (Windows allows it but it's potentially dangerous)
                ...
                if (name[len - 1] <= ' ' || name[len - 1] == '.')
                    invalidName = TRUE; // trailing spaces and dots in the directory name are undesirable
            AGAIN2:
                                   ...
                                   CPluginFSInterfaceAbstract* pluginFS)
{
    ...
    if (Type == 0) // disk
    {
        ...
        if ((int)strlen(root) < e - pathOrArchiveOrFSName || // not the root path
...
void CPathHistoryItem::GetPath(char* buffer, int bufferSize)
{
    ...
    // we must duplicate all '&' otherwise they will be underlined
    ...
    while (*s != 0)
    {
        if (*s == '&')
        {
            if (!(skipFirstAmpersand && first))
            {
                ...
                {
                    ret = FALSE;
                    memmove(s + 1, s, l - (s - buffer)); // duplicate '&', trim by one character
                    buffer[l] = 0;
                    s++;
                }
            }
...
{
    BOOL ret = TRUE; // by default we return success
    char errBuf[MAX_PATH + 200];
    if (PathOrArchiveOrFSName != NULL) // valid data present
    {
        int failReason;
        BOOL clear = TRUE;
        if (Type == 0) // disk
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    BOOL done = FALSE;
                    // if the FS interface where the path was last opened is known, we try
                    // to find it among the detached ones and use it
                         ...
                         !panel->GetPluginFS()->Contains(PluginFS)))
                    {
                        ...
                        for (i = 0; i < list->Count; i++)
                        {
                            if (list->At(i)->Contains(PluginFS))
                            {
                                ...
                                // try switching to the requested path (it was there last time, no need to test IsOurPath),
                                // while attaching the detached FS
                                ...
                                break; // end, further matches with PluginFS are not considered
                            }
                        }
                    }

                    // if the previous attempt failed and the path cannot be listed by the panel's FS interface,
                    // try to find a detached FS interface that can list the path
                    // so that a new FS is not opened unnecessarily
                    int fsNameIndex;
                                                        ...
                                                        fsNameIndex, convertPathToInternalDummy)))
                    {
                        ...
                        for (i = 0; i < list->Count; i++)
                        {
                            if (list->At(i)->IsPathFromThisFS(PathOrArchiveOrFSName, ArchivePathOrFSUserPart))
                            {
                                ...
                                // try switching to the requested path while attaching the detached FS
                                if (!panel->ChangePathToDetachedFS(i, TopIndex, FocusedName, TRUE, &failReason,
                                                                   PathOrArchiveOrFSName, ArchivePathOrFSUserPart))
                                {
                                    if (failReason == CHPPFR_SHORTERPATH) // almost success (the path is only shortened) (CHPPFR_FILENAMEFOCUSED cannot happen here)
                                    {                                     // update the FS interface record
                    ...
                    // if there is no other way, open a new FS interface or only change the path on the active one
                    if (!done)
                    {
                                                         ...
                                                         FocusedName, FALSE, 2, NULL, TRUE, &failReason))
                        {
                            if (failReason == CHPPFR_SHORTERPATH ||   // almost success (the path was only shortened)
                                failReason == CHPPFR_FILENAMEFOCUSED) // almost success (the path changed only to a file and it was focused)
                            {                                         // update the FS interface record
                                if (panel->Is(ptPluginFS))
                                    PluginFS = panel->GetPluginFS()->GetInterface();
                            }
...
BOOL CPathHistoryItem::IsTheSamePath(CPathHistoryItem& item, CPluginFSInterfaceEncapsulation* curPluginFS)
{
    ...
    if (Type == item.Type)
    {
        ...
        else
        {
            if (Type == 1) // archive
            {
                if (StrICmp(PathOrArchiveOrFSName, item.PathOrArchiveOrFSName) == 0 &&  // archive file name is case-insensitive
                    strcmp(ArchivePathOrFSUserPart, item.ArchivePathOrFSUserPart) == 0) // path inside the archive is case-sensitive
                {
                    return TRUE;
                }
            }
                ...
                if (Type == 2) // FS
                {
                    if (StrICmp(PathOrArchiveOrFSName, item.PathOrArchiveOrFSName) == 0) // FS name is case-insensitive
                    {
                        if (strcmp(ArchivePathOrFSUserPart, item.ArchivePathOrFSUserPart) == 0) // FS user part is case-sensitive
                            return TRUE;
                        if (curPluginFS != NULL && // handle the case when both FS user parts are identical because the FS returns TRUE from IsCurrentPath for them (generally we'd need a method to compare two FS user parts, but it's not worth it just for histories...)
...
void CPathHistory::ClearPluginFSFromHistory(CPluginFSInterfaceAbstract* fs)
{
    ...
    {
        NewItem->PluginFS = NULL; // the FS was just closed -> set to NULL
    }
    int i;
    for (i = 0; i < Paths.Count; i++)
    {
        ...
        if (item->Type == 2 && item->PluginFS == fs)
            item->PluginFS = NULL; // the FS was just closed -> set to NULL
    }
}
...
void CPathHistory::Execute(int index, BOOL forward, CFilesWindow* panel, BOOL allItems, BOOL removeItem)
{
    ...
    CPathHistoryItem* item = NULL; // if the path should be removed, store a pointer for later search
    ...
    if (forward)
    {
        if (HasForward())
        {
            if (ForwardIndex + index - 1 < Paths.Count)
            {
                ...
                if (!change)
                    item = NULL; // failed to change path -> keep it in history
                Lock = FALSE;
            }
    ...
    else
    {
        ...
        if (HasBackward() || allItems && HasPaths())
        {
            ...
            if (count - index >= 0) // we can move there (not the last item)
            {
    ...
    IdleRefreshStates = TRUE; // force status variables check on next Idle
    ...
    if (NewItem != NULL)
    {
        ...
        NewItem->HIcon = NULL; // the AddPathUnique method took over responsibility for destroying the icon
        delete NewItem;
        NewItem = NULL;
    }
                                    ...
                                    CPluginFSInterfaceEncapsulation* curPluginFS)
{
    ...
    if (Paths.Count > 0)
    {
        if (ForwardIndex == -1)
        {
            ...
            if (n.IsTheSamePath(*n2, curPluginFS)) // identical paths -> delete the entry
                           ...
                           CPluginFSInterfaceAbstract* pluginFS, CPluginFSInterfaceEncapsulation* curPluginFS)
{
    ...
    if (Paths.Count > 0)
    {
        ...
        {
            delete n;
            return; // identical paths -> nothing to do
        }
    }
                                 ...
                                 CPluginFSInterfaceEncapsulation* curPluginFS)
{
    ...
    if (n == NULL)
    {
        TRACE_E(LOW_MEMORY);
        if (hIcon != NULL)
            HANDLES(DestroyIcon(hIcon)); // we must destroy the icon
        return;
    }
    if (Paths.Count > 0)
    {
        ...
        for (i = 0; i < Paths.Count; i++)
        {
            ...
            if (n->IsTheSamePath(*item, curPluginFS))
            {
                ...
                { // FS case: replace pluginFS so the path opens on the last FS of this path
                    item->PluginFS = pluginFS;
                }
                delete n;
                if (i < Paths.Count - 1)
                {
                    // move the path up in the list
                    Paths.Add(item);
                    if (Paths.IsGood())
                        Paths.Detach(i); // if adding succeeded, discard the source
                    if (!Paths.IsGood())
                        Paths.ResetState();
                }
                return; // identical paths -> nothing to do
...
void CPathHistory::SaveToRegistry(HKEY hKey, const char* name, BOOL onlyClear)
{
    ...
    if (CreateKey(hKey, name, historyKey))
    {
        ...
        if (!onlyClear) // if the key is not only cleared, save the values from history
        {
            int index = 0;
            char buf[10];
            ...
            for (i = 0; i < Paths.Count; i++)
            {
                ...
                // archive & FS: use ':' as the separator of the two path parts
                // during loading we determine the path type from this character
                case 1: // archive
                case 2: // FS
                {
                    strcpy(path, item->PathOrArchiveOrFSName);
...
CUserMenuIconDataArr::GiveIconForUMI(const char* fileName, DWORD iconIndex, const char* umCommand)
{
    ...
    for (int i = 0; i < Count; i++)
    {
            ...
            strcmp(item->UMCommand, umCommand) == 0)
        {
            ...
            item->Clear();                 // we don't want to shift the array (when deleting) - slow and unnecessary, so just clear the item for faster skipping
            return icon;
        }
    }
...
CUserMenuIconBkgndReader::~CUserMenuIconBkgndReader()
{
    if (UserMenuIIU_BkgndReaderData != NULL) // they definitely will not be needed now, free them
    {
...
unsigned BkgndReadingIconsThreadBody(void* param)
{
    ...
    TRACE_I("Begin");
    // ensure GetFileOrPathIconAux works (it uses some COM/OLE stuff)
    ...
    for (int i = 0; UserMenuIconBkgndReader.IsCurrentIRThreadID(threadID) && i < bkgndReaderData->Count; i++)
    {
        ...
        {
            HANDLES_ADD(__htIcon, __hoLoadImage, umIcon); // add a handle to 'umIcon' into HANDLES
        }
        else
        {
            umIcon = NULL;
            if (item->UMCommand[0] != 0)
            { // if the previous method failed, try obtaining the icon from the system
...
void CUserMenuIconBkgndReader::StartBkgndReadingIcons(CUserMenuIconDataArr* bkgndReaderData)
{
    ...
    if (!AlreadyStopped && bkgndReaderData != NULL && bkgndReaderData->Count > 0)
    {
        ...
        if (thread != NULL)
        {
            // the main thread runs at higher priority; to keep icon reading as fast as before introducing the background thread,
            // we must also raise its priority
            SetThreadPriority(thread, THREAD_PRIORITY_ABOVE_NORMAL);

            bkgndReaderData = NULL; // data have been handed over to the thread, we will not free them here
            CurIRThreadIDIsValid = TRUE;
            CurIRThreadID = newThreadID;
            AddAuxThread(thread); // if the thread doesn't finish in time, we'll kill it before closing the program
        }
        else
    ...
    // pause briefly; if icons load quickly, the "simple" variants never show
    // different variants (less flicker) + some users reported that simultaneous icon loading into panels
    // significantly slowed loading icons into the user menu, causing the toolbar icons to appear
    // with a large delay which is ugly; this should prevent it by handling only slow
    // loading of user menu icons, which is the whole point of this trick)
    if (thread != NULL)
    {
...
void CUserMenuIconBkgndReader::EndUserMenuIconsInUse()
{
    ...
    else
    {
        ...
        { // last lock; if we have stored data to process, send them
            ...
            else // nobody is interested in the data anymore, just free them
                delete UserMenuIIU_BkgndReaderData;
            UserMenuIIU_BkgndReaderData = NULL;
            UserMenuIIU_ThreadID = 0;
        }
...
BOOL CUserMenuIconBkgndReader::EnterCSIfCanUpdateUMIcons(CUserMenuIconDataArr** bkgndReaderData, DWORD threadID)
{
    ...
    if (CurIRThreadIDIsValid && CurIRThreadID == threadID)
    {
        if (UserMenuIconsInUse > 0)
        {
            if (UserMenuIIU_BkgndReaderData != NULL) // if some are already stored, free them (entering config during loading, then color change, comes here again)
                delete UserMenuIIU_BkgndReaderData;
            UserMenuIIU_BkgndReaderData = *bkgndReaderData;
            UserMenuIIU_ThreadID = threadID;
            *bkgndReaderData = NULL; // caller handed over the data; we'll free them later
        }
        else
        {
            ret = TRUE;
...
{
    CurIRThreadIDIsValid = FALSE; // icons are now passed to the user menu (IsReadingIcons() must return FALSE)
    HANDLES(LeaveCriticalSection(&CS));
}
...
CUserMenuItem::CUserMenuItem(CUserMenuItem& item, CUserMenuIconDataArr* bkgndReaderData)
{
    ...
    if (Type == umitItem)
    {
        if (bkgndReaderData == NULL) // this is a copy for the config dialog; newly loaded icons are not propagated there (we wait until the dialog closes)
        {
            UMIcon = DuplicateIcon(NULL, item.UMIcon); // GetIconHandle() slowed things down unnecessarily
...
void CUserMenuItem::SetType(CUserMenuItemType type)
{
    if (Type != type)
    {
        if (type == umitSubmenuBegin)
        {
            // switching to a shared icon, delete the allocated one
            if (UMIcon != NULL)
            {
                HANDLES(DestroyIcon(UMIcon));
                UMIcon = NULL;
...
BOOL CUserMenuItem::GetIconHandle(CUserMenuIconDataArr* bkgndReaderData, BOOL getIconsFromReader)
{
    ...
    if (Type == umitSeparator) // separators have no icon
        return TRUE;

    // I will try to extract the icon from the specified file
    char fileName[MAX_PATH];
    fileName[0] = 0;
    DWORD iconIndex = -1;
    if (MainWindow != NULL && Icon != NULL && Icon[0] != 0)
    {
        // Icon is in the format "filename,resID"
        // decompose it
    ...
    if (bkgndReaderData == NULL && fileName[0] != 0 && // load icons right here
    ...
    if (bkgndReaderData == NULL && umCommand[0] != 0 && // load icons right here
        MainWindow->GetActivePanel() != NULL &&
    ...
    if (bkgndReaderData != NULL)
    {
        if (getIconsFromReader) // icons are already loaded, just take the correct one
        {
            UMIcon = bkgndReaderData->GiveIconForUMI(fileName, iconIndex, umCommand);
            if (UMIcon != NULL)
                return TRUE;
        }
        else // request loading the needed icon
            bkgndReaderData->Add(new CUserMenuIconData(fileName, iconIndex, umCommand));
    }

    // load the default icon from shell32.dll
...
{
    // let us find the window under the mouse cursor
    ...
    if (hWindow != NULL)
    {
        ...
        {
            // some Synaptics touchpad versions (e.g. on HP notebooks) display their own window with a scrolling symbol under the cursor;
            // in such a case we won't attempt to route to the "correct" window under the cursor because
            // the touchpad handles it itself
        ...
        // if it's a ScrollBar with a parent, post the message to the parent.
        // Scrollbars in the panels are not subclassed, so this is currently the only way
        // the panel can learn about wheel events when the cursor is over the scrollbar.
        className[0] = 0;
...
LRESULT CALLBACK MenuWheelHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    MSG* pMSG = (MSG*)lParam;
    MessagesKeeper.Add(pMSG); // if Salamander crashes we'll have the message history

    // we are interested only in WM_MOUSEWHEEL and WM_MOUSEHWHEEL
    //
    // 7.10.2009 - AS253_B1_IB34: Manison reported that horizontal scrolling did not work under Windows Vista.
    // It worked for me (through this path). After installing Intellipoint drivers v7 (previously I had none on Vista x64)
    // WM_MOUSEHWHEEL messages stopped flowing through here and went directly to the Salamander panels.
    // Therefore I disable this path and we will capture the messages only in the panel.
    // Note: we could similarly cut off WM_MOUSEWHEEL handling, but I won't risk breaking older OSes (perhaps when moving to W2K and later).
    // Note2: if it turns out we must catch WM_MOUSEHWHEEL via this hook as well, we should detect at runtime that the messages flow here and then disable processing
    // in the panels and command line.

    // 30.11.2012 - someone on the forum reported that WM_MOUSEHWEEL doesn't go through the message hook (same as before
    // for Manison with WM_MOUSEHWHEEL): https://forum.altap.cz/viewtopic.php?f=24&t=6039
    // so now we'll also catch the message in each window it could potentially go to (depending on focus)
    // and then route it so it is delivered to the window under the cursor as we always did

    // currently we will let both WM_MOUSEWHEEL and WM_MOUSEHWHEEL through and see what beta testers say

    if ((pMSG->message != WM_MOUSEWHEEL && pMSG->message != WM_MOUSEHWHEEL) || (wParam == PM_NOREMOVE))
        return retValue;

    // if the message arrived "recently" via the other channel, ignore this one
...
BOOL InitializeMenuWheelHook()
{
    ...
    HOldMouseWheelHookProc = SetWindowsHookEx(WH_GETMESSAGE, // HANDLES cannot handle this!
                                              MenuWheelHookProc,
                                              NULL, threadID);
...
BOOL ReleaseMenuWheelHook()
{
    ...
    {
        UnhookWindowsHookEx(HOldMouseWheelHookProc); // HANDLES cannot handle this!
                              ...
                              DWORD attr)
{
    ...
    if (ZIPRoot != NULL) // zip root has no backslash at the beginning or end
    {
    ...
    if (SourcePath != NULL) // source path has no trailing backslash
    {
                              ...
                              const FILETIME& lastWrite, const CQuadWord& fileSize, DWORD attr)
{
    ...
    // check if it's already here (cannot check before creating the item due to path adjustments)
    int i;
    for (i = 0; i < List.Count; i++)
    {
        ...
        {
            delete item;
            return FALSE; // already present, do not add another
        }
    }
...
const char* WINAPI FileTimeStampsEnum2(HWND parent, int enumFiles, const char** dosName, BOOL* isDir,
                                       ...
                                       int* errorOccured)
{ // we enumerate only files, enumFiles can therefore be skipped entirely
...
void CFileTimeStamps::Remove(int* indexes, int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        int index = indexes[count - i - 1];   // delete from the end - less shifting and indices stay
...
void CFileTimeStamps::CopyFilesTo(HWND parent, int* indexes, int count, const char* initPath)
{
                           ...
                           LoadStr(IDS_BROWSEARCUPDATETEXT), path, FALSE, initPath))
    {
        ...
        fromStr.Add("\0", 2); // just to be safe, append two more zeros at the end (no Add, also OK)
        toStr.Add("\0", 2);   // just to be safe, append two more zeros at the end (no Add, also OK)
        ...
        if (ok && !tooLongName)
        {
            ...
            char title[100];
            lstrcpyn(title, LoadStr(IDS_BROWSEARCUPDATE), 100); // back it up; other threads use LoadStr too
            fo.lpszProgressTitle = title;
            // perform the actual copy - wonderfully easy, unfortunately it sometimes crashes ;-)
...
void CFileTimeStamps::CheckAndPackAndClear(HWND parent, BOOL* someFilesChanged, BOOL* archMaybeUpdated)
{
    ...
    //---  remove from the list files that were not changed
    BeginStopRefresh();
    ...
    if (List.Count > 0)
    {
        ...
        // during critical shutdown we pretend the updated files do not exist and we won't manage to pack them back into the archive,
        // but we must not delete them; after restart the user must have a chance to pack the files manually into the archive
        if (!CriticalShutdown)
        {
            ...
            while (showDlg)
            {
                ...
                if (dlg.Execute() == IDOK)
                {
                    ...
                    //--- pack the changed files in groups with the same zip root and source path
                    TIndirectArray<CFileTimeStampsItem> packList(10, 5); // list of all items with the same zip root and source path
                    ...
                    while (!showDlg && List.Count > 0)
                    {
                        ...
                        {                                     // we iterate from the end because Detach is easier that way
                            CFileTimeStampsItem* item2 = List[i];
                            char* r2 = item2->ZIPRoot;
                            char* s2 = item2->SourcePath;
                            if (strcmp(r1, r2) == 0 && // same zip root (case-sensitive comparison required - update test\A.txt and Test\b.txt must not run simultaneously)
                                StrICmp(s1, s2) == 0)  // same source path
                            {
                                packList.Add(item2);
                                List.Detach(i);
                            }
                        ...
                        while (loop)
                        {
                            ...
                            else
                            {
                                ...
                                if (!loop) // "cancel"; detach files from disk cache, otherwise delete them
                                {
                                    List.Add(packList.GetData(), packList.Count);
                                    packList.DetachMembers();
                                    showDlg = TRUE; // show the Archive Update dialog again (with remaining files)
                                }
                            }
                            SetCurrentDirectoryToSystem();
                        }
...
void CTopIndexMem::Push(const char* path, int topIndex)
{
    ...
    if (ok) // follows -> store the next top index
    {
...
BOOL CTopIndexMem::FindAndPop(const char* path, int& topIndex)
{
    ...
    else // query for another path -> clear memory, a long jump occurred
    {
        Clear();
        return FALSE;
    }
...
BOOL CFileHistory::AddFile(CFileHistoryItemTypeEnum type, DWORD handlerID, const char* fileName)
{
    ...
    // search existing items to see if the new one is already present
    int i;
    for (i = 0; i < Files.Count; i++)
    {
        ...
        {
            // if so, just move it to the top position
            if (i > 0)
            {
                Files.Detach(i);
                if (!Files.IsGood())
                    Files.ResetState(); // cannot fail; it only reports lack of memory when shrinking the array
                Files.Insert(0, item);
                if (!Files.IsGood())
                {
                    Files.ResetState();
...
BOOL CFileHistory::FillPopupMenu(CMenuPopup* popup)
{
    ...
    for (i = 0; i < Files.Count; i++)
    {
        ...
        // separate the name from the path with '\t' so it appears in a dedicated column
        ...
        *(ptr + 1) = '\t';
        const char* text = "";
        // duplicate '&' so it doesn't appear as an underline
        ...
        sprintf(name + lstrlen(name), "\t(%s)", text); // append how the file is opened
        mii.ID = i + 1;
        popup->InsertItem(-1, TRUE, &mii);
    }
    if (i > 0)
...
*/
    ...
    // attach hot paths if they exist
...
{
    // since Windows Vista SelectAll works by default, so we keep select all there
    if (WindowsVistaAndLater)
        return FALSE;
...
protected:
    ...
    HWND HDialog;       // dialog to which we will send WM_USER_KEYDOWN
...
BOOL CreateKeyForwarder(HWND hDialog, int ctrlID)
{
    ...
    {
        // it could be a combobox, let's try to access its internal edit
