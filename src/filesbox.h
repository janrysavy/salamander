...
public:
    ...
    // reserves space for the arrow symbol if needed
    void SetMinWidths();

    // Determines which part contains point [xPos, yPos] (client coordinates).
    // If it returns hhtItem or hhtDivider, 'index' receives the column index from Columns.
    // If "Name" was clicked, 'extInName' is set. When clicking "Ext" it becomes TRUE, otherwise FALSE.
...
public:
    ...
    // if 'labelRect' != NULL, returns the rectangle around the text there
    ...
    void PaintHeaderLine(); // repaints the header line if it is visible
    ...
    // scroll: FALSE - item will be fully visible; TRUE - item will be at least partially visible
    ...
    // number of partially visible items (used to prioritize icon and thumbnail loading)
    ...
    // Microsoft recommends in Best Practices for Supporting Microsoft Mouse and Keyboard Devices
...
protected:
    void LayoutChilds(BOOL updateAndCheck = TRUE);                               // computes rectangles and positions child windows
    void SetupScrollBars(DWORD flags = UPDATE_VERT_SCROLL | UPDATE_HORZ_SCROLL); // updates scrollbar information
    BOOL ShowHideChilds();                                                       // shows or hides children based on Mode; returns TRUE when a change occurs
