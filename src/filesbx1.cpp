...
void CFilesBox::PaintAllItems(HRGN hUpdateRgn, DWORD drawFlags)
{
    ...
    case vmBrief:
    {
        ...
        // offsetting between drawing operations ensures items outside the clipping rectangle are not drawn
        if (clipRectType == SIMPLEREGION || clipRectType == COMPLEXREGION)
        {
            RECT tmpRect = clipRect;
...
void CFilesBox::PaintItem(int index, DWORD drawFlags)
{
    ...
    if (GetItemRect(index, &r))
    {
        ...
        case vmDetailed:
        {
            // skip visibility test for performance
...
void CFilesBox::EnsureItemVisible2(int newTopIndex, int index)
{
    ...
    TopIndex = newTopIndex;
    SetupScrollBars(UPDATE_VERT_SCROLL);
    // when the Selection bit changes the Dirty flag is set and we must not clear
    // it while scrolling because partially visible items are not redrawn
    // completely and will need to be painted again after we scroll
    PaintAllItems(hUpdateRgn, DRAWFLAG_KEEP_DIRTY);
    HANDLES(DeleteObject(hUpdateRgn));
    if (ImageDragging)
        ImageDragShow(TRUE);
...
CFilesBox::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        if (MainWindow->HasLockedUI())
            break; // when the UI is locked we want clicks on the panel to raise Salamander
    ...
    case WM_USER_MOUSEHWHEEL: // horizontal scrolling, available since Windows Vista
    {
        if (MainWindow->HasLockedUI())
            break;
    ...
    case WM_MOUSEWHEEL:
    {
        ...
        // WM_MOUSEHWHEEL messages stopped passing through the hook and went directly to the focused window; I disabled the hook
        // and now we must capture messages in windows that can have focus so they can be forwarded.
...
int CFilesBox::GetIndex(int x, int y, BOOL nearest, RECT* labelRect)
{
    ...
    case vmBrief:
    {
        ...
        if (!nearest && Configuration.FullRowSelect)
        {
            ...
            if (xPos >= ItemWidth - 10) // adjustment - we must leave space for box dragging
                itemIndex = INT_MAX;
        }
...
void CFilesBox::CheckAndCorrectBoundaries()
{
    ...
    case vmDetailed:
    {
        ...
        {
            // ensure scrolling when the window grows and we are at the right or bottom edge but can still scroll
            int newXOffset = XOffset;
