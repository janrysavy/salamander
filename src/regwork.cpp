...
BOOL CRegistryWorkerThread::StartThread()
{
    ...
    if (Thread == NULL)
    {
        if (WorkReady != NULL && WorkDone != NULL)
        {
            ...
            InUse = FALSE; // just to be safe; it cannot be TRUE here
            ...
            if (Thread != NULL)
            {
                OwnerTID = GetCurrentThreadId(); // allow use for this thread
                StopWorkerSkipCount = 0;
...
BOOL CRegistryWorkerThread::CInUseHandler::CanUseThread(CRegistryWorkerThread* t)
{
    ...
    { // work in the worker applies only to the thread that started it
        BOOL ret = !t->InUse;
        if (ret) // the job can be started in the registry worker thread
        {
            t->InUse = TRUE;
            T = t; // in the destructor T->InUse = FALSE
        }
        // else  // recursive call (due to the message loop and message distribution) = refuse to run in the thread
        return ret;
    }
    return FALSE;
}
