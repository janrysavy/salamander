...
void CProgressBar::Paint(HDC hDC)
{
    ...
    HDC hMemDC = NULL;
    if (Bitmap != NULL && Progress != -1) // caching Progress==-1 is pointless, nothing flashes there
    ...
    else
    {
        ...
        {
            progress = buff;
            progressLen = sprintf(progress, "%d %%", (int)((Progress /*+ 5*/) / 10)); // do not round the progress, otherwise 100% is visible from 99.5%-100%, which annoys some users (notable with FTP, where it can last half a minute)
        }
    ...
    case WM_USER_SETPROGRESS:
    {
        ...
        if (progress == (DWORD)-1)
        {
            if (SelfMoveTime > 0)
            {
                ...
                if (!TimerIsRunning)
                {
                    ...
                    paint = FALSE; // the change will occur on the timer, no reason to repaint now
            }
            else
                MoveBar();
        }
        else
        {
            ...
            if (progress != Progress)
                ...
                paint = textChanged; // progress didn't change; if the text didn't change either, skip repaint
                                     /*
    ...
    : CWindow(hDlg, ctrlID, ooAllocated)
{
    ...
    // measure the maximum size of the static control
    RECT r;
    GetClientRect(HWindow, &r);
    Width = r.right - r.left;
    Height = r.bottom - r.top;

    // if we should draw via cache, create a bitmap
    if (Flags & STF_CACHED_PAINT)
    {
        Bitmap = new CBitmap(); // if allocation fails, painting won't be cached
        if (Bitmap != NULL)
        {
    ...
    // obtain the default font from the control
    ...
    // obtain the initial text of the static control
    char buff[4096];
...
void CStaticText::PrepareForPaint()
{
    ...
    if (Flags & (STF_PATH_ELLIPSIS | STF_END_ELLIPSIS))
    {
        if (Flags & STF_END_ELLIPSIS)
        {
            ...
            if (fitChars < TextLen)
            {
                // it did not fit -- we must insert an ellipsis
                ...
                // search from the end to find how much to trim so we can append the ellipsis
        ...
        else
        {
            ...
            if (sz.cx > Width)
            {
                ...
                // search from the end for the path separator
                ...
                // the text from 'p' should fit entirely including the ellipsis
                if (ellipsisWidth + sz.cx - AlpDX[pIndex] > Width)
                {
                    // it does not fit => search from the start for a place to insert the ellipsis
                    while (pIndex < TextLen && (ellipsisWidth + sz.cx - AlpDX[pIndex] > Width))
                        pIndex++;

                    // insert the ellipsis and then the rest of the text
                    pIndex++;
                    strcpy(Text2, "...");
                    Text2Len = 3;
                    TextWidth = ellipsisWidth;
                ...
                else
                {
                    ...
                    // determine how many characters to keep on the left before the ellipsis
    ...
    // if the text would cross the window boundary, we must clip during painting
    if (TextWidth > Width)
    {
        TextWidth = Width;
        ClipDraw = TRUE;
...
BOOL CStaticText::SetToolTipText(const char* text)
{
    ...
    PostMessage(MainWindow->ToolTip->HWindow, WM_USER_REFRESHTOOLTIP, 0, 0); // ask the window to load the new text and repaint
...
CStaticText::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_ERASEBKGND:
    {
        // background will be drawn during WM_PAINT
        return TRUE;
    }
    ...
    case WM_PAINT:
    {
        ...
        // if we have a bitmap, draw into it, otherwise directly to the screen
        HDC hDC;
        if (Bitmap != NULL)
            hDC = Bitmap->HMemDC;
        else
        ...
        if (Text != NULL)
        {
            // under XPTheme we have to let Windows draw the background
            BOOL bkErased = FALSE;
            if (IsAppThemed())
            {
            ...
            if (Flags & STF_HANDLEPREFIX)
            {
                ...
                // clip everything; the issue is visible in the Plugins Manager 2.51 when scrolling
                // the plugin list up and down, leaving a red dot before the URL
                // if (!ClipDraw)
                drawFlags |= DT_NOCLIP;
        ...
        {
            // no text stored; we must at least paint the background
            if (IsAppThemed())
            {
                DrawThemeParentBackground(HWindow, hDC, &r);
            }
...
BOOL CHyperLink::ExecuteIt()
{
    ...
    if (File[0] != 0)
    {
        // do not switch to shellExecuteWnd, used from BugReport
...
void CColorGraph::PaintFace(HDC hdc)
{
    ...
    if (UsedProc > 0 && UsedProc < 1)
    {
        ...
        int y1 = (int)(elY0 + elY);
        int y2 = (int)elY0;
        if (y1 == y2 && UsedProc < 0.1)     // a bit ugly
...
void CButton::PaintFace(HDC hdc, const RECT* rect, BOOL enabled)
{
    ...
    if (Flags & BTF_RIGHTARROW)
    {
        ...
        if (empty)
        {
            // if the button contains only an arrow, center it in both axes
            r = *rect;
    ...
    case WM_PAINT:
    {
        ...
        if (hdc != NULL)
        {
            ...
            if (IsAppThemed())
            {
                ...
                // paint the background, the button has transparent areas
                HBRUSH hBrush = (HBRUSH)(COLOR_BTNFACE + 1);
...
void CColorArrowButton::PaintFace(HDC hdc, const RECT* rect, BOOL enabled)
{
    ...
    LOGFONT lf;
    GetObject(hFont, sizeof(lf), &lf);
    lf.lfHeight += 1; // tweak for 100% DPI where overly large text touches the rectangle
    hFont = HANDLES(CreateFontIndirect(&lf));
    HFONT hOldFont = (HFONT)SelectObject(hdc, hFont);
...
CAnimate::ThreadF(void *param)
{
  ...
  // raise the thread priority so the animation doesn't stutter
  ...
  while (TRUE)
  {
    ...
    if (animate->SleepThread)     // should we stop the animation
    {
    ...
    WaitForSingleObject(animate->HTerminateEvent, 40); // if we should exit, don't wait unnecessarily
  }
  TRACE_I("End");
  return 0;
}
    ...
    for (int i = 0; i < horizLines.Count; i++)
    {
        ...
        if (data.Label != NULL)
        {
            ...
            else
            {
                ...
                else // shorten the label so it does not cover the line and extend the line to the label
                {
