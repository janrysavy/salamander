...
void CFilterDialog::Validate(CTransferInfo& ti)
{
    ...
    if (useFilter)
    {
        ...
        // provide a buffer for MasksString, there is a size check so nothing happens
        ...
        Filter->SetMasksString(buf); // restore
    }
}
...
CFilterDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        if (*UseFilter)
        { // we want our own focus in the filter edit box
...
CCopyMoveDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        if (DirectoryHelper)
        {
            ChangeToIconButton(HWindow, IDB_BROWSE, IDI_DIRECTORY);   // the button will have a folder icon and an arrow on the right
...
void CCopyMoveMoreDialog::DisplayMore(BOOL more, BOOL fast)
{
    ...
    if (!more && !fast) // fast is TRUE when the controls hold defaults and don't need resetting
    {
        Criteria->Reset();
...
*/
                ...
                case 2: // Save options as defaults
                {
                    ...
                    if (ValidateData())
                    {
                        if (TransferData(ttDataFromWindow))
                            CopyMoveOptions.Set(Criteria->IsDirty() ? Criteria : NULL); // save new defaults
                    }
                    break;
                }
    ...
    case WM_COMMAND:
    {
        if (HIWORD(wParam) == BN_CLICKED)
        {
            ...
            // if the user clicked the mask checkbox, they probably want to edit it
            ...
            if (LOWORD(wParam) == IDC_CM_SPEEDLIMIT)
            {
                if (IsDlgButtonChecked(HWindow, IDC_CM_SPEEDLIMIT))
                    SendMessage(HWindow, WM_NEXTDLGCTL, FALSE, FALSE); // focus to the edit box
            }
    ...
    if (ti.Type == ttDataToWindow)
    {
        ...
        if (!err)
        {
            ...
            if (volumeName[strlen(volumeName) - 1] == '\\')
                volumeName[strlen(volumeName) - 1] = 0; // shorten by the last character ('\\')
...
void CDriveInfo::GrowWidth(int resID, int& width)
{
    ...
    strcat(buff, "M"); // the edit box has some margins; adding "M" compensates for them
...
CDriveInfo::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        Graph = new CColorGraph(HWindow, IDB_GRAPH); // JRYFIXME - rewrite to W10 look; see disk properties, maybe use GDI+ or our SVG?
        if (Graph != NULL)
...
void CPackDialog::Transfer(CTransferInfo& ti)
{
    ...
    if (ti.GetControl(combo, IDC_PACKER))
    {
        if (ti.Type == ttDataToWindow)
        {
            ...
            // set the position in the combo, preferredPacker == -1 -> no selection
    ...
    {
        if (PackerConfig->GetPreferedPacker() != -1) // only change the name if we have an extension
        {
...
AGAIN:
    ...
    // enumerate icons from *.ICO, *.EXE, *.DLL files, including 16-bit PE
...
HWND CWaitWindow::Create(HWND hForegroundWnd)
{
    ...
    // compute the window position; primarily center to HForegroundWindow, otherwise to MainWindow
    ...
    // so we use a hack: create the window first, then measure the client area and adjust its size
...
void CWaitWindow::PaintText(HDC hDC)
{
    ...
    if (Text != NULL)
    {
        ...
        // we won't clip so that we survive minor text extensions
...
void CConversionTablesDialog::Transfer(CTransferInfo& ti)
{
    if (ti.Type == ttDataToWindow)
    {
        ...
        // fill the listview columns Name, Mode and HotKey
        LVCOLUMN lvc;
