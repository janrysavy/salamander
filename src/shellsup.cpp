...
{
    return DropSourcePanel != NULL || // either the drag originates from us
           OurClipDataObject;         // or it is our data on the clipboard
}
                           ...
                           DWORD* dropEffect, void* param)
{
    ...
    // if the file system did not respond or returned garbage, prefer Copy
                          ...
                          DWORD keyState, int& tgtType, int srcType)
{
    ...
    if (panel->Is(ptZIPArchive) || panel->Is(ptPluginFS))
    {
        if (panel->Is(ptZIPArchive))
        {
            ...
            if (format != 0) // found a supported archive
            {
                ...
                if (PackerFormatConfig.GetUsePacker(format) &&
                        (*effect & (DROPEFFECT_MOVE | DROPEFFECT_COPY)) != 0 || // editable? + effect is copy or move?
                        ...
                        (panel->GetZIPPath()[0] == 0 || panel->GetZIPPath()[0] == '\\' && panel->GetZIPPath()[1] == 0)) // drop onto a disk path
                {
                    ...
                    *effect &= (DROPEFFECT_MOVE | DROPEFFECT_COPY); // limit effect to copy+move
                    ...
                    if (index >= 0 && index < panel->Dirs->Count) // drop on a directory
                    {
                        ...
                        if (index == 0 && strcmp(panel->Dirs->At(index).Name, "..") == 0)
                        {
                            if (l > 0 && panel->DropPath[l - 1] == '\\')
                                panel->DropPath[--l] = 0;
                            int backSlash = 0;
                            if (l == 0) // drop-path will be a drive (".." leads outside the archive)
                            {
                                tgtType = idtttWindows;
                                *effect = origEffect;
        ...
        else
        {
            ...
            {
                if (srcType == 2 /* FS */) // drag&drop from FS to FS (any file systems between themselves, restrictions only in CPluginFSInterfaceAbstract::CopyOrMoveFromFS)
                {
                    ...
                    if (index >= 0 && index < panel->Dirs->Count) // drop on a directory
                    {
                        if (panel == DropSourcePanel) // drag&drop within a single panel
                        {
                            ...
                            {                             // directory onto itself
                                panel->SetDropTarget(-1); // hide the marker (the copy goes to the current directory, not the focused subdirectory)
                                if (!rButton && (keyState & (MK_CONTROL | MK_SHIFT | MK_ALT)) == 0)
                                {
                                    tgtType = idtttWindows;
                                    return NULL; // without modifiers keep the STOP cursor (prevents accidental copying to the current directory)
                                }
                                if (effect != NULL)
                ...
                if ((*effect & posEff) != 0)
                {
                    ...
                    *effect &= posEff; // limit effect to FS capabilities
    ...
    if (index >= 0 && index < panel->Dirs->Count) // drop on a directory
    {
        if (panel == DropSourcePanel) // drag&drop within a single panel
        {
            if (panel->GetSelCount() == 0 &&
                ...
                panel->GetSel(index) != 0)
            {                             // directory onto itself
                panel->SetDropTarget(-1); // hide the marker (the copy/shortcut goes to the current directory, not the focused subdirectory)
                if (!rButton && (keyState & (MK_CONTROL | MK_SHIFT | MK_ALT)) == 0)
                    return NULL; // without modifiers keep the STOP cursor (prevents accidental copying to the current directory)
                if (effect != NULL)
                    *effect &= ~DROPEFFECT_MOVE;
        ...
        {                                 // drop on a file
            if (panel == DropSourcePanel) // drag&drop within a single panel
            {
                if (panel->GetSelCount() == 0 &&
                    ...
                    panel->GetSel(index) != 0)
                {                             // file onto itself
                    panel->SetDropTarget(-1); // hide the marker (the copy/shortcut goes to the current directory, not the focused file)
                    if (!rButton && (keyState & (MK_CONTROL | MK_SHIFT | MK_ALT)) == 0)
                        return NULL; // without modifiers keep the STOP cursor (prevents accidental copying to the current directory)
                    if (effect != NULL)
                        *effect &= ~DROPEFFECT_MOVE;
            ...
            // if it is a shortcut, analyze it
            ...
            linkTgt[0] = 0;
            if (StrICmp(file->Ext, "lnk") == 0) // isn't it a directory shortcut?
            {
                IShellLink* link;
            ...
            if (linkIsDir) // the link points to a directory, path is OK; switch to it
            {
                panel->SetDropTarget(index);
            ...
            {
                format--;
                if (PackerFormatConfig.GetUsePacker(format) && // editable?
...
{ // simpler version of the previous function for "paste" from the clipboard
    CFilesWindow* panel = (CFilesWindow*)param;
    isTgtFile = FALSE;
    tgtType = idtttWindows;
    if (panel->Is(ptZIPArchive) || panel->Is(ptPluginFS)) // not yet into archives or file systems
    {
...
void DropEnd(BOOL drop, BOOL shortcuts, void* param, BOOL ownRutine, BOOL isFakeDataObject, int tgtType)
{
    ...
    {
        BOOL again = TRUE; // keep reading while files are still being added
...
{ // WARNING: also used from CSalamanderGeneral::OpenNetworkContextMenu()
    CALL_STACK_MESSAGE_NONE
...
HRESULT AuxGetCommandString(IContextMenu2* menu, UINT_PTR idCmd, UINT uType, UINT* pReserved, LPSTR pszName, UINT cchMax)
{
    ...
    __try
    {
        ...
        // this call isn't essential for program operation, so we wrap it in a try/except block
...
void ShellActionAux7(IDataObject* dataObject, CImpIDropSource* dropSource)
{
    ...
    {
        if (dropSource != NULL)
            dropSource->Release(); // this one is ours, hopefully it won't crash ;-)
        if (dataObject != NULL)
            dataObject->Release();
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
...
{
    if (SalShExtSharedMemView != NULL) // shared memory available (drag&drop errors not handled)
    {
        ...
        if (SalGetTempFileName(NULL, "SAL", fakeRootDir, FALSE))
        {
            ...
            // jr: I found a note on the internet: "Did implementing "IPersistStream" and providing the undocumented
            // "OleClipboardPersistOnFlush" format solve the problem?" -- in case we ever need
            // to get rid of the DROPFAKE method
                    ...
                    if (dataObject != NULL && dropSource != NULL)
                    {
                        ...
                        if (fakeDataObject != NULL)
                        {
                            ...
                            if (sharedMemOK)
                            {
                                ...
                                LastWndFromGetData = NULL; // clear it in case fakeDataObject->GetData is not called
                                hr = DoDragDrop(fakeDataObject, dropSource, allowedEffects, &dwEffect);
                                DropSourcePanel = NULL;
                                // read the drag&drop results
                                // Note: returns dwEffect == 0 on MOVE, so we introduce a workaround via dropSource->LastEffect,
                                ...
                                // (in short: an optimized Move is performed, meaning no copy to the destination followed by deletion
                                //            of the original, to prevent the source from accidentally deleting the original (the move may not yet be complete), it gets
                                //            the result DROPEFFECT_NONE or DROPEFFECT_COPY)
                                if (hr == DRAGDROP_S_DROP && dropSource->LastEffect != DROPEFFECT_NONE)
                                {
                                    ...
                                    if (dropDone)
                                    {
                                        ...
                                        else // archives + FS with Copy or Move (not both) + FS with Copy+Move when dragging with the right mouse button; cursor changes don't affect the menu result, so read the operation from the copy hook (SalShExtSharedMemView->Operation)
                                            operation = SalShExtSharedMemView->Operation;
                                    }
                                    ReleaseMutex(SalShExtSharedMemMutex);

                                    if (!dropDone &&                 // the copy-hook did not react or the user chose Cancel in the drop menu (shown when dragging with the right mouse button)
                                        dwEffect != DROPEFFECT_NONE) // cancel detection: since the copy-hook failed, the returned drop effect is valid, so compare it to Cancel
                                    {
...
void GetLeftTopCornert(POINT* pt, BOOL posByMouse, BOOL useSelection, CFilesWindow* panel)
{
    if (posByMouse)
    {
        // coordinates based on mouse position
    ...
    else
    {
        if (useSelection)
        {
            // based on the context menu position
...
void RemoveUselessSeparatorsFromMenu(HMENU h)
{
    ...
    for (i = miCount - 1; i >= 0; i--)
    {
        ...
        {
            if (lastSep != -1 && lastSep == i + 1) // two separators in a row, remove one as redundant
...
BOOL ResourceGetDialogName(WCHAR* buff, int buffSize, char* name, int nameMax)
{
    ...
    if (style != 0xffff0001)
    {
        ...
        // reading the classic DLGTEMPLATE is handled in Altap Translator, likely not needed here
        return FALSE;
    }
...
// tries to load aclui.dll and extract the dialog name stored with ID 103 (Security tab)
// on success fills pageName with the dialog title and returns TRUE; otherwise returns FALSE
                 ...
                 BOOL posByMouse, BOOL onlyPanelMenu)
{
    ...
    { // no files or directories -> nothing to do
        return;
    }
        ...
        (!dragFiles && action != saCopyToClipboard || !SalShExtRegistered))
    {
        ...
        // other actions in archives are not supported yet
        return;
    }
    if (panel->Is(ptPluginFS) && dragFiles &&
        (!SalShExtRegistered ||
         !panel->GetPluginFS()->NotEmpty() ||
         !panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_MOVEFROMFS) &&    // FS supports "move from FS"
             !panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_COPYFROMFS))) // FS supports "copy from FS"
    {
    ...
    if (useSelection)
    {
        ...
        if (count != 0)
        {
            ...
            panel->GetSelItems(count, indexes, action == saContextMenu); // we abandoned this (see GetSelItems): for context menus we start from the focused item and end with the item before focus (there is an intermediate wrap to the start of the name list) - Windows does it the same, e.g. Add To Windows Media Player List on MP3 files
        }
        else
        {
            index = panel->GetCaretIndex();
    ...
    if (panel->Is(ptZIPArchive) && SalShExtRegistered)
    {
        if (dragFiles)
        {
            // if dragging a single archive subdirectory, find which one (for updating the directory line and inserting into the command line)
            int i = -1;
            if (count == 1)
                i = indexes[0];
            else if (count == 0)
        ...
        else
        {
            ...
            {
                if (SalShExtSharedMemView != NULL) // shared memory available (copy&paste errors not handled)
                {
                                    ...
                                    *fakeName = '\\';
                                    ...
                                    if (fakeDataObject != NULL)
                                    {
                                        ...
                                        if (effect != NULL)
                                        {
                                            ...
                                            if (ef != NULL)
                                            {
                                                *ef = prefferedDropEffect;
                                                                               ...
                                                                               (count == 0) ? 1 : count))
                                                {
                                                    ...
                                                    { // on successful saving the system calls fakeDataObject->AddRef() +
                                                        // save the default drop effect
                                                        ...
                                                        // our data are already on the clipboard (must be cleared when Salamander exits)
                                                        ...
                                                        if (sharedMemOK)
                                                        {
                                                            ...
                                                            *fakeName = '\\';
                                                            ...
                                                            delFakeDir = FALSE; // everything is OK, the fake dir will be used
                                                        ...
                                                        if (!sharedMemOK) // if communication with salextx86.dll or salextx64.dll cannot be established, keeping the data object on the clipboard makes no sense
                                                        {
                                                            OleSetClipboard(NULL);
                                                            OurDataOnClipboard = FALSE; // theoretically redundant (should be set in fakeDataObject's Release() a few lines below)
                                                        }
                                                        ...
                                                        // on COPY clear the CutToClip flag
                                                        ...
                                                        // on COPY clear the CutToClip flag in the other panel as well
                                            ...
                                            TRACE_E(LOW_MEMORY);
                                        fakeDataObject->Release(); // if fakeDataObject is on the clipboard, it will be released when the application exits or when removed from the clipboard
                                    }
                                    else
                                        TRACE_E(LOW_MEMORY);
                                }
    ...
    if (panel->Is(ptPluginFS))
    {
        ...
        if (count > 0)
        {
            ...
            int i;
            for (i = 0; i < panel->Dirs->Count; i++) // ".." cannot be selected, no need to test
            {
        ...
        else
        {
                ...
                panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_CONTEXTMENU)) // context-menu
            {
                ...
                {
                    if (onlyPanelMenu) // panel menu (clicking behind items in the panel)
                    {
                        panel->GetPluginFS()->ContextMenu(panel->GetPluginFS()->GetPluginFSName(),
            ...
            else
            {
                if (dragFiles && SalShExtRegistered &&
                    ...
                    (panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_MOVEFROMFS) || // FS supports "move from FS"
                     panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_COPYFROMFS)))  // FS supports "copy from FS"
                {
                    // if dragging a single FS subdirectory, determine which one (for changing
                    // the directory line and inserting into the command line)
                    int i = -1;
                    if (count == 1)
                        i = indexes[0];
                    else if (count == 0)
        ...
        // increase the thread priority again, the operation has completed
    ...
    if (!panel->Is(ptDisk) && !panel->Is(ptZIPArchive))
    {
        if (indexes != NULL)
            delete[] (indexes);
        EndStopRefresh();
        return; // to be safe, do not process other panel types
    }
                ...
                if (menu != NULL)
                {
                    ...
                    if (action == saPermissions)
                    {
                        ...
                        if (!GetACLUISecurityPageName(pageName, 200))
                            lstrcpy(pageName, "Security"); // if obtaining the name failed, use the English "Security" and silently ignore in localized versions
                    }
        ...
        if (useSelection)
        {
            ...
            else
            {
                ...
                if (menu != NULL)
                {
                    ...
                    if (anotherPanel->CutToClipChanged)
                    {
                        // before CUT and COPY clear the CutToClip flag in the other panel as well
                        anotherPanel->ClearCutToClipFlag(!samePaths);
                    }

                    if (action != saCopyToClipboard)
                    {
                        // in the CUT case set the file's CutToClip bit (ghosted)
                        int idxCount = count;
                        ...
                        for (i = 0; i < idxCount; i++)
                        {
                            ...
                            f->CutToClip = 1;
                            f->Dirty = 1;
                            if (samePaths) // mark the file/directory in the other panel (quadratic complexity, who cares...)
                            {
                    ...
                    // also set the preferred drop effect and mark the origin as Salamander
    ...
    case saContextMenu:
    {
        ...
        if (panel->Is(ptZIPArchive))
        {
            if (useSelection) // only if it concerns items in the panel (not the current path)
            {
                // if command states need to be calculated, do it (ArchiveMenu.UpdateItemsState uses them)
                MainWindow->OnEnterIdle();

                // let the enabler set states and open the menu
            ...
            {
                if (onlyPanelMenu) // context menu inside the panel (behind the items) -> only Paste
                {
                    ...
                    // If it is a path-changing paste, show it in the Paste item
                    char text[220];
                    char tail[50];
                    tail[0] = 0;
        ...
        else
        {
            ...
            if (panel->ContextMenu != NULL) // we crashed, probably due to a recursive call via the message loop in contextPopup.Track (panel->ContextMenu was cleared, likely when leaving the inner recursion)
            {
            ...
            else // ptDisk
            {
                ...
                // handle the Shift key - extended context menu, under W2K it includes Run as...
#define CMF_EXTENDEDVERBS 0x00000100 // rarely used verbs
                ...
                if (onlyPanelMenu)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // work around a TortoiseHg shell extension bug: it keeps a global mapping of item IDs
                            // when we obtain two menus (panel->ContextMenu and panel->ContextSubmenuNew) the later
                            // QueryContextMenu call overwrites this mapping, so commands from the earlier menu stop working
                            // originally this affected panel->ContextSubmenuNew which holds all commands except Open and Explore
                            // we avoid the issue by always taking commands from panel->ContextMenu
                            // only Open and Explore commands are unaffected by this bug,
                            // therefore it's enough to obtain the menu (call QueryContextMenu) from
                            // panel->ContextSubmenuNew as the second in order.
                            // NOTE: we cannot always do this, because if only the New menu is being added,
                            //       it makes more sense to obtain the panel->ContextMenu second so its
                            //       commands work (e.g. THg doesn't insert into the New menu at all,
                ...
                BOOL clipCopy = FALSE;     // is this "our copy"?
                BOOL clipCut = FALSE;      // is this "our cut"?
                BOOL cmdDelete = FALSE;    // is this "our delete"?
                BOOL cmdMapNetDrv = FALSE; // is this "our Map Network Drive"? (UNC root only, we keep it simple)
                DWORD cmd = 0;             // command number for the context menu (10000 = our paste)
                char pastePath[MAX_PATH];  // buffer for the path where "our paste" will be executed (if any)
                if (panel->ContextMenu != NULL && h != NULL)
                {
                    ...
                    char cmdName[2000]; // intentionally 2000 instead of 200, some shell extensions write twice as much (thinking: Unicode = 2 * "number of characters", etc.)
                    if (onlyPanelMenu)
                    {
                        if (panel->ContextSubmenuNew->MenuIsAssigned())
                        {
                                        ...
                                        if (AuxGetCommandString(panel->ContextMenu, mi.wID, GCS_VERB, NULL, cmdName, 200) == NOERROR)
                                        {
                                            if (stricmp(cmdName, "explore") == 0 || stricmp(cmdName, "open") == 0)
                                            {
                                                ...
                                                if (bckgndMenuInsert == 2)
                                                    break; // we do not need more items from here
                                            }
                                        }
                                    }
                                }
...
*/
                            ...
                            // add the Paste command (if it is a path-changing paste, show it in the Paste item)
                            char tail[50];
                            tail[0] = 0;
                            ...
                            // insert a separator if it's not there yet
                            MENUITEMINFO mi2;
                    ...
                    {
                        // originally the New item was added before ShellActionAux5,
                        // but under Windows XP calling ShellActionAux5 deleted the New item
                        // if an Edit/Copy operation had been executed beforehand
                        if (panel->ContextSubmenuNew->MenuIsAssigned())
                        {
                            MENUITEMINFO mi;
                    ...
                    if (GetMenuItemCount(h) > 0) // protection against a completely trimmed menu
                    {
                        CMenuPopup contextPopup;
                        contextPopup.SetTemplateMenu(h);
                        cmd = contextPopup.Track(MENU_TRACK_RETURNCMD | MENU_TRACK_RIGHTBUTTON,
                                                 pt.x, pt.y, panel->GetListBoxHWND(), NULL);
                        //            for testing only -- show the menu using the Windows API
                    ...
                    if (cmd != 0)
                    {
                        ...
                        // the Map Network Drive command is 40 on XP, 43 on W2K and only Vista defines cmdName
                        ...
                        if (cmd != 10000 && cmd != 10001 && !clipCopy && !clipCut && !cmdDelete && !cmdMapNetDrv)
                        {
                            if (cmd < 5000 && stricmp(cmdName, "rename") == 0)
                            {
                                ...
                                panel->RenameFile(specialIndex); // only a drive applies here (no need to enable "Rename")
                            }
                            else
                            {
                                ...
                                if (!useSelection && cmd < 5000 &&         // context menu for a directory
                                    ...
                                    stricmp(cmdName, "link") != 0)         // not needed for Create Shortcut
                                {
                                    char root[MAX_PATH];
                                    GetRootPath(root, panel->GetPath());
                                    if (strlen(root) >= strlen(panel->GetPath())) // menu for the entire drive - because of commands like
                                    {                                             // "format..." we must take our hands off the media
                                        CFilesWindow* win;
                                        int i;
                                        for (i = 0; i < 2; i++)
                                        {
                                ...
                                {
                                    SetCurrentDirectoryToSystem(); // so the drive can be unmapped from the panel
                                }
                                else
                                {
                                    SetCurrentDirectory(panel->GetPath()); // for file names containing spaces: ensures Open With works with Microsoft Paint (on W2K it complained "d:\documents.bmp was not found" for "D:\Documents and Settings\petr\My Documents\example.bmp")
                                }
                                ...
                                panel->FocusFirstNewItem = TRUE; // for WinZip archives as well as the New menu (works well only when the panel auto-refreshes)
                                if (cmd < 5000)
                                {
                                    BOOL changeToFixedDrv = cmd == 35; // "Format" is non-modal, requires switching to a fixed drive
                                    if (releaseLeft)
                                    {
                                        if (changeToFixedDrv)
                                        {
                                    ...
                                    // we handle cut/copy/paste, but for safety refresh clipboard enablers anyway
                                    ...
                                    // notify about the change in the current directory and its subdirectories (better safe than sorry)
                                ...
                                else
                                {
                                    if (panel->ContextSubmenuNew->MenuIsAssigned()) // an exception might have occurred
                                    {
                                        ...
                                        //---  refresh non-automatically refreshed directories
                                        // notify about the change in the current directory (a new file/directory can likely be created only there)
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (cmdMapNetDrv) // is this our "Map Network Drive"? (UNC root only, we keep it simple)
                                    {
...
void ExecuteAssociation(HWND hWindow, const char* path, const char* name)
{
    ...
    {
        if (ExecuteAssociationTlsIndex != TLS_OUT_OF_INDEXES) // new call not possible
        ...
        if (Configuration.UseSalOpen)
        {
            ...
            char execName[MAX_PATH + 200]; // +200 is reserved for longer names (stupid Windows)
            strcpy(execName, path);
            if (SalPathAppend(execName, name, MAX_PATH + 200) && SalOpenExecute(hWindow, execName))
            {
                if (ExecuteAssociationTlsIndex != TLS_OUT_OF_INDEXES) // another call is now possible
                    TlsSetValue(ExecuteAssociationTlsIndex, (void*)0);
                return; // done, executed in the salopen.exe process
            }

            // if salopen.exe fails, run the classic way (risk of open handles in the directory)
        }
    ...
    else
    {
        ...
        // ask whether Salamander should continue or if a bug report should be generated
...
// returns TRUE if it's safe to supply a special invisible window as the parent to a shell extension
// the extension might destroy that window via DestroyWindow, which would normally close Explorer but kills Salamander
// exceptions exist when we must pass the Salamander main window as the parent
BOOL CanUseShellExecuteWndAsParent(const char* cmdName)
{
    ...
    // under Windows 8 "Open With" misbehaved - selecting a custom program did not show the Open dialog
    // https://forum.altap.cz/viewtopic.php?f=16&t=6730 a https://forum.altap.cz/viewtopic.php?t=6782
    // the issue is that Invoke returns, but later MS touches the parent window which we already destroyed
    // TODO: we could keep ShellExecuteWnd alive (a child window stretched over Salamander in the background)
    // we would just verify before passing it that it still exists (no one killed it)
    // TODO2: I tried this experimentally and under W8 Open With still fails; the Open dialog isn't modal to our main window (or Find window)
    // for now we'll pass Salamander's main window in this case
    if (Windows8AndLater && stricmp(cmdName, "openas") == 0)
        return FALSE;

    // for the remaining cases (majority) we can use ShellExecuteWnd
    return TRUE;
}
...
BOOL MakeFileAvailOfflineIfOneDriveOnWin81(HWND parent, const char *name)
{
  ...
  BOOL ret = TRUE;    // WARNING: missing support for OneDriveBusinessStorages, should be implemented !!!
  if (Windows8_1AndLater && OneDrivePath[0] != 0 && strlen(name) < MAX_PATH)
  {
    char path[MAX_PATH];
    char *cutName;
    strcpy_s(path, name);
    if (CutDirectory(path, &cutName) && SalPathIsPrefix(OneDrivePath, path)) // we handle this only under the OneDrive folder
    {
      ...
      {
        // this clumsy method doesn't work: it's asynchronous and the move to offline (downloading from the network)
        // may take up to a minute or never happen, we have no control over it,
        // we'll wait until this can be done via the Win32 API
