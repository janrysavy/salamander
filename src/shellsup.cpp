                          ...
                          DWORD keyState, int& tgtType, int srcType)
{
                    ...
                    if (index >= 0 && index < panel->Dirs->Count) // drop on a directory
                    {
                        if (panel == DropSourcePanel) // drag&drop within a single panel
                        {
                            ...
                            {                             // directory onto itself
                                ...
                                {
                                    tgtType = idtttWindows;
                                    return NULL; // without modifiers keep the STOP cursor (prevents accidental copying into the current directory)
                                }
                                if (effect != NULL)
    ...
    else
    {
        ...
        {                                 // drop on a file
            ...
            // when it is a shortcut, analyze it
            ...
            {
                format--;
                if (PackerFormatConfig.GetUsePacker(format) && // Edit available?
                    ...
                    if (dataObject != NULL && dropSource != NULL)
                    {
                        ...
                        if (fakeDataObject != NULL)
                        {
                            ...
                            if (sharedMemOK)
                            {
                                ...
                                //            of the original, so the source does not accidentally delete the original (it might not have been moved yet), it gets
...
// on success fills pageName with the dialog name and returns TRUE; otherwise returns FALSE
                 ...
                 BOOL posByMouse, BOOL onlyPanelMenu)
{
    ...
    if (useSelection)
    {
        ...
        if (count != 0)
        {
            ...
            panel->GetSelItems(count, indexes, action == saContextMenu); // we abandoned this (see GetSelItems): for context menus we start from the focused item and end with the item before focus (there is an intermediate wrap to the beginning of the name list); the system does the same, see Add To Windows Media Player List on MP3 files
        }
        else
        {
            index = panel->GetCaretIndex();
    ...
    if (panel->Is(ptZIPArchive) && SalShExtRegistered)
    {
        if (dragFiles)
        {
            // if it drags a single subdirectory of the archive, determine which one (to change the path
                                    ...
                                    *fakeName = '\\';
                                    ...
                                    if (fakeDataObject != NULL)
                                    {
                                        ...
                                        if (effect != NULL)
                                        {
                                                ...
                                                *ef = prefferedDropEffect;
                                                                               ...
                                                                               (count == 0) ? 1 : count))
                                                {
                                                    ...
                                                    { // on a successful save the system calls fakeDataObject->AddRef() +
                                                        ...
                                                        // our data are already on the clipboard (must clean them up when Salamander exits)
                                            ...
                                            TRACE_E(LOW_MEMORY);
                                        fakeDataObject->Release(); // if fakeDataObject is on the clipboard, it is released when the application ends or when removed from the clipboard
                                    }
                                    else
                                        TRACE_E(LOW_MEMORY);
                                }
    ...
    if (panel->Is(ptPluginFS))
    {
        ...
        else
        {
                ...
                panel->GetPluginFS()->IsServiceSupported(FS_SERVICE_CONTEXTMENU)) // context-menu
            {
                ...
                {
                    if (onlyPanelMenu) // panel menu (click behind the items in the panel)
                    {
                        panel->GetPluginFS()->ContextMenu(panel->GetPluginFS()->GetPluginFSName(),
        ...
        if (useSelection)
        {
            ...
            else
            {
                ...
                if (menu != NULL)
                {
                    ...
                    // also set the preferred drop effect + origin from Salamander
        ...
        if (panel->Is(ptZIPArchive))
        {
            ...
            else
            {
                if (onlyPanelMenu) // context menu in the panel (behind the items) -> paste only
                {
                    ...
                    // If it is a paste that changes the directory, show it on the Paste item
                    char text[220];
                    char tail[50];
                    tail[0] = 0;
...
#define CMF_EXTENDEDVERBS 0x00000100 // rarely used verbs
                ...
                if (onlyPanelMenu)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // work around a bug in the TortoiseHg shell extension: it has a global mapping of item IDs
                            // in the THg menu and commands, i.e. in our case when two menus are obtained
                            // (panel->ContextMenu and panel->ContextSubmenuNew) this mapping gets overwritten
                            // by a later obtained menu (QueryContextMenu call), so commands from the earlier menu
                            ...
                            // to work around this problem we take advantage of the fact that from panel->ContextMenu we only take
                            ...
                            //           it is wiser to obtain panel->ContextMenu as the second menu instead,
                            //           so that its commands work (e.g. THg does not add anything to the New menu at all,
                            //           so no issue appears)
...
*/
                            ...
                            // add the Paste command (if it is a paste that changes the directory, show it in the Paste item)
                            char tail[50];
                            tail[0] = 0;
                    ...
                    else
                    {
                        ...
                        // under Windows XP calling ShellActionAux5 removed the item
                        // New (when an Edit/Copy operation had been performed beforehand)
                        if (panel->ContextSubmenuNew->MenuIsAssigned())
                        {
                            MENUITEMINFO mi;
                    ...
                    if (cmd != 0)
                    {
                        ...
                        // Map Network Drive command is 40 on XP, 43 on W2K, and only under Vista does it have a defined cmdName
                        ...
                        if (cmd != 10000 && cmd != 10001 && !clipCopy && !clipCut && !cmdDelete && !cmdMapNetDrv)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    SetCurrentDirectoryToSystem(); // so the drive can be unmapped even from the panel
                                }
                                else
                                {
                                ...
                                panel->FocusFirstNewItem = TRUE; // for both WinZip and its archives and the New menu (works well only when the panel auto-refreshes)
                                if (cmd < 5000)
                                {
                                    ...
                                    // we intercept cut/copy/paste, but still refresh the clipboard enablers just in case
                                    IdleRefreshStates = TRUE;  // force a check of the state variables on the next Idle
                                    IdleCheckClipboard = TRUE; // also let it check the clipboard
...
void ExecuteAssociation(HWND hWindow, const char* path, const char* name)
{
        ...
        TlsGetValue(ExecuteAssociationTlsIndex) == 0)       // this is not a recursive call
    {
        ...
        if (Configuration.UseSalOpen)
        {
            ...
            // if salopen.exe fails, launch the classic way (risk of open handles in the directory)
        }
...
// which the shell extension could kill with DestroyWindow (normally closes Explorer, but used to crash Salamander)
...
BOOL CanUseShellExecuteWndAsParent(const char* cmdName)
{
    ...
    // under Windows 8 Open With misbehaved – choosing a custom program did not show the Open dialog
    // https://forum.altap.cz/viewtopic.php?f=16&t=6730 a https://forum.altap.cz/viewtopic.php?t=6782
    // the problem is that the call returns from Invoke, but later Windows touches the parent window, which we have already destroyed
    // TODO: one idea would be to keep ShellExecuteWnd alive (a child window stretched over the entire Salamander area, completely in the background)
    // we would just have to verify before passing it that it is alive (no one destroyed it)
