...
HANDLE RefreshFinishedEvent = NULL; // waits for "PostMessage" processing to complete
                ...
                if (sameHandle != NULL)
                {
                    for (index = 0; index < ObjectArray.Count; index++)
                    {
                        if (sameHandle == (HANDLE)ObjectArray[index])
                        {
                            ...
                            if (r != WAIT_TIMEOUT) // if the error cleared, request the next change
                            {
                                FindNextChangeNotification((HANDLE)ObjectArray[index]); // discard this change
                            }
...
BOOL InitializeThread()
{
    ...
    // event for the "starter" of the "safe handle killer" thread
...
void TerminateThread()
{
    ...
    {
        SetEvent(TerminateEvent);              // request that the snooper finish its work
    ...
    if (SafeFindCloseThread != NULL)
    {
        ...
        {
            TerminateThread(SafeFindCloseThread, 666);          // if that fails, terminate it forcefully
...
void AddDirectory(CFilesWindow* win, const char* path, BOOL registerDevNotification)
{
    ...
    // trims the trailing characters and works with a different path
...
void ChangeDirectory(CFilesWindow* win, const char* newPath, BOOL registerDevNotification)
{
    ...
    for (i = 0; i < WindowArray.Count; i++)
        if (win == WindowArray[i])
        {
            ...
            // avoid waiting by letting another thread close it
            HANDLES(EnterCriticalSection(&SafeFindCloseCS));
...
{
    /*
#ifdef _DEBUG     // verify whether BeginSuspendMode() and EndSuspendMode() are invoked from the same function (based on the caller's return address -> cannot detect a "bug" when different functions share the same caller)
  DWORD *register_ebp;
