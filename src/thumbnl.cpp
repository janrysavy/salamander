                         ...
                         DWORD* outBuff, BOOL processTopDown)
{
    ...
    YCoeff = ColCoeff;
    // coefficients for the center and right pixel in case we need another pass
...
CShrinkImage::CreateCoeff(DWORD origLen, WORD newLen, DWORD& norm)
{
    ...
    {
        sum += origLen;
        // compute which pixel the new boundary intersects
        boundary = sum / newLen;
...
void CShrinkImage::ProcessRows(DWORD* inBuff, DWORD rowCount)
{
    ...
    for (y = Y; y < Y + rowCount; y++)
    {
        ...
        if (y == YBndr)
        {
            ...
            // for the last pixel skip computing the left part of the next pixel (there isn't one)
            for (; x2 < xBndr; x2++)
            {
                // fetch the pixel
                rgb = *inBuff++;
...
// Object cleanup—called before processing another thumbnail or when this object no longer needs one (finished or not)
void CSalamanderThumbnailMaker::Clear(int thumbnailMaxSize)
{
    ...
    if (thumbnailMaxSize != -1)
    {
        ...
        {
            // if the user changed the thumbnail size in the configuration, allocate ThumbnailBuffer and AuxTransformBuffer again
            if (ThumbnailBuffer != NULL)
            {
                free(ThumbnailBuffer);
                ThumbnailBuffer = NULL;
...
// Returns TRUE once the complete thumbnail is stored in this object (successfully obtained from the plugin)
