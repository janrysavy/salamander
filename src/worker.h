...
#define COPY_MIN_FILE_SIZE CQuadWord(4096, 0) // must be at least 1 (otherwise the DoCopyFile pre-copy allocation test stops working)
...
// 4/2012 - increased the buffer to ten times the old size; large files over the network now reach speeds comparable
// to Total Commander and finish 2-3x faster than with the previous one-tenth buffer
...
#define TRSPMETER_ACTSPEEDSTEP 200        // sample interval for calculating transfer speed in milliseconds (must not be 0)
#define TRSPMETER_ACTSPEEDNUMOFSTEPS 25   // number of samples used for transfer speed (more samples make the drop smoother when the first entry falls out of the queue)
#define TRSPMETER_NUMOFSTOREDPACKETS 40   // number of recent "packets" to remember for low-speed calculations (must not be 0)
#define TRSPMETER_STPCKTSMININTERVAL 2000 // minimum time span between the first and last stored "packet" when using them for low-speed calculations

class CTransferSpeedMeter
{
protected:
    // WARNING: access this object only while holding COperations::StatusCS

    // transfer speed calculation:
    DWORD TransferedBytes[TRSPMETER_ACTSPEEDNUMOFSTEPS + 1]; // circular queue storing bytes transferred during the last N intervals plus a working slot accumulating the current interval
    int ActIndexInTrBytes;                                   // index of the current entry in TransferedBytes
    DWORD ActIndexInTrBytesTimeLim;                          // timestamp boundary (ms) for the current entry; bytes keep accumulating until this time
    int CountOfTrBytesItems;                                 // number of slots in TransferedBytes (completed ones plus the working slot)
    ...
    int ActIndexInLastPackets;                               // write index for the next received "packet"; when full it also points to the oldest entry
    int CountOfLastPackets;                                  // count of valid entries in LastPacketsSize/LastPacketsTime
    DWORD MaxPacketSize;                                     // largest packet size we expect

public:
    BOOL ResetSpeed; // TRUE = the meter should be reset before the next reading (call JustConnected); the speed drop was so large we ended up displaying zero speed
...
public:
    ...
    // report a data chunk: 'count' bytes transferred in 'time'; 'maxPacketSize' is the largest amount expected
    // before the next BytesReceived() call
    void BytesReceived(DWORD count, DWORD time, DWORD maxPacketSize);

    // tunes 'progressBufferLimit' according to current packet data; 'lastFileBlockCount' is the limit we
    // must not cross (we consider only continuous copying of a single file; the counter is overflow-safe and
    // values > 1000000 simply mean "a lot", the exact figure is irrelevant); 'lastFileStartTime' is the GetTickCount() captured when
    // the most recent file copy started
...
// CProgressSpeedMeter
//
// measures the progress rate to estimate time remaining

#define PRSPMETER_ACTSPEEDSTEP 500         // sample interval for calculating progress speed in milliseconds (must not be 0)
#define PRSPMETER_ACTSPEEDNUMOFSTEPS 60    // number of samples used for progress speed (more samples make the drop smoother when the first entry falls out of the queue)
#define PRSPMETER_NUMOFSTOREDPACKETS 100   // number of recent "packets" to remember for low-speed calculations (must not be 0)
#define PRSPMETER_STPCKTSMININTERVAL 10000 // minimum time span between the first and last stored "packet" when using them for low-speed calculations
...
protected:
    ...
    DWORD TransferedBytes[PRSPMETER_ACTSPEEDNUMOFSTEPS + 1]; // circular queue storing bytes transferred during the last N intervals plus a working slot accumulating the current interval
    int ActIndexInTrBytes;                                   // index of the current entry in TransferedBytes
    DWORD ActIndexInTrBytesTimeLim;                          // timestamp boundary (ms) for the current entry; bytes keep accumulating until this time
    int CountOfTrBytesItems;                                 // number of slots in TransferedBytes (completed ones plus the working slot)
...
enum COperationCode
{
    ...
    ocCountSize,
    ocConvert,
    ocLabelForSkipOfCreateDir, // label to jump to when the script skips an ocCreateDirXXX; WARNING: SourceName and TargetName store the LO- and HI-DWORD of the total file sizes (including ADS) contained in the skipped directory; WARNING: Attr stores the ocCreateDirXXX index in the COperations array for that directory
...
public:
    ...
    int RemapNameFromLen;      // name remapping for MoveFiles (From -> To)
    ...
    BOOL TargetPathSupADS;      // TRUE if the copy/move target supports ADS (delete the file's ADS or the whole file before overwriting)
                                //    BOOL TargetPathSupEFS;       // TRUE if the copy/move target supports EFS (or more generally: it is NTFS rather than FAT)
    ...
    BOOL FastMoveUsed;     // is at least one file or directory being "renamed"? (then the total moved size would be misleading)
    ...
    char WorkPath1[MAX_PATH];  // non-empty string = first path processed (used for change notifications)
    BOOL WorkPath1InclSubDirs; // TRUE/FALSE = with/without subdirectories (first path)
    char WorkPath2[MAX_PATH];  // non-empty string = second path processed (used for change notifications)
...
private:
    ...
    CProgressSpeedMeter ProgressSpeedMeter; // meter for calculating "time left" (also tracks directory creation, empty copies, etc.; uses the same operation sizes as the progress meter)
    CQuadWord TransferredFileSize;          // bytes already copied/transferred (the final total should match TotalFileSize unless on-disk data change)
    ...
    DWORD LastSetupTime;            // GetTickCount() captured when we last computed the speed limit parameters plus any braking
    ...
    DWORD LastProgBufLimTestTime; // GetTickCount() from the last ProgressBufferLimit evaluation
    DWORD LastFileBlockCount;     // blocks copied since the last file started (WARNING: overflow-protected; values > 1000000 mean "a lot")
...
protected:
    ...
    // OperDlgs and OperPaused have the same number of elements and share indices (each operation uses the same index in both arrays)
    TDirectArray<HWND> OperDlgs;    // array of HWND handles for dialogs of operations in the queue
    TDirectArray<DWORD> OperPaused; // array describing queue state: 2/1/0 = "manually-paused"/"auto-paused"/"running"
...
public:
    ...
    // removes an operation from the queue (the operation finished); if 'doNotResume' is FALSE, it posts
    // a "resume" to the first operation in the queue whenever every operation is "paused";
    ...
    // moves operation 'dlg' to the end of the list and sets its state to "auto-paused"
...
// of Unicode names of all ADS (except the default ADS) - the elements and the array itself
// are allocated and must be freed by the caller; the array of names is returned only
// if no error occurred (see 'lowMemory' and 'winError') and ADS were found (the function returns TRUE);
// if 'streamNamesCount' is not NULL it returns the number of elements in 'streamNames';
// if 'lowMemory' is not NULL it returns TRUE when an out-of-memory error occurs (only possible when 'streamNames' is not NULL);
// if 'winError' is not NULL it returns the Windows error code (NO_ERROR if none occurred - if a Windows error occurs, the
// function always returns FALSE); the function returns TRUE if the file/directory contains ADS, otherwise FALSE;
// 'bytesPerCluster' is the cluster size used to compute disk space occupied by the ADS (0 = unknown size);
// in 'adsOccupiedSpace' (if not NULL) it returns the disk space occupied by the ADS;
// in 'onlyDiscardableStreams' (if not NULL) it returns TRUE if only ADS that can be discarded without prompting were found (currently only thumbnails from W2K)
