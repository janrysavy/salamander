...
**                                                                                       **
**   Built from the source code of 'LHA for UNIX, v.1.14'                                 **
**                                                                                       **
\*****************************************************************************************/
...
int LHAGetHeader(FILE* fp, LHA_HEADER* lpHeader)
{
    ...
    if (st.wDay == 0 && st.wHour == 0 && st.wMinute == 0 && st.wMonth == 0 &&
        st.wSecond == 0 && st.wYear == 1980) // LHA stores an unrepresentable date as
    {                                        // all zeros -> set minimum to 1 Jan 1980
        st.wDay = 1;
        st.wMonth = 1;
    }
    FILETIME ft;
    ...
    OemToChar(lpHeader->name, lpHeader->name); // handle Czech characters ... :-)
...
#define PROGRESS_MASK 0xffff8000 // update the progress meter position every 32 KB
...
int LHAOpenArchive(FILE*& f, LPCTSTR lpName)
{
    ...
    { // try reading a single header
        fseek(f, 0, SEEK_SET);
        return TRUE; // everything is OK, the archive is LHZ
    }

    // failed to read the first header, try whether the archive is an SFX
    ...
    {
        fclose(f);
        iLHAErrorStrId = IDS_LOWMEM; // unable to allocate buffer2, so bail out
        return FALSE;
    }
...
// TestLHAOpenArchive - tests LHAOpenArchive by running it on all
// files on the "path" (recurses into subdirectories). If it doesn't crash,
// everything is OK :-)
//
// Call it like this:
