...
//
// returns FALSE when the path is empty, otherwise TRUE (the path contains
// some non-space character)
...
BOOL ValidatePathIsNotEmpty(HWND hParent, const char* path)
{
    ...
    /* originally this function was called ValidateCommandFile and checked file
       existence, but we abandoned that idea (maybe because of network issues?)
  if (testFileExist)
  {
    // try to access the file
    if (!FileExists(myPath))
    {
      // the file was not found - it might not exist or the path may be
      // unreachable; let the user force the change
      char buff[1000];
...
void CLoadSaveToRegistryMutex::Init()
{
    ...
    // Up to this version the mutex was created in the local namespace under the
    // name SalamanderLoadSaveToRegistryMutex.
    // Now we want to provide interoperability across all sessions, so we place
    // it in the Global namespace.
    // We also append the SID (from Windows 2000 on) so that mutexes of
    // Salamander instances running under different users do not collide -- they
    // work with their own Registry trees, no synchronization needed there.
    // We could add the Salamander version into the mutex name as well (every
    // version has its own Registry tree). New versions can remove obsolete
    // configurations, so we do not do it.
    LPTSTR sid = NULL;
    if (!GetStringSid(&sid))
        sid = NULL;
...
CConfiguration::CConfiguration()
{
    ...
    AutoSave = TRUE;
    CloseShell = FALSE;
    ShowGrepErrors = FALSE; // other search tools (FAR/WinCmd/PowerDesk/Windows
                            // Find) do not show errors
    ...
    LastFocusedPage = 0;
    ConfigurationHeight = 0; // the dialog logic will not allow a smaller dialog
                             // than the largest page it contains
    ...
    SortNewerOnTop = FALSE; // by default sort like Explorer on XP, newer items
                            // at the bottom
    SortDirsByName = FALSE; // so people do not report it as a bug like they did
                            // to Ghisler
    SortDirsByExt = FALSE;  // directories have no extensions, kept for companies
                            // or users relying on the old directory ordering
    SaveHistory = TRUE;
    SaveWorkDirs = FALSE; // by default save space in the registry, the list is
                          // large
    EnableCmdLineHistory = TRUE;
    SaveCmdLineHistory = TRUE;
    //  LantasticCheck = FALSE;
    UseSalOpen = FALSE;
    NetwareFastDirMove = FALSE; // choose the slower but 100% working mode; power
                                // users can switch it
    ...
    // settings for displaying items in a panel
    FullRowSelect = FALSE;
    ...
    DrvSpecRemovableSimple = FALSE; // since 2.5b11 floppies have their own
                                    // category, so we can afford to read icons
    ...
    SkillLevel = SKILL_LEVEL_ADVANCED; // try to present as many options as
                                       // possible
...
void CConfigurationDlg::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        // ColorsChanged() calls a plug-in method (when colors change
        // PLUGINEVENT_COLORSCHANGED is fired) -> we must set the parent for
        // their message boxes
    ...
    case WM_DESTROY:
    {
        if (GetKeyState(VK_ESCAPE) & 0x8000) // prevent listing interruption in
                                             // the panel after each ESC
...
void CCfgPageRegional::Transfer(CTransferInfo& ti)
{
    ...
    else
    {
        if (stricmp(Configuration.SLGName, SLGName) != 0)
        {
            ...
            Configuration.ShowSLGIncomplete = TRUE; // if the language is not
                                                    // complete, show a message at
                                                    // startup (recruit
                                                    // translators)
        }
        // if the table has changed and the old one is already loaded we need to
        // restart Salamander
...
CCfgPageView::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // fill the list view with the Name, Mode and HotKey columns
        LVCOLUMN lvc;
        ...
        // dialog controls should stretch with its size, set splitter controls
        ...
        if (wParam == IDC_VIEW_LIST2)
        {
            ...
            case LVN_ITEMCHANGED:
            {
                ...
                if ((nmhi->uOldState & 0xF000) != (nmhi->uNewState & 0xF000))
                {
                    ...
                    LoadControls(); // when the column changes, load new data for
                                    // the radio buttons and edit line
                break;
            }
            }
            break;
...
void CCfgPageViewer::Validate(CTransferInfo& ti)
{
    ...
    if (ti.IsGood())
    {
        ...
        // provide MasksString, there is range checking so it is safe
    ...
    if (ti.IsGood())
    {
        ...
        Configuration.HexModeMasks.SetMasksString(buf); // restore HexModeMasks
...
void CCfgPageViewer::Transfer(CTransferInfo& ti)
{
    ...
    // provide MasksString, there is range checking so it is safe
    ti.EditLine(IDC_VIEW_INHEX, (char*)Configuration.HexModeMasks.GetWritableMasksString(), MAX_PATH);
    // provide MasksString, range checks ensure everything is fine
...
void CCfgPageViewer::LoadControls()
{
    ...
    logFont.lfHeight = GetWindowFontHeight(hEdit); // use the edit line font size
                                                   // for presentation
    if (HFont != NULL)
        HANDLES(DeleteObject(HFont));
...
CCfgPageViewer::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        if (HIWORD(wParam) == BN_CLICKED)
        {
            ...
            case IDB_VIEWERFONT:
            {
                /* used by the export_mnu.py script that generates salmenu.mnu
                   for the Translator. Keep synchronized with the InsertMenu()
                   call below...
...
*/
            ...
            case IDC_IV_SELECTED_TEXT:
            {
                ...
                if (button != NULL)
                {
                    /* used by the export_mnu.py script that generates
                       salmenu.mnu for the Translator. Keep synchronized with
                       the InsertMenu() call below...
MENU_TEMPLATE_ITEM CfgPageViewerMenu[] = 
{
  {MNTT_PB, 0
...
void CCfgPageUserMenu::Transfer(CTransferInfo& ti)
{
    ...
    else
    {
        SourceUserMenuItems->LoadUMI(*UserMenuItems, UserMenuIconBkgndReader.IsReadingIcons() ||
                                                         UserMenuIconBkgndReader.HasSysColorsChanged()); // after a system color change copying icons is not enough,
                                                                                                         // they must be reloaded (the dialog does not handle
                                                                                                         // icon color changes, icons might appear "broken")
    }
}

// pulls a child from the dialog and enables or disables it based on the
// 'enable' parameter; if 'enable' is FALSE and 'clear' is FALSE, it can also
// clear an edit line or a checkbox
        ...
        case IDL_MENUITEMS:
        {
            ...
            case EDTLBN_ENABLECOMMANDS:
            {
                    ...
                    UserMenuItems->At(index)->Type == umitSubmenuEnd)
                {
                    if (UserMenuItems->At(index)->Type == umitSubmenuBegin)
                    {
                        // umitSubmenuBegin - the start can always move up, down until
                        // its matching end hits the list end
                    ...
                    {
                        // umitSubmenuEnd - do not allow the end to move over a different
                        // end or start
                        if (index > 0)
                        {
...
CCfgPageHotPath::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        // ugly hack - need some message that arrives after WM_INITDIALOG
        // so that I can set the focus; hopefully this survives until the next
        // Salamander version :-)
        if (EditMode)
        {
            SetFocus(HListView);
...
{
    int Label;    // resource ID of the first color name [static before the button]
...
{
    int ItemLabel; // resource ID with the item name [combobox]
...
void CCfgPageColors::Transfer(CTransferInfo& ti)
{
    ...
    else
    {
        ...
        ColorsChanged(TRUE, TRUE, FALSE); // save time, change only color-dependent
                                          // items, do not reload icons
...
{
    // set panel paths to fixed ones (otherwise leaving the path could
    ...
    // history stored in the configuration (SelectHistory, CopyHistory,
    // EditHistory, ChangeDirHistory, FileListHistory)
    Configuration.ClearHistory();
    MainWindow->EditWindow->FillHistory();

    // Find dialog history including combo boxes of open windows
    ClearFindHistory(FALSE);

    // internal viewer history including combo boxes of open Find windows
