...
BOOL CMenuSharedResources::Create(HWND hParent, int width, int height)
{
    ...
    // determine enlargement factor for bitmaps
...
void CMenuPopup::LayoutColumns()
{
    ...
    // for strings let it compute column widths
    int i;
    for (i = 0; i < Items.Count; i++)
    {
        ...
        else
        {
            if (item->Type & MENU_TYPE_STRING)
            {
                ...
                {
                    // obtain dimensions via a query (HBMMENU_CALLBACK is used by TortoiseCVS for icons in the context menu)
                    MEASUREITEMSTRUCT mis;
                    mis.CtlType = ODT_MENU;
                    mis.CtlID = 0;
                    mis.itemID = item->ID;
            ...
            else
            {
                ...
                {
                    {
                        // obtain dimensions for the bitmap
                        BITMAP bitmap;
    ...
    for (i = 0; i < Items.Count; i++)
    {
        ...
        if (!(item->Type & MENU_TYPE_OWNERDRAW) && (item->Type & MENU_TYPE_STRING))
        {
            ...
            // stretch the menu after the loop
        }
        else
        {
...
void CMenuPopup::DrawCheckBitmapVista(HDC hDC, CMenuItem* item, int yOffset, BOOL selected)
{
    ...
    if (item->HBmpItem != NULL)
    {
        ...
        // center the check mark if the line height is bigger than the check mark height
        int myYOffset = 0;
        if (item->Height > SharedRes->TextItemHeight)
    ...
    */
        ...
        // the bitmap may actually be smaller than requested - adjust the values
        BITMAP bitmap;
        GetObject(hBitmap, sizeof(bitmap), &bitmap);

        int bmpX = 0; // from which position in the bitmap to extract the image
        // 2.12.2012: if the resolution is set to 105% DPI (or 125, 150, etc.) on Windows 7
        ...
        // I checked the repository history and believe no hacks belong here; the values must be assigned directly
        // tested under Windows 7 with various DPIs using TortoiseSVN and HG, SourceGear DiffMerge, BeyondCompare, Adobe Acrobat and it looks OK
...
void CMenuPopup::DrawCheckBitmap(HDC hDC, CMenuItem* item, int yOffset, BOOL selected)
{
        ...
        (item->HBmpUnchecked != NULL && !(item->State & MENU_STATE_CHECKED)))
    {
        ...
        if (item->State & MENU_STATE_GRAYED)
        {
            ...
            // we have a monochrome bitmap and can draw it in the disabled state
...
void CMenuPopup::DrawCheckImage(HDC hDC, CMenuItem* item, int yOffset, BOOL selected)
{
    ...
    // center the image if the line height is bigger than the image height
    int myYOffset = 0;
    if (item->Height > SharedRes->TextItemHeight)
        myYOffset += (item->Height - SharedRes->TextItemHeight) / 2;
    ...
    if (item->State & MENU_STATE_GRAYED)
    {
        ...
        // copy a monochrome version of the icon into it (white stays white)
        if (item->HIcon != NULL)
...
void CMenuPopup::DrawItem(HDC hDC, CMenuItem* item, int yOffset, BOOL selected)
{
    ...
    // do not draw the item if it lies outside the visible area
    int topLimit = 0;
    ...
    // to prevent flicker when there is a check mark, shift the background
    int xO = 0;
    if (item->Height == SharedRes->TextItemHeight &&
        !(item->Type & MENU_TYPE_OWNERDRAW) &&
    ...
    else
    {
        ...
        // in any case set and restore the font - we can't know
        // what rascals might do with the DC
        if (item->State & MENU_STATE_DEFAULT)
            hOldFont = (HFONT)SelectObject(hDC, SharedRes->HBoldFont);
        else
