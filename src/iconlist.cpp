...
BOOL CIconList::CreateOrEnlargeTmpImage(int width, int height)
{
    ...
    {
        // WARNING: using BITMAPV4HEADER caused Salamander to crash in ICM32.DLL,
        // the old BITMAPINFOHEADER is apparently sufficient
        ...
        if (HTmpImage != NULL)
            HANDLES(DeleteObject(HTmpImage)); // remove the old bitmap if there is one
...
BOOL CIconList::Create(int imageWidth, int imageHeight, int imageCount)
{
    ...
    ZeroMemory(ImageFlags, sizeof(BYTE) * imageCount); // without an alpha channel
    ...
    // auxiliary DC so we can create a compatible bitmap
    ...
    // create the cache/mask bitmap if it does not exist or is too small
...
BYTE CIconList::ApplyMaskToImage(int index, BYTE forceXOR)
{
    ...
    // scan the entire icon and determine its type (NORMAL, XOR, ALPHA)
...
SKIP:
    ...
    {
        // prepare the background color in the proper format
        DWORD bkClr = GetRValue(BkColor) << 16 | GetGValue(BkColor) << 8 | GetBValue(BkColor);
        // copy transparent mask areas to the alpha channel
        int row;
        for (row = 0; row < ImageHeight; row++)
        {
            DWORD* ptr = ImageRaw + iX + (iY + row) * BitmapWidth;
...
BOOL CIconList::ReplaceIcon(int index, HICON hIcon)
{
    ...
    // Honza: on Windows 10 the CopyImage call started crashing in debug x64 when LR_COPYFROMRESOURCE was enabled
    // FIXME: audit whether this downscaling is still needed since SalLoadIcon() now calls LoadIconWithScaleDown()
    hIcon = (HICON)CopyImage(hIconOrig, IMAGE_ICON, ImageWidth, ImageHeight, /*LR_COPYFROMRESOURCE | */ LR_COPYRETURNORG);

    // retrieve the icon's MASK and COLOR bitmaps from the handle
    ...
    // for a black-and-white icon the height should be even
    ...
    // ii.hbmColor -> HImage (if hbmColor is NULL, the XOR part resides in the lower half of hbmMask
    if (ii.hbmColor != NULL)
...
BAIL:
    ...
    // if the size was changed, the temporary icon must be destroyed
    if (hIcon != hIconOrig)
        DestroyIcon(hIcon);
...
CIconList::GetIcon(int index)
{
    ...
    // create a B&W mask and a color bitmap based on the display
    HDC hDC = HANDLES(GetDC(NULL));
    ...
    // prepare the COLOR part of the icon into HTmpImage
    int row;
    for (row = 0; row < ImageHeight; row++)
    {
    ...
    // prepare the MASK part of the icon into HTmpImage
    for (row = 0; row < ImageHeight; row++)
    {
        DWORD* imagePtr = ImageRaw + iX + (iY + row) * BitmapWidth;
        DWORD* tmpPtr = TmpImageRaw + row * TmpImageWidth;
    ...
    // create an icon from hColor and hMask
    ICONINFO ii;
    ii.fIcon = TRUE;
    ii.xHotspot = 0;
    ii.yHotspot = 0;
    ...
    ii.hbmMask = hMask;
    HICON hIcon;
    hIcon = CreateIconIndirect(&ii); // must not be in handles; exports the icon outside Salamander
...
BOOL CIconList::DrawMask(HDC hDC, int x, int y, int index, COLORREF fgColor, COLORREF bkColor)
{
    ...
    for (row = 0; row < ImageHeight; row++)
    {
        ...
        for (col = 0; col < ImageWidth; col++)
        {
            ...
            if (alpha != 0) // see the comment above
                *tmpPtr = clr;
...
BOOL CIconList::DrawALPHA(HDC hDC, int x, int y, int index, COLORREF bkColor)
{
    ...
    // in the first phase transfer the data to HMask
    SelectObject(HMemDC, HTmpImage);
...
BOOL CIconList::DrawAsAlphaLeaveBackground(HDC hDC, int x, int y, int index, COLORREF fgColor)
{
    ...
    for (row = 0; row < ImageHeight; row++)
    {
        ...
        {
            DWORD argb = *imagePtr;
            BYTE alpha = 255 - (BYTE)(argb & 0x000000FF); // all channels carry the same value considered as the alpha channel
            if (alpha != 0)
            {
...
BOOL CIconList::AlphaBlend(HDC hDC, int x, int y, int index, COLORREF bkColor, COLORREF fgColor)
{
    ...
    // is this the variant that requires XOR drawing?
...
BOOL CIconList::SetBkColor(COLORREF bkColor)
{
  ...
  */
    ...
    {
        if (ImageFlags[index] != IL_TYPE_NORMAL) // background color only makes sense for normal icons
            continue;
...
BOOL CIconList::CreateFromBitmap(HBITMAP hBitmap, int imageCount, COLORREF transparentClr)
{
    ...
    // add stripes from the source bitmap row by row
    int index = 0;
...
BOOL CIconList::ConvertToGrayscale(BOOL forceAlphaForBW)
{
    if (ImageCount > 0)
    {
        ...
        for (i = 0; i < BitmapWidth * BitmapHeight; i++)
        {
            ...
            if (forceAlphaForBW)
            {
                // when output goes to a B&W bitmap, set the threshold between white and black (tuned on Vista icons in the user menu)
                if (alpha < 200)
                    alpha = 0;
                if (brightness > 240)
                    alpha = 0;
...
BOOL CIconList::SaveToPNG(BYTE** rawPNG, DWORD* rawPNGSize)
{
    ...
    callbackData.FreeSpace = ImageWidth * ImageCount * ImageHeight * 4 * 2; // better allocate double the memory so the PNG MUST fit
    callbackData.Size = 0;
