...
// table of packer associations by extension
...
CPackACDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDB_ACSTOP:
        {
            // if searching is running the button acts as STOP, otherwise Rescan
            if (SearchRunning)
            {
            ...
            else
            {
                ...
                {
                    // if the focus was on one of the buttons we must restore it
    ...
    {
        // the searching thread sent us a new path it is working on, so trim the trailing slash
    ...
    case WM_USER_ACFINDFINISHED:
    {
        ...
        // if the thread ended because we are closing the window, continue closing
        if (WillExit)
...
// the actual (and recursive :-) ) function that searches the disk
BOOL CPackACDialog::DirectorySearch(char* path)
{
    ...
    // allocate buffer for the search mask
...
void CPackACDialog::AddToCustom(int foundIndex, int packerIndex, CPackACPacker* foundPacker)
{
    ...
    // add custom packers for the newly found archiver
    char variable[50];
    int i;
    ...
    // add custom unpackers for the newly found archiver
...
void CPackACDialog::RemoveFromCustom(int foundIndex, int packerIndex)
{
    ...
    //   2) the invoked program is a variable corresponding to this archiver

    CPackACPacker* p = NULL;
    char variable[50];
    sprintf(variable, "$(%s)", ArchiverConfig->GetPackerVariable(packerIndex));
    int i;
    // search custom packers backwards so we can delete items
    for (i = PackerConfig.GetPackersCount() - 1; i >= 0; i--)
        ...
        if (PackerConfig.GetPackerType(i) >= 0)
        {
            ...
            else if (PackerConfig.GetPackerSupMove(i))
            {
                ...
                if (!strcmp(cmd, variable))
                {
                    // calls a program on move cmd that does not exist - disable it
...
void CPackACDialog::Transfer(CTransferInfo& ti)
{
    ...
    else
    {
        ...
        int i;
        // iterate over all packers - configure paths and remove associations for those not found
...
// return whether the item is selected or if it is a header
                                  ...
                                  const CQuadWord& size, EPackExeType exeType)
{
    ...
    // the extension has been checked already; now verify the end of the string
    // and whether other requirements are met (currently only the type)
...
// ensure only the newest item is selected and add it to the array
...
void CPackACArray::InvertSelect(int index)
{
    ...
    {
        // but only one item may be selected
        int i;
        for (i = 0; i < Count; i++)
...
int CPackACListView::GetCount()
{
    ...
    {
        // ask each how many entries it has and add the heading
...
void CPackACListView::InvertSelect(int index)
{
    ...
    if (PackersTable != NULL && !FindArchiver(index, &archiver, &arcIndex))
    {
        ...
        // and repaint what is necessary
        UpdateWindow(HWindow);
    }
}

// return the found archiver for the list view item
CPackACPacker*
...
void CPackACListView::Initialize(APackACPackersTable* table)
{
    ...
    // set the columns
    InitColumns();
    // set the initial number of items in the list view
    ListView_SetItemCount(HWindow, GetCount());
    // set focus on the first list view item
    ListView_SetItemState(HWindow, 0, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
}

// set column captions and basic widths of the list view
BOOL CPackACListView::InitColumns()
{
    ...
    // we keep the checkbox state, windows asks for it
    ListView_SetCallbackMask(HWindow, LVIS_STATEIMAGEMASK);
    // done
    return TRUE;
}

// set the column widths in the list view based on the window size
void CPackACListView::SetColumnWidth()
{
    ...
    // subtract widths of the fixed columns (size, date, time)
                                   ...
                                   const CQuadWord& size, EPackExeType type)
{
    ...
    BOOL stop = FALSE;
    int totalCount = 0;
    // go through all packers to see if it is one we are looking for
    int i;
    for (i = 0; i < PackersTable->Count; i++)
    {
        // take the archiver
        ...
        // keep total count for the list view index
...
// window procedure for the list view with found archivers
LRESULT
CPackACListView::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        // handle space bar to toggle checkboxes
        if (wParam == VK_SPACE)
        {
...
// return the text that belongs to the given column
...
// dialog procedure for selecting drives to search
INT_PTR
        ...
        case IDC_ACDRVLIST:
        {
            ...
            case EDTLBN_GETDISPINFO:
            {
                ...
                else
                {
                    ...
                    while (*txt == ' ' || *txt == '\t')
                        txt++;
                    // determine length without trailing spaces
                    int len = (int)strlen(txt) - 1; // if txt is empty string, len == -1
    ...
    else
    {
        if (Dirty)
        {
            ...
            for (i = 0; i < EditLB->GetCount(); i++)
            {
                ...
                len += pathlen + 1;
                // if the path does not end with a backslash account for one
