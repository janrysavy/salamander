...
void CMenuBar::DrawItem(HDC hDC, int index, int x)
{
    ...
    // paint the stripe above and below (for Windows Vista rebar)
    RECT r2 = r;
    r2.top = 0;
    r2.bottom = 1;
    ...
    // the window repaints all bands, causing the whole menu to redraw and flicker
    // (older OSes only redrew what was needed). Caching might hide the issue, but
    // a better solution is rewriting the rebar and avoiding the broken MS code.
...
void CMenuBar::EnterMenuInternal(int index, BOOL openWidthSelect, BOOL byMouse)
{
    ...
    else
    {
        ...
        TrackHotIndex(); // the expanded menu captures the mouse
        if (ExitMenuLoop)
            leaveLoop = TRUE;
    }
    ...
    while (!leaveLoop)
    {
        ...
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE) != 0)
        {
            ...
            case WM_KEYDOWN:
            {
                ...
                {
                    leaveLoop = TRUE;
                    break; // let the message through so releasing Alt won't open the system menu
                }
                ...
                case VK_RETURN:
                {
                    ...
                    if (HotIndex != -1)
                    {
                        OpenWithSelect = TRUE; // select the first item
                        OpenByMouse = FALSE;
                        TrackHotIndex();
                        if (ExitMenuLoop)
                            leaveLoop = TRUE;
                ...
                // patch: we must not generate WM_CHAR that would reach the owner
                // menu and cause a beep (Find dialog)
...
*/
    ...
    // remove ourselves from the closing message monitoring
    ...
    // deliver the result next - the same is handled in CMenuPopup::Track
    ...
    if (hWndUnderCursor != NULL)
    {
        ...
        // and used Alt+Tab, Alt+Tab to switch between EV/SS windows, the EV window sometimes paused for 2s
...
BOOL CMenuBar::IsMenuBarMessage(CONST MSG* lpMsg)
{
    ...
    case WM_SYSKEYDOWN:
    {
        ...
        {
            WheelDuringMenu = FALSE; // if the user scrolled the wheel and then typed a number (Alt+numXXX), ignore the wheel
            HandlingVK_MENU = FALSE;
        }
        if (wParam == VK_F10)
        {
    ...
    case WM_SYSKEYUP:
    {
        if (lpMsg->wParam == VK_MENU)
        {
            ...
            {
                WheelDuringMenu = FALSE;
                return TRUE; // swallow Alt release after Alt+Wheel, otherwise the window menu opens without being shown
            }
...
CMenuBar::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        // sent by ourselves from mainwindow
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (MenuLoop)
        {
            // prevents conflicts between mouse movement and Left/Right keys
        ...
        if (!HotIndexIsTracked)
        {
            ...
            if (hitItem)
            {
                if (!HelpMode2 && !MenuLoop && !MouseIsTracked) // capture is needed only in track mode
                {
                    TRACKMOUSEEVENT tme;
            ...
            {
                if (!MenuLoop) // if in MenuLoop, keep the last selected item highlighted
                {
                    newHotIndex = -1;
                    if (MouseIsTracked)
                        WindowProc(WM_MOUSELEAVE, 0, 0);
