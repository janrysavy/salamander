...
//
// ****************************************************************************
// CPluginInterfaceEncapsulation + encapsulation of parts of the plugin interface
//
// class through which Salamander accesses CPluginInterfaceAbstract methods,
// ensures EnterPlugin and LeavePlugin are called; direct interface calls may lead to errors
...
public:
    ...
    // methods of CPluginInterfaceForArchiverAbstract without "virtual" (would be unnecessary and slower)
...
public:
    ...
    // CPluginInterfaceForViewerAbstract methods without 'virtual' to avoid overhead
...
protected:
    ...
    int BuiltForVersion;                           // valid only when the plugin is loaded: Salamander version the plugin was compiled for (see LAST_VERSION_OF_SALAMANDER in spl_vers.h)
...
public:
    ...
    // CPluginInterfaceForMenuExtAbstract methods without 'virtual' to avoid overhead
...
public:
    ...
    // returns the Salamander version this plugin was built for (see LAST_VERSION_OF_SALAMANDER in spl_vers.h)
    int GetBuiltForVersion() { return BuiltForVersion; }

    // CPluginInterfaceForFSAbstract methods without 'virtual' to avoid overhead
    ...
    // direct interface call (bypasses CPluginData + InitDLL), a call-stack message is created
    ...
    // direct interface call (bypasses CPluginData + InitDLL), call-stack-message is created;
...
public:
    ...
    // methods of CPluginInterfaceForThumbLoaderAbstract without "virtual" (it's unnecessary and would just slow things down)

    // does not need Enter/LeavePlugin because it may use only sal-general methods
    // that can be called from any thread (panel operations are not)
...
{
protected:
    CPluginInterfaceAbstract* Interface; // encapsulated interface; during DLL entry it is set to -1
    int BuiltForVersion;                 // valid only while 'Interface' is active: Salamander version the plugin was compiled for (see LAST_VERSION_OF_SALAMANDER in spl_vers.h)
...
public:
    ...
    // CPluginInterfaceAbstract methods without 'virtual' to avoid overhead
...
//
// Class through which Salamander accesses CPluginDataInterfaceAbstract methods.
// It ensures EnterPlugin and LeavePlugin are invoked. Direct calls to the
// interface may lead to errors (for example refreshing a panel while its data
...
protected:
    ...
    int BuiltForVersion;                     // valid only while 'Interface' is active: Salamander version this plugin was compiled for (see LAST_VERSION_OF_SALAMANDER in spl_vers.h)

    const char* DLLName;              // string reference from the CPluginData of the plugin that created this interface
    const char* Version;              // version string from the plugin that created this interface
...
public:
    ...
    // returns the DLL name string stored in the CPluginData that created this interface
    const char* GetDLLName() { return DLLName; }

    // returns the version string stored in the CPluginData that created this interface
    ...
    // returns the Salamander version this plugin was compiled for (see LAST_VERSION_OF_SALAMANDER in spl_vers.h)
    int GetBuiltForVersion() { return BuiltForVersion; }

    // faster variant of ReleasePluginData
    ...
    // CPluginDataInterfaceAbstract methods without 'virtual' for speed
...
//
// class through which Salamander accesses CPluginFSInterfaceAbstract methods;
// it ensures EnterPlugin and LeavePlugin are called. Direct calls to
// CPluginFSInterfaceAbstract methods may cause issues (for example, refreshing
...
protected:
    ...
    DWORD PluginFSCreateTime;  // creation time of this FS (0 == uninitialized)

    int ChngDrvDuplicateItemIndex; // index of the duplicate item in the Change Drive menu (unused if the menu lacks a duplicate for this FS) (0 = uninitialized)
...
public:
    ...
    // returns the Salamander version this plugin was compiled for (see
    ...
    // returns the index of the opened FS name
    ...
    // returns TRUE if 'fsName' belongs to the same plugin as this FS; if TRUE,
    ...
    // CPluginFSInterfaceAbstract methods without 'virtual' to avoid overhead
    ...
    // WARNING: used exclusively within the EnterPlugin+LeavePlugin section, do not call outside of it!
    ...
    // ********************************************************************************
    // WARNING: lower the thread priority before starting an operation in the plugin
    // ********************************************************************************
...
// the interface should handle repeated calls (listing may not succeed on the
// first attempt for FS or may fail completely). Either ensure changes are made
...
public:
    ...
    // helper: sets the encapsulated WinLib object
...
protected:
    ...
    // if not, it deletes the pointer and returns FALSE
...
{
protected:
    CPluginInterfaceAbstract* Plugin; // plug-in using this iface; note: may be NULL
                                      // (plugin not loaded) or -1 during the plugin's entry point

    char HelpFileName[MAX_PATH]; // if not empty, name (without path) of the .chm help file used by this plugin (optimization only, stored nowhere)

public:
    HINSTANCE LanguageModule; // if not NULL, handle to the plugin's .SLG language module
...
public:
    ...
    // helper non-virtual function returning a pointer to the panel according to PATH_TYPE_XXX
...
{
    pmitItemOrSeparator, // item or separator (Name is missing or NULL)
    pmitStartSubmenu,    // start of a submenu (an item that opens a submenu containing items up to the end marker)
    pmitEndSubmenu,      // end-of-submenu marker (used only while building the menu, other fields are blank)
};

// If this bit is set in HotKey, the user changed the key in configuration and
// Plugin::Connect()/AddMenuItems must preserve it
#define HOTKEY_DIRTY 0x00010000
#define HOTKEY_HINT 0x00020000 // 'Name' contains a tab followed by the hot key shown in the menu when the user has not assigned a custom key
...
public:
    ...
    char* Name;               // menu item text; NULL denotes a separator
    ...
    DWORD HotKey;             // hot key: LOWORD=hotkey(LOBYTE:vk, HIBYTE:mods), HIWORD=(0:user kept it, 1:user changed it, dirty)
...
public:
    ...
    char* Name;                // plugin name shown in Extensions/C.Packers/C.Unpackers dialogs
                               // max length MAX_PATH - 1
    char* DLLName;             // DLL file name, relative to "plugins" or absolute
                               // max length MAX_PATH - 1
    BOOL SupportPanelView;     // TRUE => implements ListArchive, UnpackArchive, UnpackOneFile (panel archiver/view)
    BOOL SupportPanelEdit;     // TRUE => implements PackToArchive, DeleteFromArchive (panel archiver/edit)
    BOOL SupportCustomPack;    // TRUE => implements PackToArchive (custom archiver/pack)
    BOOL SupportCustomUnpack;  // TRUE => implements UnpackWholeArchive (custom archiver/unpack)
    BOOL SupportConfiguration; // TRUE => implements Configuration (can be configured)
    BOOL SupportLoadSave;      // TRUE => implements registry load/save (persistence)
    BOOL SupportViewer;        // TRUE => implements ViewFile and CanViewFile (file viewer)
    BOOL SupportFS;            // TRUE => implements a file system
    ...
    char* LastSLGName; // name of the last .SLG file used (NULL = none yet or same language as Salamander)

    char* PluginHomePageURL; // URL of the plugin home page (NULL == no home page)

    char* ChDrvMenuFSItemName;    // filesystem command in the change-drive menu: name (max length MAX_PATH - 1)
    int ChDrvMenuFSItemIconIndex; // filesystem command in the change-drive menu: icon index (in PluginIcons; -1=no icon; index isn't validated)
    BOOL ChDrvMenuFSItemVisible;  // filesystem command in the change-drive menu: visible? (users can hide it from Plugins Manager)

    TIndirectArray<CPluginMenuItem> MenuItems; // array of menu items
    BOOL DynMenuWasAlreadyBuild;               // TRUE if BuildMenu() was already called; further calls are ignored

    char* BugReportMessage; // message shown by the Bug Report dialog when a plugin exception occurs
    char* BugReportEMail;   // e-mail shown by the Bug Report dialog when a plugin exception occurs

    CMaskGroup ThumbnailMasks;   // masks determining files for which the plugin can create thumbnails (max length MAX_GROUPMASK - 1); NULL if it generates none
    BOOL ThumbnailMasksDisabled; // TRUE only when the plugin is unloading/removing

    BOOL ArcCacheHaveInfo;    // TRUE if information about archive disk cache usage is already known (GetCacheInfo called)
    char* ArcCacheTmpPath;    // location of cached files from archives: NULL = TEMP, otherwise the cache root path
    ...
    int PluginIconIndex;          // index of the plugin icon (Plugins window/Plugins menu + Help/About Plugin and optional submenu icon) (index in PluginIcons; -1=no icon; index isn't validated)
    int PluginSubmenuIconIndex;   // icon index for the plugin submenu in the Plugins menu and optional toolbar drop-down (index in PluginIcons; -1=use PluginIconIndex; index isn't validated)
    BOOL ShowSubmenuInPluginsBar; // TRUE => show a drop-down button with the plugin submenu on the top toolbar using PluginSubmenuIconIndex

    BOOL PluginIsNethood;           // TRUE if the plugin replaces the Network item from the Change Drive menu (added for Nethood)
    BOOL PluginUsesPasswordManager; // TRUE if the plugin uses the Password Manager

    int IconOverlaysCount; // number of icon-overlay triples in the IconOverlays array
    HICON* IconOverlays;   // allocated array of icon overlays, count = 3 * IconOverlaysCount (sizes: 16 + 32 + 48)
    ...
    BOOL ShouldUnload;      // TRUE if the plugin should unload at the earliest opportunity
    BOOL ShouldRebuildMenu; // TRUE if the plugin menu should be rebuilt at the earliest opportunity
    ...
    BOOL OpenPackDlg;                // TRUE to open the Pack dialog for this plugin
    int PackDlgDelFilesAfterPacking; // Pack dialog: "Delete files after packing" checkbox: 0=default, 1=on, 2=off
    BOOL OpenUnpackDlg;              // TRUE to open the Unpack dialog for this plugin
...
protected:
    HINSTANCE DLL;                                                         // handle to the plugin's DLL file
    CPluginInterfaceEncapsulation PluginIface;                             // plugin interface (set to -1 during the entry point)
...
public:
    ...
    // loads the DLL into memory, attaches to it and verifies the stored information (SupportXXX, etc.)
    ...
    // (messages from inside the plugin are still displayed)
    // 'waitCursor' shows the Wait cursor while loading the DLL
    // if 'showUnsupOnX64' is TRUE, a message box warns about plugins unsupported on x64
    // if 'releaseDynMenuIcons' is TRUE, dynamic menu icons are released (they are reloaded before opening the menu)
    ...
    // returns the plugin name extended with "(Plugin)"; used where it's not obvious it is a plugin
    ...
    // unloads the DLL; if 'ask' is TRUE it prompts to save when "save on exit" is on,
    ...
    // searches the plugin menu items for a command with 'id'; if found, sets
    ...
    // helper method: builds one submenu level from MenuItems (recursive for nested levels);
    // 'menu' is the submenu receiving the items;
    // 'i' is the current index in MenuItems - upon return it is past the array end or on an end-submenu marker;
    // 'count' is the number of items in the submenu 'menu'; 'mask' is the precomputed state mask
    ...
    // runs a menu command identified by 'suid' (response to WM_COMMAND),
    ...
    // shows help for the menu command identified by 'suid' (response to WM_COMMAND),
    // 'parent' is the parent message box window, 'index' is this plugin index in Plugins,
    // returns TRUE if 'suid' was found among the plugin's menu items,
    ...
    // obtain archive disk-cache settings - uses a plugin call
    ...
    // returns TRUE if the plugin is an archiver and handles deleting cached files itself
    ...
    // fills 'mii::State' and 'mii::Type' according to the command for 'pluginIndex' and 'menuItemIndex'
    ...
    // extracts hot keys with the dirty bit set from 'oldMenuItems' and moves them to 'MenuItems'
    // synchronization is driven by the command ID
...
//   i < 0  => (-i - 1) is the index in CPlugins for the plugin used for "panel-arch. view/edit"
//   i >= 0 => i is the index in CArchiverConfig for an external "panel-arch. view/edit"
//
// 'Type' in CPackerConfig and CUnpackerConfig:
//   i == 0 => external archiver; data for external "custom pack/unpack" are used
//   i < 0 => (-i - 1) is the index in CPlugins for the plugin used for "custom pack/unpack"
//
// 'ViewerType' in CViewerMasks:
//   i == 0 => external viewer; data for the external "file viewer" are used
//   i == 1 => internal text/hex viewer
//   i < 0 => (-i - 1) is the index in CPlugins for the plugin used for "file viewer"
...
struct CPluginOrder
{
    ...
    BOOL ShowInBar; // used only when converting old configuration
...
protected:
    ...
    CRITICAL_SECTION DataCS; // critical section used only to synchronize data accessed by GetIndex()
    ...
    DWORD TimerTimeCounter;                           // "time" for adding timers (prevents endless loops inside CPlugins::HandlePluginFSTimers())
    ...
    // if LastPlgCmdPlugin == NULL, LastPlgCmdID is meaningless and the menu item will be disabled with default text
...
public:                     // helper variables for menu items coming from plugins:
    int LastSUID;           // helper variable for generating menu item SUIDs
    ...
    DWORD LoadInfoBase; // base for flags returned via CSalamanderPluginEntry::GetLoadInformation()
...
public:
    ...
    // saves the object to the registry (plugin info + plugin configuration),
    ...
    // removes from Data plugins whose .spl no longer exists; if 'canDelPluginRegKey' is TRUE,
    // their registry configuration is also deleted. 'notLoadedPluginNames' (buffer of size
    // 'notLoadedPluginNamesSize') receives a list of up to 'maxNotLoadedPluginNames' names of
    // plugins not loaded but with registry configuration (either removed or failed InitDLL()),
    // separated by ", "; 'numOfSkippedNotLoadedPluginNames' (if not NULL) returns the number of
    // names not stored in 'notLoadedPluginNames'. 'loadAllPlugins' is TRUE only when upgrading
    // to a new Salamander version and all plugins should be loaded; those that fail and still
    // have registry configuration are stored in 'notLoadedPluginNames'
    ...
    // version of plugins.ver was found (configuration must be saved so the process doesn't repeat
    // on the next start)
    BOOL ReadPluginsVer(HWND parent, BOOL importFromOldConfig);

    // loads all plugins and calls their ClearHistory method
    void ClearHistory(HWND parent);
    ...
    // rebuilds the menu and unloads all plugins that requested it; returns the first Salamander/menu
    // command requested by plugins (WM_COMMAND is sent immediately to the main window) along with the plugin in 'data'
    void GetCmdAndUnloadMarkedPlugins(HWND parent, int* cmd, CPluginData** data);

    // returns the plugin requesting the Pack/Unpack dialog; if none, returns NULL in 'data'
    // and -1 in 'pluginIndex'
    void OpenPackOrUnpackDlgForMarkedPlugins(CPluginData** data, int* pluginIndex);

    // sequentially returns salamand.exe and all plugins with versions; index counts from zero (in/out)
    // returns TRUE if the result is valid
    BOOL EnumInstalledModules(int* index, char* module, char* version);

    // adds a plugin, returns success
    ...
    // stores in 'uniqueKeyName' a unique registry key name for plugin private data,
    // the name is based on 'regKeyName'
    void GetUniqueRegKeyName(char* uniqueKeyName, const char* regKeyName);

    // stores in 'uniqueFSName' a unique FS name based on 'fsName';
    ...
    // fs names from previous plugin loads from which a unique name is preferably chosen
    ...
    // and CSalamanderBuildMenu. Avoid storing CPluginData pointers to look them up later.
    // NOTE: the index is valid only until the number of plugins changes
    ...
    // returns the CPluginData containing iface 'plugin', or NULL if absent
    // NOTE: the pointer is valid only until the number of plugins changes
    ...
    // returns the CPluginData whose DLLName equals dllName (DLLName is allocated once -> unique
    // plugin identifier). Returns NULL if not found.
    // NOTE: the pointer is valid only until the number of plugins changes
    CPluginData* GetPluginData(const char* dllName);

    // returns the CPluginData whose DLLName ends with 'dllSuffix'; returns NULL if missing
    // NOTE: the pointer is valid only until the number of plugins changes
    CPluginData* GetPluginDataFromSuffix(const char* dllSuffix);

    // creates an image list (color if gray == FALSE, otherwise grayscale)
    // each plugin is represented by a single icon in the list
    // a default plug icon is used when the plugin lacks its own icon
    HIMAGELIST CreateIconsList(BOOL gray);

    // adds all plugin names to the list view; if setOnly is TRUE only values are set;
    // 'numOfLoaded' receives the number of loaded plugins
    void AddNamesToListView(HWND hListView, BOOL setOnly, int* numOfLoaded);

    // adds pointers to CPluginData of plugins providing thumbnails
    // NOTE: the pointers are valid only until the number of plugins changes
    void AddThumbLoaderPlugins(TIndirectArray<CPluginData>& thumbLoaderPlugins);

    // adds plugin names to a menu
    // the number of entries is limited by maxCount
    // commands are assigned as firstID + itemIndex
    ...
    // handles WM_USER_INITMENUPOPUP for the submenu of a plugin in the Plugins menu
    ...
    // initializes menu items of a plugin; 'parent' is the message box window,
    ...
    // adds plugin buttons that have menus to the toolbar when they should be visible
    BOOL InitPluginsBar(CToolBar* bar);

    // runs the menu command identified by 'suid' (WM_COMMAND handler)
    // 'parent' is the parent message box window; returns TRUE if panel selection should be cleared
    BOOL ExecuteMenuItem(CFilesWindow* panel, HWND parent, int suid);

    // shows help for the menu command identified by 'suid' (WM_COMMAND in HelpMode)
    // 'parent' is the parent message box window; returns TRUE if the help was shown
    ...
    // one of the extensions; 'exclude' is the plugin index to ignore (it will be cleared),
    // 'view' and 'edit' receive the indexes of found archivers (the standard PackerIndex and UnpackerIndex)
    // from CPackerFormatConfig, see above), 'viewFound' and 'editFound' say whether 'view'
    // and 'edit' are valid
    void FindViewEdit(const char* extensions, int exclude, BOOL& viewFound, int& view,
                      BOOL& editFound, int& edit);

    // searches for a DLL among plugin DLL files; if it exists, returns TRUE and the plugin index
    ...
    // of the FS plugin and the index 'fsNameIndex' of the plugin's file system name
    ...
    // it also returns the index 'fsName2Index' of the file system name 'fsName2' in that plugin
    ...
    // returns the number of "panel view" plugins before the given index,
    ...
    // returns the number of "panel edit" plugins before the given index,
    ...
    // returns the number of "file viewer" plugins before the given index,
    ...
    // calls AcceptChangeOnPathNotification for all plugins (delivering the message to loaded plugins)
    void AcceptChangeOnPathNotification(const char* path, BOOL includingSubdirs);

    // reaction to the Plugins/Last Command -- if a Last Command exists, it is executed
    // 'parent' is the parent message box window; returns TRUE if the panel selection should be cleared
    ...
    // new plugins that do not yet have a record
    ...
    // gets/sets the variable in CPluginData::ShowSubmenuInPluginsBar
    ...
    // adds a new timer to PluginFSTimers; the absolute timeout is GetTickCount() + 'relTimeout'.
    ...
    // removes from PluginFSTimers either all timers belonging to 'timerOwner' when 'allTimers' is TRUE,
    ...
    // searches plugin menus for 'hotKey'; returns TRUE when found and stores the
    // location in 'pluginIndex' and 'menuItemIndex'
    BOOL FindHotKey(WORD hotKey, BOOL ignoreSkillLevels, const CPluginData* ignorePlugin, int* pluginIndex, int* menuItemIndex);

    // traverses all plugins except 'ignorePlugin' and removes the hot key when any
    // menu item has it assigned
    void RemoveHotKey(WORD hotKey, const CPluginData* ignorePlugin);

    // returns TRUE if a command uses the hot key and sets 'pluginIndex' and 'menuItemIndex'
    BOOL QueryHotKey(WPARAM wParam, LPARAM lParam, int* pluginIndex, int* menuItemIndex);

    // reaction to WM_(SYS)KEYDOWN in a panel or the edit line -- plugins search
    // their menus for this hot key; if they process the message, TRUE is returned, otherwise FALSE
    ...
    // language is chosen for a plugin if the language selected for Salamander does not exist
    void ClearLastSLGNames();

    // returns the number of plugins that can be loaded (GetLoaded() returns TRUE)
    int GetNumOfPluginsToLoad();

    // returns the FS name of the first plugin that adds a file system and has PluginIsNethood
    ...
    // invokes PasswordManagerEvent for all plugins using the Password Manager.
    // See CSalamanderGeneralAbstract::SetPluginUsesPasswordManager (loads unopened plugins)
    void PasswordManagerEvent(HWND parent, int event);
...
protected:
    ...
    // the last command executed from the Plugins menu; 'rebuildDynMenu' is TRUE if, in the case of a
    ...
    // and 'menuItemIndex' an index into MenuItems
    ...
    // adds a record to the Order array; returns the index in the array on success, otherwise -1
    int AddPluginToOrder(const char* dllName, BOOL showInBar);

    // Sorts the Orders array by plugin name so newly added plugins are alphabetical
    void QuickSortPluginsByName(int left, int right);

    // Only used when converting from the old configuration (the visibility flag was moved to CPluginData)
    BOOL PluginVisibleInBar(const char* dllName);

    // helper function that finds an index for inserting a timer with 'timeoutAbs' into PluginFSTimers
...
protected:
    ...
    DWORD LoadInfo;                  // DWORD returned by GetLoadInformation()
    TIndirectArray<char> OldFSNames; // array of old fs-names (registry names replaced when loading the plugin)
...
public:
    ...
    // specify 'dataSize' == -1 to compute the length using strlen; returns success
    ...
    // stores the required size for the value 'name'+'type' from key 'key' in 'bufferSize'; returns success
...
protected:
    ...
    BOOL CustomPack;   // TRUE -> modification "custom pack" allowed
    BOOL CustomUnpack; // TRUE -> modification "custom unpack" allowed
    BOOL PanelView;    // TRUE -> modification "panel view" allowed
    BOOL PanelEdit;    // TRUE -> modification "panel edit" allowed
    BOOL Viewer;       // TRUE -> modification "file viewer" allowed
...
// converts a path to the external format (calls the corresponding plugin method
