...
void CMenuPopup::AssignHotKeys()
{
    ...
    // indicates whether the given character is already assigned
    BOOL assigned[256];

    // only convenient characters are offered
    int i;
    for (i = 0; i < 256; i++)
        assigned[i] = !IsCharAlpha(i);

    // in the first pass walk through all menu items and detect already used hotkeys
    ...
    // try to assign a hotkey to items without one
    char buff[1000];
    for (i = 0; i < Items.Count; i++)
    {
        CMenuItem* item = Items[i];
        ...
        for (i2 = 0; !keyAssigned && i2 < 2; i2++)
        {
            ...
            while (*p != 0)
            {
                // During the first pass we try to avoid characters with diacritics so the user
                // does not have to switch from an English keyboard
                // see https://forum.altap.cz/viewtopic.php?f=23&t=4025
                WCHAR buffw[10];
                buffw[1] = 0; // ensures the second pass runs
                ...
                if (buffw[1] == 0 && !assigned[upper])
                {
                    ...
                    if (len < 1000 - 1) // we must fit into the buffer
                    {
                        ...
                        // mark the character as used if the hot key was inserted
...
BOOL CMenuPopup::SetItemInfo(DWORD position, BOOL byPosition, const MENU_ITEM_INFO* mii)
{
    ...
    {
        // string type allocates a string which will be freed
...
BOOL CMenuPopup::LoadFromHandle()
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        // copy the string if it is of string type
...
// load 'menuPopup' based on the 'menuTemplate' template
BOOL CMenuPopup::LoadFromTemplate2(HINSTANCE hInstance, const MENU_TEMPLATE_ITEM* menuTemplate, DWORD* enablersOffset, HIMAGELIST hImageList, HIMAGELIST hHotImageList, int* addedRows)
{
    ...
    while (row->RowType != MNTT_PE)
    {
        ...
        case MNTT_PB: // popup begin
        {
            ...
            subMenu->SetStyle(MENU_POPUP_UPDATESTATES); // j.r.: this is a bit too generic
...
void CMenuPopup::CheckSelectedPath(CMenuPopup* terminator)
{
    ...
    if (OpenedSubMenu != NULL)
    {
        ...
        if (selectedItem == NULL || OpenedSubMenu != selectedItem->SubMenu)
        {
            ...
            {
                // deactivate the previous item if necessary
...
int CMenuPopup::FindNextItemIndex(int firstIndex, char key)
{
    ...
    int prefixIndexFirst = -1; // first occurrence of a prefixed variant
    int charIndexFirst = -1;   // first occurrence of a variant without prefix
    int prefixIndex = -1;      // last occurrence of a prefixed variant
    int charIndex = -1;        // last occurrence of a variant without prefix
    int i;
    for (i = 0; i < Items.Count; i++)
    {
        CMenuItem* item = Items[i];
...
void CMenuPopup::OnChar(char key, BOOL* leaveMenu, DWORD* retValue)
{
    ...
    if (firstIndex == -1)
    {
        ...
        // expanding the 7Zip menu and pressing 'h', which contradicts
        ...
        // in any case we hit RTC issues, so I trim to the lower DWORD using a mask
...
void CMenuPopup::EnsureItemVisible(int index)
{
    ...
    {
        // find the bottom edge of the last visible item
...
*/
        // if an item is displayed above the top edge, the down arrow is always shown
        downArrow = TRUE;
        bottomLimit = Height - UPDOWN_ITEM_HEIGHT;

        // if we show the first item in the list, there will no longer be
        // an up arrow
        if (index == 0)
            upArrow = FALSE;
        else
        ...
        // extend the repaint region by the bottom arrow
        ...
        // extend the repaint region starting at the top edge of the previously first item
    ...
    if (TopItemY + item->YOffset + item->Height > bottomLimit)
    {
        ...
        // find the top edge of the first visible item
        int firstItemY = 0;
        for (i = 0; i < Items.Count; i++)
        {
            CMenuItem* iterator = Items[i];
        ...
        // extend the repaint region by the up arrow
        HRGN hItemRgn = HANDLES(CreateRectRgn(0, 0, Width, firstItemY));
        CombineRgn(hUpdateRgn, hUpdateRgn, hItemRgn, RGN_OR);
        HANDLES(DeleteObject(hItemRgn));

        // extend the repaint region starting at the bottom edge of the previously last item
...
void CMenuPopup::OnMouseWheel(WPARAM wParam, LPARAM lParam)
{
    ...
    int itemHeight = item->Height; // the first item cannot be a separator
    if (itemHeight < 5)
        itemHeight = 15; // use 15 pixels if it is a separator
    ...
    DWORD wheelScroll = GetMouseWheelScrollLines();        // can be WHEEL_PAGESCROLL(0xffffffff)
    wheelScroll = max(1, min(wheelScroll, (DWORD)Height)); // cap at the page height
    ...
    if (TopItemY == 0 && lastVisibleItem->YOffset + lastVisibleItem->Height <= Height)
        return; // all items are visible; scrolling here is pointless
...
void CMenuPopup::DoDispatchMessage(MSG* msg, BOOL* leaveMenu, DWORD* retValue, BOOL* dispatchLater)
{
    ...
    *dispatchLater = FALSE;
    ...
    case WM_CHAR:
    case WM_SYSCHAR: // j.r.: to support Alt+LETTER inside the menu
    {
    ...
    case WM_SYSKEYUP:
    {
        ...
        // WM_SYSCOMMAND message with uCmdType=SC_KEYMENU was sent and the window menu was activated
        // this happened in Plugins/Encrypt/Create Key/Shift+F10 in one of the edit lines
        // with passwords where our menu pops up
    ...
    case WM_KEYDOWN:
    {
        ...
        if (shiftPressed && msg->wParam == VK_F10)
        {
            ...
            if (popup != NULL && !(FirstPopup->TrackFlags & MENU_TRACK_RIGHTBUTTON))
            {
                ...
                {
                    popup->OnKeyReturn(leaveMenu, retValue); // expand a submenu or generate a command
                }
            }
            return;
        }
    ...
    case WM_NCMBUTTONUP:
    {
               ...
               msg->message == WM_RBUTTONUP))))
        {
            ...
            if (!(item->State & MENU_STATE_GRAYED) ||
                SharedRes->MenuBar != NULL && SharedRes->MenuBar->HelpMode && // Petr: opened from the menubar and in HelpMode (Shift+F1) +
            ...
            msg->message == WM_RBUTTONUP)
        {
            ...
            {
                popup->OnKeyReturn(leaveMenu, retValue); // e.g. expand a submenu or generate a command
            }
            return;
        }
        if (popup == NULL)
        {
            ...
            if (SharedRes->ExcludeRect != NULL && PtInRect(SharedRes->ExcludeRect, msg->pt) &&
                (msg->message == WM_LBUTTONDOWN || msg->message == WM_LBUTTONDBLCLK))
                *dispatchLater = FALSE; // over this rectangle do not deliver ldown to prevent reopening the menu
            else
                *dispatchLater = TRUE;
            *leaveMenu = TRUE;
            if (SharedRes->MenuBar != NULL)
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (popup->OpenedSubMenu != NULL)
        {
          ...
          SharedRes->ChangeTickCount = GetTickCount();
        */
            // John: classic menus behave like this.
            SharedRes->ChangeTickCount = GetTickCount();
        }
        return;
    }
    ...
    case WM_TIMER:
    {
        ...
        // receive IDT_THROBBER and the throbber stopped spinning while the user was in the menu
        // it would be possible to deliver only this timer, but there is currently no reason
        // to withhold the others; if something goes wrong we can send only the throbber
        //      return;
    }
...
BOOL CMenuPopup::CreatePopupWindow(CMenuPopup* firstPopup, int x, int y, int submenuItemPos, const RECT* exclude)
{
    ...
    // because the application may destroy data when closing
    ...
    if (Items.Count == 0 || FirstPopup->Closing) // the menu might have been closed during init
    {
    ...
    // handle the menu running off the bottom of the screen for menus without MENU_TRACK_VERTICAL (for example User Menu)
...
CMenuPopup::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        // we will not use CACHE - draw directly to the screen
        // thanks to the opening effect there will be no flicker
        PAINTSTRUCT ps;
        HANDLES(BeginPaint(HWindow, &ps));
        PaintAllItems(NULL);
        HANDLES(EndPaint(HWindow, &ps)); // only turns off the caret we do not have anyway
        return 0;
    }
        /*
                    ...
                    if (!leaveMenu)
                    {
                        ...
                        if (skipFirstLBtnDblclk)
                        {
                            ...
                            if (msg.message == WM_LBUTTONDBLCLK)
                                continue; // skip the message because it is a slip after lbuttondown
                        }
                        if (skipFirstLBtnUp)
                        {
                            if (msg.message == WM_LBUTTONDOWN)
