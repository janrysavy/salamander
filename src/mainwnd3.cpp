...
// critical shutdown: how much time we can spend in WM_QUERYENDSESSION before
// the OS kills us. It is 5s (5s with an open message box, 10s without pumping
...
// code for measuring time losses
/*
...
// universal callback for our MessageBox when the user clicks HELP
    ...
    if (CurrentHelpDir[0] == 0)
    {
            ...
            DirExists(CurrentHelpDir))
        {
            ...
            { // the directory from the current .slg file does not exist
                lstrcpyn(helpPath, CurrentHelpDir, MAX_PATH);
                if (_stricmp(helpSubdir, "english") == 0 || // we already tested "english" and it does not exist so do not try again
...
//
// Vista+: dynamically obtain functions to set/clear shutdown blocking reasons
//
...
void CMainWindow::SetDoNotLoadAnyPlugins(BOOL doNotLoad)
{
    ...
    else
    {
        ...
        if (!CriticalShutdown)
        {
            ...
            {
                PostMessage(LeftPanel->HWindow, WM_USER_REFRESH_DIR, 0, t1); // ensure the icon cache is repopulated (thumbnails can be shown again)
            }
            if (RightPanel->GetViewMode() == vmThumbnails)
            {
                PostMessage(RightPanel->HWindow, WM_USER_REFRESH_DIR, 0, t2); // ensure the icon cache is repopulated (thumbnails can be shown again)
            }
        }
    }
}
...
BOOL CMainWindow::SHChangeNotifyInitialize()
{
    ...
    // message WM_USER_SHCHANGENOTIFY, which will be delivered for notifications, crosses process boundaries
    // with SHCNRF_NewDelivery (also known as SHCNF_NO_PROXY) we assume responsibility
...
BOOL CMainWindow::OnAssociationsChangedNotification(BOOL showWaitWnd)
{
    ...
    // this synchronization ensures two Salamanders do not interfere with each other
    ...
    if (HANDLES(RegOpenKeyEx(HKEY_CURRENT_USER, "Control Panel\\Desktop\\WindowMetrics", 0, KEY_READ | KEY_WRITE, &hKey)) == ERROR_SUCCESS)
    {
        ...
        int val = atoi(size);
        if (val > 0) // unfortunately users set icon sizes randomly (72, 96, 128, etc.) so we cannot filter out "strange" sizes
        {
...
CMainWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        SHChangeNotifyInitialize(); // request Shell Notifications
        ...
        if (h != NULL)
        {
            ...
            // (users complained they accidentally click our AOT instead of Close)
            if (items > 2)
            {
...
*/
    ...
    {
        if (IgnoreWM_SETTINGCHANGE || LeftPanel == NULL || RightPanel == NULL) // a bug report showed WM_SETTINGCHANGE arrived right from WM_CREATE of the main window (panels didn't exist yet, causing a NULL access)
            return 0;
        ...
        if (lParam != 0 && stricmp((LPCTSTR)lParam, "Extensions") == 0)
        {
            ...
            // this branch is probably no longer used, it's some old path,
    ...
    case WM_USER_SHCHANGENOTIFY: // received thanks to SHChangeNotifyRegister
    {
        ...
        SHChangeNotification_Unlock(hLock); // ppidl translated, we can free the memory
        ppidl = NULL;
        ...
        else
        {
            ...
            else
            {
                ...
                // after media insertion automatically retry the "drive not ready" message box
        ...
        /*
    // WM_DEVICECHANGE didn't work well under Win XP when connecting the DSC F707 camera.
    ...
    */
    ...
    {
        // delay processing so the main window activates after ESC from the viewer on WinXP;
        // without this hack the main window stayed inactive and the safe-wait window never appeared
    ...
    case WM_USER_TBDROPDOWN:
    {
        ...
        {
            // user clicked a plugin icon in the PluginsBar;
    ...
    case WM_USER_SHOWWINDOW:
    {
        if (!SalamanderBusy)
        {
            ...
            BringWindowToTop(HWindow); // probably not important, but I saw it in a sample so adding it here
            if (IsIconic(HWindow))
            {
...
*/
    ...
    case WM_USER_CONFIGURATION:
    {
        ...
        UserMenuIconBkgndReader.ResetSysColorsChanged(); // we now start watching system color changes (icon reload required)
        BOOL readingUMIcons = UserMenuIconBkgndReader.IsReadingIcons();
        if (readingUMIcons) // new icons are on their way to the user menu; show them after configuration is done (on OK reload icons again so newly added ones are read)
            UserMenuIconBkgndReader.BeginUserMenuIconsInUse();
        ...
        if (res == IDOK) // values changed -> refresh everything possible
        {
            ...
            // the user might have toggled Documents
            CDriveBar* copyDrivesListFrom = NULL;
            if (DriveBar != NULL && DriveBar->HWindow != NULL)
            {
    ...
    case WM_SYSCOMMAND:
    {
        ...
        // if the user pressed Alt while the initial splash window was shown,
    ...
    case WM_APPCOMMAND:
    {
        // catch messages from newer mice (4th button and above)
    ...
    case WM_COMMAND:
    {
        if (HelpMode && (HWND)lParam == NULL && LOWORD(wParam) != CM_HELP_CONTEXT)
        {
            ...
            { // command of a plugin (submenu of Plugins menu)
                if (Plugins.HelpForMenuItem(HWindow, LOWORD(wParam)))
                    ...
                    id = CM_LAST_PLUGIN_CMD; // if the plugin has no help, show Salamander help "Using Plugins"
            }
        ...
        { // command from a plugin menu
            ...
            if (Plugins.ExecuteMenuItem(activePanel, HWindow, LOWORD(wParam)))
            {
                ...
                activePanel->SetSel(FALSE, -1, TRUE);                        // explicit repaint
...
*/
        ...
        case CM_LCHANGEDRIVE:
        {
            if (activePanel != LeftPanel)
            {
                ...
                UpdateWindow(HWindow); // draw the focus before the menu appears
            }
            ...
            // change the current drive in the right panel
        case CM_RCHANGEDRIVE:
        {
        ...
        case CM_RROOTDIR:
        {
            ...
            // toggle the left panel status line
        case CM_LEFTSTATUS:
        {
            LeftPanel->ToggleStatusLine();
            IdleRefreshStates = TRUE; // force state check on next Idle
            return 0;
        }
            // toggle the right panel status line
        case CM_RIGHTSTATUS:
        {
            ...
            // toggle the left panel directory line
        case CM_LEFTDIRLINE:
        {
            ...
            // toggle the right panel directory line
        ...
        case CM_EMAILFILES: // emailing files and directories
        {
            ...
            // if we selected an item, unselect it again
        ...
        case CM_CHANGECASE: // change case in names
        {
            ...
            if (activePanel->Is(ptDisk)) // source is disk - all operations work here
            {
                CActionType type;
                switch (LOWORD(wParam))
                {
            ...
            {
                if (activePanel->Is(ptZIPArchive)) // source is an archive - all operations work here
                {
                ...
                {
                    if (activePanel->Is(ptPluginFS)) // source is a FS - all operations work here
                    {
        ...
        case CM_COMPAREDIRS:
        {
            ...
            if (strcmp(leftPath, rightPath) == 0) // case sensitive; if this fails it's fine
            {
            ...
            if (dlg.Execute() == IDOK)
            {
                ...
                else
                {
                    if (Configuration.CompareOnePanelDirs)
                    {
                        ...
                        Configuration.CompareSubdirs = FALSE; // handles case when CompareSubdirs is enabled and a compare is run for FS and the user toggles CompareOnePanelDirs - without this, CompareSubdirs would win next time the drive dialog opens
                    }
                }
        ...
        case CM_DOSSHELL:
        {
            ...
            // There is an undocumented flag 0x400 where si.hStdOutput passes the monitor handle
            ...
            else
            {
                ...
                while (1)
                {
                    ...
                    if (ok) // distribute a notification about the change on 'path' with 'includingSubdirs'
                    {
                        ...
                        {
                            // for better I/O with plugins the EnterPlugin/LeavePlugin section
    ...
    {
        // broadcast configuration changes to plugins
        Plugins.Event(PLUGINEVENT_CONFIGURATIONCHANGED, 0);
        return 0;
    }
    ...
    case WM_USER_UNINITMENUPOPUP:
    {
        ...
        case CML_FILES_VIEWWITH:
        {
            ...
            if (hIcons != NULL)
            {
                popup->SetImageList(NULL); // detach the list to avoid owning an invalid handle
                ImageList_Destroy(hIcons);
            }
            hIcons = popup->GetHotImageList();
            if (hIcons != NULL)
            {
                popup->SetHotImageList(NULL); // detach the list to avoid owning an invalid handle
                ImageList_Destroy(hIcons);
            }
            if (popupID == CML_PLUGINS) // closing the Plugins menu; dynamic icons can be freed (rebuilt next time)
    ...
    case WM_USER_INITMENUPOPUP:
    {
        ...
        case CML_FILES_NEW:
        {
            ...
            if (activePanel == NULL)
                break;
            BeginStopRefresh(); // closed in WM_USER_UNINITMENUPOPUP/CML_FILES_NEW,
            ...
            // if the menu exists, build ours from it
        ...
        case CML_OPTIONS_PLUGINS:
        {
            ...
            popup->SetImageList(hIconsGray);
            popup->SetHotImageList(hIcons);
            // list only plugins that can be configured
    ...
    case WM_LBUTTONUP:
    {
        ...
        if (DragMode)
        {
            ...
            if (!DragFullWindows)
                EndStopStatusbarRepaint(); // resume throbber repaints when dragging with XOR split bar
            return 0;
        }
        break;
    }
    ...
    case WM_ENABLE:
    {
        if (WindowsVistaAndLater)
        {
            // Windows Vista UAC patch: launching a file from the panel may show an elevation prompt.
            // If the prompt is closed with Cancel, Salamander could lose panel focus.
            // The main window is disabled when WM_ACTIVATE or WM_SETFOCUS arrive, so Microsoft IME popups get the focus instead.
    ...
    case WM_ACTIVATE:
    {
        ...
        // hide or show the Wait window if it exists
    ...
    case WM_USER_POSTCMDORUNLOADPLUGIN:
    {
        ...
        {
            // may occur while waiting for Release(force==TRUE) of the plugin to finish
    ...
    case WM_USER_SALSHEXT_PASTE:
    {
        ...
        if (SalShExtSharedMemView != NULL) // shared memory is available (we cannot handle cut/copy&paste errors)
        {
            ...
            {
                if (SalamanderBusy)
                    SalShExtSharedMemView->SalBusyState = 2 /* Salamander is busy, postpone paste */;
                else
                {
                    SalamanderBusy = TRUE;
                    SalShExtPastedData.SetLock(TRUE);
                    LastSalamanderIdleTime = GetTickCount();
                    SalShExtSharedMemView->SalBusyState = 1 /* Salamander is not busy and waiting for a paste operation */;
                    ...
                    while (count++ < 50) // wait no longer than 5 seconds
                    {
                        ...
                        WaitForSingleObject(SalShExtSharedMemMutex, INFINITE);
                        if (SalShExtSharedMemView->PasteDone) // copy hook supplied the target path and other data
                        {
            ...
            PostMessage(HWindow, WM_USER_SALSHEXT_TRYRELDATA, 0, 0); // release data once unlocked
        }
    ...
    case WM_USER_END_SUSPMODE:
    {
        ...
        // window activated, perform a refresh
    ...
    case WM_TIMER:
    {
        ...
        case IDT_POSTENDSUSPMODE:
        {
            ...
            PostMessage(HWindow, WM_USER_END_SUSPMODE, 0, 0); // does nothing if ActivateSuspMode < 1
            break;
        }
    ...
    case WM_ACTIVATEAPP:
    {
        ...
        if (wParam == TRUE) // activating the app
        {
            ...
            {
                SetTimer(HWindow, IDT_POSTENDSUSPMODE, 200, NULL); // hopefully we'll never find out why this timer existed; commented out because it delays directory refresh by 200 ms after operations (e.g. moving a file into a subdirectory)
            }
            else
            {
                ...
                // would not restore it, only a beep; see https://forum.altap.cz/viewtopic.php?f=6&t=3791
                //
    ...
    case WM_ENDSESSION:
    {
        ...
        // is killed. In theory TRUE should be returned to call WM_ENDSESSION so this
        // could arrive, but so far all Windows versions prefer killing the app.
        //
        // here we handle so-called "critical shutdown" (including log off) which
        // has the ENDSESSION_CRITICAL flag in lParam; it can be triggered using EWX_FORCE:
        ...
        //
        // Vista+ only: we also handle shutdown with EWX_FORCEIFHUNG here; it doesn't set
        // the ENDSESSION_CRITICAL flag but forces the application to exit regardless of the return value
        ...
        // - this mode is called "forced shutdown"
        // - the system won't kill our process, no timeout is running, we won't terminate anything forcefully
        // - we must notify the user if they want to cancel the shutdown; if they cancel, the app continues normally
        //
        ...
        //   Windows kills the process and the configuration is lost; in theory making a copy each time would solve it,
        ...
        //   e.g. a prompt asking to cancel running disk operations), WM_ENDSESSION is still sent,
        //   so we don't save configuration in WM_ENDSESSION, only perform minimal cleanup
        //   (stop running disk operations)
        // - on Vista+ we first back up the configuration registry key in WM_QUERYENDSESSION (5s limit),
        //   return TRUE to continue shutdown, and the system gives us another 5s in WM_ENDSESSION to save configuration,
        ...
        // Experimentally observed behavior for three types of shutdowns:
        //
        ...
        // W2K: WM_QUERYENDSESSION, after 5s a Kill/Cancel dialog appears (Cancel aborts the shutdown),
        //      WM_QUERYENDSESSION returns TRUE -> WM_ENDSESSION arrives, after 5s a Kill/Cancel dialog (Cancel acts like Kill)
            ...
            (lParam & ENDSESSION_CRITICAL) == 0)
        {
            ...
            // incomplete state, e.g. during copying the file size is allocated but the content is not
            // written, just filled with zeros, and the configuration won't be saved.
            return 0;
        }

        if (!SaveCfgInEndSession) // configuration should not be saved (handled later)
        {                         // WaitInEndSession or XP "critical shutdown": wait for running disk operations to finish
            if (!WindowsVistaAndLater && ProgressDlgArray.RemoveFinishedDlgs() > 0)
                ProgressDlgArray.PostCancelToAllDlgs(); // dialogs and workers run in their own threads; they might exit

            while (ProgressDlgArray.RemoveFinishedDlgs() > 0)
                Sleep(200);
            return 0; // let the application close
        }
    ...
    case WM_USER_FORCECLOSE_MAINWND:
    {
        ...
        if (uMsg == WM_QUERYENDSESSION)
        {
            ...
            {
                // make sure WM_ENDSESSION doesn't arrive from the IdleCheckClipboard handler
                // when OnEnterIdle() and CannotCloseSalMainWnd were TRUE (WM_ENDSESSION would refuse to run)
                // the program is about to terminate; handling IDLE makes no sense and only delays us
        ...
        // will certainly exit, so perform at least the worst cleanup: cancel running disk operations
        ...
        // some disk operation dialogs are running
        WCHAR blockReason[MAX_STR_BLOCKREASON];
        if (ProgressDlgArray.RemoveFinishedDlgs() > 0)
        {
            ...
            else // report it in a window and wait for everything to finish; WM_ENDSESSION cannot arrive here
            {
                ...
                if (res == IDCANCEL)
                {
                    ...
                    return 0; // refuse closing/shutdown/logoff; any "forced shutdown" will be detected in WM_ENDSESSION
                }
                UpdateWindow(HWindow);
            }
        }
        ...
        { // always true: uMsg == WM_QUERYENDSESSION && (lParam & ENDSESSION_CRITICAL) != 0
            ...
            return TRUE; // continue to WM_ENDSESSION where we finish or are killed while waiting
        }
        ...
        if (uMsg != WM_ENDSESSION)
        {
            ...
            for (i = 0; i < destroyArray.Count; i++)
            {
                if (IsWindow(destroyArray[i])) // if the window still exists
                {
                    ...
                    WindowsManager.CS.Enter(); // no modifications to WindowsManager allowed
                    CFindDialog* findDlg = (CFindDialog*)WindowsManager.GetWindowPtr(destroyArray[i]);
                    if (findDlg != NULL) // if the window still exists send it a close query (otherwise pointless)
                    {
                        BOOL myPost = findDlg->StateOfFindCloseQuery == sofcqNotUsed;
                        if (myPost) // if this is not nesting (maybe possible but unlikely)
                        {
                        ...
                        while (cont)
                        {
                            ...
                            if (findDlg != NULL) // handle only if the window still exists (otherwise pointless)
                            {
            ...
            if (!endAfterCleanup)
            {
                ...
                // shredding the temporary file and the system kills us during shredding).
                // A better approach is to shred properly after system restart, handled in DeleteTmpCopy();
                // shredding does not happen during critical shutdown
                ViewerWindowQueue.BroadcastMessage(WM_CLOSE, 0, 0);

                // add a delay before unloading the plugin - if there are Find windows or the internal viewer
        ...
        // perform the bare minimum cleanup and then exit (the system may kill us earlier, current mode: kill within 5s),
        if (endAfterCleanup)
        {
            // always true: uMsg == WM_QUERYENDSESSION && (lParam & ENDSESSION_CRITICAL) != 0
            // wait up to five seconds from WM_QUERYENDSESSION for disk operations to finish
        ...
        if (uMsg == WM_QUERYENDSESSION && (lParam & ENDSESSION_CRITICAL) != 0) // this applies to Vista+
        {
            ...
            BOOL backupOK = FALSE;
            if (cfgOK) // old configuration seems OK; back it up in case saving fails
            {
                char backup[200];
                sprintf_s(backup, "%s.backup.63A7CD13", SALAMANDER_ROOT_REG); // "63A7CD13" prevents a collision with the user's key name
        ...
        if (shutdown) // during shutdown/log-off/restart show a wait window for all Saves (including plugins) and process the message loop so we aren't marked "not responding" and killed early
        {
            ...
            analysing.SetProgressMax(7 /* number from CMainWindow::SaveConfig() -- MUST stay in sync! */ +
                                     Plugins.GetPluginSaveCount()); // minus one so they can enjoy a view of 100%
            analysing.Create();
        ...
        // declare a "critical shutdown" so all routines end as quickly as possible
        ...
        EXIT_WM_USER_CLOSE_MAINWND:
            ...
            if (uMsg != WM_ENDSESSION) // during critical shutdown we don't end stop-refresh (refreshes are sent to panels)
            {
                EndStopRefresh();
                return 0; // refuse close/shutdown/logoff; a forced shutdown will be detected in WM_ENDSESSION
            }
            ...
            {
                // wait for disk operations to finish; the system might kill our process before that
                while (ProgressDlgArray.RemoveFinishedDlgs() > 0)
                    Sleep(200);
                                 ...
                                 reason + (int)strlen(reason), BUG_REPORT_REASON_MAX - ((int)strlen(reason) + 1)) > 0)
        {
            ...
            if (CriticalShutdown || // during critical shutdown there's no point in asking anything, let the system terminate us
        ...
        if (LeftPanel != NULL && RightPanel != NULL)
        {
            ...
            if (LeftPanel->PrepareCloseCurrentPath(shutdown ? analysing.HWindow : LeftPanel->HWindow, TRUE, FALSE, detachFS1, FSTRYCLOSE_UNLOADCLOSEFS /* unnecessary - plugins (including FS) already unloaded */))
            {
                ...
                {
                    canClose = TRUE; // close both panels or none
                ...
                if (canClose)
                {
                    ...
                    // Protect the list box from redraw requests after we just cut the data
        ...
        // if the window pops up the user would find both panels empty (listing released).
        ...
        {
            Plugins.ClearLastSLGNames(); // so all plugins can pick a new fallback language if needed
        ...
        // the main window is destroyed during shutdown, so the following is dead code during shutdown
        ...
        if (uMsg == WM_QUERYENDSESSION)
        {
            ...
            // main window already closed - nobody to deliver WM_ENDSESSION to, WaitInEndSession
            // and SaveCfgInEndSession need not be set
...
*/
    ...
    case WM_DESTROY:
    {
        if (!CanDestroyMainWindow)
        {
            ...
            // while (PeekMessage(&msg, HWindow, 0, 0, PM_REMOVE));  // Petr: replaced by discarding key messages only; without TranslateMessage and DispatchMessage we risk an endless loop (found when unloading Automation with memory leaks; before showing the leak message box an endless WM_PAINT loop occurred)
...
*/
    ...
    case FM_GETFILESELLFNW:
    {
        if (!GetActivePanel()->Is(ptDisk))
            return 0; // we operate only on disk paths
    ...
    case FM_REFRESH_WINDOWS:
    {
        ...
        {
            //--- refresh directories not automatically refreshed
