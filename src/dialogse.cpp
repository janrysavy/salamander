...
CCfgPageShellExt::Validate(CTransferInfo &ti)
{
  ...
  // if they want items in a submenu, the submenu name must not be empty
  BOOL submenu;
...
CCfgPageShellExt::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    ...
    case WM_DESTROY:
    {
      ...
      SECDeleteAllItems();
      ShellExtConfigVersion = 0; // next time we will reload the contents from the registry
      break;
    }
        ...
        case IDC_SE_LIST:
        {
          switch (nmhdr->code)
          {
            ...
            case EDTLBN_MOVEITEM:
            {
              ...
              SetWindowLongPtr(HWindow, DWLP_MSGRESULT, FALSE);  // allow moving
              return TRUE;
            }
