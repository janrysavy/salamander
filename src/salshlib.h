...
// we detect it in GetData according to the mouse position; this variable stores the last test result
extern HWND LastWndFromGetData;

// utter hack: we need to find out into which window the Paste will happen
// we detect it in GetData according to the foreground window; this variable stores the last test result
...
//
// data object used to detect the target of a drag & drop operation (used when
// extracting from an archive and when copying from a plug-in file system),
...
// also SALCF_FAKE_SRCFSPATH (source filesystem path) to GetData()
...
public:
    ...
    (FORMATETC* formatEtc)
    {
        if (formatEtc->cfFormat == CF_HDROP)
            return DV_E_FORMATETC; // this enforces a "NO" drop in simpler software (BOSS, WinCmd, SpeedCommander, MSIE, Word, etc.)
...
public:
    ...
    // returns TRUE if it is possible to unload the 'plugin'; if the object contains
...
private:
    ...
    BOOL CutOrCopyDone;        // FALSE = the object is only being placed on the clipboard; Release does nothing until CutOrCopyDone is TRUE
...
public:
    CFakeCopyPasteDataObject(IDataObject* winDataObject, const char* fakeDir)
    {
        ...
        LastGetDataCallTime = GetTickCount() - 60000; // initialize to 1 minute before the object was created
        CutOrCopyDone = FALSE;
    }
