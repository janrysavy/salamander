...
// text shown in the command line combo box; cached so it will not flicker
...
public:
    ...
    BOOL SetText(const char* txt); // sets only Message, returns whether a redraw is needed
    void UpdateControl();
    int GetNeededWidth();
...
public:
    ...
    void SetFont();
    void FillHistory();
    BOOL Dropped(); // listbox is expanded
