...
BOOL SalamanderActive()
{
    ...
    if (foreground == NULL)
        return TRUE; // during Salamander activation blocked by the wait window, GetForegroundWindow() returns NULL
    DWORD pid;
    ...
    while (run && GetMessage(&msg, NULL, 0, 0))
    {
        ...
        case WM_USER_SHOWWAITWND:
        {
            ...
            if (show)
            {
                ...
                // if the user cancelled the operation there, an immediate display would
                          ...
                          int delay, BOOL showCloseButton, HWND hForegroundWnd)
{
    ...
    { // can happen - if the internal viewer searches and switches to the main window, it tries to bring up another safe-wait window and the message pops up
        HANDLES(LeaveCriticalSection(&SafeWaitMessageCallerSetSection.cs));
...
BOOL GetSafeWaitWindowClosePressed()
{
    ...
    // is clicked in the viewer's wait window
    return FALSE;
}
...
CQuadWord MyGetDiskFreeSpace(const char* path, CQuadWord* total)
{
    ...
    {
        ret.Value = (unsigned __int64)availBytes /*freeBytes*/.QuadPart; // I used availBytes instead of freeBytes because a user reported that we were showing 35GB instead of 2GB of free space (it was a Novell network disk with quotas under XP)
        if (total != NULL)
        ...
        if (strlen(resPath) > 3)
        {
            ...
            while (GetCurrentLocalReparsePoint(resPath, repPointPath))
            {
                if (rootOrCurReparsePointSet != NULL && !*rootOrCurReparsePointSet && rootOrCurReparsePoint != NULL)
                {
                    ...
                    if (strlen(resPath) < strlen(repPointPath)) // if the path to the current reparse point is longer than the path obtained by resolving the subst, we must append this part after the subst root
                    {
                ...
                {                                                                               // it only makes sense to look for reparse points on fixed disks, so stop here (network paths are a problem because their reparse points will return "local paths" (C:\...), which, if used on this machine instead of the remote one they come from, lead to nonsensical results)
                    if (netPath != NULL)
                ...
                if (drvType != DRIVE_FIXED)
                    break; // reparse points only make sense on fixed disks
            }
        }
    }
}
                        ...
                        LPDWORD lpTotalNumberOfClusters)
{
    ...
    if (!IsUNCPath(ourPath) && GetDriveType(ourPath) == DRIVE_FIXED) // reparse points only make sense on fixed disks
                            ...
                            LPTSTR lpFileSystemNameBuffer, DWORD nFileSystemNameSize)
{
    ...
    {                                                                // gradually try shortening the path; on a mounted directory it can return the mounted disk parameters
        ...
        { // ourPath is ResolveSubsts(path) or a shortened version of ResolveSubsts(path)
            ...
            ResolveSubsts(resPath);
            GetRootPath(rootOrCurReparsePoint, path);
            if (strlen(resPath) < strlen(ourPath)) // if the path we return volume info for is longer than the path obtained by resolving the subst, we must append this part after the subst root
            {
                                ...
                                int* repPointType, BOOL makeRelPathAbs)
{
    ...
    // even CreateFile trims spaces/dots and then works with a different path
    ...
    if (repPointDstBuf != NULL)
    {
        ...
        { // the symlink is relative; try converting it to an absolute path
            ...
            symlinkAbsPath[0] = (WCHAR)(unsigned char)repPointDir[0]; // a bit of a hack (we rely on 'a-zA-Z' mapping to Unicode 1:1)
            symlinkAbsPath[1] = L':';
            if (*s == L'\\')
...
UINT MyGetDriveType(const char* path)
{
    ...
    if (!IsUNCPath(ourPath))
    {
        ...
        if (drvType == DRIVE_FIXED) // reparse points only make sense on fixed disks
        {                           // gradually try shortening the path; on a mounted directory it can return the mounted disk parameters
            // if it is not a root path, try traversing the reparse points as well
            BOOL cutPathIsPossible = TRUE;
            ResolveLocalPathWithReparsePoints(ourPath, path, &cutPathIsPossible, NULL, NULL, NULL, NULL, NULL);
            ...
            { // NOTE: differs from MyGetVolumeInformation because GetDriveType succeeds for any path (not just root + mounted volume)
                if (!cutPathIsPossible || !CutDirectory(ourPath))
    ...
    if (wnd != NULL)  // we have a predecessor
    {
      ...
      if (fm != NULL)
      {
        ...
        if (params != NULL)
        {
          ...
          // unfortunately when receiving mapped memory we cannot determine its size to the byte (only with page granularity)
                         ...
                         BOOL autoImportConfig, BOOL* importCfgFromFileWasSkipped)
{
    ...
    if (SalGetFileSize(file, size, err))
    {
        if (size <= CQuadWord(10000000, 0)) // above 10MB it is 100% nonsense...
        {
            ...
            if (buf != NULL) // "always true" (on error we merely avoid crashing; the user dismissed the out-of-memory message)
            {
                DWORD bytesRead;
    ...
    if (buf != NULL && (DWORD)size.Value > 0)
    {
        *(WCHAR*)((LPBYTE)buf + (DWORD)size.Value) = 0; // safety net for too short file
        ...
        { // "always false" (on error we merely avoid crashing; the user dismissed the out-of-memory message)
            free(buf);
            buf = NULL;
        }
    }
