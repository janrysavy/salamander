             ...
             char* winCodePage, DWORD* identifier, char* description)
{
    ...
    BOOL comment;
    char* text; // non-NULL means an error message
    ...
    __try
    {
        while (txt < endTxt)
        {
                    ...
                    else // name=files
                    {
                        ...
                        if (txt < endTxt && *txt == '=' && txt > beg)
                        {
                            ...
                            do
                            {
                                ...
                                while (txt < endTxt && *txt != '\r' && *txt != '\n' && *txt != '|')
                                    txt++;
                                if (beg < txt) // another conversion file (or ANSI->OEM/OEM->ANSI)
                                {
                        ...
                        else // missing '=' or it's at line start (and not doubled)
                        {
            ...
            if (text == NULL && !comment) // add code to the table
            {
            ...
            if (find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            {
                if (find.cFileName[0] != 0 && strcmp(find.cFileName, ".") != 0 && strcmp(find.cFileName, "..") != 0)
                {
                    ...
                    if (table->GetState() == ctsSuccessfullyLoaded)
                    {
                        ...
                        delete table; // discard the implicit table we don't need
                }
            }
...
BOOL CCodeTables::Init(HWND hWindow)
{
    ...
    if (!Loaded)
    {
        ...
        if (Configuration.ConversionTable[0] != '*')
        {
            ...
            {
                // if loading fails perfectly, give other tables a chance
                findBest = TRUE;
            }
        }
        else
...
void CCodeTables::InitMenu(HMENU menu, int& codeType)
{
    ...
    MENUITEMINFO mi;
    if (GetMenuItemCount(menu) == 0) // empty menu, needs filling
    {
        int count = 0;
        ...
        for (i = 0; i < Table->Data.Count; i++)
        {
            ...
            {
                // if nothing follows the separator, skip inserting it
                                    ...
                                    char* codePage)
{
    ...
    if (buf != NULL)
    {
        ...
        for (i = -1; i < Table->Data.Count; i++)
        {
            ...
            {
                if (n != NULL && winCodePageLen > 0) // not a separator and WinCodePage loaded
                {
                    // remove '&' characters, they hinder comparison
                    char buf3[200];
                    lstrcpyn(buf3, n, 200);
...
#define PENALTY_NOT_ALPHA_NOR_NUM_PENALTY_ADD1 2  // addition: non-alphanumeric char preceded by alpha/num
#define PENALTY_NOT_ALPHA_NOR_NUM_PENALTY_ADD2 1  // addition: non-alphanumeric char followed by alpha/num
#define PENALTY_NOT_ALPHA_NOR_NUM_PENALTY_ADD3 50 // addition: at least three identical non-alphanumerics preceded by alpha/num
#define PENALTY_NOT_ALPHA_NOR_NUM_PENALTY_ADD4 50 // addition: at least three identical non-alphanumerics followed by alpha/num
...
#define PENALTY_CHANGE 1                          // types of adjacent chars differ (lower/upper/number)
#define PENALTY_MAYBE_UNKNOWN_CHAR 1              // after five '?' characters - likely unknown chars in the target encoding (standard is '?')
                ...
                while (s < end)
                {
                    if (!forceText)
                    {
                        ...
                        { // disallowed character
                            if (++binar > minBinar)
                                break; // more than 0.5% disallowed characters
                        }
                        if (*s == 0)
                        {
                            if (++nulls > 10)
                    ...
                    if (IsNotAlphaNorNum[*s]) // character is neither alpha nor numeric
                    {
                        ...
                        { // spaces, tabs and line ends are ignored
                            BOOL skipChar = FALSE;
                            if (*s >= 128) // ignore one non-ASCII char (used to skip an extra apostrophe '\x92' in ASCII text)
                            {
                            ...
                            if (!skipChar)
                            {
                                if (s > (const unsigned char*)testBuf && !IsNotAlphaNorNum[*(s - 1)])
                                {
                                    ...
                                    // at least three identical characters preceded by alpha or num (frame corner is a letter - e.g. CP437 text tested on CP852)
                                ...
                                if (s + 1 < end && !IsNotAlphaNorNum[*(s + 1)])
                                {
                                    ...
                                    // at least three identical characters followed by alpha or num (frame corner is a letter - e.g. CP437 text tested on CP852)
                        ...
                        if (s + 1 < end && !IsNotAlphaNorNum[*(s + 1)]) // next character is also alpha or numeric
                        {
                            ...
                            if (c1 == 2 && c2 == 1)
                            {
                                if (s > (const unsigned char*)testBuf && IsAlpha[*(s - 1)])
                                    penalty += PENALTY_UPPER_TO_LOWER; // the word "Úrok" otherwise converts to MACCE (Intel adds space, 'U' becomes lowercase 'r')
                            }
                            else
                            {
                                if (c1 == 1 && c2 == 2)
                ...
                // config files and .inf files also look like complete garbage based on 'penalty'
                {
                    if (isText != NULL)
                        *isText = TRUE;
...
int CCodeTables::GetConversionToWinCodePage(const char* codePage)
{
    ...
    // remove '&' characters; they would hinder comparison
    char buf2[200];
