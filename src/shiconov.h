...
struct CShellIconOverlayItem
{
    ...
    BOOL GoogleDriveOverlay;                 // TRUE = Google Drive handler (their handler crashes, so we add extra synchronization)
...
protected:
    ...
    BOOL GetGDAlreadyCalled;                        // TRUE = we already checked where the Google Drive directory is located
    char GoogleDrivePath[MAX_PATH];                 // directory for Google Drive (we do not call their handler elsewhere; it is disgustingly slow and, without the extra synchronization, it crashes)
    BOOL GoogleDrivePathIsFromCfg;                  // TRUE if the Google Drive directory comes from the Google Drive configuration (FALSE = only the default is possible and Google Drive may not be installed at all)
    BOOL GoogleDrivePathExists;                     // does the Google Drive directory exist on disk?
...
public:
    ...
    // adds 'item' to the array (previously the insertion by 'priority' was incorrect)
    ...
    // in the STA threading model, so the object must be created and used in a single thread only)
