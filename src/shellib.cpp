...
BOOL IsSimpleSelection(IDataObject* pDataObject, CDragDropOperData* namesList)
{
    ...
    if (pDataObject != NULL && !IsFakeDataObject(pDataObject, NULL, NULL, 0)) // data from an archive or plug-in filesystem are not accepted here
    {
        ...
        if (pDataObject->EnumFormatEtc(DATADIR_GET, &enumFormat) == S_OK)
        {
            ...
            // If we detect truncated format names, it is almost certainly Remote Desktop
            // and we must not call pDataObject->GetData(), because we would start copying on the remote machine
            // the file to our temp and we would stay frozen during that time
                            ...
                            if (data->fWide)
                            {
                                ...
                                while (1) // double null terminated, assumes no empty strings (start)
                                {
                                    ...
                                    if (lastBackslash != NULL)
                                    {
                                        ...
                                        {
                                            if (s > fileW && *(s - 1) == L'\\')
                                                s--; // trim a possible trailing backslash
                                            int len;
                                       ...
                                       POINTL pt, DWORD* pdwEffect)
{
    ...
    if (GetCurDir != NULL)
    {
        ...
        { // if the selection is not from a single path (likely only in Find), we cannot copy/move into archives or the filesystem
    ...
    if (CurDirDropTarget != NULL) // jen idtttWindows
    {
        ...
        {                // other drop targets in the panel may still work
            ...
            *pdwEffect = DROPEFFECT_NONE;
            CurDirDropTarget->Release(); // remove the drop target so DragOver is not invoked on it
            CurDirDropTarget = NULL;
        }
        else
        {
            if (OldDataObjectIsFake)
            { // our data object (may not be from this process): default is Copy (the fake is in TEMP; on the same disk it defaulted to Move, so we bypass it like this)
                                  ...
                                  POINTL pt, DWORD* pdwEffect)
{
    ...
    if (TgtType == idtttWindows)
    {
        ...
        // if this is a "fake" directory (archive extraction, copy/move from FS), handle it here
        if (!operationDone && isFake && CurDir[0] != 0)
        {
            ...
            // Copy (the fake is in TEMP; on the same disk it defaulted to Move, so we bypass it like this)
            if ((origKeyState & MK_SHIFT) != 0 && (origKeyState & MK_CONTROL) == 0 &&
                (origEffect & DROPEFFECT_MOVE) != 0)
            {
                *pdwEffect = DROPEFFECT_MOVE;
            ...
            if (*pdwEffect != DROPEFFECT_NONE)
            {
                ...
                else // TgtType: idtttArchive, idtttArchiveOnWinPath, idtttPluginFS
                {
                        ...
                        namesList != NULL)
                    {
                        ...
                        namesList = NULL; // DoDragDropOper will free it; we will not do it here
                    }
                }
                ret = S_OK;
            }
...
void ReleaseShellib()
{
    __try
    {
        ...
        OleFlushClipboard(); // hand the data from the IDataObject we left on the clipboard over to the system (this IDataObject is released)
                                      ...
                                      const char* enumNamePrefix = NULL)
{
    ...
    if (useEnumForPIDLs) // slower variant, unfortunately necessary to obtain the share PIDL on the server
    {
                              ...
                              BOOL namesMustBeValid = FALSE)
{
    ...
    for (i = 0; i < files; i++)
    {
        ...
        // drag&drop of an invalid name means operating on a name with silently trimmed spaces/dots
        // at the end (instead of "a   " it would become "a"), which we definitely do not want
                        ...
                        *name = 0;
                                                                          ...
                                                                          IID_IShellFolder, (LPVOID*)&folder2))))
                        {
                            ...
                            *name = c;
                            ...
                            TRACE_E("BindToObject error: 0x" << std::hex << ret << std::dec); // keep dir
                        IMalloc* alloc;
...
void OpenFolderAndFocusItem(HWND hOwnerWindow, const char* dir, const char* item)
{
    ...
    if (PathContainsValidComponents((char*)mydir, FALSE))
    {
        ...
        if (item[0] != 0)         // if no item should be selected, avoid SHOpenFolderAndSelectItems because it would display the parent directory (see MSDN)
        {
...
//
//  returns TRUE if path contains a valid new path
                           ...
                           char* path, BOOL onlyNet, const char* initDir)
{
    __try
    {
        ...
        /* j.r.: under W2K, after opening the focus lands on OK instead of in the tree view (as it used to);
           ensure_visible also stops working; simply AWFUL, so we are returning to the old version of the dialog;
           maybe we can rewrite it sometime later.
    if (!onlyNet)  // Petr: the Network dialog works only in the old version - the new one cannot ask the user to log on to the server (when the current logon is not enough)
                    ...
                    if (m != NULL)
                    {
                        ...
                        else // keep only the New menu
                        {
                            ...
                            while (1)
                            {
                                ...
                                {
                                    if (mi.hSubMenu != NULL)
                                    { // look for the last submenu (user items should only appear before the Windows items; we will see over time)
                                        foundIndex = index;
...
void CMenuNew::ReleaseBody()
{
    __try
    {
        // the HMENU Menu is destroyed directly by the menu it is attached to
        if (Menu2 != NULL)
            Menu2->Release(); // this call crashes occasionally
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
