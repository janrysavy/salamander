...
public:
    ...
    // searches all masks and returns a pointer to the matching item, if found
...
struct CViewerMasksItem
{
    ...
    DWORD HandlerID; // unique ID (valid during the Salamander session)
                     // used to identify the editor when selecting from the CFileHistory
...
struct CConfiguration
{
    ...
    int IncludeDirs,            // select/deselect (*, +, -) directories as well
        ...
        CloseShell,             // close the shell after running the command line
        ...
        ClearReadOnly,          // remove the read-only flag when working with CD-ROMs
        ...
        AlwaysOnTop,            // should the main window stay Always On Top?
                                //      FastDirectoryMove,     // allow moving directories by renaming?
        SortUsesLocale,         // sort according to regional settings
        SortDetectNumbers,      // detect numbers during sorting? (see StrCmpLogicalW)
        SortNewerOnTop,         // show newer items first -- Salamander 2.0 behavior
        SortDirsByName,         // sort directories by name
        SortDirsByExt,          // emulate extensions for directories (sort by extension + show in the Ext column)
        SaveHistory,            // save histories to the configuration?
        ...
        ForceOnlyOneInstance,   // from cmdline: pretend OnlyOneInstance is enabled
        StatusArea,             // Salamander lives in the tray and won't appear in the taskbar when minimized
        SingleClick,            // is one click enough to select an item?
        ...
        NetwareFastDirMove,     // on Novell Netware use fast-dir-move (rename directory)? otherwise rename files only
        UseAsyncCopyAlg,        // Win7+ only: use asynchronous copy algorithm to network drives?
        ReloadEnvVariables,     // regenerate after environment variables change?
        QuickRenameSelectAll,   // Quick Rename/Pack selects the whole name (users disliked the new selection)
        EditNewSelectAll,       // EditNew should select the entire name (not just the base). Users requested a separate option: some always create .TXT and overwrite only the name, others use various extensions and want the whole name selected.
        ...
        ShowSLGIncomplete,      // TRUE = if IsSLGIncomplete is not empty, show the missing translation warning
        ...
        CnfrmShowNamesToCompare, // User Menu: show the Compare names dialog even when both names are filled
        CnfrmDSTShiftsIgnored,   // Compare Directories: pairs differ by exactly one or two hours; show warning that they were ignored?
        CnfrmDSTShiftsOccured,   // Compare Directories: pairs differ by exactly one or two hours; show hint that such differences can be ignored?
    ...
    int CompareIgnoreFiles;  // ignore the specified file names?
    int CompareIgnoreDirs;   // ignore the specified directory names?
    ...
    char IfPathIsInaccessibleGoTo[MAX_PATH]; // path used when the current one becomes inaccessible (network outage, media removed, ...)

    DWORD LastUsedSpeedLimit; // remembers the last used speed limit (users often repeat one number)

    BOOL QuickSearchEnterAlt; // if TRUE, quick search accepts Alt+letter

    // how items are displayed in the panel
    ...
    int ShowPanelCaption; // draw the panel caption in color within the directory line?
    int ShowPanelZoom;    // show the Zoom button in the directory line?

    char InfoLineContent[200];

    int FileNameFormat; // how to adjust filenames after reading from disk

    int SizeFormat; // how to display values in the Size column: SIZE_FORMAT_xxx

    int SpaceSelCalcSpace; // pressing Space selects and calculates occupied space
    ...
    // Internal Viewer:
    int CopyFindText; // after F3 from Find Files dialog: copy search text to viewer?
    ...
    WINDOWPLACEMENT WindowPlacement; // invalid unless SavePosition is TRUE

    BOOL WrapText; // text wrapping set via menu (stored here)
    ...
    // Tip of the Day
    //  int  ShowTipOfTheDay;         // display Tip of the Day at startup
    ...
    int LastPluginVerOP; // ACTUAL_VERSION from plugins.ver for the other platform (x86/x64); stored to detect overwriting by the other version
    ...
    char* DisabledCustomIconOverlays; // allocated list of disabled icon overlay handlers (separator ';', escape sequence for ';' is ';;')
    ...
    int DoNotDispCantLoadPluginSLG;  // TRUE = suppress warning that plugin SLG with the same name cannot be loaded as Salamander
    int DoNotDispCantLoadPluginSLG2; // TRUE = suppress warning that the last used plugin SLG cannot be loaded (selected by user or automatically)
    int UseAsAltSLGInOtherPlugins;   // TRUE = try to use AltSLGName for plugins
    char AltPluginSLGName[MAX_PATH]; // only if UseAsAltSLGInOtherPlugins is TRUE: fallback SLG module for plugins (if LoadedSLGName does not exist)
    ...
    int UseTitleBarPrefix;                       // show prefix in the title bar?
...
protected:
    ...
    void RefreshGroupIconInUMItems(); // when colors change HGroupIcon is modified, we must update it in UserMenuItems as well
...
protected:
    ...
    BOOL EditMode;
    int EditIndex;
    BOOL LabelEdit; // editing a label
...
{
protected:
    BOOL IfPathIsInaccessibleGoToChanged; // TRUE if the user edited the IfPathIsInaccessibleGoTo path
