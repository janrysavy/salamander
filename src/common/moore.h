...
public:
    ...
    // pattern buffer must have length (length + 1) characters (compatibility with strings)
...
protected:
    ...
    int* Fail1;            // failure table for the current character
    int* Fail2;            // failure table for occurrences of the substring from the right
    char* OriginalPattern; // original pattern to search for
    char* Pattern;         // pattern to search in the corresponding form (Flags)
    int Length;            // pattern length

private:
    BOOL Initialize(); // called only from SetFlags

    WORD Flags; // modify through SetFlags
};

//
// ****************************************************************************
// SearchForward
// returns the position of Pattern or -1
// text - where to search
// length - length of the text string
// start - first character indexed from 0
//
