...
// macro MESSAGES_DISABLE - detaches the module from the compilation
// macro MESSAGES_DEBUG - prints the file name and line number before the message
//                        and enables messages shown through DMESSAGE_???
...
// call once the current thread no longer needs access to the module's functions and data
void LeaveMessagesModul();

#endif // MULTITHREADED_MESSAGES_ENABLE

/// returns a pointer to the global buffer filled with a string from resources
const char* rsc(int resID);
const WCHAR* rscW(int resID);

/// returns a pointer to the global buffer filled with a string from sprintf
...
/// returns a pointer to the global buffer filled with an error description
...
extern HINSTANCE HInstance; // expected variable containing the instance handle

/// Sets the title of message boxes generated by the MESSAGExx macros. Copies the string.
...
/// Sets the parent of message boxes generated by the MESSAGExx macros
void SetMessagesParent(HWND parent);

class C__Messages
{
protected:
    C__StringStreamBuf MessagesStringBuf; // string buffer holding the data of the messages stream
...
{
protected:
    C__StringStreamBufW MessagesStringBuf; // string buffer holding the data of the messages stream
...
/**@name Macros generating message boxes.
 * If the MESSAGES_DEBUG macro is defined, message boxes display
 * on the first line the file and the line where MESSAGExx is in the code.\\
 *
 */
...
/** shows a message box with the supplied text, stays on the same thread, distributes
    messages of the calling thread */
...
/** shows a message box with the supplied text on a new thread, without distributing
    messages of the calling thread */
...
/** Displays a message box with the specified text and an information icon,
    remains on the same thread, and dispatches the calling thread's messages */
...
/** Displays a message box with the specified text and a question icon,
    remains on the same thread, and dispatches the calling thread's messages */
...
/** Displays a message box with the specified text and an error icon,
    remains on the same thread, and dispatches the calling thread's messages */
...
/** Displays a message box with the specified text and an information icon on a new thread,
...
/** Displays a message box with the specified text and a question icon on a new thread,
    without dispatching the calling thread's messages */
...
/** Displays a message box with the specified text and an error icon on a new thread,
    without dispatching the calling thread's messages */
...
/// MESSAGE_I depends on the existence of the MESSAGES_DEBUG macro
...
/// MESSAGE_Q depends on the existence of the MESSAGES_DEBUG macro
...
/// MESSAGE_TI depends on the existence of the MESSAGES_DEBUG macro
#define DMESSAGE_TI(str, buttons) MESSAGE_TI(str, (buttons))
#define DMESSAGE_TIW(str, buttons) MESSAGE_TIW(str, (buttons))

/// MESSAGE_TQ depends on the existence of the MESSAGES_DEBUG macro
...
/* These functions must raise a compilation error
const char *rsc(int resID);
const WCHAR *rscW(int resID);
const char *spf(const char *formatString, ...);
const WCHAR *spfW(const WCHAR *formatString, ...);
