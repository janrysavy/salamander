...
// the __DEBUG_WINLIB macro enables several tests for tricky WinLib bugs
...
void SetWinLibStrings(const TCHAR* invalidNumber, // "not a number" (used by numeric transfer buffers)
                      const TCHAR* error);        // title "error" (used by numeric transfer buffers)

extern HINSTANCE HInstance;
extern const TCHAR* CWINDOW_CLASSNAME;  // name of the universal window class
extern const TCHAR* CWINDOW_CLASSNAME2; // name of the universal window class - does not have CS_VREDRAW | CS_HREDRAW

#ifndef _UNICODE
extern const WCHAR* CWINDOW_CLASSNAMEW;  // name of the Unicode universal window class
extern const WCHAR* CWINDOW_CLASSNAME2W; // name of the Unicode universal window class - does not have CS_VREDRAW | CS_HREDRAW
#endif                                   // _UNICODE
...
{
    ooAllocated, // deallocate on WM_DESTROY
    ooStatic,    // set HWindow to NULL on WM_DESTROY
...
public:
    ...
    virtual ~CWindowsObject() {} // ensure derived destructors run
...
enum CTransferType
{
    ...
    ttDataFromWindow // data go from the window
};
...
{
public:
    int FailCtrlID; // INT_MAX = all good, otherwise the control ID with an error
    CTransferType Type;
...
protected:
    HWND HDialog; // dialog handle for which the transfer runs
};
...
protected:
    ...
    BOOL Modal; // affects how the dialog is destroyed
    HINSTANCE Modul;
    int ResID;
    HWND Parent;
};
...
struct CWindowData
{
    // if window objects (Wnd) live on the stack (typically modal dialogs, e.g. SalMessageBox())
    // and the thread terminates, the stack becomes invalid and the window objects are no longer accessible,
    // we prevent that by accessing the objects (Wnd) only while their window handles (HWnd) remain valid
    HWND HWnd;
    CWindowsObject* Wnd;
};
...
public:
    ...
    CWinLibCS CS; // public so we can locally prevent changes in Windows Manager
...
{
protected:
    const TCHAR* QueueName; // queue name (debugging only)
    CWindowQueueItem* Head;
    int Count;
    CWinLibCS CS; // access from multiple threads -> synchronization needed
...
public:
    ...
    BOOL Add(CWindowQueueItem* item); // add an item to the queue, return success
    void Remove(HWND hWindow);        // remove an item from the queue
    BOOL Empty();                     // return TRUE if the queue is empty
    int GetWindowCount();             // return the number of windows in the queue

    // send a message to all windows (PostMessage - windows can be in different threads)
