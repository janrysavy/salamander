...
void CElasticLayout::AddResizeCtrl(int resID)
{
    ...
    if (hChild != NULL)
    {
        ...
        // if the control extends past the current SplitY divider, stretch SplitY to cover it
...
CElasticLayout::FindMoveControls(HWND hChild, LPARAM lParam)
{
    ...
    // if the control's top edge is at or below SplitY, add it to the moving-control list
    RECT r;
    GetWindowRect(hChild, &r);
...
void CElasticLayout::FindMoveCtrls()
{
    ...
    // compute the bounding rectangle that encloses every control scheduled to move
    RECT rEnvelope = {0};
...
CPropSheetPage::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_HELP:
    {
        ...
        break; // allow F1 to fall through to the parent window
    }
    ...
    case WM_NOTIFY:
    {
        if (((NMHDR*)lParam)->code == PSN_KILLACTIVE) // PSN_KILLACTIVE (page deactivation)
        {
            ...
            else // keep the page from deactivating
                SetWindowLongPtr(HWindow, DWLP_MSGRESULT, TRUE);
            return TRUE;
        }

        if (((NMHDR*)lParam)->code == PSN_SETACTIVE) // PSN_SETACTIVE (page activation)
        {
            if (ParentDialog != NULL && ParentDialog->LastPage != NULL)
            { // remember which page was last active
        ...
        { // Finish button pressed
            // PSN_KILLACTIVE was not sent, so validate explicitly
            if (!ValidateData())
            {
                SetWindowLongPtr(HWindow, DWLP_MSGRESULT, TRUE);
                return TRUE;
            ...
            // iterate over every page to transfer its data
                                   ...
                                   LPARAM lParam)
{
    ...
    case WM_INITDIALOG: // first message: attach the object to the dialog
    {
        ...
        else
        {
            ...
            // insert hwndDlg into the window list
            if (!WindowsManager.AddWindow(hwndDlg, dlg)) // error
            {
                TRACE_ET(_T("Unable to create dialog."));
                return TRUE;
            }
            dlg->NotifDlgJustCreated(); // provides a hook for tweaking the dialog layout
        }
        break;
    }

    case WM_DESTROY: // final message: detach the object from the dialog
    {
        ...
        {
            // Petr: I moved it below wnd->WindowProc() so that messages still arrive during WM_DESTROY
            //       (Lukas needed that)
...
#define _TPD_LEFTMARGIN 4  // offset of the TreeView and Caption from the left edge
#define _TPD_TOPMARGIN 4   // offset of the TreeView and Caption from the top edge
...
CTPHCaptionWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (devCaps == -1) // use the gradient only when the display offers more than 256 colors
        {
...
#define TPH_STEPS 100
        ...
        */
        ...
        if (Text != NULL)
        {
            ...
            // srcLF.lfWeight = FW_BOLD; // on Vista bold text looks quite ugly and unreadable
...
CTPHGripWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SETCURSOR:
    {
        // force a north-south sizing cursor
...
{
    // defer forwarding WM_INITDIALOG until the window dimensions are known
    ...
    case WM_INITDIALOG:
    {
        ...
        {
            RECT rect = {0, 0, 4, 8};
            MapDialogRect(HWindow, &rect); // retrieve baseUnitX and baseUnitY for converting dialog units to pixels
        ...
        // capture the minimal dimensions so we can enforce them later
        ...
        // set the user-specified window size and lay out the controls
        int height = (int)*WindowHeight;
        RECT clipR; // ensure the dialog never exceeds the screen height
    ...
    case WM_HELP:
    {
        ...
        return TRUE; // prevent F1 from falling through to the parent even if WinLibHelp->OnHelp() is not invoked
    }
    ...
    case WM_COMMAND:
    {
        ...
        case _TPD_IDC_OK:
        {
            ...
            // walk through all pages to transfer their data
        ...
        // forward the command so Enter triggers the default buttons
    ...
    case WM_NCHITTEST:
    {
        // restrict resizing to the vertical axis
...
void CTreePropHolderDlg::LayoutControls()
{
    ...
    if (hdwp != NULL)
    {
        ...
        HANDLES(EndDeferWindowPos(hdwp));
        // hack: there is apparently a bug in the tree view/common controls: if a scrollbar appears because of the content,
        // the selected item is not repainted, so it is cropped on the right; it may be related to full-row select and
        // the Aero look. Repainting on Windows 7 does not flicker, so we can probably afford it.
...
int CTreePropHolderDlg::BuildAndMeasureTree()
{
    ...
    for (int i = 0; i < TPD->Count; i++)
    {
        ...
        RECT r;
        // Account for the TreeView_GetItemRect() return value
        if (TreeView_GetItemRect(HTreeView, TPD->At(i)->HTreeItem, &r, TRUE) && r.right - r.left > width)
            width = r.right - r.left;
    }
    // Collapse any items that should not remain expanded
    for (int i = 0; i < TPD->Count; i++)
    {
...
{
    lpw = lpdwAlign(lpw); // align the DLGITEMTEMPLATEEX structure on a DWORD boundary
    *(DWORD*)lpw = 0;     // helpID
    lpw += 2;
                             ...
                             const TCHAR* buttonHelp)
{
    if (Count > 0)
    {
        ...
        // determine the maximum page dimensions
        for (int i = 0; i < Count; i++)
        {
            At(i)->ParentDialog = this;
            ...
            WORD* t = pageTemplate + 13;            // menu: skip to the dialog class and then to its title
            if (*t == 0)
                t++; // no menu
            else
            {
        ...
        // distance from the dialog's bottom edge to the bottoms of the TreeView and child dialog
        ...
        // build the dialog template: DLG or DLGEX must match the page format,
        // otherwise controls are clipped and fonts differ between the pages and the rest of the tree property dialog
        ...
        *((BYTE*)lpw + 1) = ANSI_CHARSET;           // font charset
        ...
        // placeholder static control that the child dialog replaces during initialization
