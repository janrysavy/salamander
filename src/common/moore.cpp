...
// ****************************************************************************
// Initialize
// computation of the Fail1 and Fail2 arrays
//
...
BOOL CSearchData::Initialize()
{
    ...
    Fail2[Length] = Length - 1; // added because the algorithm touches this value; nothing else needed
...
void CSearchData::Set(const char* pattern, WORD flags)
{
    ...
    if (OriginalPattern != NULL)
    {
        ...
        OriginalPattern[Length] = 0; // for compatibility with a regular string
    }
    if (Pattern != NULL)
    {
        free(Pattern);
