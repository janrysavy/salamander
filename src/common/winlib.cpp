...
// workaround for runtime check failure in the debug build: the original macro cast rgb to WORD,
...
void ReleaseWinLib()
{
    // detach any open windows from WinLib because it is shutting down...
                       ...
                       LPVOID lpvParam)        // pointer to the window object being created
{
    ...
    if (hWnd != 0)
    {
        if (WindowsManager.GetWindowPtr(hWnd) == NULL) // if it is not yet in WindowsManager
            AttachToWindow(hWnd);                      // add it now so we can subclass the window
    }
    return hWnd;
}
...
CWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_HELP:
    {
        ...
        if (GetWindowLongPtr(HWindow, GWL_STYLE) & WS_CHILD)
            break;   // if we do not handle F1 and it is a child window, let F1 fall through to the parent
...
#endif // _UNICODE
{
...
#endif                                                // _UNICODE
                                                      //--- register the window handle in WindowsManager
    ...
    case WM_DESTROY: // last message - detach the object from the window
    {
        ...
        {
            // Petr: I moved it below wnd->WindowProc() so that during WM_DESTROY
            //       messages would still arrive (needed by Lukas)
            ...
            // because someone else in the subclassing chain already restored the original WndProc
#ifdef _UNICODE
...
#else  // _UNICODE
            if (wnd->UnicodeWnd) // if our window procedure is Unicode, we must use the "W" variants of the API functions
            {
    ...
    //--- forward to the associated window object's WindowProc(...)
    LRESULT lResult;
    if (wnd != NULL)
    ...
    case WM_COMMAND:
    {
        ...
        case IDHELP:
        {
            if (WinLibHelp != NULL && HelpID != -1)
            {
                ...
                hi.dwContextId = ResID; // WM_HELP also delivers ResID instead of HelpID, so keep it consistent
...
CDialog::CDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: // first message - attach the object to the dialog
    {
        ...
        {
            dlg->HWindow = hwndDlg;
            //--- register the dialog handle in WindowsManager
            if (!WindowsManager.AddWindow(hwndDlg, dlg)) // error
            {
                TRACE_ET(_T("Unable to create dialog."));
                return TRUE;
            }
            dlg->NotifDlgJustCreated(); // introduced to allow adjusting the dialog layout
        }
        break;
    }
    ...
    case WM_DESTROY: // last message - detach the object from the dialog
    {
        ...
        {
            // Petr: I moved it below dlg->DialogProc() so that during WM_DESTROY
            //       messages would still arrive (needed by Lukas)
            ...
            if (dlg->IsAllocated())
                ...
                dlg->HWindow = NULL; // note the detachment
        }
        return ret;
    }
    ...
    //--- forward to the dialog object's DialogProc(...) handler
    INT_PTR dlgRes;
    if (dlg != NULL)
...
CWindowQueue::~CWindowQueue()
{
    ...
    // multi-threading is no longer a risk here because the app is closing and the threads are already terminated
...
BOOL CTransferInfo::GetControl(HWND& ctrlHWnd, int ctrlID, BOOL ignoreIsGood)
{
    if (!ignoreIsGood && !IsGood())
        return FALSE; // no point in processing more
...
void CTransferInfo::EnsureControlIsFocused(int ctrlID)
{
    ...
    if (ctrl != NULL)
    {
        ...
        {                // e.g. the edit line in a combo box
...
void CTransferInfo::EditLine(int ctrlID, double& value, TCHAR* format, BOOL select)
{
    ...
    if (GetControl(HWindow, ctrlID))
    {
        ...
        case ttDataFromWindow:
        {
            ...
            if (*s == _T('-') || *s == _T('+'))
                s++;        // skip the sign
            while (*s != 0) // convert commas to dots
            {
...
void CTransferInfo::EditLine(int ctrlID, int& value, BOOL select)
{
    ...
    if (GetControl(HWindow, ctrlID))
    {
        ...
        case ttDataFromWindow:
        {
            ...
            TCHAR* endptr;
            value = _tcstoul(buff, &endptr, 10); // replacement for atoi / _ttoi, which return 2147483647 instead of 4000000000 (because it is a SIGNED INT)
            break;
        }
        }
    }
                             ...
                             BOOL ignoreOverflow, BOOL quiet)
{
    ...
    if (GetControl(HWindow, ctrlID))
    {
        ...
        case ttDataFromWindow:
        {
            ...
            while (*s != 0) // validate the number
            {
                ...
                else
                {
                    if (hexMode && num > 0x0fffffffffffffffui64 ||
                        !hexMode && num > 1844674407370955161ui64 || // maximum is 18446744073709551615ui64
            ...
            // on overflow we return the boundary values (inspired by value = _ttoi64(buff))
            if (unsignedNum)
            {
                if (overflow)
