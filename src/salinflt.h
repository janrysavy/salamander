...
{
    const char* Data;    // start of the data
    const char* DataPtr; // current position in the data
    const char* DataEnd; // end of the data

    DWORD Crc; // CRC32 of the extracted data
...
__forceinline uch NextByte(CDecompressionObject* decompress)
{
    ...
    {
        if (decompress->DataPtr == decompress->DataEnd)
            // The error state is reported only when DataPtr exceeds DataEnd
            decompress->DataPtr++;
        return 0 /* error */;
    }
}
