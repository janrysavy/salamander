...
CVersionInfo::LoadBlock(const BYTE*& ptr, CVersionBlock* parent)
{
    ...
    case vbtVar:
    {
        ...
        {
            // String and Var have no children, so we exit here
            return block;
        }
    }
    }
...
BOOL CVersionInfo::SaveBlock(CVersionBlock* block, BYTE*& ptr, const BYTE* maxPtr)
{
    ...
    BYTE* oldPtr = ptr; // store for subsequent computation of our and our children's size

    // wLength
    WORD* wLength = (WORD*)ptr; // save for setting later
    ptr += 2;

    // wValueLength
    WORD* wValueLength = (WORD*)ptr; // save for setting later
    *wValueLength = 0;
    ptr += 2;
    ...
    *((WORD*)ptr) = (block->Text) ? 1 : 0;
    ...
    // otherwise store the size including padding
    if (block->Children.Count > 0)
        *wLength = (WORD)(ptr - oldPtr);
...
BOOL CVersionInfo::UpdateResource(HANDLE hUpdateRes, int resID)
{
    ...
    memset(buff, 0, 50000);
    BYTE* ptr = buff; // note: the value will be modified
