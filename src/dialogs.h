...
public:
    // 'history' determines whether the dialog will contain a combobox (TRUE) or an edit line (FALSE)
    // 'directoryHelper' specifies if a resource with a button behind the edit line will be used to select a directory
    // 'selectionEnd' specifies up to which character the name is selected (used for quick rename), -1 selects the whole name
...
protected:
    ...
    char** History;
    int HistoryCount;
    CCriteriaData* CriteriaInOut; // used to transfer data into and out of the dialog (on OK)
...
public:
    ...
    // Data change only to values different from NULL (parameter NULL means no change)
    void SetDlgData(CProgressDlgArrItem* dlg, HANDLE dlgThread, HWND dlgWindow);

    // Removes the structure 'dlg' from the array; 'dlg->DlgThread' must be NULL
    // Call only if starting the dialog acquired via PrepareNewDlg() failed
    void RemoveDlg(CProgressDlgArrItem* dlg);

    // Removes from the array all dialogs whose threads have already finished (their handles are closed)
    // Returns the number of operation dialog threads still running (when it returns zero Salamander can be terminated)
    int RemoveFinishedDlgs();

    // Finds in the array a dialog with window 'hdlg' and stores NULL to its 'DlgWindow'
    // This way the dialog reports it is closing; the dialog thread should end shortly
    void ClearDlgWindow(HWND hdlg);

    // Returns the next open dialog; if no dialog is open, returns NULL
    // Before the first call set 'index' to 0, use the returned value of 'index' for subsequent calls (do not touch 'index' between calls)
    // Dialogs are returned cyclically (after the last one it returns to the first)
    HWND GetNextOpenedDlg(int* index);

    // Ensures all open dialogs are closed
    // Call only from the main thread (otherwise another dialog might open and not know it should terminate)
    void PostCancelToAllDlgs();

    // Sends a message to all dialogs that the icon (color) has changed and needs to be set again; call only from the main thread
    void PostIconChange();
};
...
// Returns FALSE if the progress dialog could not be opened in a new thread or if starting the worker thread failed in this dialog
// When FALSE is returned the caller must free the script 'script' manually (otherwise the script is freed after the worker thread finishes)
...
protected:
    ...
    BOOL FlushCachedData(); // sends modified data to statics and progress bars; returns TRUE if something dirty was updated

    void SetDlgTitle(BOOL minimized);
    void SetWindowIcon();

protected:
    BOOL RunningInOwnThread;                // TRUE/FALSE = dialog runs in its own thread ("background") / runs in the main thread and is modal to its parent (usually one of the panels)
    CStartProgressDialogData* ProgrDlgData; // non-NULL only if the dialog runs in its own thread and the main thread hasn't been resumed yet (waiting for dialog creation and operation start)
    ...
    HANDLE WorkerNotSuspended;    // non-signaled means the worker should enter suspend mode
    ...
    BOOL StatusPaused;            // TRUE = the operation is stopped, e.g. when querying Cancel (and other dialogs)
    ...
    BOOL CanClose; // prevents unwanted closing (handled inside this object's methods)

    BOOL TimerIsRunning; // if TRUE, a timer updates text changes and the progress bar

    BOOL FirstUserSetDialog; // TRUE until WM_USER_SETDIALOG is processed (the first call forces a repaint so the user sees the dialog at least flash)

    HWND NextForegroundWindow; // window that should be foreground after closing this dialog (on XP SP1 with a top-most window, focus sometimes went to that window instead of Salamander)
...
{
protected:
    char VolumePath[MAX_PATH]; // which drive information should be shown for (either the root or a junction point)
...
public:
    int FileNameFormat; // numbers compatible with AlterFileName
    int Change;         // which part of the name to modify  --||--
...
{
protected:
    DWORD DisplayedVersion; // version currently shown in the list
...
{
protected:
    HWND HListView; // our list view
    CToolbarHeader* Header;
    HIMAGELIST HImageList;      // image list for the list view
    ...
    char InstalledPluginsText[200]; // text taken from the list view caption when the dialog opens
...
protected:
    ...
    void InitColumns();     // add columns to the list view
    ...
    void OnSelChanged();                                                    // selected item in the list view changed
    CPluginData* GetSelectedPlugin(int* index = NULL, int* lvIndex = NULL); // returns NULL if no item is selected; 'index' returns the Plugins array index; 'lvIndex' returns the index in the list view, can be NULL
...
protected:
    HWND HListView; // list view control
    CToolbarHeader* Header;
    CPluginData* Plugin;
...
protected:
    ...
    CPluginMenuItem* GetSelectedItem(int* orgIndex); // orgIndex receives index in Plugin->MenuItems; may be NULL
...
{
public:
    // array corresponding to SalamanderConfigurationRoots; TRUE if the configuration exists, FALSE otherwise
...
public:
    ...
    // returns the index into Items; highest priority is 'selectSLGName' (if not NULL), then
    ...
    // or the Windows setting or -1 if nothing matches
...
protected:
    HWND HListView;
    CShares SharedDirs; // keep our own instance so nobody refreshes it in the background
...
protected:
    ...
    void Refresh();     // loads shared folders and adds them to the list view
    static int CALLBACK SortFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
    void SortItems();                        // sorts items based on the SortBy variable
    int GetFocusedIndex();                   // returns the index in SharetDirs or -1 if no item is selected
...
struct CConnectionItem
{
    ...
    BOOL Default;  // if TRUE, the item is FOCUSED+SELECTED after the listview is filled; only one item should have Default == TRUE
...
protected:
    ...
    // insert a new item into the Connections array
...
public:
    ...
    // change progress relatively
    void AddSize(const CQuadWord& size);

    // dispatches messages; returns FALSE if the user cancelled the operation
    BOOL Continue();
...
extern CProgressDlgArray ProgressDlgArray; // array of file-operation dialogs (only those running in their own threads)
