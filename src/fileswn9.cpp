...
BOOL SafeInvokeCommand(IContextMenu2* menu, CMINVOKECOMMANDINFO& ici)
{
    ...
    // temporarily lower thread priority so that a confused shell extension does not eat the CPU
...
BOOL CFilesWindow::ClipboardPaste(BOOL onlyLinks, BOOL onlyTest, const char* pastePath)
{
    ...
    IDataObject* dataObj;
    BOOL files = FALSE;       // find out whether the clipboard data are actually files
    BOOL filesOnClip = FALSE; // do we have something of ours on the clipboard for file/directory paste?
    ...
    if (OleGetClipboard(&dataObj) == S_OK && dataObj != NULL)
    {
        ...
        { // Salamander "fake" data object on the clipboard -> paste files/directories from the archive
                ...
                SalShExtSharedMemView->PastedDataID == SalShExtPastedData.GetDataID())
            {
                ...
                SalShExtSharedMemView->ClipDataObjLastGetDataTime = GetTickCount() - 60000; // probably no longer needed on W2K+: set last GetData() time one minute back so a later Release of the data object goes smoothly
            }
            ...
            if (pasteFromOurData)
            {
                ...
                {
                    // move clears the clipboard after itself (cut&paste), release the clipboard before
                    // starting the operation so other applications can use it for further data transfers
                    ...
                    {
                        IdleRefreshStates = TRUE;  // on next Idle force checking status variables
                ...
                PostMessage(MainWindow->HWindow, WM_USER_SALSHEXT_TRYRELDATA, 0, 0); // after unlocking we may release the data
                return TRUE;
            }
        }
        ...
        if (dataObj->EnumFormatEtc(DATADIR_GET, &enumFormat) == S_OK)
        {
            ...
            while (enumFormat->Next(1, &formatEtc, NULL) == S_OK)
            {
                if (formatEtc.cfFormat == cfIdList ||
                    formatEtc.cfFormat == cfFileContent && !onlyLinks) // file content (stored when copying from Windows Mobile (WinCE)) - cannot create shortcuts (not really usable for file content)
                {
                    files = TRUE;
                }
        ...
        DWORD effect = 0;
        if (ownRutine) // if there's a chance for our own handling, find out whether it is copy or move
        {
            DWORD dropEffect = 0;
            ...
            if (!onlyLinks && !onlyTest && ownRutine)
            {
                ...
                if (effect == 0)
                    ownRutine = FALSE; // neither copy nor move - we can't do anything else
            }
        }
        ...
        else
        {
            if (files && !onlyTest) // perform paste/pastelink
            {
                ...
                if (menu != NULL)
                {
                    ...
                    if (onlyLinks)
                        PasteLinkIsRunning++; // better assume multiple threads might compete
                ...
                // post refreshes to both panels (if they are not auto-refreshed - it won't reach FS panels)
...
BOOL CFilesWindow::ClipboardPasteToArcOrFS(BOOL onlyTest, DWORD* pasteDefEffect)
{
    ...
    if (OleGetClipboard(&dataObj) == S_OK && dataObj != NULL)
    {
        ...
        if (IsSimpleSelection(dataObj, namesList)) // find out whether these are files/directories from disk and whether they are from a single path
        {
            ret = TRUE;
            // find out whether it is copy or move
            DWORD dropEffect = DROPEFFECT_COPY | DROPEFFECT_MOVE; // if we can't determine the effect, assume both (Copy has priority)
            UINT cfPrefDrop = RegisterClipboardFormat(CFSTR_PREFERREDDROPEFFECT);
            if (OpenClipboard(HWindow))
            {
                HANDLE handle = GetClipboardData(cfPrefDrop);
                ...
                if (Is(ptZIPArchive)) // Paste into archive
                {
                        ...
                        PackerFormatConfig.GetUsePacker(format - 1)) // edit available?
                    {
                        if (dropEffect == (DROPEFFECT_COPY | DROPEFFECT_MOVE))
                            dropEffect = DROPEFFECT_COPY; // Copy has priority (safer)
                        if (dropEffect != 0)
                        {
                            DoDragDropOper(dropEffect == DROPEFFECT_COPY, TRUE, GetZIPArchive(),
                                           GetZIPPath(), namesList, this);
                            namesList = NULL;         // DoDragDropOper will deallocate it, we won't do it here
                            FocusFirstNewItem = TRUE; // if it will be a single new file, let the focus find it
...
BOOL CFilesWindow::IsTextOnClipboard()
{
    ...
    while (1)
    {
        ...
        {
            if (attempt++ <= 10)
                Sleep(10); // wait to see if the clipboard frees up (max. 100 ms)
            else
            {
                TRACE_E("OpenClipboard() has failed!");
                break;
...
BOOL CFilesWindow::PostProcessPathFromUser(HWND parent, char (&buff)[2 * MAX_PATH])
{
    if (!IsFileURLPath(buff) && IsPluginFSPath(buff))
        return TRUE; // let the FS plugin handle processing
    ...
    // expand ENV variables (frequent request on the forum and for our needs)
    // if an ENV variable doesn't exist, a directory with the same name gets a chance
...
void CFilesWindow::ClipboardPastePath()
{
    ...
    if (OpenClipboard(HWindow))
    {
        ...
        if (handle != NULL)
        {
            ...
            if (pathW != NULL)
            {
                while (*pathW != 0 && *pathW <= L' ')
                    pathW++; // remove spaces+CR+LF before the path
                if (ConvertU2A(pathW, -1, buff, _countof(buff)))
                    changePath = TRUE;
                else
                {
                    if (buff[0] != 0 && GetLastError() == ERROR_INSUFFICIENT_BUFFER)
                        changePath = TRUE; // buffer is small, ignore it (same as ANSI variant)
                }
                HANDLES(GlobalUnlock(handle));
            }
        }
        if (!changePath) // probably not Unicode, try ANSI (when Unicode is present ANSI is usually broken - without diacritics)
        {
            ...
            {
                while (*path != 0 && *path <= ' ')
                    path++; // remove spaces+CR+LF before the path
...
void CFilesWindow::ChangeFilter(BOOL disable)
{
    ...
    BeginStopRefresh(); // we don't need any refreshes
...
BOOL CFilesWindow::OnLButtonDown(WPARAM wParam, LPARAM lParam, LRESULT* lResult)
{
    ...
    BOOL noBeginDrag = FALSE; // user drags with left button and adds the right
    if (BeginDragDrop)
    {
        noBeginDrag = TRUE;
        ReleaseCapture();
    ...
    // compatibility with Windows Explorer: clicking outside (in the active or inactive panel)
    // means confirming the rename; other actions (Alt+Tab) are considered, unlike Explorer,
        ...
        if (index >= 0 && index < Dirs->Count + Files->Count)
        {
                    ...
                    (GetKeyState(VK_CONTROL) & 0x8000) == 0)
            {
                ...
                if (GetKeyState(VK_SHIFT) & 0x8000) // shift + lButton -> extended selection
                {
                    ...
                    if (first != last)
                    {
                        // group deselect/select according to the item taken when Shift was pressed
            ...
            {
                // hack for UpDir where we want to allow box dragging (d&d is pointless anyway)
                if (GetFocus() != GetListBoxHWND())
                    MainWindow->FocusPanel(this);
                if (!noBeginDrag)
                    BeginBoxSelect = 1;
        ...
        if (index >= 0 && index < Dirs->Count + Files->Count)
        {
            if (selecting)
            {
                ...
                if (GetKeyState(VK_SHIFT) & 0x8000) // shift + rButton -> extended selection
                {
                    ...
                    if (first != last)
                    {
                        // group deselect/select according to the first item
            ...
            else
            {
                if (index == 0 && index < Dirs->Count && strcmp(Dirs->At(0).Name, "..") == 0)
                {
                    // hack for UpDir where we want to allow box dragging (drag&drop is pointless anyway)
                    if (GetFocus() != GetListBoxHWND())
                        MainWindow->FocusPanel(this);
                    if (!noBeginDrag)
                        BeginBoxSelect = 1;
...
BOOL CFilesWindow::OnRButtonUp(WPARAM wParam, LPARAM lParam, LRESULT* lResult)
{
    ...
    if (DragSelect)
    {
        ...
        ReleaseCapture();
        KillTimer(GetListBoxHWND(), IDT_SCROLL); // might still be running
        DragSelect = FALSE;
    }
    if (DragBox)
        EndDragBox();
    ...
    if (contextMenu)
    {
        ...
        if (index >= 0 && index < Dirs->Count + Files->Count)
        {
            ...
            if (GetSelCount() > 0) // is there a selection?
            {
                ...
                {                                                   // test whether clicked inside the selection (otherwise clear it)
        ...
        {
            if (GetSelCount() > 0) // we must clear the selection (this menu is for the panel - clicked behind items)
            {
                SetSel(FALSE, -1, TRUE);                        // explicit repaint
                PostMessage(HWindow, WM_USER_SELCHANGED, 0, 0); // sel-change notify
                UpdateWindow(MainWindow->HWindow);
            ...
            UserWorkedOnThisPath = TRUE;
            ShellAction(this, saContextMenu, FALSE, TRUE, TRUE); // menu only for the panel (clicking behind items)
        }
        *lResult = 0;
        return TRUE; // we don't want WM_CONTEXTMENU to be generated
    }
    return FALSE;
}
...
BOOL CFilesWindow::IsDragDropSafe(int x, int y)
{
    ...
    // enough time has passed since the drag started
...
void CFilesWindow::OfferArchiveUpdateIfNeededAux(HWND parent, int textID, BOOL* archMaybeUpdated)
{
    *archMaybeUpdated = FALSE;
    if (AssocUsed) // if the user edited files from the archive we must update the archive before operating on it, otherwise we would work with old versions stored directly inside
    {
        ...
        // repack the changed files, prepare them for further use
        BOOL someFilesChanged;
        UnpackedAssocFiles.CheckAndPackAndClear(parent, &someFilesChanged, archMaybeUpdated);
        SetEvent(ExecuteAssocEvent); // start cleanup of files
        DiskCache.WaitForIdle();
        ResetEvent(ExecuteAssocEvent); // finish cleanup of files

        // if edited files might be in the disk cache, drop them so they are extracted again when accessed
        if (someFilesChanged)
        {
            char buf[MAX_PATH];
...
void CFilesWindow::OfferArchiveUpdateIfNeeded(HWND parent, int textID, BOOL* archMaybeUpdated)
{
    ...
    { // the same archive is in the other panel, we must update it too
    ...
    if (*archMaybeUpdated)
    {
        // a bit of a hack: the user pressed F5 and instead we force an archive update and panel
        // refresh; F5 must then be pressed again (we could handle it via PostMessage(uMsg,
        // wParam, lParam), but it's better if the user checks the selection and presses F5 again)
...
BOOL CFilesWindow::OnMouseMove(WPARAM wParam, LPARAM lParam, LRESULT* lResult)
{
    ...
    *lResult = 0;
    if (BeginDragDrop && (wParam & (MK_LBUTTON | MK_RBUTTON))) // "tearing" for drag&drop
    {
        ...
        {
            // the condition "!PerformingDragDrop" was added after AS 2.52 due to a state reported by ehter:
            // in the directory there must be two files (one EXE downloaded via IE so it has a security ADS) and another file
            // the second file is dragged onto the EXE, which triggers a security window that is NONMODAL and has a message loop
            // thus Salamander's main thread gets stuck in ShellAction(); a second drag&drop then caused a crash
            // with this patch we disable new drag&drop, just like Windows Explorer does
            // the solution isn't clean because Salamander only seemingly runs (the main thread isn't in our message loop)
            if (!PerformingDragDrop)
            {
                if (Is(ptZIPArchive) && AssocUsed) // if the user edited files from the archive we must update the archive before operating on it, otherwise we'd work with old versions of the edited files stored directly in the archive
                {
                ...
                else
                {
                    if (IsDragDropSafe((short)LOWORD(lParam), (short)HIWORD(lParam))) // minimum time
                    {
                        ...
                        // if certain items are selected and we drag by some unselected one,
                        // clear the selection
                ...
                else
                {
                            ...
                            (GetKeyState(VK_CONTROL) & 0x8000) == 0)
                    {
                        if (GetKeyState(VK_SHIFT) & 0x8000) // shift + lButton -> extended selection
                        {
                            ...
                            if (first != last)
                            {
                                // group deselect/select according to the item captured when Shift was pressed
                                SetSelRange(SelectItems, first, last);
                                PostMessage(HWindow, WM_USER_SELCHANGED, 0, 0);
                            }
...
// copies the mask to the DIB to locate transparent/opaque areas and sets the alpha channel accordingly
...
CFilesWindow::CreateDragImage(int cursorX, int cursorY, int& dxHotspot, int& dyHotspot, int& imgWidth, int& imgHeight)
{
    ...
    if (selCount == 0)
    {
        ...
        if (GetViewMode() == vmBrief || GetViewMode() == vmDetailed)
        {
            ListBox->XOffset = 0; // affects item offset in detailed mode
            ...
            if (ContainsAlpha(lpBits, width, height))
            {
                ...
                //        and will be completely transparent
    ...
    if (bitsPerPixel <= 8)
    {
        ...
        // for now we take the whole image
...
*/
    ...
    SelectObject(hDC, hOldBmp); // release bitmap before adding to the imagelist
...
BOOL CopyUNCPathToClipboard(const char* path, const char* name, BOOL isDir, HWND hMessageParent, int nestingLevel)
{
    ...
    nestingLevel++; // note: also controls error messaging, we must increment here
    ...
    if (buff[0] == '\\' && buff[1] == '\\')
    {
        ...
        strcat(buff, name); // append the focused item name
    ...
    // if it's not UNC, it might be a SUBST drive
    ...
    // only on the top level we show an error message
    if (nestingLevel == 1)
    {
        ...
        // all possibilities failed -- give up and show an error message
        // the path cannot be converted to UNC; it's neither shared nor mapped
        strcpy(buff, path);
        SalPathAddBackslash(buff, 2 * MAX_PATH);
        strcat(buff, name);
...
BOOL CFilesWindow::CopyFocusedNameToClipboard(CCopyFocusedNameModeEnum mode)
{
    ...
    if (FocusedIndex == 0 && FocusedIndex < Dirs->Count &&
        ...
        return FALSE; // skip the up-directory entry
...
BOOL CFilesWindow::BuildColumnsTemplate()
{
    ...
    {
        // in brief mode the header line is not shown, but we ensure the Name column is added
        // so that tests like CFilesWindow::IsExtensionInSeparateColumn work
        CColumn column;
        column.CustomData = 0;
    ...
    for (i = 0; i < STANDARD_COLUMNS_COUNT; i++)
    {
        ...
        if (i == 0 || ViewTemplate->Flags & item->Flag)
        {
            ...
            if (i == 0) // column "Name"
            {
                ...
                else // just to be safe make the strings double-null-terminated
                {
...
void CFilesWindow::OnHeaderLineColWidthChanged()
{
    ...
    for (i = 0; i < Columns.Count; i++)
    {
        ...
        if (column->ID == COLUMN_ID_CUSTOM) // column added by a plugin
        {
