...
void CPluginsDlg::SetColumnWidths()
{
    ...
    int i;
    for (i = 0; i <= 3; i++)
        ListView_SetColumnWidth(HListView, i, i < 3 ? LVSCW_AUTOSIZE_USEHEADER : LVSCW_AUTOSIZE); // do not stretch the last column, the content is wider than the header
    ...
    //  SCROLLBARINFO si; // would need to be loaded dynamically, we don't bother
    //  si.cbSize = sizeof(si);
    ...
    case WM_COMMAND:
    {
        ...
        case IDB_PLUGINADD:
        {
            ...
            if (SafeGetOpenFileName(&ofn))
            {
                ...
                while (1)
                {
                    ...
                    // finish when two null characters are found
        ...
        case IDB_PLUGINFOCUS:
        {
            ...
            if (p != NULL && IsPluginUnsupportedOnX64(p->DLLName))
            {
                ...
                // IDS_PLUGINISX86ONLY is not an ideal text but it will do and we will not bother translators
            ...
            if (p != NULL)
            {
                ...
                char* s = p->DLLName;
                if ((*s != '\\' || *(s + 1) != '\\') && // not UNC
                    (*s == 0 || *(s + 1) != ':'))       // not "c:" -> path relative to plugins subdirectory
                {
                    GetModuleFileName(HInstance, buf, MAX_PATH);
                    s = strrchr(buf, '\\') + 1;
                    strcpy(s, "plugins\\");
    ...
    if (ti.Type == ttDataFromWindow)
    {
        ...
        for (i = 0; i < Plugin->MenuItems.Count; i++)
        {
            if (HotKeys[i] != 0)
            {
                ...
                Plugin->MenuItems[i]->HotKey = HotKeys[i]; // assign the hotkey to this plugin
            }
        }
    }
}
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_ASSIGN:
        {
            ...
            for (i = 0; i < Plugin->MenuItems.Count; i++)
                if (HOTKEY_GET(HotKeys[i]) == hotKey)
                    HotKeys[i] = 0;
            HotKeys[orgIndex] = (HotKeys[orgIndex] & ~HOTKEY_MASK) | HOTKEY_DIRTY; // prevent this change from being overridden by the plugin's Connect()
...
CCfgPageConfirmations::AddItem(HTREEITEM hParent, int iImage, int textResID, int* value)
{
    ...
    if (iImage == -1)
    {
        ...
        item.Variable = value;
        item.Checked = 0;
        List.Add(item); // put the handles into an array for easy access
    }
    return ret;
}
...
CCfgPageConfirmations::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // dialog elements should stretch with the dialog size, set splitter controls
    ...
    case WM_DESTROY:
    {
        // according to MSDN, TreeView does not destroy the image list, but the
        // W2K checked build complains during the following ImageList_Destroy
        // call, so remove the image list just to be safe
        if (HTreeView != NULL)
...
CCfgPageDrives::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        // terrible hack - need a message that arrives
...
void CCfgPageViewers::LoadControls()
{
    ...
    SendMessage(GetDlgItem(HWindow, IDE_ARGUMENTS), EM_SETSEL, 0, -1); // select all so the browse overwrites the content
    SendMessage(GetDlgItem(HWindow, IDE_INITDIR), WM_SETTEXT, 0,
                (LPARAM)(empty ? "" : item->InitDir));
    SendMessage(GetDlgItem(HWindow, IDE_INITDIR), EM_SETSEL, 0, -1); // select all so the browse overwrites the content
    DisableNotification = FALSE;
}
...
void CCfgPageMainWindow::Transfer(CTransferInfo& ti)
{
    ...
    // back up data so we can detect changes
...
BOOL CCfgPageMainWindow::InitIconCombobox()
{
    ...
    // since Vista, if font aliasing is set to Standard, the combobox had aliased font while the rest of the dialog had the classic non-aliased one; set the correct font
...
CCfgPageAppearance::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        {
            // notify about changes in the edit line
...
void CCfgPageChangeDrive::SetDrivesToListbox(int resID, DWORD drives)
{
    ...
    // focus ends at the end; move it back to the beginning
...
void CCfgPageChangeDrive::InitList(int resID)
{
    ...
    // set the height according to the control; LBS_NOINTEGRALHEIGHT is used because with some fonts the height would otherwise be zero
