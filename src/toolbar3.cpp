...
// fill the Items array with all buttons that the toolbar can contain
BOOL CTBCustomizeDialog::EnumButtons()
{
    ...
    TLBI_ITEM_INFO2 tii;
    tii.Mask = TLBI_MASK_STYLE; // insert separator as the first list item
...
void CTBCustomizeDialog::FillLists()
{
    ...
    int i;
    for (i = 0; i < AllItems.Count; i++)
        if (i == 0 || !PresentInToolBar(AllItems[i].ID)) // if not a virtual separator, check presence in the toolbar
        {
            int ret = (int)SendMessage(HAvailableLB, LB_ADDSTRING, 0, 1); // 1 is a dummy value, work around a WinXP bug
            if (ret != LB_ERR)
    ...
    for (i = 0; i < ToolBar->Items.Count; i++)
    {
        ...
        int ret = (int)SendMessage(HCurrentLB, LB_ADDSTRING, 0, 1); // 1 is a dummy value, work around a WinXP bug
        if (ret != LB_ERR)
...
void CTBCustomizeDialog::OnAdd()
{
    ...
    if (ToolBar->InsertItem2(cIndex, TRUE, &AllItems[data]))
    {
        ...
        SendMessage(HAvailableLB, WM_SETREDRAW, TRUE, 0);
        int ret = (int)SendMessage(HCurrentLB, LB_INSERTSTRING, cIndex, 1); // 1 is a dummy value, work around a WinXP bug
        if (ret != LB_ERR)
...
void CTBCustomizeDialog::OnRemove()
{
    ...
    if (ToolBar->RemoveItem(cIndex, TRUE))
    {
        ...
        if (data != 0) // separator
        {
            ...
            int ret = (int)SendMessage(HAvailableLB, LB_INSERTSTRING, index, 1); // 1 is a dummy value, work around a WinXP bug
            if (ret != LB_ERR)
...
void CTBCustomizeDialog::MoveItem(int srcIndex, int tgtIndex)
{
    ...
    int ret = (int)SendMessage(HCurrentLB, LB_INSERTSTRING, tgtIndex - delta, 1); // 1 is a dummy value, work around a WinXP bug
    if (ret != LB_ERR)
        SendMessage(HCurrentLB, LB_SETITEMDATA, ret, data);
...
void CToolBar::Customize()
{
    ...
    // notify about the start of customization
    ...
    // restore the original button settings
    Customizing = FALSE;
    InvalidateRect(HWindow, NULL, FALSE);

    // notify about the end of customization
