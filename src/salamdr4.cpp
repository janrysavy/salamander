...
BOOL CTruncatedString::TruncateText(HWND hWindow, BOOL forMessageBox)
{
    ...
    else
    {
        ...
        if (forMessageBox)
        {
            // for message boxes—limit the substring to 400 pixels so it fits even on 640x480
            int chars = SubStrLen;
            int maxWidth = 400;
            SIZE sz;
        ...
        else
        {
            ...
            if (sz.cx > maxWidth)
            {
                ...
                // copy the leading part that includes the truncated substring
...
// StrToUInt64
//
// Converts a number (it may include a leading '+') to unsigned __int64.
                       ...
                       const CQuadWord* lpParArray)
{
    ...
    if (input != NULL && *input++ == '{' && *input++ == '!' && *input++ == '}' && nOutMax > 0)
    {
        while (*input != 0)
        {
            ...
            {
                if (*input == '{') // expand the curly brace
                {
                    input++;
...
//
// Writes into lpOut a string based on the files and dirs variables:
...
BOOL CopyHTextToClipboardW(HGLOBAL hGlobalText, int textLen)
{
    ...
    if (OpenClipboard(NULL))
    {
        if (EmptyClipboard())
        {
            ...
            if (SetClipboardData(CF_UNICODETEXT, hGlobalText) == NULL) // then store the Unicode text
                err = GetLastError();
        }
        else
            err = GetLastError();
        ...
        IdleRefreshStates = TRUE;  // on the next idle force checking the status variables
...
{
    if (TransferIsDir) // handle directories differently
    {
