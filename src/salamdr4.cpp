...
// CTruncatedString
//
// See the help file for a description
//
...
BOOL CTruncatedString::Set(const char* str, const char* subStr)
{
    ...
    if (subStr != NULL)
    {
        ...
        {
            len -= 2; // subtract %s that will be omitted
...
{
    // if nothing needs truncation, exit
    if (SubStrIndex == -1)
        return TRUE;
    ...
    int fitChars;
    int alpDx[8000]; // used for calculating widths
    int textLen = (int)strlen(Text);
    char* truncated = (char*)malloc(textLen + 1 + 3); // 3: space reserved for ellipsis in extreme cases
    if (truncated == NULL)
    {
        TRACE_E(LOW_MEMORY);
        ret = FALSE;
    ...
    else
    {
        ...
        if (forMessageBox)
        {
            // for message boxes -- just ensure the substring is no wider than 400 pixels (still fits under 640x480)
            int chars = SubStrLen;
            int maxWidth = 400;
            SIZE sz;
            ...
            {
                // first part with truncated substring
                ...
                memcpy(TruncatedText, Text, textLen + 1); // just copy -- it fits
        }
        else
        {
            // single line for dialogs
            // determine the maximum width we can use
            RECT r;
            ...
            if (sz.cx > maxWidth)
            {
                ...
                // subtract from the truncatable part
...
// StrToUInt64
//
// Converts a number (it may start with '+') to unsigned __int64.
// The parameter len specifies the maximum number of processed characters.
// If 'isNum' is not NULL, TRUE is returned in it when the entire string
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// NOTE: whenever ExpandPluralString is modified, update ValidatePluralStrings
//        in the TRANSLATOR project as well
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                       ...
                       const CQuadWord* lpParArray)
{
    ...
    if (input != NULL && *input++ == '{' && *input++ == '!' && *input++ == '}' && nOutMax > 0)
    {
        while (*input != 0)
        {
            ...
            {
                input++;
                if (output >= outputNullTerm) // output buffer must also hold the terminating null
                {
                    lpOut[nOutMax - 1] = 0;
            ...
            {
                if (*input == '{') // perform expansion of the curly braces
                {
                    input++;

                    // extract the appropriate parameter value from the array
                    unsigned __int64 arg;
                    const char* parInd = input;
                    int parIndVal = 0;
                    while (*parInd >= '0' && *parInd <= '9')
                        parIndVal = 10 * parIndVal + *parInd++ - '0';
                    if (*parInd == ':' && parInd > input) // index specified, use it
                    {
                    ...
                    while (*input != '}' && *input != 0)
                    {
                        ...
                        // if this is the last string without range limit,
                        // or the value of arg is less than or equal to the boundary
                        if (numStrLen == 0 || arg <= num)
                        {
                            // insert the appropriate substring into the output string
                            int i;
...
//
// Writes to lpOut a string depending on variables files and dirs:
...
// The selectedForm variable controls insertion of the word "selected".
//
// forDlgCaption is TRUE/FALSE when the text is/isn't intended for a dialog caption
// (capital letters are required in English).
//
...
int ExpandPluralFilesDirs(char* lpOut, int nOutMax, int files, int dirs, int mode, BOOL forDlgCaption)
{
    ...
    if (nOutMax > 200)
        nOutMax = 200;
    nOutMax -= 20; // make room for numbers of files and dirs
...
int ExpandPluralBytesFilesDirs(char* lpOut, int nOutMax, const CQuadWord& selectedBytes, int files, int dirs, BOOL useSubTexts)
{
    ...
    if (nOutMax > 200)
        nOutMax = 200;
    nOutMax -= 30; // make room for numbers of files and dirs
...
BOOL LookForSubTexts(char* text, DWORD* varPlacements, int* varPlacementsCount)
{
    ...
    *varPlacementsCount = 0;
    ...
    const char* src = text; // read characters from this pointer
    char* dst = text;       // write the result to this pointer
    char* var = NULL;       // pointer to the first character of variable
...
BOOL CViewTemplates::Load(HKEY hKey)
{
    ...
    for (i = 0; i < VIEW_TEMPLATES_COUNT; i++)
    {
        ...
        if (i == 6 && Configuration.ConfigVersion < 23)
            continue; // for IDS_TYPES_VIEW we want the default columns
        HKEY actKey;
        ...
        if (multibyte != NULL)
        {
            ...
            if (multibyteStr != NULL)
            {
                if (textLen > 0 && WideCharToMultiByte(CP_ACP, 0, str, textLen, multibyteStr, mbLen + 1, NULL, NULL) == 0)
                    err = GetLastError();
                multibyteStr[mbLen] = 0; // terminating null
    ...
    if (OpenClipboard(NULL))
    {
        if (EmptyClipboard())
        {
            ...
            if (text != NULL)
            {
                ...
                err = AddMultibyteToClipboard(text, textLen); // store text first in multibyte form
                HANDLES(GlobalUnlock(hGlobalText));
            }
            else
                err = GetLastError();

            if (SetClipboardData(CF_UNICODETEXT, hGlobalText) == NULL) // then store text in multibyte
                err = GetLastError();
        }
        else
            err = GetLastError();
        ...
        IdleRefreshStates = TRUE;  // on next Idle we force checking status variables
        IdleCheckClipboard = TRUE; // also let clipboard be checked
    }
    else
    {
        err = GetLastError();
    ...
    if (OpenClipboard(NULL))
    {
        if (EmptyClipboard())
        {
            ...
            if (text != NULL)
            {
                if (textLen == -1)
                    textLen = lstrlen(text);
                err = AddUnicodeToClipboard(text, textLen); // store text first in Unicode
                HANDLES(GlobalUnlock(hGlobalText));
            }
            else
                err = GetLastError();

            if (SetClipboardData(CF_TEXT, hGlobalText) == NULL) // then store text in multibyte
                err = GetLastError();
        }
        else
            err = GetLastError();
...
//****************************************************************************
//
// Internal function to obtain column content
//
...
{
    if (TransferIsDir && !TransferFileData->SizeValid) // directories with unknown size only
    {
    ...
    else
    {
        ...
        case SIZE_FORMAT_KB: // NOTE: the same code appears later, search for this constant
        {
...
char InternalGetTypeAux3[MAX_PATH + 4]; // extension in lowercase, DWORD-aligned

void WINAPI InternalGetType()
{
    if (TransferIsDir) // directories must be handled differently
    {
    ...
    {
        if (TransferAssocIndex == -2) // we haven't searched for the extension yet
        {
            if (TransferFileData->Ext[0] != 0)
            {
                ...
                *((DWORD*)InternalGetTypeAux1) = 0;
                ...
                TransferAssocIndex = -1; // without extension -> cannot be in Associations
        }
...
{
    TransferLen = 0;
    // NOTE: to display more attributes, GetAttrsString() and DISPLAYED_ATTRIBUTES must be updated
...
//****************************************************************************
//
// Internal function to get index of simple icons for FS with their own icons (pitFromPlugin)
//
...
BOOL CSalamanderView::InsertStandardColumn(int index, DWORD id)
{
    ...
    for (i = 0; i < STANDARD_COLUMNS_COUNT; i++)
    {
        if (GetStdColumn(i, FALSE)->ID == id) // desired standard column found
        {
...
BOOL CSalamanderView::SetColumnName(int index, const char* name, const char* description)
{
    ...
    { // check of double (twice null-terminated) strings + non-empty second one + their copy
...
BOOL CSalamanderView::DeleteColumn(int index)
{
    ...
    if (!Panel->Columns.IsGood())
        Panel->Columns.ResetState(); // cannot fail, the array just wasn't shrunk
    return TRUE;
}
...
//
// Holds a list of files the user opened with View or Edit.
//
...
CFileHistoryItem::CFileHistoryItem(CFileHistoryItemTypeEnum type, DWORD handlerID, const char* fileName)
{
    ...
    // try to load the icon from the system
...
BOOL CFileHistoryItem::Execute()
{
    ...
    case fhitOpen:
    {
        ...
        if (ptr != NULL)
        {
            *ptr = 0; // split the path into directory and file name
            HCURSOR hOldCur = SetCursor(LoadCursor(NULL, IDC_WAIT));
            MainWindow->SetDefaultDirectories(); // so the launched process inherits the correct current directories
...
BOOL SalOpenExecute(HWND hWindow, const char* fileName)
{
    ...
    // initialization required to run SalOpen
    ...
    if (SalOpenSharedMem != NULL)
    {
        ...
        *ptr = 0;
        ...
        // launch the salopen.exe process
        STARTUPINFO si;
        ...
        PROCESS_INFORMATION pi;
        {
            ...
            {
                { // when waiting, DDE associations don't work (.html, .h, .cpp, etc.)
...
BOOL IsPluginFSPath(const char* path, char* fsName, const char** userPart)
{
    ...
    while (LowerCase[*name] >= 'a' && LowerCase[*name] <= 'z' ||
        ...
        name++;
    // test if the FS name meets all conditions (':' follows and length >= 2)
    if (*name == ':' && name - start >= 2 && name - start < MAX_PATH)
    {
        ...
        // pointer in 'path' to the first character of plugin-defined part (after the first ':')
        if (userPart != NULL)
            *userPart = name + 1;
        return TRUE;
    }
