...
BOOL CStatusWindow::SetSubTexts(DWORD* subTexts, DWORD subTextsCount)
{
    ...
    // build the cursor tracking array
    BuildHotTrackItems();
...
void CStatusWindow::BuildHotTrackItems()
{
    ...
    if (Border == blTop)
    {
        ...
        if (Text != NULL)
        {
            ...
            // get the positions of all characters
            SIZE s;
            ...
            if (FilesWindow->Is(ptDisk) || FilesWindow->Is(ptZIPArchive))
            {
                ...
                else
                {
                    ...
                    // for UNC paths strip the final backslash
            ...
            else
            {
                if (FilesWindow->Is(ptPluginFS))
                {
                    ...
                    while (1)
                    {
                        ...
                        if (chars == lastChars)
                            chars++; // that would cause an infinite loop, so we handle it
                        if (chars > pathLen)
                            chars = pathLen;
...
void CStatusWindow::DestroyWindow()
{
    ...
    if (Throbber || DelayedThrobber)
        SetThrobber(FALSE, 0, TRUE); // need to kill the timer
...
void CStatusWindow::SetThrobber(BOOL show, int delay, BOOL calledFromDestroyWindow)
{
    ...
    if (show)
    {
        if (DelayedThrobber) // waiting to be shown
        {
            ...
            if (Throbber /* just fixing an inconsistent state */ ||
                delay <= 0 || !SetTimer(HWindow, IDT_DELAYEDTHROBBER, delay, NULL))
            {
                DelayedThrobber = FALSE; // should appear immediately or timer setup failed, so show it now
                DelayedThrobberShowTime = 0;
            }
            else
            {
        ...
        else
        {
            if (!Throbber && delay > 0)
            {
                if (HWindow != NULL && SetTimer(HWindow, IDT_DELAYEDTHROBBER, delay, NULL))
                    DelayedThrobber = TRUE; // not shown yet + should appear with a delay + window visible (if not, only DelayedThrobberShowTime is set)
                DelayedThrobberShowTime = GetTickCount() + delay;
                if (DelayedThrobberShowTime == 0)
                    DelayedThrobberShowTime++; // 0 is an invalid value
            }
    ...
    {
        if (DelayedThrobber) // waiting to be shown but the throbber should hide, stop waiting
        {
            if (HWindow == NULL)
    ...
    if (HWindow != NULL && !DelayedThrobber && Throbber != show)
    {
        ...
        if (Throbber)
        {
            ThrobberFrame = 0; // starting frame -> animate from the first slot
...
{
    static int NewID = 0; // throbber ID must be unique (single counter for both panels)
    ThrobberID = NewID++;
    if (ThrobberID == -1)
        ThrobberID = NewID++;
    return ThrobberID;
...
void CStatusWindow::GetHotText(char* buffer, int bufSize)
{
    ...
    if (HotItem != NULL && Text != NULL)
    {
        ...
        // for a Directory Line with a plugin FS we must allow the plugin a final path tweak (adding ']' for VMS paths under FTP)
...
BOOL CStatusWindow::FindHotTrackItem(int xPos, int& index)
{
    ...
    for (i = 0; i < HotTrackItems.Count; i++)
    {
        ...
        // if there is an ellipsis after the root folder, shift xPos by its width
...
void CStatusWindow::Paint(HDC hdc, BOOL highlightText, BOOL highlightHotTrackOnly)
{
    ...
    // space reserved for the toolbar
    if (isDirectoryLine)
    ...
    if (Text != NULL)
    {
        BOOL truncateEnd = TRUE; // shorten the end (TRUE) or after the root folder (FALSE)
        int visibleChars = 0;
        ...
        // initialize all areas with zero values
        SecurityRect = tmpR;
        ...
        if (isDirectoryLine)
        {
            ...
            if (Size != NULL)
            {
                ...
                if (tmpR.right - tmpR.left < s.cx)
                    goto SKIP_MEASURING; // size field does not fit - abort measuring
        ...
        if (tmpR.right > tmpR.left + TextEllipsisWidthEnv)
        {
            ...
            {
                // the text does not fit within the desired width -> need to shorten it
                if (isDirectoryLine && HotTrackItems.Count > 1 &&
                    HotTrackItems[0].Pixels + TextEllipsisWidthEnv <= textWidth)
                {
                    // for the top directory line we shorten after the root folder of the path
                    EllipsedChars = 0;
                    EllipsedWidth = 0;
                    ...
                    truncateEnd = FALSE; // shorten from the inside
                }
                else
                {
                    // for the bottom info line search from the end for a character
    ...
    SKIP_MEASURING:
        ...
        // display the main text if we have room for it
        if (TextRect.right > TextRect.left)
        {
            // determine in advance which part of the text should be rendered prominently for ClearType
            // we must exclude it from drawing as normal text
            ...
            // draw the first part of the text (up to hotItem; if the text starts with hotItem we draw nothing)
            if (firstClipChar != 0)
            {
                if (truncateEnd)
                { // no truncation or the end was trimmed
                ...
                { // trimmed portion after the root folder
                    // root cast
            ...
            // draw the second part of the text (beyond hotItem) -- truncation at the end
            ...
            // draw the second part of the text (beyond hotItem) -- shortening in the middle
            // this shortening is used only for paths in the directory line (!truncateEnd)
            ...
            { // trimmed portion after the root folder
                ...
                {
                    ExtTextOut(dc, TextRect.left + AlpDX[rootChars - 1], textY, 0, NULL, "...", 3, NULL); // "..."
                    firstChar += EllipsedChars;                                                           // skip the omitted characters
                }
                else
                {
                    if (firstChar < rootChars + EllipsedChars) // need to skip a backslash that would fall into the ellipsis
        ...
        {
            // JRYFIXME: proper support for the HOT color needed, see PaintSymbol; for now a hack via SVGSTATE_DISABLED
...
private:
    ...
    BOOL UseUnicode;                  // is Unicode text in DataObject? otherwise we try ANSI
...
public:
    ...
    // returns a directory (there must be exactly one)
            ...
            if (path != NULL)
            {
                ...
                if (path != NULL)
                {
                    ...
                    if (!IsPluginFSPath(Buffer))
                    {
                        ...
                        if ((l != 2 || Buffer[0] != '\\' || Buffer[1] != '\\') && // not the "\\\\" Nethood root path
                            l > 0 && Buffer[l - 1] == '\\')
                            Buffer[--l] = 0;             // trailing '\\' not welcome
...
CStatusWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        if (ShowThrobber)
        {
            ...
            if (ti < 0)
                ti = 0;
            DelayedThrobberShowTime = 0; // we no longer need this value; SetThrobber will set it again, so assign an invalid one
    ...
    case WM_SIZE:
    {
        ...
        if (Width != r.right || Height != r.bottom)
        {
            Width = r.right;
            Height = r.bottom;
            ItemBitmap.Enlarge(Width, Height); // allocate bitmap in ItemBitmap.HMemDC
        }
    ...
    {
        DWORD id = (DWORD)wParam; // FIXME_X64 - verify cast to (DWORD)
    ...
    case WM_MOUSEMOVE:
    {
        ...
        // handle text tearing and start drag&drop
        ...
        if (MouseCaptured)
        {
            ...
            if (isInSizeRect)
            {
                // drive-info works only if the FS supports it
                if (FilesWindow->Is(ptDisk) || FilesWindow->Is(ptZIPArchive) ||
        ...
        if (MouseCaptured && uMsg == WM_LBUTTONUP && (HotItem != NULL || HotSize || HotZoom || HotSecurity))
        {
            ...
            if (x <= GetSystemMetrics(SM_CXDRAG) && y <= GetSystemMetrics(SM_CYDRAG))
            {
                if (HotItem != NULL)
                {
                    if (Border & blTop)
                    {
                        ...
                        if (HotItem != lastItem)
                        {
                            ...
                            FilesWindow->ChangeDir(path, -1, NULL, 2 /* as if using back/forward in history */, NULL, FALSE);
                        }
                        else
                        {
                            // click on the last component -- change dir
                            SendMessage(MainWindow->HWindow, WM_COMMAND, MAKEWPARAM(CM_ACTIVE_CHANGEDIR, 0), 0);
                        }
                    }
                    if (Border & blBottom)
                ...
                if (HotZoom)
                {
                    ...
                    UpdateWindow(MainWindow->HWindow); // so that the next WM_MOUSEMOVE arrives after the window is repainted
                }
    ...
    case WM_ERASEBKGND:
    {
        ...
        // j.r.: all status bars should use a single CBitmap cache
    ...
    case WM_TIMER:
    {
        if (wParam == IDT_THROBBER)
        {
            if (StopStatusbarRepaint == 0)
            {
                ...
                NeedToInvalidate = TRUE;
                InvalidateIfNeeded();
                //        RepaintThrobber();  // j.r. FIXME RepaintThrobber() would be better, but it misbehaves during window resize
            }
            else
                PostStatusbarRepaint = TRUE;
        }
...
CStatusWindow::CreateDragImage(const char* text, int& dxHotspot, int& dyHotspot, int& imgWidth, int& imgHeight)
{
    ...
    ItemBitmap.Enlarge(sz.cx, sz.cy); // allocate bitmap in ItemBitmap.HMemDC
    ...
    SelectObject(ItemBitmap.HMemDC, ItemBitmap.HOldBmp); // release the bitmap from HMemDC for a moment
...
{
    // the font size might have changed
    InvalidateRect(HWindow, NULL, TRUE);
    BuildHotTrackItems();
}
