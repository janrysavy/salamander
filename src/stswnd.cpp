...
void CStatusWindow::GetHotText(char* buffer, int bufSize)
{
    ...
    if (HotItem != NULL && Text != NULL)
    {
        ...
        // For a Directory Line with a plugin FS allow the plugin to finalize the path (adding ']' for VMS paths over FTP)
...
void CStatusWindow::Paint(HDC hdc, BOOL highlightText, BOOL highlightHotTrackOnly)
{
    ...
    if (Text != NULL)
    {
        ...
        if (tmpR.right > tmpR.left + TextEllipsisWidthEnv)
        {
            ...
            if (textWidth < AlpDX[TextLen - 1])
            {
                ...
                {
                    // For the bottom info line search backward for a character
    ...
    SKIP_MEASURING:
        ...
        if (TextRect.right > TextRect.left)
        {
            ...
            // Draw the first part of the text (up to hotItem; if the text starts at hotItem, draw nothing)
            if (firstClipChar != 0)
            {
                if (truncateEnd)
...
CStatusWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (MouseCaptured)
        {
            ...
            if (isInSizeRect)
            {
                // Drive info works only when the FS supports drive info
                if (FilesWindow->Is(ptDisk) || FilesWindow->Is(ptZIPArchive) ||
        ...
        if (MouseCaptured && uMsg == WM_LBUTTONUP && (HotItem != NULL || HotSize || HotZoom || HotSecurity))
        {
            ...
            if (x <= GetSystemMetrics(SM_CXDRAG) && y <= GetSystemMetrics(SM_CYDRAG))
            {
                ...
                if (HotZoom)
                {
                    ...
                    UpdateWindow(MainWindow->HWindow); // So the next WM_MOUSEMOVE arrives at the already redrawn window
                }
