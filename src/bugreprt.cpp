...
BOOL GetProcessorSpeed(DWORD* mhz)
{
    __try
    {
        ...
        if (f.QuadPart == 1000) // GetTickCount is used, measure a longer sample
            divider = 2;        // 0.5 s
...
// generated while enumerating modules
struct CModuleInfo
{
                                ...
                                FPrintLine PrintLine, void* param)
{
    ...
    __try
    {
        ...
        if (Exception != NULL)
        {
            ...
            if (IsDebuggerPresent())
            {
                ...
                // outside MSVC everything worked correctly, so note that we are running from the debugger
    ...
    __try
    {
        ...
        if (Exception != NULL && Exception->ContextRecord != NULL)
        {
            ...
            PrintLine(param, "", FALSE);
#ifndef _DEBUG // avoid breaking into the debugger in debug builds
            if (ctxtRec->ContextFlags & CONTEXT_CONTROL)
            {
                ...
                while (i < 32 + 64)
                {
                    // esp may point somewhere invalid - guard against it
                    __try
                    {
...
#endif //_DEBUG  // prevent breaking into the debugger in debug builds
        }
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
...
#define LS_BUFSIZE 2048
    ...
    __try
    {
        if (Exception != NULL)
        {
            ...
            // and display EIP plus the call stack
            PrintLine(param, "Stack Back Trace:", FALSE);
            BOOL firstTime = TRUE;
            int i;
            for (i = 0; i < CCallStack::CallStacks.Count; i++)
            {
                // !!! access to CCallStack::CallStacks should be protected by a critical section
                // otherwise we risk touching invalid memory
                CCallStack* stack;
                ...
                if (stack != NULL)
                {
                    __try // a rather dumb safeguard against modifications to CCallStack::CallStacks; also guard against GetThreadHandleFromID
                    {
                ...
                if (stack != NULL && hThread != NULL)
                {
                    ...
                    {
                        // if this isn't our thread, suspend it so we can retrieve its context
              ...
              //     ebp-0x8   ebp-0x4   ebp+0x0   ebp+0x4   ebp+0x8   ebp+0xC
                            ...
                            if (threadWithException || GetThreadContext(hThread, &ctx))
                            {
                                ...
                                PrintLine(param, buf, TRUE);
                                // esp may point somewhere invalid - be safe
                                __try
                                {
                                    // *(ebp) is the frame pointer of the calling function
                                    // *(ebp + 4) is the return address (where execution of the caller continues after this function returns)
  ...
  */
    ...
    __try
    {
        ...
        if (TRUE)
        {
            ...
            DWORD netDrives; // bit mask of network drives
                ...
                if ((mask & i) || (netDrives & i)) // drive is accessible
                {
                    ...
                    if (driveType == 2)
                    {
                        ...
                        if (medium == 350 || medium == 525 || medium == 800 || medium == 1)
                            getMoreInfo = FALSE; // avoid spinning the floppy during MyGetVolumeInformation()
                    }
                    ...
                    if (getMoreInfo)
                    {
                        ...
                        //---  GetDiskFreeSpace - display information
                        if (!err)
                        {
