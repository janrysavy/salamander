    ...
    : CWindow(ooStatic)
{
    ...
    HotExt = FALSE;
    DownIndex = -1; // no column button is pressed
    DownVisible = FALSE;
    DragIndex = -1; // no column is being resized
...
void CHeaderLine::PaintAllItems(HDC hDC, HRGN hUpdateRgn)
{
    ...
    // This optimization caused problems during an FTP client connection:
  ...
  */
    ...
    for (i = 0; i <= columnsCount; i++)
    {
        if (i < columnsCount)
            ...
            width = 60000; // the right side is definitely off-screen, so painting will still occur
...
#define SORT_BITMAP_W 8 // width of the bitmap used for sorting
#define SORT_BITMAP_H 8 // height of the bitmap used for sorting
...
void CHeaderLine::SetMinWidths()
{
    ...
    for (i = 0; i < columnsCount; i++)
    {
        ...
        // if the column is sortable, add bitmap width and spacing
...
CHeaderLine::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY: // if the cursor is over the header line with a highlighted item, clear the highlight before destruction
    case WM_CANCELMODE:
    {
