...
struct SPackExpData
{
    ...
    // these variables exist because we cannot obtain the DOS name of a non-existent file
    // we therefore return a substitute DOS name which is renamed to the desired long name
    ...
    BOOL DOSTmpFilePossible;  // TRUE while ArcName can be replaced with the DOS fallback name
    char* DOSTmpFile;         // fallback name for ArcName (non-NULL only if DOSTmpFilePossible is TRUE)
};
...
// Variables expanded in the command line and the current directory when
...
// Function to initialize the spawn executable name with full path
...
void CPackerFormatConfig::AddDefault(int SalamVersion)
{
    ...
    case 3: // new in beta2
    case 4: // beta3 with old configuration (contains $(SpawnName))
...
{
    // j.r. macro disabled because PackIsArchive is heavily called from CFilesWindow::CommonRefresh()
...
// initializes the configuration from another configuration
BOOL CArchiverConfig::Load(CArchiverConfig& src)
{
    // clear what we have (if any)
    DeleteAllArchivers();
...
// sets the path to the packer executable at the given index
...
// sets the path to the unpacker executable at the given index
// (called from the transfer when closing the auto-configuration dialog)
...
BOOL CArchiverConfig::Save(int index, HKEY hKey)
{
    ...
    //   of the archiver (UID is used now), so storing it has no meaning
...
// j.r. reworked configuration loading -- it searches the default list and if a
//      registry item matches, its values are used, otherwise it is ignored.
//      The old method caused problems because users manually deleted the key
//      contents and there was no dialog path to restore the original list.
...
BOOL CArchiverConfig::Load(HKEY hKey)
{
    ...
    // UID of the archiver (Title was previously used, but it is translated now and unusable)
    if (ret && Configuration.ConfigVersion > 64)
        ret &= GetValue(hKey, SALAMANDER_PPC_UID, REG_DWORD, &uid, sizeof(DWORD));
    // load the unpacker executable if different from the packer
    if (!exesAreSame)
        if (ret)
    ...
    if (ret)
    {
        ...
        for (i = 0; i < Archivers.Count; i++)
        {
            ...
            // use paths from entries that are complete and whose title matches the default value
            if (Configuration.ConfigVersion <= 64 && stricmp(title, arch->Title) == 0 || // Title is now translated and cannot be used
            ...
            if (CutDirectory(path))
            {
                ...
                while (1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            int count = 4; // copy '.' plus at most 3 allowed characters of the 8.3 extension
                ...
                if (h != INVALID_HANDLE_VALUE)
                {
                    ...
                    strcpy(data->DOSTmpFile, buff2); // obtained a substitute name
                }
                else
                {
...
PackExpExeName(unsigned int index, BOOL unpacker = FALSE)
{
    ...
    else
    {
        ...
        // with a long name; I no longer feel like patching this and risking that it won't work
        buff[0] = '\0';
...
// ****************************************************************************
// tables assigning evaluation functions to individual variables
//
...
//         DOSTmpName is NULL if a long name cannot be replaced by a DOS fallback name,
//           otherwise it points to a buffer at least MAX_PATH long
//   OUT:  buffer is the command line with variables expanded
//         DOSTmpName is the name of the temporary file (or empty if not replaced)
...
//   in PackErrorHandlerPtr with your own handler that processes the error as needed.
...
//   IN:   parent is the parent of message boxes
...
//
//   Runs the external program given (with parameters) in cmdLine
//
//   RET: returns TRUE on success, FALSE on error
//        on error it calls the callback *PackErrorHandlerPtr
//   IN:  parent is the parent window for message boxes
//        cmdLine is the command line to execute
//        currentDir is the full current directory for the program or NULL if it doesn't matter
...
BOOL PackExecute(HWND parent, char* cmdLine, const char* currentDir, TPackErrorTable* const errorTable)
{
    ...
    // if we haven't determined the spawn path yet, do it now
    ...
    //       (even newly started ones) and other software (at least during Paste) if we
