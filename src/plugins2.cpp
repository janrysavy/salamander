...
// and converts it to grayscale (leaving the transparent color intact).
//
...
LoadDIB(HKEY hKey, const char *valueName, HBITMAP *hBitmap)
{
  ...
  BYTE *buff = (BYTE*)malloc(bufferSize + 4);  // +4 because CStaticHuffmanCodec::DecodeBuffer() reads four bytes past the buffer (it doesn't use the values but reading past the allocation sometimes caused an exception)
  if (buff == NULL)
  {
    TRACE_E(LOW_MEMORY);
    return FALSE;
  }
  *(DWORD *)(buff + bufferSize) = 0;  // clear the overflow so it does not contain random values
  DWORD bufferSize2 = bufferSize;
  res = SalRegQueryValueEx(hKey, valueName, 0, &gettedType, buff, &bufferSize2);
  if (res != ERROR_SUCCESS || bufferSize2 != bufferSize)
  {
...
BOOL CPlugins::FindLastCommand(int* pluginIndex, int* menuItemIndex, BOOL rebuildDynMenu, HWND parent)
{
    ...
    if (LastPlgCmdPlugin != NULL)
    {
        ...
        for (i = 0; i < Data.Count; i++)
        {
            ...
            if (stricmp(p->DLLName, LastPlgCmdPlugin) == 0)
            {
                if (p->GetLoaded()) // if the plugin isn't loaded we pretend we didn't find anything
                {
                    ...
                    // updated menu version (otherwise the last-cmd item could disappear after opening the submenu)
        ...
        // command not found, clear Last Command to avoid "reviving" old commands after reloading a plugin
        if (LastPlgCmdPlugin != NULL)
            free(LastPlgCmdPlugin);
        LastPlgCmdPlugin = NULL;
    }
...
BOOL CPlugins::ExecuteCommand(int pluginIndex, int menuItemIndex, CFilesWindow* panel, HWND parent)
{
    ...
    if (pluginData->InitDLL(parent)) // PluginIfaceForMenuExt must be initialized for GetMenuItemStateType()
    {
        MENU_ITEM_INFO dummy;
        if (pluginData->GetMenuItemStateType(pluginIndex, menuItemIndex, &dummy))
        {
...
void CPlugins::InitMenuItems(HWND parent, CMenuPopup* root)
{
    ...
    // add submenus for plugins that have items and clear their SUIDs
    ...
    for (i = 0; i < Order.Count; i++)
    {
        ...
        if (p->SupportDynMenuExt || p->MenuItems.Count > 0) // dynamic menu or it has some items
        {
            ...
            if (!containVisibleItem)
            {
                ...
                for (j = 0; j < p->MenuItems.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        { // if this is a submenu we must skip all its items and nested submenus
                            int level = 1;
            ...
            if (containVisibleItem)
            {
                ...
                DuplicateAmpersands(pluginName, 299); // plugin name may contain '&'

                mi.String = pluginName;
                mi.ImageIndex = (p->PluginSubmenuIconIndex != -1) ? orderIndex : -1;
                mi.SubMenu = new CMenuPopup();
                p->SubMenu = (CMenuPopup*)mi.SubMenu; // assign this submenu to the plugin
    ...
    // compute a new CPluginsStateCache::ActualStateMask and other variables when appropriate
    ...
    if (FindLastCommand(&pluginIndex, &menuItemIndex, TRUE, parent))
    {
        ...
        if (menuItem->Name != NULL) // display the item even if it doesn't match the current skill level
        {
            ...
            DuplicateAmpersands(lastCmdStr, 299); // plugin name may contain '&'
            strcat(lastCmdStr, ": ");
            ...
            DuplicateAmpersands(lastCmdStr + cmdNameOffset, 500); // restore && if it was present in the command
            setToDefaultItem = FALSE;
        }
    }
    if (setToDefaultItem)
    {
        // the old command isn't available, insert a disabled default item
    ...
    else
    {
        ...
        else
        {
            if (MainWindow->GetActivePanel()->Is(ptPluginFS))
            {
                ...
                {                   // using the interface we find the plugin index in the active panel
    ...
    else
    {
        ...
        else
        {
            if (MainWindow->GetNonActivePanel()->Is(ptPluginFS))
            {
                ...
                if (fs->NotEmpty()) // should be "always true"
                {                   // using the interface we find the plugin index in the inactive panel
...
{
    // check whether it's one of the plugin submenus
    int i;
    for (i = 0; i < Data.Count; i++)
    {
        CPluginData* p = Data[i];
...
void SaveFSNames(HKEY itemKey, TIndirectArray<char>* fsNames)
{
    char buf[1000];
    buf[0] = 0;
    int remainingSize = sizeof(buf); // we store the fs-names into 'buf', separated by ':'
    int i;
...
void CPlugins::Load(HWND parent, HKEY regKey)
{
    ...
    if (regKey != NULL)
    {
        ...
        while (OpenKey(regKey, buf, itemKey))
        {
            ...
            else // new version (functions stored in a DWORD using bits)
            {
                DWORD functions = 0;
                ok = GetValue(itemKey, SALAMANDER_PLUGINS_NAME, REG_SZ, name, MAX_PATH) &&
                     GetValue(itemKey, SALAMANDER_PLUGINS_DLLNAME, REG_SZ, dllName, MAX_PATH) &&
                ...
                // these values may be missing in the configuration
            ...
            if (ok)
            {
                ...
                else
                {
                                  ...
                                  loadOnStart, lastSLGName, pluginHomePageURL[0] != 0 ? pluginHomePageURL : NULL))
                    {
                        ...
                        // assign icons here because AddPlugin() would be useless (values change
                        // after the first plugin load and until then we don't have an icon bitmap)
                        ...
                        if (p != NULL && OpenKey(itemKey, SALAMANDER_PLUGINS_MENU, menuKey))
                        {
                            ...
                            while (OpenKey(menuKey, buf2, menuItemKey))
                            {
                                ...
                                // saving registry space and allowing conversion of old configurations
                                ...
                                // IconIndex is stored only when different from -1 (no icon)
                                // saving registry space and allowing conversion of old configurations;
                                ...
                                // Type is stored only when different from pmitItemOrSeparator
                                // saving registry space and allowing conversion of old configurations
                                if (!GetValue(menuItemKey, SALAMANDER_PLUGINS_MENUITEMTYPE, REG_DWORD, &type, sizeof(DWORD)))
                                    type = pmitItemOrSeparator;

                                // HotKey is stored only when different from 0
                                // saving registry space and ensuring backward conversion
                                ...
                                { // regular item or start-submenu menu entry
    ...
    if (regKey != NULL)
    {
        ...
        for (i = 0; i < Data.Count; i++)
        {
            ...
            if (CreateKey(regKey, buf, itemKey))
            {
                ...
                if (p->LastSLGName != NULL && p->LastSLGName[0] != 0) // store it if not an empty string
                {
                    SetValue(itemKey, SALAMANDER_PLUGINS_LASTSLGNAME, REG_SZ, p->LastSLGName, -1);
                }
                if (p->PluginHomePageURL != NULL && p->PluginHomePageURL[0] != 0) // store it if not an empty string
                {
                    SetValue(itemKey, SALAMANDER_PLUGINS_HOMEPAGE, REG_SZ, p->PluginHomePageURL, -1);
                }
                if (p->ThumbnailMasks.GetMasksString()[0] != 0) // store it if not an empty string
                {
                ...
                { // store new values
                    ...
                    for (i2 = 0; i2 < p->MenuItems.Count; i2++)
                    {
                        ...
                        if (CreateKey(menuKey, buf2, menuItemKey))
                        {
                            ...
                            {                                                              // store "state" only for an item or a separator with "call-get-state"
                                DWORD state = p->SupportDynMenuExt ? -1 : item->StateMask; // dynamic menu: this hack handles the situation when a plugin with a dynamic menu switches to a static one after loading and fails in the entry point (the dynamic menu contents remain and if it lacks call-get-state items, the menu might appear even without loading the plugin)
                            ...
                            // saving registry space and ensuring backward conversion;
                            // for dynamic menus the icon is not stored, therefore its index is also omitted
                            ...
                            // HotKey is stored only when it is not zero
                            // saving registry space and ensuring backward conversion
                            if (item->HotKey != 0)
                            {
...
CPlugins::GetPluginDataFromSuffix(const char* dllSuffix)
{
    ...
    if (dllSuffix != NULL)
    {
        ...
        for (i = 0; i < Data.Count; i++)
        {
            ...
            char* s = data->DLLName;
            if ((*s != '\\' || *(s + 1) != '\\') && // not UNC
                (*s == 0 || *(s + 1) != ':'))       // not "c:" -> path relative to plugins
            {
...
void CPlugins::CheckData()
{
    ...
    if (!DefaultConfiguration) // not the default plugin configuration -> remove all old ZIP+TAR+PAK
    ...
    for (i = 0; i < PackerFormatConfig.GetFormatsCount(); i++)
    {
        if (PackerFormatConfig.GetUsePacker(i))
        {
            ...
            {
                TRACE_E("Invalid packer index in PackerFormatConfig, ext = " << PackerFormatConfig.GetExt(i)); // this error is reported when importing configuration from version 2.0 because UnCAB shipped with 2.0 incorrectly claimed it could pack archives; the issue is corrected here -- no further handling needed
                               ...
                               TIndirectArray<char>* oldFSNames)
{
    ...
    int number = 2;
    lstrcpyn(uniqueFSName, fsName, MAX_PATH - 9); // leave 9 characters at the end for numbers when searching for a unique name
    ...
    if (oldFSNames != NULL)
    {
        for (i = 0; i < oldFSNames->Count; i++)
        {
            ...
            { // match except for suffix; check if suffix is numeric (numbers are added when searching for a unique name)
                char* num = s + offset;
    ...
    for (i = 0; i < Data.Count; i++)
    {
        if (i == 0 && uniqueFSNames != NULL)
        {
            ...
            for (j = 0; j < uniqueFSNames->Count; j++)
            {
                ...
                {
                    if (!oldFSNameUsed)
                        sprintf(uniqueFSName + offset, "%d", number++); // change key name
                            ...
                            BOOL& editFound, int& edit)
{
    ...
    // obtain an array of extensions from the extension string
    char ext[300];
    ...
    while (s > ext)
    {
        ...
        if (s >= ext)
            *s = 0;
        extArray.Add(s + 1); // either the first extension or one of the list
    }
    ...
    for (i = 0; i < Data.Count; i++)
    {
        ...
        while (s > ext2)
        {
            ...
            {
                if (StrICmp(s + 1, extArray[j]) == 0) // the sets of extensions intersect
                {
    ...
    if (!viewFound || !editFound)
    {
        ...
        // of the externalArchivers array in CPlugins::FindViewEdit
        struct
        {
            const char* ext;
            int index;
...
{ // returns TRUE if plugins from 'foundFiles' should be installed and all plugins loaded
    ...
    if (file != INVALID_HANDLE_VALUE)
    {
        ...
        {
            if (read == 0)
                isEOF = TRUE;              // EOF, no sense reading further, just process the remaining buffer
            ...
            if (start < eol) // not an empty line
            {
                ...
                if (firstRow) // first row of the file
                {
                    firstRow = FALSE;
                    ...
                    else       // newer version, we must process the whole file
                    {
                        ...
                        Configuration.LastPluginVer = ver; // remember the number of this plugins.ver
...
BOOL CPlugins::ReadPluginsVer(HWND parent, BOOL importFromOldConfig)
{
    ...
    // we will look for plugins added in the plugins.ver file
    ...
    if (SearchForAddedSPLs(buf, s, foundFiles))
    {
        ...
        for (int i = 0; i < Data.Count; i++)
        {
            ...
            )
            {
                ...
                Data[i]->InitDLL(parent, TRUE, FALSE); // suppress excessive cursor blinking
...
void CPlugins::GetCmdAndUnloadMarkedPlugins(HWND parent, int* cmd, CPluginData** data)
{
    ...
    for (i = 0; i < Data.Count; i++)
    {
        ...
        if (p->ShouldRebuildMenu)
        {
            ...
            if (p->GetLoaded()) // if the user manually unloaded the plugin meanwhile, discard the rebuild request
            {
        ...
        if (p->Commands.Count > 0)
        {
            ...
            *cmd = p->Commands[0];        // the first command in queue
            ...
            if (!p->Commands.IsGood())
            {
                ...
                p->Commands[0] = -1; // prevent endless cycle in case removing the command fails
            }
            *data = p;
        ...
        )
        {
                ...
                (err == ERROR_FILE_NOT_FOUND || err == ERROR_PATH_NOT_FOUND || err == ERROR_BAD_PATHNAME))
            {
                ...
                if (notLoadedPluginNames != NULL && Data[i]->RegKeyName != NULL && Data[i]->RegKeyName[0] != 0 &&
                    _stricmp(Data[i]->DLLName, "fsearch\\fsearch.spl") != 0 && // we want to hush FSearch (no need to warn it's gone)
                    _stricmp(Data[i]->DLLName, "eroiica\\eroiica.spl") != 0 && // hush Eroiica (no need to warn it's gone)
                    _stricmp(Data[i]->DLLName, "unace\\unace.spl") != 0 &&     // hush UnACE (no need to warn it's gone)
                    _stricmp(Data[i]->DLLName, "diskcopy\\diskcopy.spl") != 0) // hush DiskCopy (no need to warn it's gone)
                {
    ...
    if (foundFiles.IsGood())
    {
        *s = 0; // correct the path in buf
        ...
        for (int i = 0; i < foundFiles.Count; i++)
        {
            ...
            if (!Plugins.FindDLL(pluginName, index)
#ifdef _WIN64                                            // FIXME_X64_WINSCP - this will likely need a different approach... (ignoring missing WinSCP in the 64-bit Salamander)
                && !IsPluginUnsupportedOnX64(pluginName) // only a problem in internal debug builds (an incomplete plugin compiles even though it's not released to the public)
#endif                                                   // _WIN64
            )
            {
                _snprintf_s(textProgress, _TRUNCATE, "%s\n%s", LoadStr(IDS_AUTOINSTALLPLUGINS), pluginName);
...
BOOL CPlugins::EnumInstalledModules(int* index, char* module, char* version)
{
    ...
    else
    {
        if (*index > 0 && *index - 1 < Data.Count)
        {
            ...
            char* s = data->DLLName;
            if ((*s != '\\' || *(s + 1) != '\\') && // not UNC
                (*s == 0 || *(s + 1) != ':'))       // not "c:" -> path relative to packers
            {
                GetModuleFileName(HInstance, module, MAX_PATH);
                s = strrchr(module, '\\') + 1;
                strcpy(s, "plugins\\");
...
BOOL CPlugins::QueryHotKey(WPARAM wParam, LPARAM lParam, int* pluginIndex, int* menuItemIndex)
{
    ...
    BYTE vk = UpperCase[(BYTE)wParam]; // we should be case-insensitive; the user might have caps lock on and the hotkey edit line always treats the key as uppercase
    if (vk == 0)
        return FALSE; // plugin hotkeys do not work when vk == 0
...
BOOL CPlugins::HandleKeyDown(WPARAM wParam, LPARAM lParam, CFilesWindow* activePanel, HWND hParent)
{
    ...
    if (QueryHotKey(wParam, lParam, &pluginIndex, &menuItemIndex))
    {
        ...
        // because it would trigger command A -- I flush the message queue
        ...
        // therefore WM_KEYDOWN and WM_SYSKEYDOWN must be used, which in turn send WM_CHAR and WM_SYSCHAR
        MSG msg;
