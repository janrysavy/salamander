        ...
        while (1)
        {
            if (convertDlg.Execute() == IDOK)
            {
                ...
                else
                {
                    ...
                    // when the main window is disabled-after reactivating a disabled main window the active panel
                    // does not have focus)
                    ...
                    // change in the directory displayed in the panel and, if work was done in subdirectories, there as well
                    ...
                    if (!res || !StartProgressDialog(script, LoadStr(IDS_CONVERTTITLE), NULL, &dlgData))
                    {
                        if (script->IsGood() && script->Count == 0)
                        {
                            ...
                            // none of the files may have matched the mask filter
                    ...
                    else // remove selected indexes (operation runs in another thread)
                    {
                        SetSel(FALSE, -1, TRUE);                        // explicit repaint
                        PostMessage(HWindow, WM_USER_SELCHANGED, 0, 0); // selection change notify
                        UpdateWindow(MainWindow->HWindow);
...
void CFilesWindow::FindFile()
{
    ...
    { // try to open Find for the FS in the panel; if it succeeds, there is no point opening the standard Find dialog
        SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_NORMAL);
        ...
        if (i >= Dirs->Count && i < Dirs->Count + Files->Count)
        {
            ...
            else
            {
                if (Is(ptZIPArchive))
                {
                    ...
                    // disk cache settings for the plugin (standard values change only for the plugin)
                ...
                else
                {
                    if (Is(ptPluginFS))
                    {
                        ...
                        return; // viewing on the FS is already done
                    }
                    else
                    {
                 ...
                 int enumFileNamesLastFileIndex)
{
    ...
    {
        // attempt to find a viewer with the matching HandlerID
        int j;
                    ...
                    { // plug-in viewers only
                        ...
                        if (plugin != NULL && plugin->SupportViewer)
                        {
                            if (!plugin->CanViewFile(name))
                                continue; // try to find another viewer, this one won't handle it
                        }
                        else
...
void CFilesWindow::EditFile(char* name, DWORD handlerID)
{
    ...
    if (handlerID != 0xFFFFFFFF)
    {
        // attempt to find an editor with the matching HandlerID
        int i;
...
void CFilesWindow::EditNewFile()
{
    ...
    while (1)
    {
        ...
        // ------------------
        // For EditNew, smart selection of only the name makes no sense because people also change the extension, see our forum:
        ...
        if (!Configuration.EditNewSelectAll)
        {
            ...
            if (first)
            {
                ...
                first = FALSE; // after an error we get the full file name, so select it all
            }
        }
        ...
        if (dlg.Execute() == IDOK)
        {
            ...
            // we do this only for the last component; the previous ones already exist and it doesn't matter (the system handles it)
            // or they are checked during creation and an error is shown (we simply don't clean them, letting the user do some work)
...
void CFilesWindow::FillEditWithMenu(CMenuPopup* popup)
{
    ...
    for (i = 0; i < masks->Count; i++)
    {
        ...
        // if users have multiple mask rows associated with one viewer/editor,
...
void CFilesWindow::CreateDir(CFilesWindow* target)
{
    ...
    CREATE_AGAIN:
        ...
        if (dlg.Execute() == IDOK)
        {
            ...
            // clean the name from undesirable characters at the beginning and end
            // we do this only for the last component; the previous ones already exist and it doesn't matter (the system handles it)
            // or they are checked during creation and an error may be raised (we simply don't clean them so the user has to try)
            char* lastCompName = strrchr(path, '\\');
            MakeValidFileName(lastCompName != NULL ? lastCompName + 1 : path);
    ...
    else
    {
            ...
            GetPluginFS()->IsServiceSupported(FS_SERVICE_CREATEDIR)) // FS is in the panel
        {
            ...
            if (!cancel) // not a cancel operation
            {
                if (!ret)
                {
                    CTruncatedString subject;
    ...
    if (formatedFileName[0] != 0 && *s == 0)
    {
                                                 ...
                                                 f->DosName != NULL && strlen(f->DosName) + l < MAX_PATH))
        {
            ...
            *mayChange = TRUE;
            ...
            {
                if (StrICmp(path, tgtPath) != 0 && // if it isn't just a case change
                    ...
                    if (find != INVALID_HANDLE_VALUE)
                    {
                            ...
                            StrICmp(tgtName, data.cFileName) != 0)            // full name differs
                        {
                            ...
                            if (SalPathAppend(tmpName, data.cFileName, MAX_PATH))
                            {
                                ...
                                {                    // the file again, then return the temporary file to its original name
...
void CFilesWindow::QuickRenameEnd()
{
    ...
    if (QuickRenameWindow.HWindow != NULL && QuickRenameWindow.GetCloseEnabled())
    {
        ...
        // avoid cycles caused by WM_KILLFOCUS and similar messages
...
BOOL CFilesWindow::HandeQuickRenameWindowKey(WPARAM wParam)
{
    ...
    if (Is(ptDisk))
    {
        ...
        // to cancel the operation, so they would have to press Escape. Explorer behaves this way now.
