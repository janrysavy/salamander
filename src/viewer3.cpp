...
void CViewerWindow::OnVScroll()
{
    if (VScrollWParam != -1 && VScrollWParam != VScrollWParamOld)
    {
        ...
        if (EnableSetScroll || SeekY != oldSeekY)
        {
            ...
            // ends with the scrollbar not updated after answering No; this hack fixes it (probably invalidating the
...
CViewerWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_VSCROLL:
    {
        if (FileName != NULL)
        {
            ...
            case SB_THUMBTRACK:
            {
                // the actual scrolling runs from a timer because USB mice and MS scrollbars misbehave otherwise:
                // when the viewer is fullscreen, repainting the whole window takes long enough that the stubborn scrollbar waits,
                // so dragging feels like chewing gum. Posting the scroll message or deferring painting did not help;
            ...
            case CM_EXTSEL_DOWN:
            {
                ...
                while (1)
                {
                    ...
                    { // search for the end of the selection in LineOffset without the last line (if it is only partially visible,
                        ...
                        // from the beginning of the line and not from the end of the previous line, so the selection of 'endSelLineIndex' must respect this
                ...
                case CM_EXTSEL_HOME:
                {
                    ...
                    else // go to the end of the previous line
                    {
                        if (endSelLineIndex > 0)
                        {
                            ...
                            // artificially (the upper line is wrapped, so we can move one character left),
                    ...
                    if (scrollUp)
                    {
                        ...
                        if (scrolled) // scrolled without repainting; the first line spans SeekY to firstLineEndOff
                        {
                                ...
                                SeekY <= firstLineEndOff) // just to be safe
                            {
                                ...
                                // there is no offset difference between the beginning and end of a wrapped line, create one
                                // artificially (the top line is wrapped, so we can move one character left),
                                // it must be a backward block; otherwise the block would end at the end of the previous line
                                if (WrapText && newEndSel == EndSelection && newEndSel > 0)
                                    newEndSel--; // should always be > 0
                                EndSelection = newEndSel;
                            ...
                            if (scrolled) // scrolled without repainting; the first line spans SeekY to firstLineEndOff
                            {
                                ...
                                SeekY <= firstLineEndOff) // just to be safe
                                {
                                    if (!GetOffsetFromXInText(&curX, &curOff, EndSelectionPrefX, -1, SeekY, firstLineCharLen,
                                        ...
                                        return 0;
                                    EndSelection = curOff;
                                    // ensure this line is not only scrolled but also repainted (selection changes)
                                    InvalidateRows(minRow, maxRow, FALSE);
                                }
        ...
        if (MouseDrag)
        {
            ...
            if (GetOffset(x, y, off, fatalErr) && !fatalErr)
            {
                if (EndSelection != off)
                {
                    ...
                    // (the right edge is missing); when extending the block further we must repaint the previous line
                    // so the right edge is drawn (this redraws the previous line unnecessarily outside the described case,
                    ...
                    // repaint the end of the previous line (its right edge was erased)
...
*/
                    ...
                    // insert the auto-detect command at the very top of the submenu and follow it with a separator
