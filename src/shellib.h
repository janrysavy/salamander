...
// Creates a context menu interface for the given directory
...
// Detects whether this is a path inside NetHood (a directory containing the target.lnk file),
...
// the folder for the shortened ID list from the desktop, and calls GetDisplayNameOf with the given 'flags'
// for the last ID from that folder); on success it returns TRUE and stores the name in 'name' (buffer of size 'nameSize');
// does not deallocate 'pidl'; 'alloc' is an interface obtained through CoGetMalloc
...
public:
    // Last effect returned by GiveFeedback - introduced because
    ...
    // (in short: an optimized Move is done, which means that it does not create a copy at the destination followed by deleting
...
public:
    ...
    (DWORD dwEffect)
    {
        if (DragFromPluginFSWithCopyAndMove && !DragFromPluginFSEffectIsFromPlugin)
        {
            ...
            { // Copy should be performed, but Move is offered -> force this situation, show the Copy cursor, and store Copy in LastEffect
...
// leaves the remaining operations to the system IDropTarget object from IShellFolder

// Record stored in the data passed to the copy and move callback
...
// Container of data passed to the copy and move callback
...
// destroys the 'data' object, and 'param' is the parameter passed to the CImpDropTarget constructor
...
// Callback that returns the target directory for the point 'pt'
...
// Callback invoked when the mouse enters or leaves the target
...
private:
    ...
    int TgtType; // see CIDTTgtType; idtttWindows also for archives and FS without the option to drop the current data object
...
// Releases a CCopyMoveData instance
