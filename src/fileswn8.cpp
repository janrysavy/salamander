...
BOOL CFilesWindow::DeleteThroughRecycleBin(int* selection, int selCount, CFileData* oneFile)
{
    ...
    // Verify that the path doesn't contain components ending with a space or dot;
    // Recycle Bin goes crazy and deletes on a different path (it quietly trims
    ...
    if (!PathContainsValidComponents(path, TRUE))
    {
        ...
        return FALSE; // quick dirty bloody hack - Recycle Bin simply cannot
                      // handle names ending with a space or dot (it deletes a
                      // different name created by trimming those characters,
    ...
    do
    {
        ...
        if (oneFile->Name[oneFile->NameLen - 1] <= ' ' || oneFile->Name[oneFile->NameLen - 1] == '.')
        {
            ...
            SalMessageBox(MainWindow->HWindow, textBuf, LoadStr(IDS_ERRORTITLE), MB_OK | MB_ICONEXCLAMATION);
            return FALSE; // quick dirty bloody hack - Recycle Bin simply
            //           cannot handle names ending with a space or dot
            //           (it deletes a different name created by trimming
            //           those characters, which we definitely don"t want)
        }
        // oneFile points to the selected item or the caret position in the file box
    ...
    ;             // for now we simply keep the selection, the return value doesn't work
}
...
void CFilesWindow::FilesAction(CActionType type, CFilesWindow* target, int countSizeMode)
{
    ...
    if (!FilesActionInProgress)
    {
        ...
        if (type != atCountSize && countSizeMode != 0)
            countSizeMode = 0; // just to be sure (only countSizeMode is tested later)

        FilesActionInProgress = TRUE;

        BeginSuspendMode(); // snooper takes a break
        BeginStopRefresh(); // just so that change notifications are not distributed
        ...
        if (countSizeMode == 0 && count > 0)
        {
            ...
            if (indexes == NULL)
            {
                ...
                EndStopRefresh();
                EndSuspendMode(); // snooper starts again now
                return;
            }
            else
            {
        ...
        char path[2 * MAX_PATH + 200]; // +200 is a reserve (Windows create paths longer than MAX_PATH)
        ...
        else
        {
            if (target->Is(ptZIPArchive))
            {
                ...
                // if packing to the archive in the other panel isn't possible, leave the path empty
            ...
            else
            {
                if (target->Is(ptPluginFS) && (type == atCopy || type == atMove))
                {
                    ...
                    {
                        // this only adjusts the destination path text in the plugin -> no point lowering the thread priority
                        int selFiles = 0;
                        int selDirs = 0;
                        ...
                        {
                            path[0] = 0; // error obtaining the target path
                        }
                        else
                        {
                            // convert the path to external format (before showing in the dialog)
        ...
        if (type == atDelete)
        {
            ...
            {
                recycle = 0;              // none
                canUseRecycleBin = FALSE; // it won't work because Windows doesn't support it
            }
            else
            {
                if (invertRecycleBin)
        ...
        CFileData* f = NULL;
        char formatedFileName[MAX_PATH + 200]; // +200 is a reserve (Windows create paths longer than MAX_PATH)
        char expanded[200];
        ...
        else // count-files directories and files
        {
        ...
        case atDelete:
        {
            ...
            case 0:
                ...
                break;
            case 1:
                break; // this message isn't shown because DeleteThroughRecycleBin does the confirmation
            case 2:
        ...
        char subject[MAX_PATH + 100 + 200]; // +200 is a reserve (Windows create paths longer than MAX_PATH)
        if (resID != 0)
        {
        ...
        char nextFocus[MAX_PATH + 200]; // +200 is a reserve (Windows create paths longer than MAX_PATH)
        nextFocus[0] = 0;
        char* mask = NULL;
        switch (type)
        {
        ...
        case atMove:
        {
            ...
            while (1)
            {
                if (strlen(path) >= 2 * MAX_PATH)
                    path[0] = 0; // path is too long; not an ideal solution but I'm not up for better now :(
...
BOOL EmailFilesAddDirectory(CSimpleMAPI* mapi, const char* path, BOOL* errGetFileSizeOfLnkTgtIgnAll)
{
    ...
    do
    {
             ...
             (file.cFileName[1] != 0 && (file.cFileName[1] != '.' || file.cFileName[2] != 0))))
        {
            ...
            {
                // links: size == 0, the file size must be obtained via GetLinkTgtFileSize() additionally
                BOOL cancel = FALSE;
                ...
                else
                {
                    ...
                    for (int i = 0; i < alloc; i++)
                    {
                        if (indexes[i] >= 0 && indexes[i] < Dirs->Count + Files->Count)
                        {
                            ...
                            {
                                // links: f->Size == 0, file size must be obtained via GetLinkTgtFileSize() additionally
                                BOOL cancel = FALSE;
...
BOOL CFilesWindow::OpenFocusedInOtherPanel(BOOL activate)
{
    ...
    if (buff[0] != 0)
    {
        ...
        if (activate && this == MainWindow->GetActivePanel())
        {
            ...
            {
                // if the path changed in the other panel, activate it
