...
CTipOfTheDayWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    ...
    case WM_PAINT:
    {
      ...
      // everyone complain when the icons look so nice? ;-)
...
BOOL CDisconnectDialog::OnDisconnect()
{
    ...
    while ((index = ListView_GetNextItem(HListView, iStart, LVNI_SELECTED)) != -1)
    {
        ...
        // just disconnect items but leave them in Connections, which will be cleaned
        ...
        if (Connections[index].Type == citNetwork)
        {
            ...
            if (WNetDisconnectDialog1(&conn) != NO_ERROR)
            {
                ...
                // without invalidating and updating the main window, the area under the Disconnect dialog isn't redrawn
                // the dialog uses save-bits and its remembered background probably isn't invalidated,
...
void CDisconnectDialog::EnumConnections()
{
    ...
    // if at least one network drive was inserted, add a group before the first network item
    ...
    if (fsList != NULL)
    {
        ...
        if (count > 0)
        {
            ...
            // check item text uniqueness and index duplicate items
            for (i = pluginGroupIndex; i < Connections.Count; i++)
            {
                ...
                for (x = i + 1; x < Connections.Count; x++)
                {
                    ...
                    {
                        if (!freePluginPath) // first match found, index also the first duplicate item
                        {
    ...
    // if at least one file system was inserted, add a group before the first plugin item
...
void CCompareDirsDialog::Transfer(CTransferInfo& ti)
{
    ...
    // provide MasksString; there's range validation so nothing can break
...
CCmpDirProgressDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // when opening the dialog set PluginMsgBoxParent to this dialog (main thread only)
    ...
    case WM_COMMAND:
    {
        ...
        // do not let the command propagate or the dialog will close
        return TRUE;
    }
    }
    return CCommonDialog::DialogProc(uMsg, wParam, lParam);
...
CErrorReadingADSDlg::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        if (Title != NULL)
        {
            ...
            Title = NULL; // comes from LoadStr; its lifetime ends soon (it will be overwritten), so null it out
        }
...
{
    // if we ever show another attribute,
    // InternalGetAttr() and DISPLAYED_ATTRIBUTES need to be extended
    int l = 0;
...
CDriveSelectErrDlg::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // WARNING: unfortunately mount points return DRIVE_NO_ROOT_DIR when no media is inserted, unbelievable ... so
        ...
        if (drvType == DRIVE_REMOVABLE || drvType == DRIVE_CDROM || drvType == DRIVE_NO_ROOT_DIR)
        {
            ...
            case DRIVE_FIXED: // mount point, determine the root removable drive
            {
            ...
            case DRIVE_NO_ROOT_DIR:
                setTimer = FALSE;
                break; // no idea what's going on, better skip periodic tests
            }
...
void CCfgPageIconOvrls::Transfer(CTransferInfo& ti)
{
    ...
    else
    {
        ...
        { // configuration changed -> notify that it takes effect after Salamander restarts
...
CCfgPageIconOvrls::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // dialog elements should stretch with its size, set splitter controls
