...
// of WM_USER_ADDLOG; parameters will be copied to the log data (they can be freed after the call returns)
...
#define WM_USER_FLASHICON WM_APP + 214  // flash the status icon after Find is activated
...
// Length of the mapped view; must be larger than the regex line length + EOL +
...
public:
    ...
    // sets the base to which Set appends more text and clears the dirty flag
    void SetBase(const char* buf);
    // append to the base value set via SetBase
...
struct CGrepData
{
    BOOL FindDuplicates; // search for duplicates?
    DWORD FindDupFlags;  // FIND_DUPLICATES_xxx; meaningful only if 'FindDuplicates' is TRUE
    int Refine;          // 0: search new data, 1 & 2: search within existing data; 1: intersect with old data; 2: subtract from old data
    BOOL Grep;           // use grep?
...
public:
    ...
    // builds ItemName based on NamedText and LookInText
    void BuildItemName();

    // WARNING: saving is optimized; only changed values are stored. Before saving
    // to a key it has to be cleared first
...
public:
    ...
    // called for the local copy of the object that will be used for searching
    // must be called before Contains()
    ...
    // Note: the method requires the full path ending with a slash
...
class CFindAdvancedDialog: public CCommonDialog
{
  ...
  public:
    ...
    void EnableControls();   // handles disabling/enabling controls
    void LoadTime();
...
struct CFoundFilesData
{
    ...
    // 1) while searching for duplicate files when comparing content,
    //    it contains a pointer to CMD5Digest with the computed MD5 of the file
    ...
    // Different marks boundaries of duplicate groups
    ...
    // otherwise fills the buffer 'text' (must be at least 50 characters long) with the value
...
public:
    ...
    void SetDifferentByGroup(); // sets Different based on Group so it alternates at group boundaries
    ...
    // should be called only when the search thread is not running
    ...
    DWORD GetSelectedListSize();                     // returns how many bytes are needed to store all selected
                                                     ...
                                                     // if nothing is selected, returns 2 (two terminators)
    ...
    // if 'forceRemove' is TRUE, selected items are removed without checks
...
enum CStateOfFindCloseQueryEnum
{
    ...
    sofcqSentToFind,  // request sent; the response has not arrived or the user has not answered "stop searching?" yet
...
protected:
    ...
    int FileNameFormat; // how to adjust filenames loaded from disk, taken from global configuration due to synchronization issues
    ...
    CMenuBar* MenuBar;
    HWND HStatusBar;
    HWND HProgressBar; // status bar child window shown for some operations in a special field
    ...
    BOOL FlashIconsOnActivation; // once activated, we flash the status icons

    char FindNowText[100];

public:
    CStateOfFindCloseQueryEnum StateOfFindCloseQuery; // main thread asks the Find thread whether the window can close; unsynchronized, used only during shutdown
...
public:
    ...
    // the main window calls this for all Find dialogs when colors change
    void OnColorsChange();

    void SetProcessingEscape(BOOL value) { ProcessingEscape = value; }

    // allows Alt+C and other hot keys belonging to hidden controls
...
protected:
    ...
    void SetTwoStatusParts(BOOL two, BOOL force = FALSE); // sets one or two status bar parts; sizes depend on the current text
    ...
    void InsertDrives(HWND hEdit, BOOL network); // fills hEdit with fixed drives (and network drives if requested)
    ...
    void UpdateStatusText(); // if not in search mode, shows the count and total size of selected items

    // OLE clipboard operations

    // creates a context menu for the selected items and calls ContextMenuInvoke with lpVerb
    // returns TRUE if Invoke was called, otherwise FALSE on failure
    ...
    // if found, copies it to the buffer and returns TRUE
    // if not found or the buffer is too small, returns FALSE
