...
CSharesItem::CSharesItem(const char* localPath, const char* remoteName, const char* comment)
{
    ...
    if (localPath != NULL && localPath[0] != 0 && localPath[1] == ':')
    {
        ...
        SalPathAddBackslash(buff, MAX_PATH); // ensure a lone "c:" becomes a root path
        if (SalGetFullName(buff))            // root "c:\", others without '\\' at the end
        {
            ...
            if (LocalPath != NULL && RemoteName != NULL && Comment != NULL)
            {
                ...
                if (s == NULL || *(s + 1) == 0) // root path; s==NULL is just for safety but should never happen
                    LocalName = LocalPath;
                else
                    LocalName = s + 1;
            }
...
{
    if (LocalPath != NULL)
        free(LocalPath); // LocalName points into LocalPath, therefore we don't free it
    if (RemoteName != NULL)
        free(RemoteName);
    if (Comment != NULL)
        free(Comment);
    ...
    do
    {
        ...
        if (res == ERROR_SUCCESS || res == ERROR_MORE_DATA)
        {
            ...
            for (i = 1; i <= er; i++)
            {
                ...
                // we skip specials because Explorer doesn't show them
...
{
    HANDLES(EnterCriticalSection(&CS));
    // clear the Wanted array
    Wanted.DestroyMembers();

    // insert only those shares that reside on the requested path
    char buff[MAX_PATH];
    lstrcpyn(buff, path, MAX_PATH);
    if (buff[0] != 0)                        // when searching shares from This Computer we must not append a backslash
    ...
    for (i = 0; i < Data.Count; i++)
    {
        ...
        {
            int index;
            if (!GetWantedIndex(item->LocalName, index)) // insert the matching share into Wanted only if it's not already there
            {
...
BOOL CShares::GetUNCPath(const char* path, char* uncPath, int uncPathMax)
{
    ...
    int longestIndex = -1; // index in the Data array holding the longest matching share
    ...
    if (longestIndex != -1)
    {
        ...
        // from the original path append directories beyond the share
        if (strlen(item->LocalPath) < strlen(path))
        {
            const char* s = path + strlen(item->LocalPath);
            if (*s == '\\')
                s++; // skip a possible backslash
            strcat(unc, s);
        }
