...
CSharesItem::CSharesItem(const char* localPath, const char* remoteName, const char* comment)
{
    ...
    if (localPath != NULL && localPath[0] != 0 && localPath[1] == ':')
    {
        ...
        SalPathAddBackslash(buff, MAX_PATH); // in case it was only "c:", we want to create the root
...
void CShares::PrepareSearch(const char* path)
{
    ...
    if (buff[0] != 0)                        // when searching for shares from this_computer we must not append a backslash
...
BOOL CShares::GetUNCPath(const char* path, char* uncPath, int uncPathMax)
{
    ...
    int longestIndex = -1; // index into Data holding the longest matching share
    ...
    for (i = 0; i < Data.Count; i++)
    {
        ...
        {
            // look for the longest share that still matches the requested 'path'
    ...
    if (longestIndex != -1)
    {
        ...
        // append directories from the original path beyond the share
