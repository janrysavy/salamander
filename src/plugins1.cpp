...
void EnterPlugin()
{
    if (AlreadyInPlugin == 0)
    {
        ...
        // and any change to the following code must also be made in CSalamanderGeneral::GetMsgBoxParent
...
void LeavePlugin()
{
    ...
    {
        // measure to avoid interrupting panel listing after each ESC in the plugin (mainly
            ...
            StopRefresh == 0) // if we haven't left stop-refresh mode yet, sending the message is pointless
        {
            ...
            // post a request to dispatch notifications about path changes
...
CPluginFSInterfaceEncapsulation::GetFSIcon(BOOL& destroyIcon)
{
    ...
    if (IsServiceSupported(FS_SERVICE_GETFSICON))
    {
        ...
        if (r != NULL && destroyIcon) // add the handle for the returned icon to HANDLES
...
void CSalamanderConnect::AddViewer(const char* masks, BOOL force)
{
    ...
    if (Viewer || force)
    {
        ...
        if (!Viewer && force) // this is an update, not an installation, so check whether it is already listed
        {
            ...
            while (s > ext)
            {
                ...
                while (--ss >= s + 1 && *ss <= ' ')
                    ;
                *(ss + 1) = 0; // trim trailing spaces from the mask
                ss = s + 1;
                while (*ss != 0 && *ss <= ' ')
                    ss++;     // skip spaces at the beginning of the mask
                if (*ss != 0) // if the mask is not empty, add it to the array
                {
                    extArray.Insert(0, ss); // one of the extensions, O(n^2) to preserve order
                    if (!extArray.IsGood())
                        extArray.ResetState();
                }
            }

            // trim the mask list by removing entries that are already registered...
            int i;
            ...
            // merge the mask 'masks' again (the useful remainder)
            s = ext2;
            int k;
            for (k = 0; k < extArray.Count; k++)
            {
                if (extArray[k][0] == ';' && s != ext2)
                    *s++ = ' '; // space is necessary (otherwise the previous ';' wouldn't act as a separator)
                strcpy(s, extArray[k]);
...
void CSalamanderConnect::ForceRemoveViewer(const char* mask)
{
    ...
    for (i = 0; i < MainWindow->ViewerMasks->Count; i++)
    {
        if (MainWindow->ViewerMasks->At(i)->ViewerType == -Index - 1) // correct plug-in
        {
            ...
            ext2[len] = 0;
            char* s = ext2 + len;
            // find and remove the extension 'mask', side effect: ';' replaced by 0
            while (s > ext2)
            {
                while (--s >= ext2)
                {
                ...
                if (s >= ext2)
                    *s = 0;
                if (StrICmpIgnoreSpacesOnStartAndEnd(s + 1, mask) == 0) // we are looking for this mask, will delete it
                {
...
void CSalamanderConnect::AddPanelArchiver(const char* extensions, BOOL edit, BOOL updateExts)
{
    ...
    while (s > ext)
    {
        ...
        if (s >= ext)
            *s = 0;
        extArray.Insert(0, s + 1); // one of the extensions, O(n^2) to preserve order
        if (!extArray.IsGood())
            extArray.ResetState();
    }
    ...
    for (i = 0; i < PackerFormatConfig.GetFormatsCount(); i++)
    {
        ...
        {
            if (PackerFormatConfig.GetUnpackerIndex(i) == -Index - 1) // and the plugin is set at least for "view"
            {
                found = TRUE;
            }
            else
        ...
        while (s > ext2)
        {
            ...
            for (j = 0; j < extArray.Count; j++)
            {
                if (found || StrICmp(s + 1, extArray[j]) == 0) // upgrade or extension sets have a non-empty intersection
                {
                    ...
                    {
                        extArray.Delete(j); // it's already in ext2, it needn't be in ext
                        if (!extArray.IsGood())
                            extArray.ResetState();
                    }
    ...
    BOOL newItem = FALSE;
    if (index == -1) // extensions contain brand new types; we may add a new entry later
    {
        newItem = TRUE;
        ext2[0] = 0;
    }
    ...
    // prepare ext2 with a union of existing extensions and "extensions" or just new extensions
    len = (int)strlen(ext2);
    if (len > 0 && ext2[len - 1] == ';')
        len--;
    ...
    else
    {
        if (PanelView) // view
        {
            /*    // commented out because otherwise ::Connect only needs to call AddPanelArchiver
      // with parameter 'edit'==TRUE and even a plugin that does not provide panel edit
      // would be marked as "panel edit" for the given extensions, which is wrong
      if (newItem && edit)
      {
        packerIndex = -Index - 1;
        usePacker = TRUE;
...
*/
    ...
    if (updateExts && newItem) // all plugin entries were removed by the user; add at least the new ones
    {
        ...
        {
            packerIndex = unpackerIndex = -Index - 1; // view and possibly edit
    ...
    if (updateExts && !newItem &&          // update the extension list (only if the record exists)
        ...
        change)                            // only when changed (otherwise it would add unwanted extensions
    {                                      // supported archiver extensions)
...
NEXT_ROUND:
    ...
    {
        if (PackerFormatConfig.GetUnpackerIndex(i) == -Index - 1) // if the plug-in is configured at least for viewing
        {
            char ext[300];
...
void CSalamanderConnect::SetBitmapWithIcons(HBITMAP bitmap)
{
    ...
    if (p != NULL)
    {
        //  copy the 'bitmap' into our DIB so we
                                                ...
                                                const char* extensions, const char* fsName)
{
    ...
    // this is loading an installed plugin or adding a new plugin,
    ...
    if (Plugin->SupportFS)
    {
        ...
        else
        {
            ...
            {
                Plugin->FSNames.Delete(i); // remove all fs-names except the first
                if (!Plugin->FSNames.IsGood())
                    Plugin->FSNames.ResetState(); // deletion occurred, nothing else matters
            }
        }
...
CSalamanderPluginEntry::LoadLanguageModule(HWND parent, const char* pluginName)
{
    ...
    { // the SLG doesn't exist or isn't the expected one (completely different file or at least another version)
        ...
        if (lang == NULL) // find all .slg files on disk for the plugin and let the user choose if there's more than one
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        if (Configuration.UseAsAltSLGInOtherPlugins) // fallback language defined but unavailable for this plugin, let the user choose another one
                ...
                lang = HANDLES_Q(LoadLibrary(path));
                if (lang == NULL || !IsSLGFileValid(Plugin->GetPluginDLL(), lang, languageID, NULL))
                { // shouldn't theoretically happen (dialog verifies the SLG module)
                    if (lang != NULL)
                        HANDLES(FreeLibrary(lang));
                    lang = NULL;
    ...
    if (DLL == NULL && IsGood())
    {
        ...
        else // connect to the DLL
        {
            ...
            if (entry != NULL)
            {
                ...
                {
                    if (PluginHomePageURL != NULL)
                    { // delete the plugin URL just before calling the entry point so it remains available if loading fails due to an old plugin version (the user can use the URL to obtain a new version)
                        free(PluginHomePageURL);
        ...
        { // the return value of CPlugins::GetFirstNethoodPluginFSName() changed - it affects UNC root paths (whether up-dir is shown); refresh needed
...
BOOL CPluginData::Remove(HWND parent, int index, BOOL canDelPluginRegKey)
{
    ...
    if (!unloaded)
    {
        ...
        {
            // the plugin may unload: let it delete any remaining temp files in the disk cache
            ...
            if (unloaded)
            {
                ...
                // remove icon overlays when unloading the plugin
                ReleaseIconOverlays();
    ...
    if (unloaded)
    {
        // adjust "file viewer" - remove records for this plugin and handle array shifting of Plugins
        ...
        for (k = 0; k < 2; k++)
        {
            ...
            for (i = 0; i < viewerMasks->Count; i++)
            {
                ...
                if (type < 0) // not external or internal -> plug-in viewer
                {
                    ...
                    {
                        if (type > index) // Plugins array shifts -> decrease 'type' by one
                        {
                            type--;
        ...
        // adjust "custom pack" - remove records for this plugin and handle shifting of the Plugins array
        int i;
        for (i = 0; i < PackerConfig.GetPackersCount(); i++)
        {
        ...
        // adjust "custom unpack" - remove records for this plugin and handle shifting of the Plugins array
        for (i = 0; i < UnpackerConfig.GetUnpackersCount(); i++)
        {
        ...
        // adjust "panel view/edit" - modify or remove records for this plugin
        ...
        for (i = 0; i < PackerFormatConfig.GetFormatsCount(); i++)
        {
            ...
            if (usePack && pack < 0) // not an external "edit"
            {
                ...
                {
                    if (pack > index) // Plugins array shifts -> decrease 'pack' by one
                    {
                        pack--;
            ...
            if (removePack || removeUnpack) // a replacement for "view" and/or "edit" is needed
            {
                ...
                if (newView < 0 && -newView - 1 > index)
                    newView++; // Plugins array shifts -> adjustment needed
                if (newEdit < 0 && -newEdit - 1 > index)
                    newEdit++; // Plugins array shifts -> adjustment needed
        ...
        if (SupportLoadSave && canDelPluginRegKey) // if the plugin supports load/save configuration and we can delete its registry key (not an import from an older Salamander)
...
BOOL CPluginData::GetMenuItemStateType(int pluginIndex, int menuItemIndex, MENU_ITEM_INFO* mii)
{
    ...
    if (item->StateMask == -1) // query the item state directly from the plugin?
    {
        DWORD state = 0;
    ...
    for (; i < MenuItems.Count; i++)
    {
        ...
        if (item->SkillLevel & CfgSkillLevelToMenu(::Configuration.SkillLevel)) // apply the skill-level menu reduction
        {
            ...
            if (item->Name == NULL) // separator or failed allocation of start-submenu name
            {
                ...
                if (item->StateMask == -1) // query separator visibility directly from the plugin?
                {
            ...
            else // a regular menu item or submenu
            {
                ...
                item->SUID = Plugins.LastSUID; // save which SUID was assigned
                ...
                if (item->Type == pmitStartSubmenu && !hidden) // let the submenu be populated
                {
                    ...
                    else
                    {
                        if (mi.SubMenu == NULL)
                        {
                            mi.Mask &= ~MENU_MASK_SUBMENU; // turn the submenu into a regular item; it will run command 0, hopefully that's fine
                            TRACE_E(LOW_MEMORY);
                        }
            ...
            if (!hidden)
            {
                ...
                skipSubMenu = TRUE; // item hidden due to hidden state
        }
        else
...
void CPluginData::InitMenuItems(HWND parent, int index, CMenuPopup* menu)
{
    ...
    CHECK_MENU_AGAIN:
        ...
        BOOL ok = TRUE;
        if (SupportDynMenuExt) // dynamic menu: rebuild it if the plugin switches to static during load; verify below
        {
            if (GetLoaded())
                BuildMenu(parent, FALSE); // already loaded -> rebuild the menu manually
            else
                InitDLL(parent, FALSE, TRUE, TRUE, FALSE); // not loaded -> the menu will rebuild itself during plugin load
        ...
        {
            int i = 0;
            DWORD mask = GetMaskForMenuItems(index); // a plugin load might have changed something
...
BOOL CPluginData::BuildMenu(HWND parent, BOOL force)
{
    ...
    {
        DynMenuWasAlreadyBuild = TRUE; // prevent needless rebuilds, especially when building the menu for Last Command and again when opening a plugin submenu containing the Last Command
        if (PluginDynMenuIcons != NULL)
...
void CPluginData::EnsureShareExistsOnServer(HWND parent, int panel, const char* server, const char* share)
{
    ...
    if (InitDLL(parent, TRUE) &&     // do not report load errors; this only provides extra info, nothing serious happens if skipped
        PluginIsNethood &&           // for the case when the plugin stops replacing Network (SetPluginIsNethood() not called) during this load
        PluginIfaceForFS.NotEmpty()) // PluginIsNethood is independent of PluginIfaceForFS, so check it separately
    {
