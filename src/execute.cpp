...
{
    // we need to refresh the selection because the dumb combobox forgot it
...
const char* WINAPI ExecuteExpDrive(HWND msgParent, void* param) // drive ("D:", "\\server\share") without trailing backslash
{
...
const char* WINAPI ExecuteExpNamePart(HWND msgParent, void* param)
{
    ...
    //  if (ss != NULL && ss != data->Buffer)   // extension is present ('.' not at the start, e.g. ".cvspass")
...
const char* WINAPI ExecuteExpExtPart(HWND msgParent, void* param)
{
    ...
    //  if (s != NULL && s != data->Buffer)   // extension is present ('.' not at the start, e.g. ".cvspass")
...
struct CFileDataExpData
{
    ...
    DWORD ValidFileData; // bitmask of valid data in CFileData
...
const char* WINAPI FileDataExpFileDateOnlyForDisk(HWND msgParent, void* param)
{
    ...
    if (FileTimeToLocalFileTime(&data->FileData->LastWrite, &ft) &&
        ...
        strcmp(data->FileData->Name, "..") == 0) // for UP-DIR the empty value is 1.1.1602 0:00:00.000
    {
        return STR_FILE_DATA_NONE;
    }
    if (stIsReady)
...
const char* WINAPI FileDataExpFileAttr(HWND msgParent, void* param)
{
    ...
    { // attributes are not valid -> nothing to show
        return STR_FILE_DATA_NONE;
    }
    char attr[20];
...
const char* WINAPI ExecuteExpListOfSelFullNames(HWND msgParent, void* param) // list of full names (without paths) separated by spaces
{
    CExecuteExpData* data = (CExecuteExpData*)param;
...
CSalamanderVarStrEntry InitDirExpArray[] =
    {
        ...
        {EXECUTE_FULLPATH, ExecuteExpFullPath2}, // trailing backslashes are used; I added a call to
        {EXECUTE_WINDIR, ExecuteExpWinDir2},     // RemoveDoubleBackslahesFromPath so we could probably switch to the
        {EXECUTE_SYSDIR, ExecuteExpSysDir2},     // versions with a backslash at the end -- we would just trim the
...
BOOL ValidateCommandFile(HWND msgParent, const char* varText, int& errorPos1, int& errorPos2)
{
    ...
    {
        // edit line is empty or only spaces -- select all
        errorPos1 = 0;
        errorPos2 = -1;
        return FALSE;
    }
                   ...
                   char* buffer, int bufferLen, BOOL ignoreEnvVarNotFoundOrTooLong)
{
                        ...
                        &data, ignoreEnvVarNotFoundOrTooLong))
    {
        ...
        // but if they point to the root they do; then "$(SalDir)\\Editor" becomes
