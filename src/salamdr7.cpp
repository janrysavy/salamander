...
// on Altap's discussion thread 6188 it turned out that the function is not ideal for Salamander.
...
// The second problem is that it discards variables inherited by the process when it is launched. For Windows Explorer neither issue
// is a problem, because under x64 Windows it is always x64 and at the same time it does not inherit any special variables,
// because it is started by the system rather than by the user.
//
...
//
// We see the best solution as using the system RegenerateUserEnvironment in a smarter way.
// When the process starts, fetch the environment variables using the GetEnvironmentStrings() API. Then call
// RegenerateUserEnvironment() (it takes 4 ms, so it is not a problem) and fetch the environment variables again.
...
protected:
    ...
    // sorts the array by name in a case-insensitive manner
    void QuickSort(int left, int right);

    // if it finds the 'name' item in the array, it returns its index; otherwise it returns -1
    // assumes the array is sorted alphabetically, uses interval bisection
...
void CEnvVariables::LoadFromProcess()
{
    ...
    while (*p != 0)
    {
        ...
        while (*p != 0)
            p++;
        // if this is not the current directory for drives, store the found item in the array
        // We ignore:
        // =::=::\
    ...
    // note that the array returned by GetEnvironmentStrings() looks sorted, but when environment variables are set the new ones are appended,
...
void RegenEnvironmentVariables()
{
    ...
    // based on it, use the differences captured when Salamander started to “patch” our process
    newVars.ApplyDifferencesToCurrentProcess(&EnvVariablesDiff);
}
...
BOOL GetResolvedPathMountPointAndGUID(const char* path, char* mountPoint, char* guidPath)
{
    ...
    if (!IsUNCPath(rootPath) && GetDriveType(rootPath) == DRIVE_FIXED) // it only makes sense to look for reparse points on fixed disks
    {
        ...
        strcpy(resolvedPath, rootPath); // for non-DRIVE_FIXED disks we take the root path, GetVolumeNameForVolumeMountPoint needs a mount point and searching for it by gradually shortening the path still seems too time-consuming (at least for network paths + with cards mount points in subdirectories hopefully are not a threat, right?)
