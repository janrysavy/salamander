...
// StrCmpLogicalW has been available since Windows XP and Explorer uses it for this comparison
int StrCmpLogicalEx(const char* s1, int l1, const char* s2, int l2, BOOL* numericalyEqual, BOOL ignoreCase)
{
    ...
    const char* beg1 = s1;         // beginning of the section (text or number)
    const char* end1 = s1;         // end of the section (text or number)
    const char* strEnd2 = s2 + l2; // end of string 's2'
    const char* beg2 = s2;         // beginning of the section (text or number)
    const char* end2 = s2;         // end of the section (text or number)
    int suggestion = 0;            // suggested result (0 / -1 / 1 = none / s1<s2 / s1>s2) - e.g. "001" < "01"

    BOOL findDots = WindowsVistaAndLater && !SystemPolicies.GetNoDotBreakInLogicalCompare(); // TRUE = names are also split at dots (not only numbers)
    ...
    while (1)
    {
        ...
        if (isStr1) // text (even empty) or a dot
        {
            if (findDots && end1 < strEnd1 && *end1 == '.')
                end1++; // dot: if we split on dots, consume them one by one
            else        // plain text (even empty)
            {
        ...
        if (isStr2) // text (even empty) or a dot
        {
            if (findDots && end2 < strEnd2 && *end2 == '.')
                end2++; // dot: if we split on dots, consume them one by one
            else        // plain text (even empty)
            {
        ...
        if (isStr1 || isStr2) // comparing text, dots or a mixed pair (text, dot or number); everything except two numbers compares as strings
        {
            int ret;
        ...
        else // comparing two numbers
        {
            if (numBeg1 == NULL)
            {
                if (numBeg2 == NULL) // both numbers are zero
                {
                    if (suggestion == 0) // we only care about the first suggested result
                    {
                ...
                else // first number is zero, second is non-zero
                {
                    if (numericalyEqual != NULL)
                        *numericalyEqual = FALSE;
                    return -1; // "00" < "1"
            ...
            {
                if (numBeg2 == NULL) // first number is non-zero, second is zero
                {
                    if (numericalyEqual != NULL)
                        *numericalyEqual = FALSE;
                    return 1; // "1" > "00"
                ...
                {
                    if (end1 - numBeg1 > end2 - numBeg2) // first number has more digits than the second
                    {
                        if (numericalyEqual != NULL)
                            *numericalyEqual = FALSE;
                        return 1; // "100" > "99"
                    ...
                    {
                        if (end1 - numBeg1 < end2 - numBeg2) // second number has more digits than the first
                        {
                            if (numericalyEqual != NULL)
                                *numericalyEqual = FALSE;
                            return -1; // "99" < "100"
                        ...
                        else // numbers have the same digit count; compare by value (equivalent to string comparison)
                        {
                            ...
                            if (ret != 0) // the values differ
                            {
                                if (numericalyEqual != NULL)
                                    *numericalyEqual = FALSE;
                                return ret;
                            }
                            else // numbers are equal; if the zero prefix length differs, store it in the suggested result
                            {
                                if (suggestion == 0) // we only care about the first suggested result
                                {
                                    if (end1 - beg1 > end2 - beg2)
...
//--- first by Name
  ...
  int res1 = RegSetStrICmpEx(f1.Name, (*f1.Ext != 0) ? (f1.Ext - 1 - f1.Name) : f1.NameLen,
                             ...
                             &numericalyEqual1);
  if (!numericalyEqual1) return res1;   // names differ (not even numerically equal)
//--- Names are equal, extension decides
  BOOL numericalyEqual2;
  int res2 = RegSetStrICmpEx(f1.Ext, f1.NameLen - (f1.Ext - f1.Name),
                             f2.Ext, f2.NameLen - (f2.Ext - f2.Name),
                             &numericalyEqual2);
  if (numericalyEqual2 && res1 != 0) return res1; // extensions match or are numerically equal and names only match numerically (name comparison has priority)
  else return res2;
*/
    //--- compare the entire Name (including Ext), like Explorer
    return RegSetStrICmpEx(f1.Name, f1.NameLen, f2.Name, f2.NameLen, NULL);
}

int CmpNameExt(const CFileData& f1, const CFileData& f2)
{
    /*  // older version: we compared the name and extension separately
//--- first by Name
  BOOL numericalyEqual1;
  int res1 = RegSetStrICmpEx(f1.Name, (*f1.Ext != 0) ? (f1.Ext - 1 - f1.Name) : f1.NameLen,
                             f2.Name, (*f2.Ext != 0) ? (f2.Ext - 1 - f2.Name) : f2.NameLen,
...
//--- Names match, extension decides
  ...
  {
    if (res2 != 0 || f1.Name == f2.Name) return res2; // identical paths must match
  }
//--- identical names (archives or FS) - try to see if case differs at least
...
*/
    ...
    if (res != 0 || f1.Name == f2.Name)
        return res; // identical paths must compare as equal
                    //--- identical names (archives or FS) - try to see if case differs at least
...
BOOL LessExtName(const CFileData& f1, const CFileData& f2, BOOL reverse)
{
    ...
    if (!numericalyEqual1)
        return reverse ? res1 > 0 : res1 < 0; // extensions differ (neither identical nor numerically equal)
                                              //--- extensions are equal, Name decides
    BOOL numericalyEqual2;
    ...
    if (numericalyEqual2 && res1 != 0)
        return reverse ? res1 > 0 : res1 < 0; // extensions are equal or numerically equal and names only match numerically (name comparison has priority)
    else
    {
        if (res2 == 0 && f1.Name != f2.Name) // identical names (archives or FS) - try to see if the case differs at least
        {
            res1 = RegSetStrCmpEx(f1.Ext, f1.NameLen - (int)(f1.Ext - f1.Name),
                                  f2.Ext, f2.NameLen - (int)(f2.Ext - f2.Name),
                                  &numericalyEqual1);
            if (!numericalyEqual1)
                return reverse ? res1 > 0 : res1 < 0; // extensions differ (not even numerically equal)
            //--- extensions are again equal, Name decides
            res2 = RegSetStrCmpEx(f1.Name, (*f1.Ext != 0) ? (int)(f1.Ext - 1 - f1.Name) : f1.NameLen,
                                  f2.Name, (*f2.Ext != 0) ? (int)(f2.Ext - 1 - f2.Name) : f2.NameLen,
                                  &numericalyEqual2);
            if (numericalyEqual2 && res1 != 0)
                return reverse ? res1 > 0 : res1 < 0; // names are equal or numerically equal and extensions only match numerically (extension comparison has priority)
        }
...
BOOL LessAttrNameExt(const CFileData& f1, const CFileData& f2, BOOL reverse)
{
    ...
    // if we add support for showing another attribute,
    // the DISPLAYED_ATTRIBUTES mask must be expanded

    // switch to alphabetical sorting like Explorer and Speed Commander
    DWORD f1Attr = 0;
    DWORD f2Attr = 0;
    ...
    //--- attributes are equal, then Name
    int res = CmpNameExt(f1, f2);
    return reverse ? res > 0 : res < 0;
}
