...
int StrCmpLogicalEx(const char* s1, int l1, const char* s2, int l2, BOOL* numericalyEqual, BOOL ignoreCase)
{
    ...
    BOOL findDots = WindowsVistaAndLater && !SystemPolicies.GetNoDotBreakInLogicalCompare(); // TRUE = split names also on dots (not only numbers)
    ...
    while (1)
    {
        ...
        if (isStr1) // text (even empty) or a dot
        {
            if (findDots && end1 < strEnd1 && *end1 == '.')
                end1++; // dot: when we split on them, take them one by one
        ...
        if (isStr2) // text (even empty) or a dot
        {
            if (findDots && end2 < strEnd2 && *end2 == '.')
                end2++; // dot: when we split on them, take them one by one
            else        // text (even empty)
            {
        ...
        if (isStr1 || isStr2) // compare text, dots, or mixed pairs of text, dot, or number (everything except two numbers compares as strings)
        {
            int ret;
            ...
            else
            {
                ...
                else // both numbers are non-zero
                {
                    ...
                    else
                    {
                        ...
                        else // numbers have the same number of digits; compare their value (equivalent to string comparison)
                        {
                            ...
                            else // the numeric values match; if they differ by the number of leading zeros, take it into the suggested result
                            {
                                if (suggestion == 0) // only the first suggested result matters to us
                                {
                                    if (end1 - beg1 > end2 - beg2)
...
LABEL_SortNameExtAux:
    ...
    if (left < j)
    {
        if (i < right)
        {
            if (j - left < right - i) // both halves need to be sorted, so recurse on the smaller one and handle the other via "goto"
            {
