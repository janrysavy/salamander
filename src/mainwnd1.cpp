    ...
    for (i = 0; i < HOT_PATHS_COUNT; i++)
    {
        ...
        if (CreateKey(hKey, keyName, actKey))
        {
            ...
            if (*name == 0 && *path == 0 && visible == TRUE)
            {
                ...
                // not ready for configuration merging, but neither is the rest of our config
                ClearKey(actKey);
                CloseKey(actKey);
...
BOOL CHotPathItems::Load(HKEY hKey)
{
    ...
    for (i = 0; i <= HOT_PATHS_COUNT; i++)
    {
        ...
        {
            // when there were only 10 hot paths, the tenth entry was stored under key '0'
...
BOOL CMainWindow::ToggleTopToolBar(BOOL storePos)
{
    ...
    else
    {
        ...
        IdleRefreshStates = TRUE; // on next Idle, enforce state checks
...
BOOL CMainWindow::ToggleBottomToolBar()
{
    ...
    if (BottomToolBar->HWindow != NULL)
    {
        ...
        BottomToolBar->SetState(btbsCount); // next time it shows we'll pour in a valid state
...
// font for our GUI (the panel font can be set in the configuration)
...
void CMainWindow::GetFormatedPathForTitle(char* path)
{
    ...
    // plugin FS without support for obtaining the window title path can show only the full path
    CFilesWindow* panel = GetActivePanel();
    if (panel == NULL)
    {
        path[0] = 0;
    ...
    case TITLE_BAR_MODE_COMPOSITE:
    {
                                                             ...
                                                             2, path, 2 * MAX_PATH))
        {
            ...
            if (path[0] != 0)
            {
                char* trimStart = NULL; // place where "..." is inserted, followed by 'trimEnd'
                char* trimEnd = NULL;
                if (panel->Is(ptDisk) || panel->Is(ptZIPArchive))
                {
                    ...
                    while (path[chars] != 0)
                    {
                        // looking for the last component we want to keep
                ...
                else
                {
                    if (panel->Is(ptPluginFS))
                    {
                        ...
                        if (panel->GetPluginFS()->GetNextDirectoryLineHotPath(path, pathLen, chars) &&
                            chars < pathLen) // end of path isn't a separator; GetNextDirectoryLineHotPath bug
                        {
                            // we isolated the root
                            trimStart = path + chars;
                            int lastChars = chars;
                            while (panel->GetPluginFS()->GetNextDirectoryLineHotPath(path, pathLen, chars))
                            {
                                ...
                                if (chars < pathLen)
                                    ...
                                    break; // end of path isn't a divider; GetNextDirectoryLineHotPath bug
                            }
...
*/
    ...
    int HeaderLineItem = -1; // will be filled with the item index if the user clicked something
...
*/
        ...
        if (hit == mwhteLeftHeaderLine || hit == mwhteRightHeaderLine)
        {
            ...
            if (hdrLine != NULL)
            {
                // find which header line item the point is over
                POINT hdrP;
                hdrP.x = xPos;
                hdrP.y = yPos;
...
*/
    ...
    if (panelClass)
    {
        ...
        case 1:
        {
            ...
            // the user changed the view configuration - rebuild the columns
            if (leftPanel)
...
void CMainWindow_RefreshCommandStates(CMainWindow* obj)
{
    ...
    BOOL linkOnDisk = FALSE;                         // disks only: cursor on a link (file or directory with FILE_ATTRIBUTE_REPARSE_POINT)
    ...
    DWORD pasteDefEffect = EnablerPasteDefEffect;    // default paste effect, may combine DROPEFFECT_COPY+DROPEFFECT_MOVE (Copy or Cut?)
    ...
    BOOL validPluginFS = FALSE;                      // does the panel have an initialized PluginFS interface?
    ...
    BOOL dirHistory = FALSE; // is there a directory available in history?
    ...
    if (activePanel != NULL && nonActivePanel != NULL && obj->LeftPanel != NULL && obj->RightPanel != NULL)
    {
        ...
        if (archive)
        {
            ...
            {
                archiveEdit = PackerFormatConfig.GetUsePacker(format - 1); // has edit?
            }
        }
        ...
        if (!leftUpDir)
            leftRootDir = FALSE; // already at root (no up-dir exists)
        else
            leftRootDir = TRUE; //!obj->LeftPanel->Is(ptDisk) || !IsUNCRootPath(obj->LeftPanel->GetPath());
        if (!rightUpDir)
            rightRootDir = FALSE; // already at root (no up-dir exists)
        else
    ...
    // if any of them changes, obj->IdleStatesChanged will be set
    ...
    obj->CheckAndSet(&EnablerPasteFiles, pasteFiles);                   // store clipboard state for the next RefreshCommandStates()
    obj->CheckAndSet(&EnablerPastePath, pastePath);                     // store clipboard state for the next RefreshCommandStates()
    obj->CheckAndSet(&EnablerPasteLinks, pasteLinks);                   // store clipboard state for the next RefreshCommandStates()
    obj->CheckAndSet(&EnablerPasteSimpleFiles, pasteSimpleFiles);       // store clipboard state for the next RefreshCommandStates()
    obj->CheckAndSet(&EnablerPasteDefEffect, pasteDefEffect);           // store clipboard state for the next RefreshCommandStates()
...
{
    // screen colors or depth changed; new imagelists have been created
    ...
    if (HPToolBar != NULL)
    {
        ...
        HPToolBar->CreateButtons();   // HFavoritIcon handle changed
    }
