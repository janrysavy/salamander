                                              ...
                                              BOOL& convertPathToInternal)
{
    ...
    {
        if (Plugins.AreFSNamesFromSamePlugin(PluginFS.GetPluginFSName(), fsName, fsNameIndex)) // compare whether the file systems are from the same plug-in
        {
...
void CFilesWindowAncestor::SetPath(const char* path)
{
    ...
    if (!Is(ptPluginFS)) // pluginFS handles changes differently...
    {
        ...
        // handle suppression of auto refresh
        if (SuppressAutoRefresh)
            MonitorChanges = FALSE;

        if (MonitorChanges)
            AddDirectory((CFilesWindow*)this, Path, DriveType == DRIVE_REMOVABLE || DriveType == DRIVE_FIXED);
        else // if changes are not monitored, Snooper does not call SetAutomaticRefresh -> do it here
        {
