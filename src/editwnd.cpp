...
BSHandlerSubclassProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        // handle Ctrl+Backspace for deleting a word
        if (wParam == VK_BACK)
        {
            ...
            if (controlPressed && !altPressed && !shiftPressed)
            {
                ...
                //          if (iStart == iEnd) // nothing may be selected
                //          {
                char buff[10000];
...
int GetCmdLineLimit()
{
  ...
  Measured limits when launching via COMSPEC:
    ...
    8156                             Vista + Win7 with COMSPEC=C:\Windows\system32\cmd.exe
                                    (longer COMSPEC means a smaller limit)
*/
...
CEditLine::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        case '\r':
        {
            ...
            else
            {
                ...
                if (panel->Is(ptDisk)) // running commands on disk -> executed in DOS Prompt
                {
                    ...
                    char cmd[SALCMDLINE_MAXLEN + MAX_PATH]; // COMSPEC is likely only a few characters; MAX_PATH is plenty (no extra buffer for /K, etc.)
    ...
    case WM_DESTROYCLIPBOARD:
    {
        ...
        IdleCheckClipboard = TRUE; // request clipboard checking as well
        break;
    }
    ...
    case WM_MOUSEWHEEL:
    {
        ...
        // It worked for me via this method. After installing IntelliPoint drivers v7 (previously on Vista x64
        // I had no special drivers) WM_MOUSEHWHEEL messages stopped passing through the hook and flowed directly
        // to the focused window; I disabled the hook and now we must catch the messages in windows that may get focus so
        ...
        // so now we also catch the message in each window that can get focus and then route it so it reaches the window under the cursor
...
*/
        ...
        if (wParam >= '0' && wParam <= '9')
        {
            ...
            // Shift+number from the edit line won't work (you need to type '*' and others)
            if ((controlPressed && !shiftPressed && !altPressed) /* ||
        ...
        case VK_SPACE:
        {
            if (controlPressed && !altPressed)
            {
                ...
                if (s != NULL)
                {
                    if (shiftPressed) // DOS-style path
                    {
...
public:
    ...
    // returns a directory or file (there must be exactly one)
        ...
        else
        {
            ...
            if (GetNameFromDataObject(pDataObject, path)) // at most MAX_PATH characters are placed into 'path'
            {
                ...
                if (!IsPluginFSPath(path))
                {
                    ...
                    if (l > 0 && path[l - 1] == '\\')
                        path[--l] = 0;             // trailing '\\' is not welcome
...
CEditWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        RECT cr;
        GetClientRect(HWindow, &cr);
        // ensure the double-line frame around the combo is not cleared during painting
        RECT r;
        r.left = 0;
        r.top = 0;
        r.right = cr.right;
        ...
        // draw our own frame (one outer gray and inner sunken)
        ...
        if (WindowsVistaAndLater)
        {
            ...
            // combobox edge, otherwise garbage remains
        ...
        // ensure the double-line frame around the button is not cleared during painting
        int sbWidth = GetSystemMetrics(SM_CXVSCROLL);
...
void CEditWindow::RestoreContent()
{
    ...
    {
        // if the old window state (contents and selection) was saved, restore it
