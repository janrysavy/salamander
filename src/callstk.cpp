...
__declspec(allocate(".i_cst$a")) const _PVFV i_callstk = (_PVFV)1; // place i_callstk at the start of the .i_cst section

#pragma section(".i_cst$z", read)
__declspec(allocate(".i_cst$z")) const _PVFV i_callstk_end = (_PVFV)1; // place i_callstk_end at the end of the .i_cst section
...
CCallStack MainThreadStack; // ensure the call-stack object exists before constructors run in the main thread
...
struct CTBRData
{
    ...
    DWORD ShellExtCrashID; // if not equal to -1, this is an exception during shell execute
...
CCallStack::CCallStack(BOOL dontSuspend)
{
    ...
    if (FirstCallstack)
    {
        // Create a thread for bug report generation; in PROTECH/VKO an infinite
        // recursion once resulted in the report being generated only when it
    ...
    */
        ...
        // When a DLL (plugin, shell extension) that uses a different RTL is
        // loaded into Salamander, that RTL installs this filter during its
        // initialization. Starting with MSVC 2005 various sanity checks no
        // longer throw exceptions but directly call UnhandledExceptionFilter(),
        // which shows the Watson dialog. Without this adjustment the bug report
        // from TortoiseSVN shell extension was lost. The call below should
...
CCallStack::~CCallStack()
{
    ...
    if (MonitoredItems != NULL)
    {
        ...
        {
            while (1) // walk the queue again and print final states of critical call-stack invocations
            {
    ...
    if (FirstCallstack)
    {
        ...
        if (BugReportThread != NULL)
        {
            // ending this thread reenters here because CCallStack exited for it as well
...
void CCallStack::ReleaseBeforeExitThread()
{
    ...
    if (stack != NULL)
        stack->ReleaseBeforeExitThreadBody(); // unintercepted threads have TLS set to NULL
    else
    {
...
void InformAboutIconOvrlsHanCrash(const char* iconOvrlsHanName)
{
   ...
   we let the script resolve button hotkey collisions by pretending it is a menu
...
*/
        ...
        {
            HKEY hSalamander;
            if (OpenKey(HKEY_CURRENT_USER, SalamanderConfigurationRoots[0], hSalamander)) // write data only if a configuration exists in the registry (which should always be true)
            {
                HKEY actKey;
                if (OpenKey(hSalamander, SALAMANDER_CONFIG_REG, actKey)) // write data only if a configuration exists in the registry (which should always be true)
                {
                    CloseKey(actKey);
...
BOOL CCallStack::CreateBugReportFile(EXCEPTION_POINTERS* Exception, DWORD threadID, DWORD ShellExtCrashID, const char* bugReportFileName)
{
    ...
    // the lower the chance this routine crashes (the libraries may be corrupt
    // since Salamander is already crashing)
    BOOL ret = TRUE;
            ...
            if (file != INVALID_HANDLE_VALUE)
            {
                __try
                {
                    ...
                    if (stack != NULL)
                    {
                        ...
                        stack->DontSuspend = TRUE; // so call-stacked functions can be called
                    }
...
{
    // WARNING - an exception occurred and before signaling salmon.exe to create
    // the minidump we should call only the bare minimum API (the system may be
    // in critical sections, with even more restrictions than inside DllMain)

    // in the DEBUG build we check for a debugger; in SDK/RELEASE builds we skip this test
#if defined(_DEBUG) && !defined(ENABLE_BUGREPORT_DEBUGGING)
    if (IsDebuggerPresent())              // when debugging, showing the Bug Report dialog makes no sense
        return EXCEPTION_CONTINUE_SEARCH; // pass the exception to the debugger
#endif
    ...
    // Although delayed compared to the minidump, this increases the chance of having a valid dump
...
*/
    ...
    TBRData.Exception = e;
    TBRData.ExitProcess = FALSE; // redundant output parameter, but keep it
    ...
    if (BugReportThread != NULL)
    {
        ...
        DWORD waitRet = WaitForSingleObject(TBRData.EventProcessed,
                                            INFINITE); // wait indefinitely so stepping in the debugger doesn't terminate
#else
        DWORD waitRet = WaitForSingleObject(TBRData.EventProcessed,
                                            6000); // give the background thread 6s to create the report
#endif
        if (waitRet != WAIT_OBJECT_0)
            reportInThisThread = TRUE; // on error or timeout try again here
        else
            reportInThisThread = TBRData.EventProcessedRet == FALSE; // if generation failed in the thread, try here too
    }

    // if the bug-report thread didn't run or generation fails there, try in this thread
    if (reportInThisThread)
    {
        // Concurrent generation of two reports caused issues (with a 200 ms
        // timeout both .BUG and .DMP were incomplete) - suspend the background
        // thread
        if (BugReportThread)
            ...
            if (MonitoredItems[act].CallerAddress == callerAddress) // found
            {
                ...
                if (item->MonitoringStartPerfTime >= oldestPerfTime) // we still monitor it
                {
                    ...
                    {                                                                                                                                                                                 // this trace is printed only once (after exceeding the critical time, the final state is printed later)
                        DWORD aproxMinCallsCount = (DWORD)((SpeedBenchmark * totalTime / (CALLSTK_BENCHMARKTIME * CCallStack::SavedPerfFreq.QuadPart / 1000)) / (3 * CALLSTK_MONITOR_SHOWINFORATIO)); // assume the call-stack macro is three times slower than measured
...
#endif                             // (defined(_DEBUG) || defined(CALLSTK_MEASURETIMES)) && !defined(CALLSTK_DISABLEMEASURETIMES)
        stack->Push(format, args); // unintercepted threads have TLS set to NULL
#if (defined(_DEBUG) || defined(CALLSTK_MEASURETIMES)) && !defined(CALLSTK_DISABLEMEASURETIMES)
    ...
    if (stack != NULL)
    {
        ...
        if (internalPushPerfTime > 0) // only if Push was called recursively
        {
            ...
            __int64 realPerfTime = (endTime.QuadPart - StartTime.QuadPart) -
                                   (stack->IgnoredPushPerfTimeCounter.QuadPart - IgnoredPushPerfTimeCounterStart.QuadPart); // subtract ignored (unmeasured) Push times so they don't skew the ratio
                ...
                ((realPerfTime * 1000) / CCallStack::SavedPerfFreq.QuadPart) > CALLSTK_MINWARNTIME)
            {
                DWORD aproxMinCallsCount = (DWORD)(((CCallStack::SpeedBenchmark * (internalPushPerfTime + stack->IgnoredPushPerfTimeCounter.QuadPart - IgnoredPushPerfTimeCounterStart.QuadPart) * 1000) /
                                                   ...
                                                   3); // assume the call-stack macro is three times slower than measured
        ...
        stack->Pop(printCallStackTop); // unintercepted threads have TLS set to NULL
#else                                  // (defined(_DEBUG) || defined(CALLSTK_MEASURETIMES)) && !defined(CALLSTK_DISABLEMEASURETIMES)
        stack->Pop(); // unintercepted threads have TLS set to NULL
#endif                                 // (defined(_DEBUG) || defined(CALLSTK_MEASURETIMES)) && !defined(CALLSTK_DISABLEMEASURETIMES)
    }
    else
    {
