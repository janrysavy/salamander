...
enum CSecurityIconState
{
    ...
    sisUnsecured = 0x01, // Icon of an unlocked lock
    sisSecured = 0x02    // Icon of a locked lock
};
...
protected:
    ...
    char* Text;
    int TextLen; // Number of characters in 'Text' without the terminator
    char* Size;
    ...
    int ThrobberID;                // Throbber identifier (-1 = invalid)
    ...
    RECT SecurityRect; // Rectangle where the lock was drawn
    int MaxTextRight;
    BOOL MouseCaptured;
    BOOL RButtonDown;
    BOOL LButtonDown;
    ...
    DWORD SubTextsCount; // Number of items in SubTexts
...
public:
    ...
    // Sets 'text' into the status line; 'pathLen' specifies the path length (the rest is the filter),
    ...
    void SetThrobber(BOOL show, int delay = 0, BOOL calledFromDestroyWindow = FALSE); // Call only from the main (GUI) thread, just like the other methods
    ...
    BOOL SetDriveIcon(HICON hIcon);     // The icon is copied into the image list - the caller must destroy it
...
protected:
    ...
    // Creates an image list with a single item used to show drag progress
    // After the drag finishes, the image list must be released
    // Input is the point relative to which dxHotspot and dyHotspot are calculated
