...
CSalamanderDirectory GlobalEmptySalDir(FALSE); // returned as an empty sal-dir (instead of NULL) for archives only
...
int CZIPUnpackProgress::AddSize(int size, BOOL delayedPaint)
{
    ...
    if (!delayedPaint)
    {
        // draw the texts immediately
        FlushDataToControls();
    }

    // repaint changed data (text + progress bars) every 100ms
    DWORD ticks = GetTickCount();
    if (ticks - LastTickCount > 100)
    {
        LastTickCount = ticks;
        // if we didn't paint recently, do it now
        if (delayedPaint)
            FlushDataToControls();
    }

    DispatchMessages(); // give user messages a moment
...
void CZIPUnpackProgress::NewLine(const char* txt, BOOL delayedPaint)
{
    ...
    while (1) // output several lines to the dialog
    {
        ...
        // store text in a cache shown on WM_TIMER

        // cache index cycles through entries
        CacheIndex++;
            ...
            else
            {
                if (s < sEnd)
                    *s++ = *txt++;
                    ...
                    txt++; // ignore the rest of the text (it won't fit in the dialog)
            }
        }
        *s = 0;
    ...
    // don't call anything heavy here
    DispatchMessages(); // give user messages a moment
}
...
CZIPUnpackProgress::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        if (ResID == IDD_ZIPUNPACKPROG && FileProgress) // the text "Total:" must be replaced with "File:"
            SetDlgItemText(HWindow, IDT_PROGTITLE, LoadStr(IDS_UNPACKFILEPROGRESS));

        SetWindowText(HWindow, Title);
        // the entire object assumes that allocations might fail
        int i;
        for (i = 0; i < ZIP_UNPACK_NUMLINES; i++)
        {
    ...
    case WM_COMMAND:
    {
        // if the user clicked the Cancel button and hasn't confirmed yet, ask again
        if (LOWORD(wParam) == IDCANCEL && !Cancel)
        {
            // the Cancel button must be enabled
            if (IsWindowEnabled(GetDlgItem(HWindow, IDCANCEL)))
            {
                // repaint explicitly now to avoid drawing under the message box
                FlushDataToControls();

                // ask the user whether to abort the operation
                Cancel = (SalMessageBox(HWindow, LoadStr(IDS_CANCELOPERATION), LoadStr(IDS_QUESTION),
                                        MB_YESNO | MB_ICONQUESTION) == IDYES);
            }
        }
        // do not let the command fall through or the dialog would close
        return TRUE;
    }
    }
...
{
    if (MainThreadID != GetCurrentThreadId()) // Petr: just complain; not going to search where this is called wrongly
...
HWND CSalamanderGeneral::GetMsgBoxParent()
{
    ...
    // if the following code changes, it must also be changed in EnterPlugin so the check works
                ...
                const char* error, const char* title)
{
    ...
    {
        resID = IDD_ERROR3;
        noSkip = FALSE; // not used when resID==0
        break;
    }
                                      ...
                                      char** archiveOrFS, BOOL convertFSPathToExternal)
{
    ...
    {
        char buf[2 * MAX_PATH];
        int offset = -1; // buffer offset for computing archiveOrFS (-1 means NULL)
        ...
        if (l > bufferSize)
            return bufferSize == 0; // if the user doesn't want the path returned, it's not an error
        memcpy(buffer, buf, l);
...
CSalamanderGeneral::GetPanelPluginData(int panel)
{
    ...
    if (p != NULL)
    {
        ...
        if (iface != NULL && p->PluginData.GetPluginInterface() != Plugin)
            iface = NULL; // object belongs to a different plugin -> return nothing
        return iface;
    }
    return NULL;
}
...
CSalamanderGeneral::GetPanelItem(int panel, int* index, BOOL* isDir)
{
    ...
    if (p != NULL && index != NULL)
    {
        ...
        if (i < p->Files->Count + p->Dirs->Count) // enumerate next item
        {
            *index = i + 1; // next call will return the following item
            if (isDir != NULL)
...
BOOL CSalamanderGeneral::GetPanelSelection(int panel, int* selectedFiles, int* selectedDirs)
{
    ...
    if (p != NULL)
    {
        ...
        if (count > 0)
        {
            ...
            int i;
            for (i = 0; i < dirs->Count; i++) // ".." cannot be selected, so the test is redundant
            {
        ...
        return p->Dirs->Count + p->Files->Count > 0 && // panel is not empty
               (i != 0 || count > 0 || p->Dirs->Count == 0 ||
                strcmp(p->Dirs->At(0).Name, "..") != 0); // focus is not on ".." or at least something is selected
    }
    return FALSE;
}
...
CSalamanderGeneral::GetPanelSelectedItem(int panel, int* index, BOOL* isDir)
{
    ...
    if (p != NULL && index != NULL)
    {
        ...
        while (i < p->Files->Count + p->Dirs->Count) // searching for the next selected item
        {
            ...
            {
                *index = i + 1; // next call will continue from the next item
                if (isDir != NULL)
                    *isDir = i < p->Dirs->Count;
        ...
        *index = -1; // enumeration finished, no more selected items
    }
    return NULL;
}
    ...
    if (p != NULL)
    {
        ...
        if (p->Dirs->Count > 0)
        {
            ...
            if (first <= file && file <= last)
                index = (int)(file - first); // it's a directory
        }
        if (index == -1 && p->Files->Count > 0)
        {
            ...
            if (first <= file && file <= last)
                index = p->Dirs->Count + (int)(file - first); // it's a directory
        }
        if (index != -1)
...
// returns the position of the source panel (left or right?), returns PANEL_LEFT or PANEL_RIGHT
...
// activates the other panel (like the TAB key); panels marked via PANEL_SOURCE and PANEL_TARGET
// naturally swap because of this
...
void CSalamanderGeneral::PostUnloadThisPlugin()
{
    ...
    if (MainThreadID == GetCurrentThreadId())
    { // called from an entry point where Plugin is -1 (only to locate plugin data)
        // before WM_USER_POSTCMDORUNLOADPLUGIN arrives, Plugin would be changed according to the entry point result
        CPluginData* data = Plugins.GetPluginData(Plugin);
        if (data != NULL)
        {
    ...
    else // outside the entry point, Plugin is already set...
    {
        ...
        {
            // check for calls during entry point execution (Plugin is set to -1)
            if ((INT_PTR)Plugin == -1)
            {
...
void CSalamanderGeneral::PostMenuExtCommand(int id, BOOL waitForSalIdle)
{
    ...
    if (waitForSalIdle)
    {
        ...
        { // called from an entry point where Plugin is -1 (only to locate plugin data)
            ...
            if (data != NULL)
            {
                data->Commands.Add(500 + id); // salCmd values range from 0 to 499; 500 is the first free number
                ExecCmdsOrUnloadMarkedPlugins = TRUE;
            }
            else
            {
    ...
    {
        if (MainThreadID == GetCurrentThreadId())
        { // check for calls from the entry point (Plugin is set to -1)
            if ((INT_PTR)Plugin == -1)
            {
...
void CSalamanderGeneral::RefreshPanelPath(int panel, BOOL forceRefresh, BOOL focusFirstNewItem)
{
    ...
    if (p != NULL)
    {
        ...
        { // for archives enforce a hard refresh by corrupting the archive marker
...
void CSalamanderGeneral::PostRefreshPanelPath(int panel, BOOL focusFirstNewItem)
{
    ...
    if (p != NULL)
    {
        // post a hard refresh request
        HANDLES(EnterCriticalSection(&TimeCounterSection));
        int t1 = MyTimeCounter++;
        HANDLES(LeaveCriticalSection(&TimeCounterSection));
        p->FocusFirstNewItem = focusFirstNewItem; // not synchronized (may be called outside the main thread), but it shouldn't matter
...
BOOL CSalamanderGeneral::PostRefreshPanelFS2(CPluginFSInterfaceAbstract* modifiedFS, BOOL focusFirstNewItem)
{
    ...
    if (MainWindow != NULL)
    {
        // no synchronization issue because PluginFS is cleared only after CloseFS,
        // which should terminate the thread monitoring FS changes (PostRefreshPanelFS2
        // shouldn't be called after CloseFS)
...
BOOL CSalamanderGeneral::CloseDetachedFS(HWND parent, CPluginFSInterfaceAbstract* detachedFS)
{
    ...
    if (MainWindow->DetachedFSList->IsGood()) // ensure Delete succeeds
    {
        CDetachedFSList* list = MainWindow->DetachedFSList;
        int i;
        for (i = 0; i < list->Count; i++)
        {
            if (list->At(i)->GetInterface() == detachedFS)
            {
                ...
                BOOL dummy;
                if (fs->TryCloseOrDetach(FALSE, FALSE, dummy, FSTRYCLOSE_PLUGINCLOSEDETACHEDFS)) // the FS has no objection to being closed
                {
...
BOOL CSalamanderGeneral::CopyTextToClipboard(const char* text, int textLen, BOOL showEcho, HWND echoParent)
{
    ...
    // j.r.: removed parameter 'text' that didn't have to be null-terminated
    if (text == NULL)
    {
                            ...
                            const char* fileNameInCache, int& error)
{
    ...
    viewUniqueName[0] = 0;
    const char* fileName; // name of the file that will be sent to the viewer
    if (useCache)
    {
        // verify that 'fileNameInCache' is not invalid (name without a path)
        const char* s = NULL;
        ...
        // insert file 'pluginData->FileName' into the disk cache under the name 'fileNameInCache'
        while (1)
        {
            ...
            {
                if (!exists)
                    Sleep(100); // the file exists -> almost impossible, handle it anyway
                else            // fatal error
                {
                    error = 3;
                    ::DeleteFile(pluginData->FileName);
                ...
                break; // name found in the disk cache, all OK
        }
        ...
        else // we obtained the temp file successfully, must call NamePrepared()
        {
            CQuadWord size(0, 0);
            HANDLE file = HANDLES_Q(CreateFile(fileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                               NULL, OPEN_EXISTING, 0, NULL));
            if (file != INVALID_HANDLE_VALUE)
            { // ignore errors; file size is not that important
                DWORD err;
    ...
    // GetWindowPlacement reads the taskbar, so if the taskbar is at the top or left,
    // the values are offset by its size. Correct them.
    RECT monitorRect;
    RECT workRect;
    ...
    // we don't want a minimized viewer, even if the main window is minimized
    ...
    if (pluginSPL != NULL) // viewer from a plug-in
    {
        ...
        if (data != NULL && data->SupportViewer)
        {
            if (data->InitDLL(MainWindow->HWindow)
                /*&& PluginIfaceForViewer.NotEmpty()*/) // pointless; downgrade isn't possible and InitDLL checks the interfaces
            {
                HANDLE lock = NULL;
                ...
                else
                {
                    ...
                    {
                        if (lockOwner) // add the 'lock' handle to HANDLES (disk cache will want to close it)
    ...
    // if we didn't assign a name in the disk cache, release the entry...
    if (useCache && !diskCacheNameClosed)
    {
        DiskCache.ReleaseName(viewUniqueName, FALSE);
        //    ::DeleteFile(fileName);   // the cache already deleted the file and freed fileName
    }
                                                ...
                                                const char* fileNameInCache, int& error)
{
    ...
    // ensure the call doesn't come from outside the main thread or from the entry point
    if (MainThreadID != GetCurrentThreadId() || (INT_PTR)Plugin == -1)
    {
        if (MainThreadID == GetCurrentThreadId()) // if both errors occur (other thread + not yet finished entry point), entry point has priority
...
void CSalamanderGeneral::ExecuteAssociation(HWND parent, const char* path, const char* name)
{
    ...
    MainWindow->SetDefaultDirectories(); // so that the starting process inherits the correct current directories
...
CSalamanderGeneral::SalCheckPath(BOOL echo, const char* path, DWORD err, HWND parent)
{
    ...
    return ::SalCheckPath(echo, path, err, TRUE, parent); // the value of 'postRefresh' doesn't matter (StopRefresh is certainly > 0)
}
    ...
    else
    {
        if (bufferSize > 0 && auxDataSize > 0) // copy at least what fits
        {
            ...
            if (auxType == SALCFGTYPE_STRING)
                ((char*)buffer)[bufferSize - 1] = 0; // terminate strings with a null
        }
        ret = FALSE;
    }
    return ret;
                                          ...
                                          HANDLE fileLock)
{
    ...
    {
        if (!fileExists) // some good soul deleted it directly from disk
        {
            // cannot prepare the file, notify the disk cache that we give up
...
void CSalamanderGeneral::UnlockFileInCache(HANDLE fileLock)
{
    ...
    SetEvent(fileLock); // start cleaning the file
    DiskCache.WaitForIdle();
    ResetEvent(fileLock); // finish cleaning the file
}
                                         ...
                                         const CQuadWord& newFileSize, BOOL* alreadyExists)
{
    ...
    // verify that 'nameInCache' is not invalid (name without a path)
    ...
    // insert file 'newFileName' into the disk cache under the name 'uniqueFileName'
    BOOL exists;
    ...
    else // we obtained the temp file successfully, must call NamePrepared()
    {
        ...
        DiskCache.ReleaseName(uniqueFileName, TRUE); // keep the prepared file in the cache even if it isn't locked
        return TRUE;
    }
}
...
BOOL CSalamanderGeneral::IsANSIText(const char* text, int textLen)
{
    ...
    while (s < end)
    {
        if (*s < ' ' && *s != '\a' && *s != '\b' && *s != '\r' && // *s must not be 0 here because of Unicode files ("0A 00" cannot expand to "0D 0A 00")
            *s != '\f' && *s != '\n' && *s != '\t' && *s != '\v' &&
            *s != '\x1a' && *s != '\x04' && *s != '\x06')
                                              ...
                                              int* type)
{
    ...
    while (SalCommandsArray[index].SalCmd != -1)
    {
        ...
        {
            // command states must be evaluated; SalCommandsArray relies on them
                                                ...
                                                BOOL* enabled, int* type)
{
    ...
    if (index != NULL && *index >= 0 && SalCommandsArray[*index].SalCmd != -1)
    {
        if (*index == 0)
        {
            // we must count command states; SalCommandsArray uses them
            MainWindow->OnEnterIdle();
        }
                                                    ...
                                                    BOOL forDlgCaption)
{
    ...
    if (selectedFiles + selectedDirs <= 1) // one selected item or the focus
    {
        ...
        char formatedFileName[MAX_PATH]; // CFileData::Name is at most MAX_PATH-5 characters - Salamander limitation
...
{
    if (dropTarget != NULL)
    { // this ugly hack removes the activated state without an active application (visually inactive, but WM_ACTIVATEAPP with "activate" already arrived)
        HWND tgtWnd = dropTarget;
        HWND tmp;
        while ((tmp = ::GetParent(tgtWnd)) != NULL && IsWindowEnabled(tmp))
            tgtWnd = tmp;
        if (MainWindow != NULL && tgtWnd != MainWindow->HWindow)
        { // do this only if the operation is outside our Salamander
...
BOOL CSalamanderGeneral::AddPluginFSTimer(int timeout, CPluginFSInterfaceAbstract* timerOwner,
                                          DWORD timerParam) // FIXME_X64 - review Salamander's interface to ensure parameters can hold x64 pointers; for example 'timerParam'
{
...
public:
    virtual HBITMAP WINAPI LoadPNGBitmap(HINSTANCE hInstance, LPCTSTR lpBitmapName, DWORD flags, COLORREF unused)
    {
        ...
        if (hBitmap != NULL) // handle is passed to the plug-in which must destroy it; remove it from Salamander HANDLES
...
void CSalamanderGeneral::DisconnectFSFromPanel(HWND parent, int panel)
{
    ...
    { // change path to the last visited Windows path
        BOOL tryNet = FALSE;
        DWORD err;
        DWORD lastErr;
        BOOL pathInvalid;
                                            ...
                                            pathInvalid, cut, TRUE))
        {
            ...
            if (ChangePanelPathToDisk(panel, buf, &failReason) ||
                failReason != CHPPFR_INVALIDPATH) // except for "invalid path" errors (e.g. refusing to close FS, etc.)
            {
                rescueOrFixed = FALSE;
            }
        }
                                                ...
                                                int menuY, const char* netPath, char* newlyMappedDrive)
{
    ...
    if (p != NULL)
    {
        BeginStopRefresh(); // no refreshes needed (call comes from the plug-in, refreshes are already disabled in EnterPlugin)
        ...
        if (forItems)
        {
            ...
            if (count != 0)
            {
                ...
                p->GetSelItems(count, indexes, TRUE); // deprecated approach (see GetSelItems): context menus start from the focused item and wrap around
            }
            else
            {
        ...
        else
        {
            ...
            if (p->ContextMenu != NULL && h != NULL)
            {
                ...
                // handle pressed Shift - extended context menu; under W2K it includes Run as...
#define CMF_EXTENDEDVERBS 0x00000100 // rarely used verbs
                ...
                int cmd = 0;
                if (GetMenuItemCount(h) > 0) // safeguard against a completely trimmed menu
                {
                ...
                if (cmd != 0)
                {
                    ...
                    char cmdName[2000]; // deliberately 2000 instead of 200; shell extensions sometimes write twice as much (consider Unicode = 2 * "number of characters")
                        ...
                        forItems && netPath[2] != 0)
                    {
                        ...
                        {
                            char newDrive = 0;
                            p->ConnectNet(TRUE, root, FALSE /* called from a plugin; we must not change the panel path or we'd
                                                 return to a deallocated FS object */
                                          ,
                                          &newDrive);
                            if (newlyMappedDrive != NULL)
                    ...
                    else
                    {
                        ...
                        IdleRefreshStates = TRUE;  // on next Idle force status variables check
...
void CSalamanderGeneral::SetPluginIconOverlays(int iconOverlaysCount, HICON* iconOverlays)
{
    ...
    if (MainThreadID != GetCurrentThreadId())
    { // call error: we don't free icons, we don't care...
                                                     ...
                                                     BOOL fileLockOwner, BOOL removeAsSoonAsPossible)
{
    ...
    if (!fileExists) // newly downloaded file copy
    {
        if (newFileOK) // download succeeded
        {
            DiskCache.NamePrepared(uniqueFileName, newFileSize);
        }
        else
        {
            DiskCache.ReleaseName(uniqueFileName, FALSE); // download failed, nothing to cache
            return;                                       // nothing more to handle
        }
    }

    if (fileLock != NULL) // we have a viewer "lock" object; link the viewer with the disk cache
    {
        DiskCache.AssignName(uniqueFileName, fileLock, fileLockOwner,
                             (fileExists || removeAsSoonAsPossible) ? crtDirect : crtCache); // for files already in the disk cache we use crtDirect because it doesn't affect the "lifetime" setting (it stays as the file author intended)
    }
    else // viewer didn't open or just lacks the "lock" object
    {
        DiskCache.ReleaseName(uniqueFileName, !fileExists && !removeAsSoonAsPossible); // if 'removeAsSoonAsPossible' is FALSE, try to leave a copy in the disk cache (for a new file we don't change its "lifetime")
    }
}
...
{
    Clear(NULL); // plug-in data are released only in the root Salamander directory
    FreeAddCache();
}
...
void CSalamanderDirectory::AllocAddCache()
{
    if (AddCache == NULL)
    {
        ...
        // if cache allocation fails it's fine; we work fully without it
    }
}
...
// 's' - output: points past the first name in 'path'
// 'i' - output: index of the found subdirectory (which should process 's')
// 'file' - input: source data for creating the directory if needed
// 'pluginData' - input: interface for creating plug-in specific data for the new directory (if needed)
// 'archivePath' - input: full path in the archive ('path' and 's' point into it)
                                   ...
                                   CPluginDataInterfaceAbstract* pluginData, const char* archivePath)
{
    ...
    if (i == Dirs.Count) // we must create it
    {
        CFileData data;
        //--- name
        data.Name = (char*)malloc((s - path) + 1); // allocate
        if (data.Name == NULL)
        {
            TRACE_E(LOW_MEMORY);
            return FALSE;
        }
        memcpy(data.Name, path, s - path); // copy the text
        data.Name[s - path] = 0;
        data.NameLen = s - path;
        //--- extension
        if (!Configuration.SortDirsByExt)
            data.Ext = data.Name + data.NameLen; // directories have no extension
        else
        {
            const char* ss = s;
            while (--ss >= path && *ss != '.')
                ;
            if (ss >= path)
                data.Ext = data.Name + (ss - path + 1); // in Windows, ".cvspass" is considered an extension ...
        ...
        data.Archive = 0;
        data.SizeValid = 0;
        data.Dirty = 0; // optional, just for completeness
        data.CutToClip = 0;
        ...
        //--- adding a salamander directory corresponding to the new directory
        /*
    CSalamanderDirectory *dir = new CSalamanderDirectory(IsForFS, ValidData, Flags);
...
BOOL CSalamanderDirectory::AddFile(const char* path, CFileData& file, CPluginDataInterfaceAbstract* pluginData)
{
    ...
    // zero out variables the plug-in doesn't define
    ...
    file.Archive = 0;
    file.SizeValid = 0;
    file.Dirty = 0; // optional, just for completeness
    file.CutToClip = 0;
    file.IconOverlayDone = 0;

    // if we cached the path during the previous add, we can insert the file directly in its place
    if (path != NULL && AddCache != NULL && pathLen > 0 &&
        pathLen == AddCache->PathLen && memcmp(path, AddCache->Path, pathLen) == 0)
    {
        // the cache contained our path, add directly
    ...
    // if the addition succeeded and we're using the cache, remember the path
...
BOOL CSalamanderDirectory::AddDir(const char* path, CFileData& dir, CPluginDataInterfaceAbstract* pluginData)
{
    ...
    dir.Archive = 0;
    dir.SizeValid = 0;
    dir.Dirty = 0; // optional, just for completeness
    dir.CutToClip = 0;
...
CSalamanderDirectory::GetSalDir(int i) const
{
        ...
        if (i >= 0 && i < SalamDirs.Count)
    {
        ...
        if (salDir == NULL)
            salDir = &GlobalEmptySalDir; // empty directory -> return the global empty directory
        return salDir;
    }
    else return NULL;
}
        ...
        if (path != NULL)
    {
        ...
        if (*path != 0) // not this directory, find the subdirectory
        {
            ...
            if (salDir != NULL ||                    // already allocated
                (salDir = AllocSalamDir(i)) != NULL) // or we managed to allocate a new object
            {
                                ...
                                CPluginDataInterfaceAbstract* pluginData, const char* archivePath)
{
    ...
    BOOL newDir = TRUE;
    if ((Flags & SALDIRFLAG_IGNOREDUPDIRS) == 0) // if we should check for duplicate directories
    {
        int i;
        for (i = 0; i < Dirs.Count; i++)
        {
        ...
        if (!newDir)                // updating existing data
        {
            ...
            if (Dirs[i].Name != NULL)
                free(Dirs[i].Name);
            Dirs[i].Name = dir.Name; // better use the new name (for possible data after '\0' in the string)
            Dirs[i].Ext = dir.Ext;
            ...
            // the rest of Dirs[i] should be zeroed just like the rest of 'dir'
        }
    }
    if (newDir)
    {
...
*/
        if (IsForFS && dir.NameLen == 2 && dir.Name[0] == '.' && dir.Name[1] == '.')
        {
            ...
            { // we already have one up-dir
                TRACE_E("CSalamanderDirectory::AddFile(): you can add up-dir (\"..\") at most once!");
                return NULL;
            }
            SalamDirs.Insert(0, NULL); // insert NULL (the object will be allocated when first needed)
            if (!SalamDirs.IsGood())
            {
                SalamDirs.ResetState();
                return NULL;
            ...
            for (i = 0; i < Dirs.Count; i++)
            {
                if (SalDirStrCmpEx(Dirs[i].Name, Dirs[i].NameLen, path, (int)(s - path)) == 0)
                {
                        ...
                        (salDir = AllocSalamDir(i)) != NULL) // or we managed to allocate a new object
                    {
                        ...
                        return NULL; // low memory error (as if the directory didn't exist)
                }
            }
        }
        else
    ...
    if (path != NULL)
    {
        ...
        if (*path != 0) // some subdirectory
        {
            ...
            for (i = 0; i < Dirs.Count; i++)
            {
                ...
                {
                    if (*s == 0 || *(s + 1) == 0)
                        return &Dirs[i]; // last path component = the desired parent directory
                    else
                    {
                        CSalamanderDirectory* salDir = SalamDirs[i];
                        if (salDir != NULL ||                    // already allocated
...
CSalamanderDirectory::GetSize(int* dirsCount, int* filesCount, TDirectArray<CQuadWord>* sizes)
{
    ...
    for (i = 0; i < Files.Count; i++)
    {
        size += Files[i].Size;
        if (sizes != NULL)
            sizes->Add(Files[i].Size); // add failure is handled in the output dialog
    }
    if (filesCount != NULL)
            ...
            for (i = 0; i < Dirs.Count; i++)
            {
                if (SalDirStrCmpEx(Dirs[i].Name, Dirs[i].NameLen, path, (int)(s - path)) == 0)
                {
                    ...
                    if (salDir != NULL)
                        ...
                        return CQuadWord(0, 0); // contains nothing; otherwise it would have been allocated
                }
            }
        }
        else
        ...
        if (*path != 0) // some subdirectory
        {
            ...
            for (i = 0; i < Dirs.Count; i++)
            {
                if (SalDirStrCmpEx(Dirs[i].Name, Dirs[i].NameLen, path, (int)(s - path)) == 0)
                {
                    ...
                    {
                        if (readOnly)
                            return &GlobalEmptySalDir; // read-only access -> return the global empty directory
                        else                           // for write access
                        {
