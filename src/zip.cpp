...
{
    if (MainThreadID != GetCurrentThreadId()) // Petr: just yell; I do not have the energy to track down every wrong call
...
HWND CSalamanderGeneral::GetMsgBoxParent()
{
    ...
    // if the following code changes, it must also be updated in EnterPlugin so the check keeps working
    ...
    if (p != NULL)
    {
        ...
        return p->Dirs->Count + p->Files->Count > 0 && // the panel is not empty
               (i != 0 || count > 0 || p->Dirs->Count == 0 ||
                strcmp(p->Dirs->At(0).Name, "..") != 0); // the focus is not on the parent directory (".."), or at least one item is selected
    }
    return FALSE;
}
...
void CSalamanderGeneral::SelectPanelItem(int panel, const CFileData* file, BOOL select)
{
    ...
    if (p != NULL)
    {
        int index = -1; // index of 'file' within the panel
        if (p->Dirs->Count > 0)
        {
...
void CSalamanderGeneral::PostUnloadThisPlugin()
{
    ...
    else // outside the entry point the Plugin is certainly set...
    {
        ...
        {
            // check for a call while the entry point is running (Plugin is set to -1)
            if ((INT_PTR)Plugin == -1)
            {
...
BOOL CSalamanderGeneral::CopyTextToClipboard(const char* text, int textLen, BOOL showEcho, HWND echoParent)
{
    ...
    // j.r. removed the text parameter, which did not have to be null-terminated
    if (text == NULL)
    {
                            ...
                            const char* fileNameInCache, int& error)
{
    ...
    if (useCache)
    {
        // verify that 'fileNameInCache' (a name without path) is valid
        const char* s = NULL;
    ...
    // do not want a minimized viewer, even if the main window is minimized
        ...
        if (data != NULL && data->SupportViewer)
        {
                ...
                /*&& PluginIfaceForViewer.NotEmpty()*/) // redundant, because downgrade is impossible and InitDLL checks the interfaces
            {
                ...
                else
                {
                    ...
                    {
                        if (lockOwner) // add the handle for 'lock' to HANDLES (the disk cache will want to close it and will look it up)
    ...
    else
    {
        if (bufferSize > 0 && auxDataSize > 0) // copy at least what fits
        {
            ...
            if (auxType == SALCFGTYPE_STRING)
                ((char*)buffer)[bufferSize - 1] = 0; // trim the string with a null terminator
        }
        ret = FALSE;
    }
    return ret;
                                         ...
                                         const CQuadWord& newFileSize, BOOL* alreadyExists)
{
    ...
    // verify that 'nameInCache' (a name without a path) is valid
...
public:
    virtual HBITMAP WINAPI LoadPNGBitmap(HINSTANCE hInstance, LPCTSTR lpBitmapName, DWORD flags, COLORREF unused)
    {
        ...
        if (hBitmap != NULL) // the handle is handed over to the plug-in; the plug-in is responsible for destroying it, so remove it from Salamander HANDLES
...
#define CMF_EXTENDEDVERBS 0x00000100 // rarely used verbs
                ...
                if (cmd != 0)
                {
                        ...
                        forItems && netPath[2] != 0)
                    {
                        ...
                        {
                            char newDrive = 0;
                            p->ConnectNet(TRUE, root, FALSE /* called from a plug-in; must not change the panel path, otherwise we would return to a deallocated FS object */
                                          ,
                                          &newDrive);
                            if (newlyMappedDrive != NULL)
                                                     ...
                                                     BOOL fileLockOwner, BOOL removeAsSoonAsPossible)
{
    ...
    {
        if (newFileOK) // the download succeeded
        {
            DiskCache.NamePrepared(uniqueFileName, newFileSize);
        }
        else
