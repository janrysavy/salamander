            ...
            else
            {
                if (size != NULL)
                {
                    *size = f->Size;
                    ...
                    { // the size of the linked target file must be taken from data->DiskDirectoryTree
            ...
            while (1)
            {
                ...
                else
                {
                    ...
                    else // we are at the end of a directory -> exit
                    {
                        ...
                        else // we definitely exited the tree
                        {
                            dir = "";
                        ...
                        if (strlen(dir) == strlen(curZIPPath))
                        {
                            ...
                            data->CurrentIndex++; // we already processed this directory
...
void CFilesWindow::UnpackZIPArchive(CFilesWindow* target, BOOL deleteOp, const char* tgtPath)
{
    ...
    BeginStopRefresh(); // sniffer thread takes a break

    //---  obtain the files and directories to work with
    char subject[MAX_PATH + 100]; // text for the Unpack dialog (what is being unpacked)
    ...
    if (data.IndexesCount > 1) // valid selection
    {
        ...
        {
            TRACE_E(LOW_MEMORY);
            EndStopRefresh(); // sniffer thread resumes now
            return;
        }
        else
        {
    ...
    char changesRoot[MAX_PATH]; // directory from which changes on disk are relevant
    changesRoot[0] = 0;
    ...
    if (!deleteOp) // copy
    {
        ...
        if (target != NULL && target->Is(ptDisk))
        {
            ...
            target->UserWorkedOnThisPath = TRUE; // default action = operate on the path in the target panel
        }
        else
            path[0] = 0;
    ...
    _DLG_AGAIN:
        ...
        if (tgtPath != NULL || dlg.Execute() == IDOK)
        {
            ...
            //---  for disk paths convert '/' to '\\' and drop duplicate '\\'
            ...
            //---  adjust the entered path -> absolute, without '.' or '..'
            ...
            BOOL mustBePath = (len == 2 && LowerCase[path[0]] >= 'a' && LowerCase[path[0]] <= 'z' &&
                               path[1] == ':'); // a path like "c:" must remain a directory even after expansion (not a file)
            ...
            {
                // instead of a 'switch', use 'if' so that 'break' and 'continue' work
                if (pathType == PATH_TYPE_WINDOWS) // Windows path (disk + UNC)
                {
                    char newDirs[MAX_PATH]; // if a directory is being created for the operation, remember its name (can be removed on error)
                    newDirs[0] = 0;

                    if (pathIsDir) // the existing part of the path is a directory
                    {
                        if (*secondPart != 0) // some of the path does not exist
                        {
                            ...
                            // determine the original path (where new directories started)
                            ...
                            if (!ok)
                            {
                                ...
                                // if directory creation failed, report changes immediately (the user may
            ...
            //---  refresh directories that are not refreshed automatically
            // changes on the target path and its subdirectories (creating new directories and extracting
            // files/directories)
            MainWindow->PostChangeOnPathNotification(changesRoot, TRUE);
            // change in the directory containing the archive (should not occur during unpack, but refresh just in case)
            MainWindow->PostChangeOnPathNotification(GetPath(), FALSE);
        }
    }
    else // delete
    {
        //---  ask whether the user really means it
                        ...
                        int* containsDirLinks, char* linkName)
{
    ...
    *end = 0; // restore the path
    ...
    else
    {
        ...
        if (dir != NULL)
        {
            newF.PluginData = -1; // -1 just like that, ignored
            newF.Association = 0;
            newF.Selected = 0;
            newF.Shared = 0;
            newF.Archive = 0;
            newF.SizeValid = 0;
            newF.Dirty = 0; // unnecessary, just for form's sake
            newF.CutToClip = 0;
        ...
        do
        {
            ...
            if (containsDirLinks != NULL && GetTickCount() - lastBreakCheck > 200)
            {
                ...
                {
                    *containsDirLinks = 2; // after interruption simulate an error so the search ends immediately
                    ok = FALSE;
            ...
            if (dir != NULL)
            {
                ...
                {                                                                // for a symlink determine the target file size
                    ...
                    { // only if the path is not too long (otherwise the error is shown during enumeration)
                ...
                if (!Configuration.SortDirsByExt && (file.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) // a directory, definitely on disk
                {
            ...
            if (file.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) // directory, definitely on disk
            {
                ...
                if (dir != NULL)
                {
                    ...
                    if (addDirOK)
                        salDir = dir->GetSalamanderDir(newF.Name, FALSE); // allocate a sal-dir for writing
                    else
                    {
                        free(newF.Name);
                        if (newF.DosName != NULL)
                ...
                else // we are only looking for the first directory link
                {
                    if ((file.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) != 0)
                    {
                        ...
                        *end2 = 0;
                        _snprintf_s(linkName, MAX_PATH, _TRUNCATE, "%s\\%s", path, file.cFileName); // truncation is fine, it is only message text
                        ok = FALSE;
                        testFindNextErr = FALSE;
                        break;
                    }
                                        ...
                                        BOOL getLinkTgtFileSize, int* containsDirLinks, char* linkName)
{
    ...
    while (index < data->IndexesCount)
    {
        ...
        // to be safe skip ".." (there was a bug report in "1.6 beta 5" about this; I do not understand how it could happen)
        ...
        if (isDir) // directory
        {
            ...
            if (dir != NULL)
            {
                ...
                if (addDirOK)
                {
                    newF.Name = NULL; // already in dir, must not call free() on it - if an error occurs later
                    newF.DosName = NULL;
                    salDir = dir->GetSalamanderDir(f->Name, FALSE); // allocate a sal-dir for writing
                }
                if (salDir == NULL)
                    goto RETURN_ERROR;
            }
            else
            {
                if ((f->Attr & FILE_ATTRIBUTE_REPARSE_POINT) != 0) // directory link found, stop...
                {
                    *containsDirLinks = 1;
                    ...
                    _snprintf_s(linkName, MAX_PATH, _TRUNCATE, "%s\\%s", path, f->Name); // truncation is fine, it is only message text
                    break;
                }
            }
...
void CFilesWindow::Pack(CFilesWindow* target, int pluginIndex, const char* pluginName, int delFilesAfterPacking)
{
    ...
    if (nameByItem) // if only one item is selected, the archive inherits its name
    {
    ...
    // if nothing is selected, choose the focused item and store its name
    ...
    if (delFilesAfterPacking == 0 || // Petr: changed default - user must always enable deletion, it is too dangerous
    ...
    if (dlg.Execute() == IDOK)
    {
        ...
        if (SalGetFullName(fileBuf, &errTextID, Is(ptDisk) ? GetPath() : NULL, nextFocus))
        {
            ...
            if (PackerConfig.Move)
            {
                ...
                // handling cases where packing fails for a file inside a linked directory is impossible
                PackerConfig.Move = FALSE;
                goto _PACK_AGAIN;
            }
        ...
        //--- confirmation for adding (updating) an existing archive
...
void CFilesWindow::Unpack(CFilesWindow* target, int pluginIndex, const char* pluginName, const char* unpackMask)
{
    ...
    if (i >= Dirs->Count && i < Dirs->Count + Files->Count)
    {
        ...
        else // choose the unpacker based on the extension
        {
            ...
            { // if none is preferred, pick the first one (so users do not stare at an empty combo)
        ...
        if (CUnpackDialog(HWindow, path, pathAlt, mask, &str, &UnpackerConfig, &delArchiveWhenDone).Execute() == IDOK)
        {
            ...
            if (text == NULL)
            {
                ...
                if (CheckAndCreateDirectory(path, NULL, TRUE, NULL, 0, newDir, FALSE, TRUE))
                {
                    ...
                    else // extraction succeeded (no Cancel, Skip may have occurred)
                    {
                                    ...
                                    while (1)
                                    {
                                        ...
                                        if (!DeleteFile(name) && !skipAll)
                                        {
                                            ...
                                            if (res == IDCANCEL) // cancel
                                            {
                                                ...
                                                nextFocus[0] = 0; // keep the cursor on the archive, do not jump to the unpacked directory
                                                break;
                                            }
                    ...
                    {
                        // changes in the directory where files were extracted
...
void CFilesWindow::CalculateOccupiedZIPSpace(int countSizeMode)
{
    ...
    if (Is(ptZIPArchive) && (ValidFileData & VALID_DATA_SIZE)) // only if CFileData::Size is valid (sizes defined via plugin data are unlikely for archives, so we ignore them here)
    {
        BeginStopRefresh(); // sniffer thread will wait
        ...
        // sort if we counted more than one selected directory or all of them
...
void CFilesWindow::AcceptChangeOnPathNotification(const char* path, BOOL includingSubdirs)
{
    ...
    if ((Is(ptDisk) || Is(ptZIPArchive)) && (!AutomaticRefresh || GetNetworkDrive()))
    {
        ...
        // FS paths in 'path' are excluded automatically because they can never match GetPath())
        char path1[MAX_PATH];
        char path2[MAX_PATH];
    ...
    if (Is(ptDisk) && !refresh &&           // only disks have free space (archives do not and FS is handled elsewhere)
        HasTheSameRootPath(path, GetPath()) // same root -> possible change in free disk space

        /* && (!AutomaticRefresh ||  // commented out because notifications do not come for subdirectory changes on auto-refreshed paths and the free-space value stayed stale
       !IsTheSamePath(path, GetPath()))*/
        ) // this path is not monitored for changes -> refresh definitely will not arrive
    {
...
void CFilesWindow::IconOverlaysChangedOnPath(const char* path)
{
    ...
    if ((int)(GetTickCount() - NextIconOvrRefreshTime) >= 0 &&             // refresh of icon overlays occurs at NextIconOvrRefreshTime; before that it makes no sense to track changes
        !IconOvrRefreshTimerSet && !NeedIconOvrRefreshAfterIconsReading && // icon overlay refresh not yet scheduled
        ...
        IsTheSamePath(path, GetPath()))
    {
        ...
        // try to refresh immediately (provided it did not come too soon after the previous)
