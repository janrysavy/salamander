...
// structure with the request and enumeration results
...
BOOL CALLBACK CloseAllOwnedEnabledDialogsEnumProc(HWND wnd, LPARAM lParam)
{
    ...
    return TRUE; // walk through all windows; a single owner can have multiple dialogs side by side
}
                          ...
                          BOOL* isFileSelected, BOOL select)
{
    ...
    if (FileNamesEnumDone == NULL)
        return FALSE; // this probably never happens, but we still handle it (error: "source does not exist")
    ...
    if (OpenClipboard(hWindow))
    {
        ...
        if (handle != NULL)
        {
            ...
            if (textW != NULL)
            {
                ...
                // for example Miranda appends additional Unicode data behind the text, so we
                // prefer to find the end of the null-terminated string ourselves, without crossing
                // but not beyond the end of the allocated memory block
                const WCHAR* s = textW;
...
HWND CShellExecuteWnd::Create(HWND hParent, const char* format, ...)
{
    ...
    if (HWindow != NULL)
        ...
        return hParent; // in case of an error return at least the correct parent
}
...
CShellExecuteWnd::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY:
    {
        ...
        // now ensure this thread gets stuck until the bug report dialog is closed
...
/*
    ...
    // give the security descriptor a NULL DACL by passing TRUE with a null ACL pointer here
...
if (windowsVistaAndLater) // FIXME: I have not encountered a situation on Vista where I had to deal with integrity levels (between AsAdmin / regular applications)
{
  ...
  ConvertStringSecurityDescriptorToSecurityDescriptor(
      ...
      &pSD,
      NULL);
  ACL* saclPtr = NULL;                // not allocated
  ...
  GetSecurityDescriptorSacl(
      ...
      &saclPtr,
      &fSaclDefaulted);
  SetSecurityDescriptorSacl(secAttr.lpSecurityDescriptor, TRUE, saclPtr, FALSE);
}
*/
...
LONG SalRegQueryValue(HKEY hKey, LPCSTR lpSubKey, LPSTR lpData, PLONG lpcbData)
{
    ...
    if (ret == ERROR_SUCCESS && lpData != NULL)
    {
        ...
        {
            if ((DWORD)*lpcbData < dataBufSize) // only values of type REG_SZ and REG_EXPAND_SZ reach this point, so one null terminator is enough
            {
