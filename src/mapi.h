...
protected:
    ...
    CQuadWord TotalSize;           // total size of files (from FileNames)

public:
    CSimpleMAPI();
    ~CSimpleMAPI();

    // tries to load mapi32.dll and link required functions
    ...
    // increases TotalSize by the file size and stores its own copy of the name
    // returns TRUE on success, otherwise FALSE
    // the array is cleared when Release is called
...
// creates a new thread that calls mapi->SendMail(); this keeps the message
// window modeless
// returns TRUE if the thread could be created, otherwise FALSE
// whether successful or not, it deletes the 'mapi' pointer
//
// CSimpleMAPI does not protect data access with critical sections, so use the
// function this way:
// 1) allocate a CSimpleMAPI instance in the main thread
// 2) add files to the allocated instance using AddFile
