...
BOOL GetGoogleDrivePath(char* gdPath, int gdPathMax, CSQLite3DynLoadBase** sqlite3_Dyn_InOut, BOOL* pathIsFromConfig)
{
    ...
    *pathIsFromConfig = FALSE;
    ...
    WCHAR widePath[MAX_PATH]; // we cannot handle longer paths anyway
    char mbPath[MAX_PATH];    // ANSI or UTF-8 path
    ...
    if (SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, 0 /* SHGFP_TYPE_CURRENT */, sDbPath) == S_OK)
    {
        ...
        if (pathOK)
        {
            ...
            if (sqlite3_Dyn->OK)
            {
                sqlite3* pDb;
                sqlite3_stmt* pStmt;
                char utf8Select[] = "SELECT data_value FROM data WHERE entry_key = 'local_sync_root_path';"; // UTF-8 string (any extra character must be converted from ANSI to UTF-8)

                // sqlite3_open_v2 requires a UTF-8 path, so convert from ANSI to UTF-8
                    ...
                    ConvertU2A(widePath, -1, mbPath, _countof(mbPath), FALSE, CP_UTF8))
                {
                    ...
                    if (!iSts)
                    {
                        ...
                        if (!iSts)
                        {
                            ...
                            TRACE_I("SQLite: cannot prepare " << sDbPath); // happens when GD is installed but the user is "not signed in"
                    }
                    else
            ...
            if (sqlite3_Dyn_InOut != NULL)
                *sqlite3_Dyn_InOut = sqlite3_Dyn; // return the loaded sqlite.dll for reuse (it might have been loaded before the call)
            else
                delete sqlite3_Dyn; // unload sqlite.dll; no one is waiting for it
        }
        else
...
/*
// returns the module (DLL) containing the given function address
// (gets DLL module handle for the specified function address)
// if we care about the currently running code, this MS-specific trick also works:
...
void InitShellIconOverlaysAuxAux(CLSID* clsid, const char* name)
{
    ...
    if (CoCreateInstance(*clsid, NULL,
        ...
        iconOverlayIdentifier != NULL) // probably redundant, just to be safe
    {
        ...
        if (iconOverlayIdentifier->GetOverlayInfo(iconFile, MAX_PATH, &iconIndex, &flags) == S_OK)
        {
            ...
            {
                int priority; // priority: handlers with the lowest number are queried first
                ...
                if (iconOverlay[ICONSIZE_16] != NULL && iconOverlay[ICONSIZE_32] != NULL && iconOverlay[ICONSIZE_48] != NULL)
                {
                    ...
                    {
                        isGoogleDrive = TRUE;
                        // Google Drive handlers are called only for subdirectories where Google Drive resides
        ...
        TRACE_I("InitShellIconOverlays(): unable to create object for: " << name); // e.g. "Offline Files" reports this on a clean XP
}
...
void InitShellIconOverlays()
{
                                         ...
                                         0, KEY_ENUMERATE_SUB_KEYS, &key))) == ERROR_SUCCESS)
    {
        ...
        DWORD i = 0;
        while (1)
        { // enumerate icon overlay handlers one by one
            FILETIME dummy;
            DWORD nameLen = MAX_PATH;
            if ((errRet = RegEnumKeyEx(key, i, name, &nameLen, NULL, NULL, NULL, &dummy)) == ERROR_SUCCESS)
            {
                int s = 0; // insert the new name; about 15 items so no quick-sort needed
        ...
        // walk through the sorted list of icon overlay handlers (Explorer orders handlers alphabetically)
        // take only the first 15; Explorer even uses only the first 11
                        ...
                        if (CLSIDFromString(oleTxtClsId, &clsid) == NOERROR)
                        {
                            ...
                            if (clsIDKey != NULL)
                            {
                                ...
                                if ((errRet = HANDLES_Q(RegOpenKeyEx(clsIDKey, txtClsId, 0, KEY_QUERY_VALUE, &classKey))) == ERROR_SUCCESS)
                                {
                                    ...
                                    {
                                        if (errRet != ERROR_FILE_NOT_FOUND) // this error is reported when a handler has no description (obviously not an error, on Vista it affects e.g. Offline Files)
                                        {
                                ...
                                {
                                    // Petr: After the 30.8.2015 Google Drive update the CLSID key
                                    //       for GDriveSharedOverlay was missing in the registry.
                                    //       No difference in overlay display compared to Explorer was found,
                                    //       so this annoying message was avoided by removing the
                                    //       GDriveSharedOverlay entry from ShellIconOverlayIdentifiers
...
BOOL IsNameInListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    {
        static char buf[MAX_PATH]; // also used from Bug Report; avoid stack overhead
        ...
        while (*s != 0)
        {
            ...
            if (d >= end)
            {
                d = end - 1; // for long names (should not happen) overwrite the last character (null terminator)
...
BOOL AddToListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    static char n[2 * MAX_PATH]; // also used from Bug Report; avoid stack overhead
    char* d = n;
    const char* s = name;
    while (*s != 0)
    {
...
CShellIconOverlayItem::~CShellIconOverlayItem()
{
    // Visual C++ 2015 disliked the __try / __except block in the destructor; the x64 linker complained:
    // error LNK2001: unresolved external symbol __C_specific_handler_noexcept
    // moving it into a function solved the problem
    Cleanup();
}
...
BOOL CShellIconOverlays::Add(CShellIconOverlayItem* item /*, int priority*/)
{
    ...
    // sorting by priority is pointless; MS states it is used only if other prioritization methods fail
    // in practice handlers are processed alphabetically, priority is probably used only for this:
    // link, share and slow file overlays have priority 10, so for such files only overlays with
    // a higher priority (lower number than 10) are taken
    /*
  int i;
...
void CreateIconReadersIconOverlayIdsAuxAux(CLSID* clsid, const char* name, IShellIconOverlayIdentifier** ids, int i)
{
    ...
    if (CoCreateInstance(*clsid, NULL, CLSCTX_INPROC_SERVER, IID_IShellIconOverlayIdentifier,
        ...
        iconOverlayIdentifier != NULL) // probably redundant, just to be safe
    {
        // call the usual methods just for formality (as if we were Explorer showing the overlays)
        OLECHAR iconFile[MAX_PATH];
        int iconIndex;
        DWORD flags;
...
void CreateIconReadersIconOverlayIdsAux(CLSID* clsid, const char* name, IShellIconOverlayIdentifier** ids, int i)
{
    ...
    __except (CCallStack::HandleException(GetExceptionInformation(), -1, name))
    {
        ...
        //    ExitProcess(1);
        TerminateProcess(GetCurrentProcess(), 1); // harsher exit (this still performs some actions)
    }
}
                               ...
                               int i, WCHAR* wPath, const char* name, DWORD shAttrs)
{
    ...
    {
        if (res != S_FALSE && res != 0x80070002) // 0x80070002 means "file not found"; "Offline Files" returns it for everything that is not offline-available
                            ...
                            int i, WCHAR* wPath, const char* name, DWORD shAttrs)
{
    ...
    return FALSE; // only for the compiler
}
                                        ...
                                        BOOL isGoogleDrivePath)
{
    ...
    wPath[MAX_PATH - 1] = 0; // just to be sure
    strcpy(aName, name);
    ...
    // the second one deallocates and causes heap corruption (hard to say why, their bug);
    // the critical section slows down reading in both panels simultaneously by about 2x,
    // so we try to use it only when GD is active (in its directory)
    BOOL isGD_CS_entered = FALSE;
    for (int i = 0; i < Overlays.Count; i++)
    {
        CShellIconOverlayItem* overlay = Overlays[i];
        if (overlay->Priority > minPriority)
            continue; // usage: link, share and slow-file overlays have priority 10, so we use only overlays with a higher priority (lower number than 10)
        ...
        if (overlay->GoogleDriveOverlay)
        {
            if (!isGoogleDrivePath)
                continue; // call Google Drive handlers only for their directory and subdirectories (they are slow and crash without extra synchronization)
            if (!isGD_CS_entered)
            {
...
void CShellIconOverlays::InitGoogleDrivePath(CSQLite3DynLoadBase** sqlite3_Dyn_InOut, BOOL debugTestOverlays)
{
    ...
    { // test only if we will advertise Google Drive on the toolbar and in the change drive menu
        firstCall = FALSE;
        BOOL found = FALSE;
        for (int j = 0; j < Overlays.Count; j++)
        {
        ...
        if (!found)
        {
            // Google Drive installs a version according to Windows (x86 / x64). Thus Salamander x86
            // on x64 Windows (and vice versa) will not find GD icon handlers and that is not an error.
#ifdef _WIN64
            if (Windows64Bit)
#else  // _WIN64
            if (!Windows64Bit)
