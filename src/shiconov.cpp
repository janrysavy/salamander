...
BOOL GetGoogleDrivePath(char* gdPath, int gdPathMax, CSQLite3DynLoadBase** sqlite3_Dyn_InOut, BOOL* pathIsFromConfig)
{
    ...
    *pathIsFromConfig = FALSE;
    ...
    if (SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, 0 /* SHGFP_TYPE_CURRENT */, sDbPath) == S_OK)
    {
        ...
        if (pathOK)
        {
            // load it only if sqlite.dll has not already been loaded
            ...
            if (sqlite3_Dyn->OK)
            {
                sqlite3* pDb;
                sqlite3_stmt* pStmt;
                char utf8Select[] = "SELECT data_value FROM data WHERE entry_key = 'local_sync_root_path';"; // UTF-8 string (if any extra character is added, it must be converted ANSI->UTF-8)
                            ...
                            if (iSts == SQLITE_ROW)
                            {
                                    ...
                                    (int)strlen(mbPath) < gdPathMax)
                                {
                                    ...
                                    *pathIsFromConfig = TRUE;
                                    ...
                                    TRACE_E("SQLite: cannot get value (or value too big or not convertible to an ANSI string) from " << sDbPath);
                            }
                            else
            ...
            if (sqlite3_Dyn_InOut != NULL)
                *sqlite3_Dyn_InOut = sqlite3_Dyn; // return the loaded sqlite.dll for reuse (it may have been loaded before this function was called)
            else
...
void InitShellIconOverlaysAuxAux(CLSID* clsid, const char* name)
{
    ...
    if (CoCreateInstance(*clsid, NULL,
        ...
        iconOverlayIdentifier != NULL) // probably redundant, just playing it safe
    {
...
void InitShellIconOverlaysAux(CLSID* clsid, const char* name)
{
    ...
    __except (CCallStack::HandleException(GetExceptionInformation(), -1, name))
    {
        ...
        //    ExitProcess(1);
        TerminateProcess(GetCurrentProcess(), 1); // forceful exit (this still calls a few things)
    }
}
...
void InitShellIconOverlays()
{
                                         ...
                                         0, KEY_ENUMERATE_SUB_KEYS, &key))) == ERROR_SUCCESS)
    {
        ...
        { // enumerate all icon overlay handlers one by one
            ...
            {
                int s = 0; // insert the new name, there are about 15, so no need for quick sort
        ...
        // take only the first 15; Explorer actually takes only the first 11
                        ...
                        if (CLSIDFromString(oleTxtClsId, &clsid) == NOERROR)
                        {
                            ...
                            if (clsIDKey != NULL)
                            {
                                ...
                                if ((errRet = HANDLES_Q(RegOpenKeyEx(clsIDKey, txtClsId, 0, KEY_QUERY_VALUE, &classKey))) == ERROR_SUCCESS)
                                {
                                    ...
                                    {
                                        if (errRet != ERROR_FILE_NOT_FOUND) // this error is reported when the handler has no description (apparently not a bug, on Vista this includes Offline Files)
                                        {
...
BOOL IsNameInListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    {
        static char buf[MAX_PATH]; // also called from the Bug Report - avoid growing the stack
...
BOOL AddToListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    static char n[2 * MAX_PATH]; // also called from the Bug Report - avoid growing the stack
    char* d = n;
    const char* s = name;
    while (*s != 0)
    {
...
void CreateIconReadersIconOverlayIdsAuxAux(CLSID* clsid, const char* name, IShellIconOverlayIdentifier** ids, int i)
{
    ...
    if (CoCreateInstance(*clsid, NULL, CLSCTX_INPROC_SERVER, IID_IShellIconOverlayIdentifier,
        ...
        iconOverlayIdentifier != NULL) // probably redundant, just playing it safe
    {
        // call the usual methods for completeness (as if Explorer were about to display the overlays)
        OLECHAR iconFile[MAX_PATH];
        int iconIndex;
        DWORD flags;
...
CShellIconOverlays::CreateIconReadersIconOverlayIds()
{
    ...
    if (Overlays.Count > 0)
    {
        ...
        TRACE_I("CShellIconOverlays::CreateIconReadersIconOverlayIds(): there are no icon overlay handlers!");
    return ids;
}
                                        ...
                                        BOOL isGoogleDrivePath)
{
    ...
    // the other frees and the heap gets corrupted (hard to say why, their bug). The critical section slows things down
    // considerably (by 2x) when reading both panels at once, so we try to use it only when GD is active (inside its directory)
...
void CShellIconOverlays::InitGoogleDrivePath(CSQLite3DynLoadBase** sqlite3_Dyn_InOut, BOOL debugTestOverlays)
{
    ...
    { // test only when we will advertise Google Drive on the toolbar and in the change drive menu
        ...
        if (!found)
        {
            // Google Drive installs the variant matching Windows (x86 / x64). Therefore Salamander x86
            // on x64 Windows (and vice versa) does not find the GD icon handlers, which is expected.
#ifdef _WIN64
            if (Windows64Bit)
#else  // _WIN64
            if (!Windows64Bit)
