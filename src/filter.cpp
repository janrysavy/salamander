...
void CFilterCriteria::Reset()
{
    ...
    // Set 'From' to one day earlier and 'To' to the current date
    SYSTEMTIME st;
    GetSystemTime(&st);
    SystemTimeToFileTime(&st, (FILETIME*)&To);
    From = To - (unsigned __int64)10000000 * 60 * 60 * 24; // one day earlier

    // internal variables
    NeedPrepare = FALSE; // calling PrepareForTest is unnecessary in these situations
    UseMinTime = FALSE;
    UseMaxTime = FALSE;
}
...
// returns the maximum number that after conversion to bytes still fits into CQuadWord
...
void CFilterCriteria::PrepareForTest()
{
    ...
    if (TimeMode == fctmDuring)
    {
        ...
        GetLocalTime(&st);
        // weekday is redundant information; we won't use it
        st.wDayOfWeek = 0;
        ...
        case fctuWeeks:
        {
            // For days and weeks, the time portion does not matter; we only consider the date
            st.wHour = 0;
            st.wMinute = 0;
            st.wSecond = 0;
            st.wMilliseconds = 0;
        ...
        case fctuYears:
        {
            // For months and years, the time portion does not matter; we only consider the date
            st.wHour = 0;
            st.wMinute = 0;
            st.wSecond = 0;
            st.wMilliseconds = 0;
            ...
            // if the day does not exist, move back to an existing one
        ...
        {
            // since 2.52b1, we ignore future files for "Modified during"; see report on the forum
    ...
    if (TimeMode == fctmFromTo)
    {
        if (UseFromDate)
        {
            ...
            {
                st.wMilliseconds = 0; // the control returns suspicious values, so trim them
                if (!UseFromTime)
                {
                    st.wHour = 0;
                    st.wMinute = 0;
        ...
        if (UseToDate)
        {
            ...
            if (FileTimeToSystemTime((FILETIME*)&To, &st))
            {
                ...
                {
                    // we want the absolute maximum time; stick right to the end of the range
...
BOOL CFilterCriteria::Test(DWORD attributes, const CQuadWord* size, const FILETIME* modified)
{
    ...
    if (ok && TimeMode != fctmIgnore)
    {
        ...
        if (!FileTimeToLocalFileTime(modified, (FILETIME*)&time))
            time = 0; // on error use zero (to keep the behavior deterministic)
...
{
    // space optimization in the Registry: store only non-default values
    // therefore clear the target key before saving
    CFilterCriteria def;
    ...
    // note: starting with 2.53 we'll "forget" the times in disabled FROM/TO controls (see TimeMode == fctmFromTo)
    // if users request the old behavior, we might disable the checkboxes in the Date controls by default,
    // which would violate UseFromDate/UseToDate; we would enable the checkbox only when the user enables the control via radio buttons
    if (From != def.From && TimeMode == fctmFromTo && (UseFromDate || UseFromTime)) // there's no point storing times when they aren't used (controls would insert the current time)
    {
    ...
    if (To != def.To && TimeMode == fctmFromTo && (UseToDate || UseToTime)) // there's no point storing times when they aren't used (controls would insert the current time)
    {
...
BOOL AttributeCheckBox(CTransferInfo* ti, int ctrlID, DWORD attribute, DWORD* attrMask, DWORD* attrValue)
{
    ...
    if (ti->GetControl(HWindow, ctrlID))
    {
        ...
        case ttDataFromWindow:
        {
            ...
            {
                *attrMask &= ~attribute;
                *attrValue &= ~attribute; // just for formality
            }
            else
            {
                *attrMask |= attribute;
...
void CFilterCriteriaDialog::FillUnits(int editID, int comboID, int* units, BOOL appendSizes)
{
    ...
    // read the control value from the edit line
    char buff[100];
    char buff2[100];
...
void CFilterCriteriaDialog::Validate(CTransferInfo& ti)
{
    ...
    // and switch to a temporary copy
    ...
    if (Data->TimeMode == fctmDuring)
    {
        ...
        if (ti.IsGood())
        {
            // verify that PrepareForTest converted the time to a valid date
            Data->PrepareForTest();
            if (!Data->UseMinTime)
            {
...
void CFilterCriteriaDialog::EnableControls()
{
    ...
    // disable the control with focus; handle it
...
CFilterCriteriaDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        for (i = 0; resID[i] != -1; i++)
        {
            ...
            // drawing the dialog on a slow machine looked strange
            // (UpDown was drawn after all the other controls)
