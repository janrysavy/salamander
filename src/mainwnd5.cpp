...
int MyCompareFileTime(FILETIME* lf, FILETIME* rf, int* foundDSTShifts, int* compResNoDSTShiftIgn)
{
    ...
    if (res != 0)
    {
        ...
        // compute the difference between the times and round it
...
// compare two files 'l' and 'r' by size
...
// on success returns TRUE and sets 'different' (TRUE if a difference was found, otherwise FALSE)
// on error or user abort returns FALSE and sets 'canceled' (TRUE if the user canceled, otherwise FALSE)

#define COMPARE_BUFFER_SIZE (2 * 1024 * 1024) // buffer size for comparison in bytes (the whole buffer need not be used)
#define COMPARE_BLOCK_SIZE (32 * 1024)        // size of a contiguous block read from a file; NOTE: COMPARE_BUFFER_SIZE must be divisible by COMPARE_BLOCK_SIZE
                           ...
                           BOOL* different, BOOL* canceled)
{
    ...
    *canceled = FALSE;
    ...
    if (!progressDlg->Continue()) // give the dialog a chance to repaint at 100% of the previous file (opening a file is the longest step, 3ms when copying small files of 0.1ms)
        *canceled = TRUE;
        ...
        if (hFile2 != INVALID_HANDLE_VALUE)
        {
            if (!*canceled)
            {
                ...
                while (TRUE)
                {
                    ...
                    // read another chunk from 'file1' into buffer1
                    // read the whole buffer in 32KB blocks from one file (on W2K and XP this is fastest when both files are on one physical disk; on Vista it's only slightly slower than sequential reading)
                    read1 = 0;
                    ...
                    readingIsFast1 = WindowsVistaAndLater &&                                                                                           // on W2K/XP this should not speed things up, so we will not tempt fate
                                     GetTickCount() - readBegTime < (DWORD)(((unsigned __int64)read1 * COMPARE_BUF_TIME_LIMIT) / COMPARE_BUFFER_SIZE); // measure the speed so it is over 1 MB/s
                                                                                                                                                       /*
          // read the entire buffer at once from one file (on Vista this is slightly faster than 32KB blocks when both files are on the same physical disk and the buffer is large)
...
*/
                    ...
                    // read another chunk from 'file2' into buffer2
                    ...
                    for (int block = 0; block < blockCount;)
                    {
                        ...
                        { // file contents differ, no point continuing reading
                            *different = TRUE;
                            ret = TRUE;
                            break;
                        }
...
// the source path is the panel path combined with 'subPath'
// 'hWindow' is the window for displaying message boxes
// 'progressDlg' advances the progress after reading X files/directories
...
// directories are determined by the paths in the left and right panel and the variables 'leftSubDir' and 'rightSubDir'
// leftFAT and rightFAT say whether the respective panel is on a FAT system; if an archive is opened,
// the corresponding xxxFAT will be FALSE
// 'flags' specifies comparison criteria from the COMPARE_DIRECTORIES_xxx family
// the function returns TRUE on successful comparison and sets 'different' (TRUE
// if the directories differ, otherwise FALSE).
// on error or user abort it returns FALSE and sets the variable 'canceled' (TRUE if aborted by the user, otherwise FALSE)
                    ...
                    BOOL getTotal, CQuadWord* total, int* foundDSTShifts)
{
    ...
    { // local block to limit lifetime of leftFiles & rightFiles (saves memory before recursing into subdirectories)
        CFilesArray leftFiles;
        CFilesArray rightFiles;

        // the variable 'left' was set during the previous file comparison
        ...
        // now the number of directories and files is the same in both panels
        ...
        // postpone comparing content because it's slow and if
        ...
        for (i = 0; i < leftFiles.Count; i++)
        {
            ...
            {
                int isDSTShift = 0; // 1 = times of the current file pair differ by exactly one or two hours, 0 = they don't
        ...
        if (timeDiffWithDSTShiftExists) // we found no other difference, report an unignored DST time shift including a warning
        {
            (*foundDSTShifts)++;
            *different = TRUE;
            return TRUE;
        ...
        if (flags & COMPARE_DIRECTORIES_BYCONTENT)
        {
            ...
            for (i = 0; i < leftFiles.Count; i++)
            {
                ...
                {
                    // files have different length, so they differ in content
                    *different = TRUE;
                    return TRUE;
                }
            }
...
void CMainWindow::CompareDirectories(DWORD flags)
{
    ...
    if (ignFileNames)
    {
        ...
        {
            TRACE_E("CMainWindow::CompareDirectories: CompareIgnoreFilesMasks is invalid."); // likely only when configuration is manually edited in the registry
            return;
        }
    }
    BOOL ignDirNames = (flags & COMPARE_DIRECTORIES_IGNDIRNAMES) != 0;
    if (ignDirNames)
    {
        ...
        {
            TRACE_E("CMainWindow::CompareDirectories: CompareIgnoreDirsMasks is invalid."); // likely only when configuration is manually edited in the registry
            return;
        }
    }

    BeginStopRefresh(); // the sniffer takes a break

    // lower the thread priority to "normal" (so the operation doesn't overburden the machine)
    SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_NORMAL);

    //--- open the progress dialog
    BOOL displayDialogBox = (flags & COMPARE_DIRECTORIES_BYCONTENT) != 0 || // for quick actions there's no point showing the dialog
    ...
    int foundDSTShifts = 0; // how many times file times differed by exactly one or two hours, and this was the only difference in the selected criteria

    //--- shallow copy of Files and Dirs arrays; we must work on a copy because while the progress dialog is shown
    //    a panel redraw would reveal we changed sorting or moved the selection
    ...
    if (leftFiles != NULL && leftDirs != NULL && rightFiles != NULL && rightDirs != NULL)
    {
        ...
        BOOL getTotal;
        if (flags & COMPARE_DIRECTORIES_BYCONTENT)
            getTotal = TRUE; // TRUE: we don't mark differences, accumulate the size of files to compare into 'total'
        else
            getTotal = FALSE;  // FALSE: mark differences, compare files by content
        CQuadWord total(0, 0); // total number of bytes that may need to be compared in the worst case

        // the 'dirSubTotal' array holds sizes of individual subdirectories (files contained in them)
    ...
    ONCE_MORE:
                ...
                if (r < right->Count) // both panels contain files
                {
                    ...
                    else
                    {
                        ...
                        else // left == right
                        {
                            ...
                            int isDSTShift = 0; // 1 = times of the current file pair differ by exactly one or two hours, 0 = they don't (or times are not compared at all)
                            ...
                            if (flags & COMPARE_DIRECTORIES_BYCONTENT)
                            {
                                ...
                                {
                                    if (!selectLeft && !leftIsNewer || !selectRight && !rightIsNewer) // if both files are already flagged, no point comparing contents
                                    {
                                        // if both files are zero length they must be identical
                                        if (leftFile->Size != CQuadWord(0, 0))
                                        {
                                            if (!getTotal)
                                            {
                            ...
                            if (!getTotal)
                            {
                                if (leftIsNewerNoDSTShiftIgn && !selectLeft || rightIsNewerNoDSTShiftIgn && !selectRight)
                                    foundDSTShifts += isDSTShift; // count only time differences of files not already marked for another reason -- if we don't need to show a complex DST warning, don't show it
        ...
        // Sal2.0 and TC compare without directories by ignoring their names as well
                    ...
                    if (r < right->Count) // both panels contain directories
                    {
                        ...
                        else
                        {
                            ...
                            else // left == right
                            {
                                ...
                                {
                                    // store the subpath into left/rightSubDir
                                    // for ptDisk it is relative to the panel path and for
                                    // ptZIPArchive relative to the archive path
                                    ...
                                    else
                                    {
                                        if (canceled)
                                            goto ABORT_COMPARE;
                                        else // if not 'canceled', an error message was shown and the user wants to continue
                                        {
                                            if (!getTotal)
                                                select = TRUE;
                                        }
    ...
    ABORT_COMPARE:
        ...
        // previously the selection was cleared at the start of the compare and updated continuously
        // if an icon redraw request arrived from the icon reader during that time,
        // the dirty bit of the file/directory was cleared and at the end it wasn't redrawn
        // (it remained unselected even though it should have been marked)

        // now we clear the selection at the end and between changing the selection state and calling
    ...
    //--- show a confirmation if files differing by exactly one or two hours appeared
    //    either warn that not all files have the same time or note that the found difference can be ignored
    ...
    EndStopRefresh(); // now the sniffer starts again
}
