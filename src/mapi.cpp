...
BOOL CSimpleMAPI::Init(HWND hParent)
{
    ...
    if (HLibrary == NULL)
    {
        ...
        if (HLibrary == NULL)
        {
            ...
            // I found msoemapi.dll there which exports what we need and works,
            // so why not try it...
...
BOOL CSimpleMAPI::SendMail()
{
    ...
    ULONG ret = MAPISendMail(0,
                             /*(ULONG)hParent*/ 0, // the dialog will be modeless
                             &message,
    ...
    // when cancelling the connection wizard (when not configured)
    /*
  if (ret != 0)
  {
    char buff[50];
