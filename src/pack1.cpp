...
// the order in the externalArchivers array inside CPlugins::FindViewEdit
...
//
//   In the string buffer find the item at the given index. Items may be
//   separated by any number of spaces, tabs or newlines.
//   (the vertical bar character (ASCII 0xB3) was added because of ACE)
//   When skipping the item with index nameidx (usually the file name),
//   the only item separator is a newline as the name can contain spaces or tabs.
//   Called from PackScanLine().
//
//   RET: returns a pointer to the given item in buffer or NULL if it cannot be found
//   IN:  buffer is a line of text for analysis
//        index is the ordinal number of the item to find
...
char* PackGetField(char* buffer, const int index, const int nameidx, const char separator)
{
    ...
    // the requested item for the given archiver does not exist
    if (index == 0)
        return NULL;
    ...
    // skip leading spaces and tabs (if any)
...
//
//   Analyzes one entry from the archive program listing.
//   Usually it is a single line but it can span multiple lines -
//   it must contain all information about a single file stored
//   in the archive. Called from PackList().
//
//   RET: returns TRUE on success, FALSE on error
//        on error the callback function *PackErrorHandlerPtr is called
//   IN:  buffer is the line of text to analyze - it is modified by the parsing
//        index is the index in PackTable for the appropriate row
                  ...
                  const SPackBrowseTable* configTable, BOOL ARJHack)
{
    ...
    // remove trailing separators from the name
    ...
    // read the attributes in the line and if any of them is D or d,
    // it is also a directory so append a backslash
    if (*(tmpfname - 1) != '\\')
    {
    ...
    *tmpfname-- = '\0';
    ...
    // and pomptr2 possibly holds its path
    ...
    while (s >= pomptr && *s != '.')
        s--;
    if (s >= pomptr)
        //  if (s > pomptr)  // ".cvspass" is an extension on Windows...
    ...
    tmpbuf = PackGetField(buffer, idx,
                          configTable->NameIdx,
                          configTable->Separator);
    // zero it in case we did not read the item (default time)
    t.wHour = 0;
    t.wMinute = 0;
    t.wSecond = 0;
    t.wMilliseconds = 0;
    if (tmpbuf != NULL)
    {
        ...
        // if no item separator follows we can read seconds
        if (*tmpbuf != '\0' && *tmpbuf != '\n' && *tmpbuf != '\t' &&
            *tmpbuf != ' ')
        {
            // skip the separator
    ...
    // set the remaining structure members (those not zeroed in AddFile/Dir)
...
//        archiveFileName is the name of the archive file to list
              ...
              CPluginDataInterfaceAbstract*& pluginData, CPluginData*& plugin)
{
    ...
    // if we have not yet determined the spawn path, do it now
    ...
    // We no longer need these handles; the child will close the duplicates, keep only StdOutRd
    HANDLES(CloseHandle(StdOutWr));
    HANDLES(CloseHandle(StdErrWr));
    ...
    while (1)
    {
        ...
        // buffer processed; now see how much remains and move it to the start
    ...
    //
    // now the main part - parsing the archiver output into our structures
    //
    ...
    for (i = 0; i < lineArray.Count; i++)
    {
        ...
        if (lines == 1)
        {
            // determine whether we span two or four lines (ARJ32 hack)
        ...
        line = NULL;
        if (!ret)
            return FALSE; // no need to call the error function, PackScanLine already did

        // reset variables
        lines = 0;
    }

    // if we ended somewhere other than in the footer, we have a problem
    if (validData < 2)
        return (*PackErrorHandlerPtr)(NULL, IDS_PACKERR_PARSE);
...
//   RET: returns TRUE on success, FALSE on error
//        on error the callback function *PackErrorHandlerPtr is called
//   IN:  archiveFileName is the archive we work with
        ...
        if (!strncmp(txtPtr, "DIR", 3) || !strncmp(txtPtr, "FILE", 4))
        {
            ...
            while (1)
            {
                ...
                if (!strncmp(txtPtr, "NAME=", 5))
                {
                    ...
                    if (newfile.Ext != NULL) // ".cvspass" is an extension on Windows ...
...
//        panel is the Salamander file panel
//        archiveFileName is the archive we extract from
//        targetDir is the path where files are extracted
//        archiveRoot is the directory in the archive we extract from
//        nextName is the callback for enumerating names to extract
...
//        archiveFileName is the archive we unpack
//        targetDir is the path where files are extracted
//        archiveRoot is the path in the archive to extract from, or NULL
                             ...
                             SalEnumSelection nextName, void* param, BOOL needANSIListFile)
{
    ...
    //
    // We must adjust the path in the archive to the required format
    //
    ...
    //
    // Now create the temporary directory for extraction
    ...
    // if an error occurred and the user chose cancel, end the operation +
    ...
    if (*rootPath != '\0')
    {
        ...
        // because of the locale and long names :-(
        char* r = rootPath;
...
//
//   RET: Returns the mask currently processed or NULL when done
...
//        size is always 0
//        the string of masks is modified (";"->"\0" when separating masks, ";;"->";" with the trailing ";" removed)
                                ...
                                const CFileData** fileData, void* param, int* errorOccured)
{
    ...
    while (1)
    {
        while (ptr >= *(char**)param && *ptr == ';')
            ptr--;
        if (((endPtr - ptr) & 1) == 1)
            *endPtr-- = 0; // ignore ';' only if odd (even count later becomes ";;"->";")
    ...
    // otherwise find the last semicolon (only if their count is odd, otherwise ";;"->";") - before the last mask
...
//        archiveFileName is the archive from which we extract
...
//        newFileName (if not NULL) is the new name of the extracted file (the file
                       ...
                       BOOL* renamingNotSupported)
{
    ...
    // find the extracted file - the name may not match due to locale and long names :-(
