...
char ThreadPath[MAX_PATH];                 // input for the active thread
BOOL ThreadValid;                          // result of the active thread
DWORD ThreadLastError;                     // result of the active thread

CRITICAL_SECTION CheckPathCS; // critical section for check-path, needed because it may be called from multiple threads (not only the main one)

// optimization: the first check-path thread is not terminated and is reused
...
HANDLE CPFirstEnd = NULL;      // event for testing termination of the first check-path thread
DWORD CPFirstExit;             // replacement exit code for the first check-path thread (it does not terminate)

char CheckPathRootWithRetryMsgBox[MAX_PATH] = ""; // drive root (including UNC) for which a "drive not ready" message box with Retry+Cancel buttons was shown (used for automatic retry after inserting media)
HWND LastDriveSelectErrDlgHWnd = NULL;            // the "drive not ready" dialog with Retry+Cancel buttons (used for automatic retry after inserting media)

DWORD WINAPI ThreadCheckPathF(void* param);

CRITICAL_SECTION OpenHtmlHelpCS; // critical section for OpenHtmlHelp()

// non-blocking read of CD drive volume name:
CRITICAL_SECTION ReadCDVolNameCS;        // critical section for accessing data
UINT_PTR ReadCDVolNameReqUID = 0;        // request UID (to check if someone still waits for the result)
char ReadCDVolNameBuffer[MAX_PATH] = ""; // IN/OUT buffer (root/volume name)
...
BOOL InitializeCheckThread()
{
    ...
    // try to launch the first check-path thread
    DWORD ThreadID;
    ThreadCheckPath[0] = HANDLES(CreateThread(NULL, 0, ThreadCheckPathF, (void*)0, 0, &ThreadID));
    if (ThreadCheckPath[0] == NULL) // it failed, but that's OK ...
    {
    ...
    for (i = 0; i < NUM_OF_CHECKTHREADS; i++)
    {
        if (ThreadCheckPath[i] != NULL)
        {
            ...
            { // it has nothing left to run, terminate it
                ...
                WaitForSingleObject(ThreadCheckPath[i], INFINITE); // wait until the thread really ends, sometimes it takes quite a while
            }
            ThreadCheckState[i] = ctsNotRunning;
...
CPF_AGAIN:
    ...
    {
        CPFirstFree = TRUE;                          // for entering the thread, otherwise an unnecessary safeguard ;-)
    ...
    // this can hang here, therefore we do the whole circus around it
    BOOL threadValid = (SalGetFileAttributes(threadPath) != 0xFFFFFFFF);
    DWORD error = GetLastError();
    if (!threadValid && error == ERROR_INVALID_PARAMETER) // reported on the root of removable media (CD/DVD, ZIP)
        error = ERROR_NOT_READY;                          // a bit hacky, but it's a "not ready" issue rather than "invalid parameter" ;-)

    // work around attribute reading bug (since W2K attribute reading can be disabled in Properties/Security) at least on fixed disks
        ...
        threadPath[1] == ':')
    {
        ...
        if (GetDriveType(root) == DRIVE_FIXED)
        {
            ...
            {
                // the path seems OK after all (without the fixed disk check it cannot be used; FindFirstFile
                // probably works from cache because a disconnected network drive happily starts listing,
                // therefore it is unusable for check-path; we had to replace it)
                threadValid = TRUE;
    ...
    if (i == 0) // first check-path thread (optimization: runs continuously)
    {
        CPFirstFree = TRUE; // now everything proceeds smoothly until WaitForSingleObject(CPFirstStart, INFINITE)
    }
    ...
    int ret;
    if (ThreadCheckState[i] & ctsActive) // does the main thread want the results?
    {
    ...
    {
        CPFirstExit = ret;
        SetEvent(CPFirstEnd); // WARNING, immediately switches to the main thread (higher priority)
...
DWORD SalCheckPath(BOOL echo, const char* path, DWORD err, BOOL postRefresh, HWND parent)
{
    ...
    // guard against multiple calls from different threads
    HANDLES(EnterCriticalSection(&CheckPathCS));

    // guard against repeated calls from a single thread
    static BOOL called = FALSE;
    if (called)
    {
        // so far only the case of deactivation/activation after ESC in CheckPath() is known; are there others?
...
RETRY:
    ...
    TEST_AGAIN:
        ...
        if (!runThread)
        {
            ...
            else // network path -> hand off to one of the worker threads
            {
        ...
        else
        {
            ...
            {
                ResetEvent(CPFirstEnd); // clear any previous termination
            ...
            if (success)
            {
                ...
                if (freeThreadIndex == 0)    // first check-path thread, check for completion
                {
                    if (WaitForSingleObject(CPFirstEnd, 200) != WAIT_TIMEOUT) // 200 ms grace period
                    {
                        exit = CPFirstExit; // replacement return value
                    }
                    else
                ...
                {
                    // after 3 seconds show the "ESC to cancel" window
                    ...
                    while (1)
                    {
                        if (ThreadCheckState[freeThreadIndex] & ctsCanTerminate)
                        {
                            if (UserWantsToCancelSafeWaitWindow())
                            {
                                ...
                                // the last system call to finish - on network paths it can take a few seconds
                                // so calling TerminateThread is pointless, the thread will finish just as fast
                                //                TerminateThread(ThreadCheckPath[freeThreadIndex], exit);
                                //                WaitForSingleObject(ThreadCheckPath[freeThreadIndex], INFINITE);  // wait until the thread really ends, sometimes it takes quite a while
                                break;
                            }
                        }
                ...
                {
                    valid = FALSE;
                    lastError = ERROR_USER_TERMINATED; // custom error
    ...
    if ((err == ERROR_USER_TERMINATED || echo) && !valid)
    {
        ...
        case ERROR_NOT_READY:
        {
            ...
            if (path[0] == '\\' && path[1] == '\\')
            {
                ...
                drive[strlen(drive) - 1] = 0; // we don't need the trailing '\\'
            }
            else
            {
                drive[0] = path[0];
...
BOOL SalCheckAndRestorePath(HWND parent, const char* path, BOOL tryNet)
{
    ...
    if ((err = SalCheckPath(FALSE, path, ERROR_SUCCESS, TRUE, parent)) != ERROR_SUCCESS)
    {
        ...
        if (tryNet && err != ERROR_USER_TERMINATED)
        {
            ...
            else // if the user has no account on the requested machine
            {
                // perform a UNC path accessibility test and possibly let the user log in
        ...
        if (!ok)
        {
            if (pathInvalid ||                                                // interrupted reconnection or unsuccessful attempt to reconnect
                err == ERROR_USER_TERMINATED ||                               // CheckPath interrupted by the ESC key
                SalCheckPath(TRUE, path, err, TRUE, parent) != ERROR_SUCCESS) // display other errors
            {
                return FALSE;
            }
        }
    ...
    if (tryNet) // if we haven't tried restoring the network connection yet
    {
        // perform a UNC path accessibility test and possibly let the user log in
        BOOL pathInvalid;
        if (CheckAndConnectUNCNetworkPath(parent, path, pathInvalid, FALSE))
...
_CHECK_AGAIN:
    ...
    while ((err = SalCheckPath(FALSE, path, ERROR_SUCCESS, TRUE, parent)) != ERROR_SUCCESS)
    {
        ...
        { // Vista: when the physical connection changes (e.g. Wi-Fi then LAN) it inexplicably reports this error, but it works on the second try, so we do the retry for the user
        ...
        if (tryNet) // we haven't tried it yet
        {
            tryNet = FALSE;
            if (LowerCase[path[0]] >= 'a' && LowerCase[path[0]] <= 'z' &&
                path[1] == ':') // this is a normal path (not UNC)
            {
            ...
            if (pathInvalid)
                break; // CutDirectory won't help here...
        }
        lastErr = err;
        if (!IsDirError(err))
            break; // CutDirectory won't help here...
...
PARSE_AGAIN:
    ...
    else // Windows/archive paths
    {
        ...
        BOOL mustBePath = (len == 2 && LowerCase[path[0]] >= 'a' && LowerCase[path[0]] <= 'z' &&
                           path[1] == ':'); // a path like "c:" must remain a path after expansion (not a file)

        if (nextFocus != NULL && !mustBePath) // select next focus - only "name" or "name with trailing backslash"
        {
            char* s = strchr(path, '\\');
            if (s == NULL || *(s + 1) == 0)
            {
                            ...
                            pathBufSize, curPathIsDiskOrArchive))
        {
            ...
            {
                if (curPath == NULL) // nothing to replace an empty path with (interpreted as the current directory)
                {
                    if (error != NULL)
            ...
            else
            {
                if (errTextID == IDS_INCOMLETEFILENAME)
                {
                    ...
                    {
                        // return FALSE silently - exception enabling further processing
                        // of relative paths on the filesystem
                        return FALSE;
                    }
                }
                else
        ...
        if (text == NULL)
        {
            ...
            { // tip for users: operations from an archive to the archive root must end with '\\', otherwise it only overwrites an existing file
            ...
            // we won't test network paths if we have just accessed them
            BOOL tryNet = !curPathIsDiskOrArchive || curPath == NULL || !HasTheSameRootPath(root, curPath);

            // verify/attach the root path; if the root works, the rest of the path should work too
        ...
        FIND_AGAIN:
            ...
            {
                char* end2 = end;
                while (*--end2 != '\\') // it's certain there's at least one '\\' after the root path
                {
                    if (*end2 == '*' || *end2 == '?')
                        hasMask = TRUE;
                }
                ...
                {
                    CutSpacesFromBothSides(end2 + 1); // spaces at the beginning and end of the mask are junk, they only cause trouble (e.g. "*.* " + "a" = "a.")
                    end = end2;
                    lastChar = *end;
                    *end = 0;
                }
            ...
            while (end > afterRoot) // still not just the root
            {
                int len2 = (int)strlen(path);
                if (path[len2 - 1] != '\\') // paths ending with backslash behave differently (classic vs. UNC): UNC reports success, classic ERROR_INVALID_NAME; extracting from an archive on a UNC path to path "" reported unknown archive (to PackerFormatConfig.PackIsArchive went e.g. "...test.zip\\" instead of "...test.zip")
                {
                    ...
                    if (attrs != 0xFFFFFFFF) // this part of the path exists
                    {
                        if ((attrs & FILE_ATTRIBUTE_DIRECTORY) == 0) // it's a file
                        {
                            if (lastChar != 0 || backslashAtEnd || mustBePath) // is there a backslash after the archive name?
                            {
                                ...
                                else // should be an archive (path to a file given), scream
                                {
                            ...
                            else // not shortened yet and no '\\' at the end -> overwriting a file
                            {
                                // an existing path must not include a file name, trim it...
                                isDir = FALSE;
                                while (*--end != '\\')
                                    ;            // it's certain there's at least one '\\' after the root path
                    ...
                    else
                    {
                        ...
                        if (err != ERROR_FILE_NOT_FOUND && err != ERROR_INVALID_NAME &&
                            ...
                            err != ERROR_DIRECTORY) // strange error - just print it
                        {
            ...
            *end = lastChar; // fix 'path'
            ...
            if (text == NULL)
            {
                ...
                { // path ends with a non-existing directory (not a mask), clean unwanted characters at the beginning and end
                         ...
                         const char* dirName, const char* curDiskPath, char*& mask)
{
                            ...
                            pathIsDir, backslashAtEnd, dirName, curDiskPath, mask, newDirs, NULL))
    {
        if (mask - 1 > path && *(mask - 2) == '\\' &&
            (mask - 1 > afterRoot || *path == '\\'))           // not the root or it's a UNC root
        {                                                      // remove the unnecessary trailing backslash
        ...
        if (newDirs[0] != 0) // create new directories on the target path
        {
            ...
            // --- refresh of manually refreshed directories (runs after EndStopRefresh,
            // so actually after the operation finishes)
            char changesRoot[MAX_PATH];
            memmove(changesRoot, path, secondPart - path);
            changesRoot[secondPart - path] = 0;
            // path change - new subdirectories were created on the path (needed even if
            // the directories could not be created) - a change without subdirectories (only subdirs were created)
            MainWindow->PostChangeOnPathNotification(changesRoot, FALSE);

            if (!ok)
            {
                char* e = path + strlen(path); // fix 'path' (merge of 'path' and 'mask')
                if (e > path && *(e - 1) != '\\')
                    *e++ = '\\';
                if (e != mask)
                    memmove(e, mask, strlen(mask) + 1); // append the mask if needed
                         ...
                         SGP_IsTheSamePathF isTheSamePathF)
{
    ...
    {
        if (*secondPart != 0) // there is also a non-existing part of the path
        {
            // analyze the non-existing part of the path - file/directory + mask?
            char* s = secondPart;
            BOOL hasMask = FALSE;
            ...
            if (maskFrom != secondPart) // there is a path before the mask
            {
            ...
            if (hasMask)
            {
                // split into path (ending with backslash) and mask
            ...
            {
                if (!backslashAtEnd) // just a name (mask without '*' or '?')
                {
                    if (selCount > 1 &&
                ...
                else // name with a slash at the end -> directory
                {
                    ...
                    SalPathAddBackslash(path, 2 * MAX_PATH); // the path must always end with a backslash, ensure it...
                    mask = path + strlen(path) + 1;
                    strcpy(mask, "*.*");
                }
            }
            CutSpacesFromBothSides(mask); // spaces at the beginning and end of the mask are useless, they only cause trouble

            if (tmpNewDirs[0] != 0) // the new directories still need to be created
            {
        ...
        else // no non-existing part of the path (the path exists completely)
        {
            if (dirName != NULL && curPath != NULL &&
                !backslashAtEnd && selCount <= 1) // without '\\' at the end (force directory) + single source
            {
                ...
                if (name >= afterRoot && *name != 0)
                {
                    *(name - 1) = 0;
                    ...
                    { // renaming a directory to the same name (case changes allowed)
                        // ensure we split into path (ending with backslash) and mask
                        memmove(name + 1, name, strlen(name) + 1);
                        *(name - 1) = '\\';
                        *name++ = 0;

                        mask = name;
                        // CutSpacesFromBothSides(mask); // cannot do it here: a directory with exactly this name exists; without spaces it would be a different directory (not a problem: the "illegal" directory existed before the operation, nothing new is created)
                        return TRUE; // leave the Copy/Move dialog loop and perform the operation
                    }
                    *(name - 1) = '\\';
                }
            ...
            // simple destination path with a universal mask
            SalPathAddBackslash(path, 2 * MAX_PATH); // the path must always end with a backslash, ensure it...
            mask = path + strlen(path) + 1;
            strcpy(mask, "*.*");
            return TRUE; // leave the Copy/Move dialog loop and perform the operation
    ...
    else // overwriting a file - 'secondPart' points to the file name in 'path'
    {
        ...
        if (*nameEnd == 0 && !backslashAtEnd) // rename/overwrite existing file
        {
            ...
            *secondPart++ = 0;
            ...
            mask = secondPart;
            // CutSpacesFromBothSides(mask); // cannot do it here: a file of exactly this name exists; without spaces it would be another file (not a problem: the "illegal" file existed before, no new "illegal" one is created)
            return TRUE; // leave the Copy/Move dialog loop and perform the operation
        }
        else // path into an archive? not possible here...
        {
...
BOOL FileNameIsInvalid(const char* name, BOOL isFullName, BOOL ignInvalidName)
{
    ...
    if (*s == ':')
        return TRUE;
    if (ignInvalidName)
        return FALSE; // dots and spaces at the end don't bother us now (a directory of that name may exist)
...
{
    // if the name ends with a space or dot, we must append '\\'; otherwise MoveFile
    ...
    if (!MoveFile(srcName, destName))
    {
        ...
        { // could be a Novell issue (MoveFile fails on files with the read-only attribute)
...
BOOL CSystemPolicies::GetMyCanRun(const char* fileName)
{
    ...
    if (strlen(p) >= MAX_PATH)
        return RestrictRun == 0; // disallow running if only selected commands are permitted (this one could not be separated from the command line)
    char name[MAX_PATH];
...
void CSystemPolicies::LoadFromRegistry()
{
    ...
    {
        // according to MSDN, values can be of type DWORD or BINARY:
        ...
        if (RestrictRun && !LoadList(&RestrictRunList, HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer\\RestrictRun"))
            RestrictRun = 0; // out of memory; disable this option
        ...
        if (DisallowRun && !LoadList(&DisallowRunList, HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer\\DisallowRun"))
            DisallowRun = 0; // out of memory; disable this option
        CloseKeyAux(hKey);
    }
...
DWORD SalGetFileAttributes(const char* fileName)
{
    ...
    // if the path ends with a space or dot we must append '\\'; otherwise GetFileAttributes
    // trims spaces/dots and works on a different path. It doesn't work for files,
    // but it's still better than getting attributes of another file/directory (for "c:\\file.txt   "
    // it operates on "c:\\file.txt")
...
BOOL ClearReadOnlyAttr(const char* name, DWORD attr)
{
    ...
    {
        // remove only RO (on hardlinks it changes attributes of other links to the same file, so keep it minimal)
    ...
    else
    {
        ...
        if (!SetFileAttributes(name, FILE_ATTRIBUTE_ARCHIVE)) // cannot read attributes, try to write them (we no longer care if it's needed)
...
BOOL IsLantasticDrive(const char* path, char* lastLantasticCheckRoot, BOOL& lastIsLantasticPath)
{
    ...
    if (path[0] != '\\') // not UNC - may not be a network path (cannot be LANTASTIC)
    {
...
BOOL IsNetworkPath(const char* path)
{
    if (path[0] != '\\' || path[1] != '\\')
    {
        ...
        return TRUE; // a UNC path is always networked
}

HCURSOR SetHandCursor()
{
    // use the system cursor -- avoid unnecessary blinking when changing cursors
    return SetCursor(LoadCursor(NULL, IDC_HAND));
}

void WaitForESCRelease()
{
    int c = 20; // wait up to 1/5 second for ESC to be released (so it won't immediately abort directory reading)
    while (c--)
    {
...
BOOL IsDeviceNameAux(const char* s, const char* end)
{
    ...
    // try whether this is a reserved name
    ...
    // try whether this is a reserved name followed by a digit '1'..'9'
...
BOOL SalIsValidFileNameComponent(const char* fileNameComponent)
{
    ...
    // test for white spaces at the beginning (commented out by Petr because file and directory names may start with spaces)
    ...
    BOOL simple = TRUE; // TRUE = risk of names like "lpt1" or "prn" and similar reserved names, so we add '_'
    BOOL wasSpace = FALSE;
...
void SalMakeValidFileNameComponent(char* fileNameComponent)
{
    ...
    BOOL testSimple = TRUE;
    BOOL simple = TRUE; // TRUE = risk of names like "lpt1", "prn" and other critical names, so we add '_'
    BOOL wasSpace = FALSE;
    // removing leading white spaces (commented out by Petr because file and directory names may start with spaces)
    /*
...
*/
    ...
    // trim white spaces and '.' at the end (the file system would do it anyway, it's clearer immediately)
    s--;
    while (s >= start && (*s <= ' ' || *s == '.'))
        s--;
    if (s >= start)
        *(s + 1) = 0;
    else // empty string or sequence of '.' and white spaces -> replace with name "_" (the system trims all this anyway)
    {
        strcpy(start, "_");
        simple = FALSE;
        testSimple = FALSE;
    ...
    if (simple && IsDeviceNameAux(start, fileNameComponent)) // append '_' for simple names
    {
        *fileNameComponent++ = '_';
        *fileNameComponent = 0;
    }
...
{
    static char path[MAX_PATH]; // called from an exception handler, stack may be full
    if (buf != NULL)
        buf[0] = 0;
...
{
    char* s = path - 1; // convert '/' to '\\' and remove duplicate backslashes (except at the start where they denote a UNC path or \\.\C:)
    while (*++s != 0)
    {
        if (*s == '/')
            *s = '\\';
