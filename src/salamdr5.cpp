...
// optimization: the first check-path thread is not terminated - it is reused
...
DWORD CPFirstExit;             // replacement exit code of the first check-path thread (it is not terminated)
...
CPF_AGAIN:
    ...
    ThreadCheckState[i] |= ctsCanTerminate; // the main thread can now terminate it
    ...
    if (!threadValid && error == ERROR_INVALID_PARAMETER) // reported on the root of removable media (CD/DVD, ZIP)
        error = ERROR_NOT_READY;                          // a bit of a hack, but it is a "not ready" issue and not "invalid parameter" ;-)

    // work around an attribute-reading error (since W2K you can disable reading attributes in Properties/Security) at least on fixed disks
    ...
    if (i == 0) // first check-path thread (optimization: runs continuously)
    {
        CPFirstFree = TRUE; // everything should now run smoothly until WaitForSingleObject(CPFirstStart, INFINITE)
    }
...
BOOL SalCheckAndRestorePath(HWND parent, const char* path, BOOL tryNet)
{
    ...
    if ((err = SalCheckPath(FALSE, path, ERROR_SUCCESS, TRUE, parent)) != ERROR_SUCCESS)
    {
        ...
        if (tryNet && err != ERROR_USER_TERMINATED)
        {
            ...
            {
                // test the accessibility of the UNC path and let the user log in if needed
        ...
        if (!ok)
        {
            if (pathInvalid ||                                                // interrupted connection restoration or an unsuccessful attempt to restore
    ...
    if (tryNet) // if we have not yet tried to restore the network connection
    {
        // test the accessibility of the UNC path and let the user log in if needed
        BOOL pathInvalid;
        if (CheckAndConnectUNCNetworkPath(parent, path, pathInvalid, FALSE))
...
_CHECK_AGAIN:
    ...
    while ((err = SalCheckPath(FALSE, path, ERROR_SUCCESS, TRUE, parent)) != ERROR_SUCCESS)
    {
        ...
        { // Vista: when the physical connection changes (e.g. Wi-Fi and then LAN) it inexplicably reports this error and the second attempt succeeds, so we take care of this annoyance for the user
...
PARSE_AGAIN:
    ...
    else // Windows/archive paths
    {
                            ...
                            pathBufSize, curPathIsDiskOrArchive))
        {
            ...
            {
                if (curPath == NULL) // nothing to replace the empty path (interpreted as the current directory)
                {
                    if (error != NULL)
            ...
            else
            {
                if (errTextID == IDS_INCOMLETEFILENAME)
                {
                    ...
                    {
                        // return FALSE without informing the user - an exception that allows further processing
        ...
        if (text == NULL)
        {
            ...
            // do not test network paths if we have just been accessing them
            BOOL tryNet = !curPathIsDiskOrArchive || curPath == NULL || !HasTheSameRootPath(root, curPath);

            // check/reconnect the root path; if the root path works, the rest of the path should work as well
        ...
        FIND_AGAIN:
            ...
            if (end > afterRoot) // not just the root yet
            {
                ...
                {
                    CutSpacesFromBothSides(end2 + 1); // spaces at the beginning and end of the mask must go; they only cause trouble (e.g. "*.* " + "a" = "a. ")
                    end = end2;
                    lastChar = *end;
                    *end = 0;
                }
            ...
            while (end > afterRoot) // not just the root yet
            {
                ...
                if (path[len2 - 1] != '\\') // paths ending with a backslash behave differently (classic and UNC): UNC returns success, the classic path ERROR_INVALID_NAME; unpacking from an archive located on a UNC path to the path "" reported an unknown archive (PackerFormatConfig.PackIsArchive received "...test.zip\\" instead of "...test.zip")
                {
                         ...
                         const char* dirName, const char* curDiskPath, char*& mask)
{
                            ...
                            pathIsDir, backslashAtEnd, dirName, curDiskPath, mask, newDirs, NULL))
    {
        ...
        if (newDirs[0] != 0) // create new directories on the target path
        {
            ...
            // path change - creation of new subdirectories on the path (required even if
                         ...
                         SGP_IsTheSamePathF isTheSamePathF)
{
    ...
    if (pathIsDir) // the existing part of the path is a directory
    {
        if (*secondPart != 0) // there is also a non-existent part of the path here
        {
            ...
            CutSpacesFromBothSides(mask); // spaces at the beginning and end of the mask must go; they only cause trouble
                ...
                !backslashAtEnd && selCount <= 1) // without '\\' at the end of the path (force directory) + one source
            {
                ...
                if (name >= afterRoot && *name != 0)
                {
                    *(name - 1) = 0;
                    ...
                    { // renaming a directory to the same name (case differences aside, identity is possible)
                        // ensure the split into a path (ending with a backslash) and a mask
                        ...
                        *name++ = 0;
                        ...
                        mask = name;
                        // CutSpacesFromBothSides(mask); // cannot do that here: a directory with exactly this name exists; removing the spaces would point to a different directory (no problem: the "invalid" directory already existed before the operation, nothing new "invalid" will arise)
                        return TRUE; // leave the Copy/Move dialog loop and perform the operation
                    }
                    *(name - 1) = '\\';
                }
    ...
    else // file overwrite - 'secondPart' points to the file name in the 'path'
    {
        ...
        if (*nameEnd == 0 && !backslashAtEnd) // renaming/overwriting an existing file
        {
            ...
            *secondPart++ = 0;
            ...
            mask = secondPart;
            // CutSpacesFromBothSides(mask); // cannot do that here: a file with exactly this name exists; removing the spaces would point to a different file (no problem: the "invalid" file already existed before the operation, nothing new "invalid" will arise)
            return TRUE; // leave the Copy/Move dialog loop and perform the operation
        }
...
BOOL ClearReadOnlyAttr(const char* name, DWORD attr)
{
    ...
    {
        // clear only the RO flag (with hard links it changes the attributes of the other hard links to the same file, so keep it to the minimum)
...
{
    // use the system cursor -- avoid unnecessary flicker when changing the cursor
...
BOOL SalIsValidFileNameComponent(const char* fileNameComponent)
{
    ...
    // test whitespace at the beginning (Petr: commented out because spaces at the beginning of file and directory names are simply allowed)
    ...
    if (s - fileNameComponent > MAX_PATH - 4)
        return FALSE;
    // test whitespace and '.' at the end of the name (the file system would trim them)
    s--;
    if (s >= start && (*s <= ' ' || *s == '.'))
        return FALSE;
...
void SalMakeValidFileNameComponent(char* fileNameComponent)
{
    ...
    BOOL simple = TRUE; // TRUE = risk of "lpt1", "prn" and other critical names, better append '_'
    BOOL wasSpace = FALSE;
    // removal of leading whitespace (Petr: commented out because spaces at the beginning of file and directory names are simply allowed)
    /*
...
*/
    ...
    // trim whitespace and '.' at the end of the name (the file system would do it anyway, so it will be clear immediately)
    s--;
    while (s >= start && (*s <= ' ' || *s == '.'))
        s--;
    if (s >= start)
        *(s + 1) = 0;
    else // empty string or a sequence of '.' characters and whitespace -> replace with the name "_" (the system trims all of this as well)
    {
        strcpy(start, "_");
        simple = FALSE;
        testSimple = FALSE;
    ...
    while (*fileNameComponent != 0)
    {
        ...
        {
            wasSpace = TRUE;
            *fileNameComponent = ' '; // replace all whitespace with ' '
        }
        else
        {
            if (testSimple && wasSpace)
