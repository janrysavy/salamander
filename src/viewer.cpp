...
int CharWidth = 1,  // character width in points; we divide by this value, so never set it to zero
    CharHeight = 1; // character height in points; we divide by this value, so never set it to zero
                     ...
                     BOOL changeOnlyHistory)
{
    ...
    if (changeOnlyHistory || ti.GetControl(hwnd, ctrlID))
    {
        ...
        else
        {
            ...
            // everything is fine, add it to history
            if (ti.IsGood())
            {
                if (Text[0] != 0)
                {
...
void CFindSetDialog::Transfer(CTransferInfo& ti)
{
    ...
    { // initialize the search text according to the selection in the viewer (the parent of this dialog)
        CWindowsObject* win = WindowsManager.GetWindowPtr(Parent);
        if (win != NULL && win->Is(otViewerWindow)) // just to be sure, test whether this window is the viewer
        {
...
CFindSetDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDCANCEL:
        {
            HexMode = CancelHexMode; // keep Cancel consistent
        ...
        case IDC_REGEXP_BROWSE:
        {
            ...
            if (item != NULL)
            {
                ...
                {
                    // open help on the page dedicated to regular expressions
                    OpenHtmlHelp(NULL, HWindow, HHCDisplayContext, IDH_REGEXP, FALSE);
                }
                if (item->Keyword != EXECUTE_HELP && !regular)
                {
                    // user chose an expression -> tick the checkbox for regular search
...
CViewerGoToOffsetDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        { // switch HEX = change offset from decimal to hex and back
      ...
      FindDialog(HLanguage, IDD_FINDSET, IDD_FINDSET)
{
    ...
    // dummy bitmap -- correct size set in WM_SIZE
    if (!Bitmap.CreateBmp(NULL, 1, 1))
        TRACE_E("Unable to create bitmap or memory DC for viewer.");

    CreateViewerBrushs();
    SetViewerFont(); // uses Bitmap (must be allocated at least as 1x1) and Width (must be initialized at least to 0)
...
CViewerWindow::~CViewerWindow()
{
    ...
    {
        SetEvent(Lock);
        Lock = NULL; // from now on it is only up to the disk cache
    }
    if (Buffer != NULL)
        free(Buffer);
    if (FileName != NULL)
...
int GetHexOffsetMode(unsigned __int64 fileSize, int& hexOffsetLength)
{
    ...
    {
        hexOffsetLength = 4;
        return 1; // at least 4 digits
    }
    --fileSize;                                              // the largest possible offset is one less than the file size
    if (fileSize <= CQuadWord(0x0000FFFF, 0x00000000).Value) // 4 digits are enough
    {
        hexOffsetLength = 4;
        return 1;
    }
    ...
    {
        if (fileSize <= CQuadWord(0xFFFFFFFF, 0x00000000).Value) // 8 digits are enough
        {
            hexOffsetLength = 9;
            return 2;
        }
        ...
        {
            if (fileSize <= CQuadWord(0xFFFFFFFF, 0x0000FFFF).Value) // 12 digits are enough
            {
                hexOffsetLength = 14;
                return 3;
            }
            else // 16 digits are required
            {
                hexOffsetLength = 19;
                return 4;
            }
...
void CViewerWindow::Paint(HDC dc)
{
    ...
    if (EnablePaint && !ExitTextMode && FileName != NULL && Width > 0 && Height > 0)
    {
        ...
        fullLine.bottom = CharHeight /*Height*/; // j.r. W2K performance patch
        ...
        {
            // determine the range of lines that need to be redrawn
            RECT clipRect;
            ...
            case vtHex:
            {
                ...
                for (i = 0; i < lines; i++)
                {
                    ...
                    if (len == 0 && i + 1 != lines && SeekY != 0)
                    {
                        ...
                        if (size != FileSize)
                        {
                            setFindOffset = FALSE; // do it during the next drawing pass
                            ViewSize = 0;
                            FindNewSeekY(SeekY, fatalErr);
                            if (fatalErr || ExitTextMode)
                                break;
                            FirstLineSize = LastLineSize = 0;
                            // when viewing a growing text file where the last line ended
                            // moving to a new line while scrolling down at EOF caused bad redraws
                            // expected also in HEX view; the following invalidate triggers a full repaint
                            InvalidateRect(HWindow, NULL, FALSE);
                            break;
                        }
                    }
                    if (i + 1 != lines)
                        ViewSize += len; // only fully visible lines
                    ...
                    int lineLen = (int)(s - line); // output the line
                    if (OriginX < lineLen)
                    {
                        int u1, u2;
                        ...
                        {
                            RECT myLine = fullLine; // myLine is the area with text that will be drawn using a slow BitBlt
                            myLine.right = min(myLine.right, (lineLen + 1) * CharWidth);
                            FillRect(Bitmap.HMemDC, &myLine, BkgndBrush);
                            if (lineLen > OriginX)
                            {
                                char* text = line + OriginX; // shift text according to OriginX
                                u1 -= (int)OriginX;
                                if (u1 < 0)
                                    u1 = 0;
                                u2 -= (int)OriginX;
                                ...
                                if (t2 < 0)
                                    t2 = 0;
                                // output text backwards - due to italics
                            ...
                            // BitBlt the entire line to the screen
                            BitBlt(dc, BORDER_WIDTH, CharHeight * i, (lineLen + 1) * CharWidth, // extend line by one char due to italics
                    ...
                    {
                        r.left = BORDER_WIDTH;
                        if (FileSize > 0) // JR: up to AS2.52b1 we displayed a 0-byte file in hex without shading the first line (visible when resizing the window)
            ...
            case vtText:
            {
                ...
                __int64 lineOffset = SeekY; // offset of the line start in bytes
                BOOL EOL = FALSE;           // TRUE if the last line ended with EOL (another line exists and may be empty)
                ...
                for (int i = 0; i < lines; i++)
                {
                    ...
                    if (len == 0 && i + 1 != lines && SeekY != 0)
                    {
                        ...
                        if (size != FileSize)
                        {
                            ...
                            // when viewing a growing text file where the last line ended
                            // moving to a new line while scrolling down at EOF caused bad redraws
                            // the following invalidate triggers a full repaint
                            InvalidateRect(HWindow, NULL, FALSE);
                            break;
                        }
                    }
                    ...
                    if (len == 0)
                    {
                        ...
                        if (redrI < clipFirstRow)
                            redrI = clipFirstRow; // avoid unnecessary repainting
                        r.left = BORDER_WIDTH;
                        ...
                        if (EOL) // adding the last empty line to LineOffset cannot be ignored
                        {
                    ...
                    unsigned char* st;                                               // start of the buffer with line content
                    unsigned char* s2;                                               // processed character from the line buffer
                    __int64 lineLen = 0;                                             // line length in characters (tab is not one character)
                    BOOL lineEndIsWrapped = FALSE;                                   // is the line end wrapped due to wrap mode?
                    ...
                    __int64 startSel = min(StartSelection, EndSelection);            // start of selection - offset in bytes
                    if (startSel == -1)
                        startSel = 0;
                    __int64 endSel = max(StartSelection, EndSelection); // end of selection - offset in bytes
                    if (endSel == -1)
                        endSel = 0;
                    if (startSel == endSel)
                        startSel = endSel = 0;
                    BOOL startSelDone = startSel <= lineOffset; // start of selection already drawn
                    BOOL endSelDone = endSel < lineOffset;      // end of selection already drawn

                    __int64 tabSpaces = 0; // offset due to tabs
                    EOL = FALSE;
                    lineEOLSize = 0;
                    while (len != 0)
                    {
                        ...
                        while (len--)
                        {
                            if (*s2 == '\r') // end of line '\r' or '\r\n'
                            {
                                ...
                                {
                                    if (!onlyOne)
                                    { // the line hasn't ended yet; beyond the boundary there may be '\n'
                        ...
                        // check text line length (if over 10000 characters we offer HEX mode)
                        ...
                        if (len == -1) // line continues into a not yet loaded segment
                        {
                    ...
                    if (fatalErr || ExitTextMode)
                        break;
                    LineOffset.Add(lineOffset);                             // offset of the line start
                    LineOffset.Add(lineOffset + fullLineLen - lineEOLSize); // offset of the line end (before EOL)
                    LineOffset.Add(lineLen);                                // line length in displayed characters (TAB counts as more characters)
                    if (!startSelDone)
                        startSel += tabSpaces;
                    if (!endSelDone)
                        endSel += tabSpaces;
                    ...
                    if (ScrollToSelection)
                    {
                        ...
                        { // for wide strings try to display as much as possible, so xRollLimit -> 0
                        ...
                        if (startSel < lineOffset + lineLen)
                        {
                            ...
                            if (originX != OriginX) // a change occurred
                            {
                                setFindOffset = FALSE; // resetting FindOffset is unlikely here, but if needed we'll do it during the next drawing pass
                                OriginX = originX;
                                InvalidateRect(HWindow, NULL, FALSE);
                                break; // need to start again
                            }
                            else
                    ...
                    if (i + 1 < lines)
                    {
                        ViewSize += fullLineLen; // only fully visible lines
                        if (i + 2 == lines)
                    ...
                    if (OriginX < lineLen)
                    {
                        ...
                        {
                            RECT myLine = fullLine;                                        // myLine is the area with text that will be drawn using a slow BitBlt
                            myLine.right = min(myLine.right, (int)(len2 + 1) * CharWidth); // one extra character because of italics
...
void ClearViewerHistory(BOOL dataOnly)
{
    ...
    if (!dataOnly)
    {
        // also trim the combobox in open Find windows
...
void ReleaseViewer()
{
    ...
    ClearViewerHistory(TRUE); // we only want to drop data
    if (ViewerContinue != NULL)
...
void CViewerWindow::SetViewerFont()
{
    ...
    else
    {
        ...
        // Vista: the Fixedsys font contains characters that do not have the expected width (even though it is fixed-width), so
        // we measure individual characters and map those with incorrect width to a replacement character with the proper width
        if (!WindowsXP64AndLater && !ViewerFontMeasured) // before XP64 and Vista we never hit this quirk, so we won't even test it (on XP, W2K, NT4, etc.)
        {
        ...
        if (!ViewerFontMeasured)
        {
            ...
            for (x = 0; x < 256; x++)
            {
                ...
                if (DrawTextEx(Bitmap.HMemDC, ch, 1, &rect, DT_LEFT | DT_TOP | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE, NULL))
                {
                    ...
                    {
                        if (x == 0xB7 /* middle dot */) // if the 'middle dot' is also incorrectly wide, substitute with a space which should be OK
                        {
                            substChar = ' ';
                            int z;
                            for (z = 0; z < x; z++)
