...
// #define ENABLE_SH_MENU_EXT     // if this macro is defined, the context menu is added as well (not just the "copy hook")
...
//
// it is part of Open Salamander - for configuration and mutual communication
// and at the same time it is part of SHELLEXT.DLL - for presentation and mutual communication
//
...
// Altap Salamander 2.52 beta 1              - 252B1a  (changing how the mutex is created/opened led to incompatibility with older versions)
...
// Altap Salamander 3.0                      - 300B5   (accidentally stayed as "3.0 beta 5", instead we released 3.0)
...
// basic item representing one row in the context menu
...
// return the index of the created item or -1 on error
// refItem returns a pointer to the added item, may be == NULL
...
// name of the mutex for accessing the shared memory (used when opening the mutex via OpenMutex,
// previously created via CreateMutex)
extern const char* SALSHEXT_SHAREDMEMMUTEXNAME;
// name of the shared memory (used when opening the shared memory via OpenFileMapping, previously
// created via CreateFileMapping)
extern const char* SALSHEXT_SHAREDMEMNAME;
// name of the event for sending the request to execute Paste in the source Salamander, used only on
...
#define WM_USER_SALSHEXT_TRYRELDATA WM_APP + 143 // [0, 0] - SalamExt reports paste-data unblocking (see CSalShExtSharedMem::BlockPasteDataRelease); if the data are no longer protected, let them be destroyed
...
{
    int Size; // structure size (for version detection + protection against overwriting memory)
    ...
    int PostMsgIndex;                 // index of the posted WM_USER_SALSHEXT_PASTE message that SalamExt is waiting for (after the wait timeout expires the index increases -> Salamander then skips the message once it arrives)
    BOOL BlockPasteDataRelease;       // probably no longer needed since W2K+: if TRUE, fakedataobj->Release() does not discard the paste data in Salamander
    int SalBusyState;                 // 0 = checking whether Salamander is "busy"; 1 = Salamander is not "busy" and is already waiting for the paste operation request; 2 = Salamander is "busy", postpone the paste
...
// write the entries required for the library to operate into the registry
// parameters are the path to the library, FALSE/TRUE to skip/test the DLL version by
// loading it, 0 or the registry view (32-bit or 64-bit) where the data should be written
