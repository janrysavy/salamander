...
unsigned ThreadViewerMessageLoopBody(void* parameter)
{
    ...
    if (ok) // if the window was created, run the application loop
    {
        ...
        MSG msg;
        HWND viewHWindow = view->HWindow; // keep a copy, because WM_QUIT leaves the window object unallocated
...
{
    // try to set the internal viewer state so no further error occurs before WM_USER_VIEWERREFRESH arrives
                                    ...
                                    __int64* firstLineCharLen, BOOL addLineIfSeekIsWrap)
{
    ...
    else
    {
        if (!fatalErr && !takeLineBegin && firstLineCharLen != NULL)
        {
            ...
            *firstLineCharLen = seek - lineBegin + tabAdd; // line length in bytes plus the extra from tabs
        }
    }
